...
//
//CCustomJewel::CCustomJewel() { this->Init(); } // OK
//
		...
		while (true)
		{
			...
			while (true)
			{
				...
				else if (section == 1)
				{
					...
					CUSTOM_JEWEL_SUCCESS_INFO sinfo;

					memset(&sinfo, 0, sizeof(sinfo));

					sinfo.Index = lpMemScript->GetNumber();

					sinfo.Level = lpMemScript->GetAsNumber();

					sinfo.Option1 = lpMemScript->GetAsNumber();

					sinfo.Option2 = lpMemScript->GetAsNumber();

					sinfo.Option3 = lpMemScript->GetAsNumber();

					sinfo.NewOption = lpMemScript->GetAsNumber();

					sinfo.SetOption = lpMemScript->GetAsNumber();

					sinfo.SocketOption = lpMemScript->GetAsNumber();

					for (size_t i = 0; i < item_info.size(); i++)
					{
						if (item_info[i].Index == sinfo.Index)
						{
							item_info[i].SuccessInfo = sinfo;
							break;
						}
					}
				}
				else if (section == 2)
				{
					...
					CUSTOM_JEWEL_FAILURE_INFO finfo;

					memset(&finfo, 0, sizeof(finfo));

					finfo.Index = lpMemScript->GetNumber();

					finfo.Level = lpMemScript->GetAsNumber();

					finfo.Option1 = lpMemScript->GetAsNumber();

					finfo.Option2 = lpMemScript->GetAsNumber();

					finfo.Option3 = lpMemScript->GetAsNumber();

					finfo.NewOption = lpMemScript->GetAsNumber();

					finfo.SetOption = lpMemScript->GetAsNumber();

					finfo.SocketOption = lpMemScript->GetAsNumber();

					for (size_t i = 0; i < item_info.size(); i++)
					{
						if (item_info[i].Index == finfo.Index)
						{
							item_info[i].FailureInfo = finfo;
							break;
						}
					}
				}
...
void runtime_unique_document_file(std::string filename, std::vector<CUSTOM_BOW_INFO>& item_info, size_t size)
{
	CMemScript* lpMemScript = new CMemScript;
	...
	try
	{
		...
		while (true)
		{
			...
			info.Index = CustomBowIndexCount++;

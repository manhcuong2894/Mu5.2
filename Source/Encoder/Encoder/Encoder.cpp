...
typedef struct 
{
	...
	BYTE			MocNapPaymentReserved[504];
...
namespace
{
	...
	void EnsureDirectoryTreeForFile(const std::string& filePath)
	{
		std::string normalized = filePath;
		std::replace(normalized.begin(), normalized.end(), '/', '\\');

		size_t start = ((normalized.size() > 1 && normalized[1] == ':') ? 2 : 0);
		size_t pos = normalized.find('\\', start);

		while (pos != std::string::npos)
		{
			std::string directory = normalized.substr(0, pos);
	...
	bool TryGetXmlAttribute(const std::string& element, const char* attributeName, std::string& value)
	{
		std::string token = std::string(attributeName) + "=\"";
		size_t start = element.find(token);
		...
		value = element.substr(start, end - start);
		return true;
	}

	bool TryParseXmlIntAttribute(const std::string& element, const char* attributeName, int& value)
	{
		std::string token;

		if (!TryGetXmlAttribute(element, attributeName, token))
		{
			return false;
		}
	...
	bool TryParseXmlFloatAttribute(const std::string& element, const char* attributeName, float& value)
	{
		std::string token;
		...
		char* end = 0;
		value = static_cast<float>(strtod(token.c_str(), &end));
	...
	bool ResolveDefaultItemPositionXml(std::string& xmlPath)
	{
		static const char* candidates[] =
		{
			"MainInfo\\Local\\ItemPosition.xml",
			".\\MainInfo\\Local\\ItemPosition.xml",
			"..\\MainInfo\\Local\\ItemPosition.xml",
			"..\\..\\MainInfo\\Local\\ItemPosition.xml",
			"..\\..\\..\\MainInfo\\Local\\ItemPosition.xml",
		...
		if (GetModuleFileNameA(0, modulePath, MAX_PATH) > 0)
		{
			std::string executableDirectory(modulePath);
			size_t separator = executableDirectory.find_last_of("\\/");

			if (separator != std::string::npos)
			{
				executableDirectory.resize(separator);

				for (size_t i = 0; i < _countof(candidates); ++i)
				{
					std::string candidatePath = executableDirectory + "\\" + candidates[i];

					if (FileExistsA(candidatePath))
					{
	...
	std::string GetDefaultItemPositionBmdPath()
	{
		...
		std::wstring exportBase(wFilename);
		RemoveQuotes(exportBase);

		if (exportBase.empty())
		{
			std::string filePath = "Release\\Data\\Local\\ItemTRSData.bmd";
			EnsureDirectoryTreeForFile(filePath);
			return filePath;
		}

		std::string filePath = WStringToUTF8(exportBase) + "\\Data\\Local\\ItemTRSData.bmd";
		EnsureDirectoryTreeForFile(filePath);
		return filePath;
	}

	bool LoadItemPositionXml(const std::string& xmlPath, std::vector<ITEM_POSITION_INFO>& items, std::string& error)
	{
		std::ifstream file(xmlPath.c_str(), std::ios::binary);
		...
		while ((cursor = xml.find("<Item", cursor)) != std::string::npos)
		{
			size_t itemTagEnd = cursor + 5;

			if (itemTagEnd < xml.size())
			{
				unsigned char nextChar = static_cast<unsigned char>(xml[itemTagEnd]);

				if (isalnum(nextChar) != 0 || nextChar == '_')
				{
					cursor = itemTagEnd;
					continue;
				}
			...
			size_t endTag = xml.find('>', cursor);

			if (endTag == std::string::npos)
			{
				error = "Invalid XML format near <Item> tag.";
				return false;
			}

			std::string element = xml.substr(cursor, (endTag - cursor) + 1);
			ITEM_POSITION_INFO info;
			memset(&info, 0, sizeof(info));

			if (!TryParseXmlIntAttribute(element, "Index", info.index) ||
				!TryParseXmlFloatAttribute(element, "ScreenX", info.X) ||
				!TryParseXmlFloatAttribute(element, "ScreenY", info.Y) ||
				!TryParseXmlFloatAttribute(element, "AngleX", info.angle[0]) ||
				!TryParseXmlFloatAttribute(element, "AngleY", info.angle[1]) ||
				!TryParseXmlFloatAttribute(element, "AngleZ", info.angle[2]) ||
				!TryParseXmlFloatAttribute(element, "Scale", info.scale))
			{
				error = "Invalid or missing ItemPosition attribute near index entry.";
				return false;
			}
		...
		if (items.empty())
		{
			error = "No <Item ... /> nodes found in XML.";
			return false;
		}

		std::sort(items.begin(), items.end(), [](const ITEM_POSITION_INFO& left, const ITEM_POSITION_INFO& right)
			{
		...
		for (size_t i = 1; i < items.size(); ++i)
		{
			...
			{
				std::ostringstream duplicatedIndex;
				duplicatedIndex << "Duplicated item index in XML: " << items[i].index;
				error = duplicatedIndex.str();
				return false;
			}
		}
	...
	bool EncodeItemPositionXml(const std::string& xmlInputPath, const std::string& bmdOutputPath, bool silentWhenMissing)
	{
		std::string inputPath = xmlInputPath;

		if (inputPath.empty() && !ResolveDefaultItemPositionXml(inputPath))
		{
			if (!silentWhenMissing)
			{
				MessageBoxA(0, "ItemPosition.xml not found.", "Encoder", MB_OK | MB_ICONWARNING);
			}

			return false;
		}

		if (!FileExistsA(inputPath))
		{
			std::string message = "ItemPosition.xml not found: " + inputPath;
			MessageBoxA(0, message.c_str(), "Encoder", MB_OK | MB_ICONWARNING);
			return false;
		}

		std::vector<ITEM_POSITION_INFO> items;
		std::string error;

		if (!LoadItemPositionXml(inputPath, items, error))
		{
			MessageBoxA(0, error.c_str(), "Encoder", MB_OK | MB_ICONERROR);
			return false;
		}

		std::string outputPath = bmdOutputPath.empty() ? GetDefaultItemPositionBmdPath() : bmdOutputPath;
		EnsureDirectoryTreeForFile(outputPath);

		PackFileEncrypt(outputPath, reinterpret_cast<BYTE*>(items.data()), static_cast<int>(items.size()), sizeof(ITEM_POSITION_INFO), 0xE2F1, true, true);

		return true;
	}

	std::string TCharToUtf8(const _TCHAR* value)
	{
#ifdef UNICODE
...
void EncoderEnviron()
{
	...
	info.AntihackEnable = (GetPrivateProfileInt("Antihack", "AntihackEnable", 1, ".\\MainConnect.ini") != 0) ? 1 : 0;

	info.AntiHackPort = (WORD)GetPrivateProfileInt("Antihack", "AntiHackPort", 55111, ".\\MainConnect.ini");

	GetPrivateProfileString("Antihack", "AntihackVersion", "PREMIUM", info.AntihackVersion, sizeof(info.AntihackVersion), ".\\MainConnect.ini");
...
void SaveMainFileInfo(const MAIN_FILE_INFO& mainInfo, const std::string& filePath)
{
	...
	std::vector<CUSTOM_BOW_INFO> CustomBow;

	//------------------------------------------------------------------------------------------------------------
	runtime_unique_document_file("Local\\CustomItem.txt", CustomItem, MAX_CUSTOM_ITEM);

	runtime_unique_document_file("Local\\CustomJewel.txt", CustomJewelInfo, MAX_CUSTOM_JEWEL);
	...
	runtime_unique_document_file("Local\\CustomBow.txt", CustomBow, MAX_CUSTOM_BOW);
	//------------------------------------------------------------------------------------------------------------
...
#endif // SHUTDOWN_LEVEL_WING3
	//----[Qua Phuc Loi]----
	info.GiftCode        = GetPrivateProfileInt("GIFTS", "GiftCode", 0, ".\\MainInfo.ini");
	info.NapDau          = GetPrivateProfileInt("GIFTS", "NapDau", 0, ".\\MainInfo.ini");
	info.NapNgay         = GetPrivateProfileInt("GIFTS", "NapNgay", 0, ".\\MainInfo.ini");
	info.NapThang        = GetPrivateProfileInt("GIFTS", "NapThang", 0, ".\\MainInfo.ini");
	info.NapTichLuy      = GetPrivateProfileInt("GIFTS", "NapTichLuy", 0, ".\\MainInfo.ini");
	info.TieuPhiNgay     = GetPrivateProfileInt("GIFTS", "TieuPhiNgay", 0, ".\\MainInfo.ini");
	info.TieuPhiThang    = GetPrivateProfileInt("GIFTS", "TieuPhiThang", 0, ".\\MainInfo.ini");
	info.TieuPhiTichLuy  = GetPrivateProfileInt("GIFTS", "TieuPhiTichLuy", 0, ".\\MainInfo.ini");

	info.EnableNapBankButton = 1;
	//------------------------------------------------------------------------------------------------------------
...
int _tmain(int argc, _TCHAR* argv[]) // OK
{
	...
	if (argc > 1 && (_tcsicmp(argv[1], _T("itemtrs")) == 0 || _tcsicmp(argv[1], _T("-itemtrs")) == 0 || _tcsicmp(argv[1], _T("/itemtrs")) == 0))
	{
		std::string inputPath = (argc > 2) ? TCharToUtf8(argv[2]) : "";
		std::string outputPath = (argc > 3) ? TCharToUtf8(argv[3]) : "";

		ENCODE_END

		CLEAR_END

		return EncodeItemPositionXml(inputPath, outputPath, false) ? 0 : 1;
	}

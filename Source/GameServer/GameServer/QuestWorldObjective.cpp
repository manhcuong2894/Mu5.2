...
bool CQuestWorldObjective::GetQuestWorldObjective(LPOBJ lpObj,int QuestIndex,int QuestGroup,QUEST_WORLD_OBJECTIVE* lpObjective,int ObjectiveNumber) // OK
{
	...
	for(int n=0;n < this->m_count;n++)
	{
		...
		if(lpInfo->RequireIndex != QuestIndex) continue;
		if(lpInfo->RequireGroup != QuestGroup) continue;
		
		// UI only needs to verify class, not current state, to show future rewards
		if(lpInfo->RequireClass[lpObj->Class] == 0 || lpInfo->RequireClass[lpObj->Class] > (lpObj->ChangeUp+1)) continue;

		if(matchedCount != ObjectiveNumber)
		{
			matchedCount++;
			continue;
		}

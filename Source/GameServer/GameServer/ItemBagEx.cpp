		...
		while (true)
		{
			...
			while (true)
			{
				...
				else if (section == 4)
				{
					...
					{
						info.RequireClass[n] = 0; // Reset tất cả class = 0
					}
					// Bắt tự định nghĩa Class được thả luôn ở 1 cột duy nhất
					int classNhanItem = lpMemScript->GetAsNumber();
					if (classNhanItem >= 0 && classNhanItem < MAX_CLASS)
					{
						info.RequireClass[classNhanItem] = 1; // Chỉ thằng này được ăn
					}

...
static bool ChaosMixCloseInventoryHasSerial(LPOBJ lpObj, DWORD serial)
{
	if (serial == 0)
	{
		return false;
	...
	{
		if (lpObj->Inventory1[n].IsItem() != 0 && lpObj->Inventory1[n].m_Serial == serial)
		{
			return true;
		}
	}
...
static bool ChaosMixCloseHasItem(LPOBJ lpObj)
{
...
static void ChaosMixCloseRestoreItems(LPOBJ lpObj)
{
	...
	for (int n = 0; n < CHAOS_BOX_SIZE; n++)
	{
		...
		if (ChaosMixCloseInventoryHasSerial(lpObj, lpObj->ChaosBox[n].m_Serial) != false)
		{
			continue;
		}

		if (gItemManager.InventoryInsertItem(lpObj->Index, lpObj->ChaosBox[n]) == 0xFF)
		{
			LogAdd(LOG_RED, "[ChaosMixCloseRollback] Cannot restore item Account:%s Name:%s Slot:%d Index:%d Serial:%u", lpObj->Account, lpObj->Name, n, lpObj->ChaosBox[n].m_Index, lpObj->ChaosBox[n].m_Serial);
		}
	}
}
...
void CChaosBox::GetCountIncrease(CItem* lpItem, int& ItemCount)
{
	if (gItemStack.GetItemMaxStack(lpItem->m_Index, lpItem->m_Level) != 0)
	{
		ItemCount += lpItem->m_Durability;
	}
	else
	{
		ItemCount++;
...
void CChaosBox::ChaosItemMix(LPOBJ lpObj, bool GetRate) // OK
{
	int ChaosCount = 0;
	int ItemCount = 0;
	int ItemMoney = 0;
	...
	for (int n = 0; n < CHAOS_BOX_SIZE; n++)
	{
		...
		{
			//ChaosCount++;
			this->GetCountIncrease(&lpObj->ChaosBox[n], ChaosCount);
		...
		{
			//ItemCount++;
			this->GetCountIncrease(&lpObj->ChaosBox[n], ItemCount);
			lpObj->ChaosBox[n].OldValue();
			ItemMoney += lpObj->ChaosBox[n].m_OldBuyMoney;
		}
...
void CChaosBox::DevilSquareMix(LPOBJ lpObj, bool GetRate) // OK
{
	int ChaosCount = 0;
	int EyeCount = 0;
	int EyeLevel = 0;
...
void CChaosBox::PlusItemLevelMix(LPOBJ lpObj, int type, bool GetRate) // OK
{
	int ChaosCount = 0;
	int BlessCount = 0;
	int SoulCount = 0;
	...
	for (int n = 0; n < CHAOS_BOX_SIZE; n++)
	{
		...
		{
			//BlessCount++;
			this->GetCountIncrease(&lpObj->ChaosBox[n], BlessCount);
		}
		else if (lpObj->ChaosBox[n].m_Index == GET_ITEM(14, 14))
		{
			//SoulCount++;
			this->GetCountIncrease(&lpObj->ChaosBox[n], SoulCount);
		}
	...
	if ((GetLargeRand() % 100) < lpObj->ChaosSuccessRate)
	{
		...
		gObjCustomLogPlusChaosMix(lpObj, type, item.m_Index, 0);
	...
	{
		gObjCustomLogPlusChaosMix(lpObj, type, lpObj->ChaosBox[ItemSlot].m_Index, 1);
...
void CChaosBox::DinorantMix(LPOBJ lpObj, bool GetRate) // OK
{
	int ChaosCount = 0;
	int UniriaCount = 0;
...
void CChaosBox::FruitMix(LPOBJ lpObj, bool GetRate) // OK
{
	int ChaosCount = 0;
	int CreationCount = 0;
	...
	for (int n = 0; n < CHAOS_BOX_SIZE; n++)
	{
		...
		{
			//CreationCount++;
			this->GetCountIncrease(&lpObj->ChaosBox[n], CreationCount);
		}
	}
...
void CChaosBox::Wing2Mix(LPOBJ lpObj, int type, bool GetRate) // OK
{
	int ChaosCount = 0;
	int FeatherCount = 0;
	int SleeveCount = 0;
	...
	//pLog->LogMessage(MCD_SEND, "[Wing2Mix: %d]", type);
	...
	//pLog->LogMessage(MCD_SEND, "[Wing2Mix: %d][ChaosCount: %d][FeatherCount: %d][SleeveCount: %d][WingItemCount: %d][ItemCount: %d]", type, ChaosCount, FeatherCount, SleeveCount, WingItemCount, ItemCount);

	//gLog.Output(LOG_CHAOS_MIX, "[Wing2Mix][ChaosCount: %d][FeatherCount: %d][SleeveCount: %d][WingItemCount: %d][ItemCount: %d]", ChaosCount, FeatherCount, SleeveCount, WingItemCount, ItemCount);
	if (GetRate != true) //CB_GETMIXRATE
	{
		if (ChaosCount != 1 || (type == 0 && FeatherCount != 1) || (type == 1 && SleeveCount != 1) || WingItemCount != 1)
		{
			//pLog->LogMessage(MCD_SEND, "[Wing2Mix: %d][ERROR][GCChaosMixSend: %d]", type, 7);
			this->GCChaosMixSend(lpObj->Index, 7, 0);
			return;
		}
	}
	...
	if (lpObj->Money < ((DWORD)lpObj->ChaosMoney))
	{
		//pLog->LogMessage(MCD_SEND, "[Wing2Mix: %d][ERROR][GCChaosMixSend: %d]", type, 2);
		this->GCChaosMixSend(lpObj->Index, 2, 0);
		return;
	}
	...
	if ((GetLargeRand() % 100) < lpObj->ChaosSuccessRate)
	{
		...
		//pLog->LogMessage(MCD_SEND, "[Wing2Mix][Success][%s][%s] - (Type: %d, ChaosSuccessRate: %d, ChaosMoney: %d)", lpObj->Account, lpObj->Name, type, lpObj->ChaosSuccessRate, lpObj->ChaosMoney);
	}
	else
	{
		this->ChaosBoxInit(lpObj);
		...
		//pLog->LogMessage(MCD_SEND, "[Wing2Mix][Failure][%s][%s] - (Type: %d, ChaosSuccessRate: %d, ChaosMoney: %d)", lpObj->Account, lpObj->Name, type, lpObj->ChaosSuccessRate, lpObj->ChaosMoney);
...
void CChaosBox::BloodCastleMix(LPOBJ lpObj, bool GetRate) // OK
{
	int ChaosCount = 0;
	int ScrollCount = 0;
	int ScrollLevel = 0;
...
void CChaosBox::Wing1Mix(LPOBJ lpObj, bool GetRate) // OK
{
	int ChaosCount = 0;
	int ChaosItem = 0;
	int ItemCount = 0;
...
void CChaosBox::PetMix(LPOBJ lpObj, int type, bool GetRate) // OK
{
	int ChaosCount = 0;
	...
	for (int n = 0; n < CHAOS_BOX_SIZE; n++)
	{
		...
		else if (lpObj->ChaosBox[n].m_Index == GET_ITEM(13, 31) && lpObj->ChaosBox[n].m_Level == 0)
		{
			...
			//this->GetCountIncrease(&lpObj->ChaosBox[n], SoulOfDarkHorseCount);
		}
		else if (lpObj->ChaosBox[n].m_Index == GET_ITEM(13, 31) && lpObj->ChaosBox[n].m_Level == 1)
		{
			SoulOfDarkSpiritCount++;
			//this->GetCountIncrease(&lpObj->ChaosBox[n], SoulOfDarkSpiritCount);
		}
		else if (lpObj->ChaosBox[n].m_Index == GET_ITEM(14, 13))
		{
			//BlessCount++;
...
void CChaosBox::SiegePotionMix(LPOBJ lpObj, int type, bool GetRate) // OK
{
	int BlessCount = 0;
	int SoulCount = 0;
...
void CChaosBox::LifeStoneMix(LPOBJ lpObj, bool GetRate) // OK
{
	int ChaosCount = 0;
	int BlessCount = 0;
	int SoulCount = 0;
	...
	for (int n = 0; n < CHAOS_BOX_SIZE; n++)
	{
		...
		{
			//GuardianCount++;
			this->GetCountIncrease(&lpObj->ChaosBox[n], GuardianCount);
		}
	}
...
void CChaosBox::SeniorMix(LPOBJ lpObj, bool GetRate) // OK
{
	...
	if (gServerInfo.m_CastleSiegeSeniorMixLimitType == -1)
	{
	...
	else if (gServerInfo.m_CastleSiegeSeniorMixLimitType != 0)
	{
		if ((this->m_SeniorMixAccumulated >= gServerInfo.m_CastleSiegeSeniorMixQuantityLimit) && this->m_SeniorMixLimitDay == SystemTime.wDay && this->m_SeniorMixLimitMonth == SystemTime.wMonth && this->m_SeniorMixLimitYear == SystemTime.wYear)
		{
			this->m_SeniorMixAccumulated = 0;
			this->GCChaosMixSend(lpObj->Index, 0, 0);
			return;
	...
	this->m_SeniorMixAccumulated++;

	if (gServerInfo.m_CastleSiegeSeniorMixLimitType > 0)
	{
		if (this->m_SeniorMixAccumulated >= gServerInfo.m_CastleSiegeSeniorMixQuantityLimit)
		{
...
void CChaosBox::PieceOfHornMix(LPOBJ lpObj, bool GetRate) // OK
{
	int ChaosCount = 0;
...
void CChaosBox::BrokenHornMix(LPOBJ lpObj, bool GetRate) // OK
{
	int ChaosCount = 0;
...
void CChaosBox::HornOfFenrirMix(LPOBJ lpObj, bool GetRate) // OK
{
	int ChaosCount = 0;
...
void CChaosBox::HornOfFenrirUpgradeMix(LPOBJ lpObj, bool GetRate) // OK
{
	int ChaosCount = 0;
...
void CChaosBox::ShieldPotionMix(LPOBJ lpObj, int type, bool GetRate) // OK
{
...
void CChaosBox::JewelOfHarmonyItemPurityMix(LPOBJ lpObj, bool GetRate) // OK
{
	int GemStoneCount = 0;
...
void CChaosBox::JewelOfHarmonyItemSmeltMix(LPOBJ lpObj, bool GetRate) // OK
{
	int ItemCount = 0;
	int ItemSlot = 0;
...
void CChaosBox::JewelOfHarmonyItemRestoreMix(LPOBJ lpObj, bool GetRate) // OK
{
	int ItemCount = 0;
	int ItemSlot = 0;
...
void CChaosBox::Item380Mix(LPOBJ lpObj, bool GetRate) // OK
{
	int GuardianCount = 0;
	int HarmonyCount = 0;
	int ItemCount = 0;
...
void CChaosBox::IllusionTempleMix(LPOBJ lpObj, bool GetRate) // OK
{
	int ChaosCount = 0;
	int ScrollCount = 0;
	int ScrollLevel = 0;
...
void CChaosBox::FeatherOfCondorMix(LPOBJ lpObj, bool GetRate) // OK
{
	int ChaosCount = 0;
	int CreationCount = 0;
	int SoulPack10 = 0;
...
void CChaosBox::Wing3Mix(LPOBJ lpObj, bool GetRate) // OK
{
	int ChaosCount = 0;
	int CreationCount = 0;
	int SoulPack10 = 0;
...
void CChaosBox::ChaosCardMix(LPOBJ lpObj, int type, bool GetRate) // OK
{
...
void CChaosBox::CherryBlossomMix(LPOBJ lpObj, int type, bool GetRate) // OK
{
...
void CChaosBox::SocketItemCreateSeedMix(LPOBJ lpObj, bool GetRate) // OK
{
	int ChaosCount = 0;
	int CreationCount = 0;
	int HarmonyCount = 0;
	...
	for (int n = 0; n < CHAOS_BOX_SIZE; n++)
	{
		...
		{
			//HarmonyCount++;
			this->GetCountIncrease(&lpObj->ChaosBox[n], HarmonyCount);
		}
...
void CChaosBox::SocketItemCreateSeedSphereMix(LPOBJ lpObj, bool GetRate) // OK
{
	int ChaosCount = 0;
	int CreationCount = 0;
	int SeedCount = 0;
...
void CChaosBox::SocketItemMountSeedSphereMix(LPOBJ lpObj, BYTE info, bool GetRate) // OK
{
...
void CChaosBox::SocketItemUnMountSeedSphereMix(LPOBJ lpObj, BYTE info, bool GetRate) // OK
{
	if (info >= MAX_SOCKET_ITEM_OPTION_TABLE)
	{
		this->GCChaosMixSend(lpObj->Index, 0, 0);
...
void CChaosBox::ImperialGuardianMix(LPOBJ lpObj, bool GetRate) // OK
{
...
void CChaosBox::ChestMix(LPOBJ lpObj, bool GetRate) // OK
{
#if(GAMESERVER_UPDATE>=501)
...
void CChaosBox::LuckyItemCreateMix(LPOBJ lpObj, bool GetRate) // OK
{
...
void CChaosBox::LuckyItemRefineMix(LPOBJ lpObj, bool GetRate) // OK
{
#if(GAMESERVER_UPDATE>=602)
...
void CChaosBox::CGChaosMixRecv(PMSG_CHAOS_MIX_RECV* lpMsg, int aIndex) // OK
{
	...
	case CHAOS_MIX_SOCKET_ITEM_MOUNT_SEED_SPHERE:
		this->SocketItemMountSeedSphereMix(lpObj, lpMsg->subtype);
		break;
	case CHAOS_MIX_SOCKET_ITEM_UN_MOUNT_SEED_SPHERE:
		this->SocketItemUnMountSeedSphereMix(lpObj, lpMsg->subtype);
		break;
...
void CChaosBox::CGChaosMixCloseRecv(int aIndex) // OK
{
	...
	{
		if (ChaosMixCloseHasItem(lpObj) != false && lpObj->IsChaosMixCompleted == 0 && lpObj->IsCastleNPCUpgradeCompleted == 0)
		{
			LogAdd(LOG_RED, "[ChaosMixCloseRollback] Account:%s Name:%s Transaction:%d Interface:%d/%d", lpObj->Account, lpObj->Name, lpObj->Transaction, lpObj->Interface.use, lpObj->Interface.type);

			gObjInventoryRollback(aIndex);
			ChaosMixCloseRestoreItems(lpObj);
			this->ChaosBoxInit(lpObj);
	...
	if (ChaosMixCloseHasItem(lpObj) != false && lpObj->IsChaosMixCompleted == 0 && lpObj->IsCastleNPCUpgradeCompleted == 0)
	{
		LogAdd(LOG_RED, "[ChaosMixCloseRollback] Account:%s Name:%s Transaction:%d", lpObj->Account, lpObj->Name, lpObj->Transaction);
...
void CChaosBox::CGChaosMixInfo(BYTE* lpMsg, int aIndex, int size)
{
	...
	//lpObj->m_crMixRateShow.lock();

	PMSG_CHAOX_MIX_GET_RATE_RECV* Data = (PMSG_CHAOX_MIX_GET_RATE_RECV*)lpMsg;

	if (Data->MixID < 0 || Data->MixID > 512)
	{
		//lpObj->m_crMixRateShow.unlock();
		return;
	}
	...
	lpObj->ShowRateLock = 100;  //Set trang thai get Rate
	...
	{
	case CHAOS_MIX_CHAOS_ITEM:
		this->ChaosItemMix(lpObj, TRUE);
		break;
	case CHAOS_MIX_DEVIL_SQUARE:
		this->DevilSquareMix(lpObj, TRUE);
		break;
	case CHAOS_MIX_PLUS_ITEM_LEVEL1:
		this->PlusItemLevelMix(lpObj, 0, TRUE);
		break;
	case CHAOS_MIX_PLUS_ITEM_LEVEL2:
		this->PlusItemLevelMix(lpObj, 1, TRUE);
		break;
	case CHAOS_MIX_DINORANT:
		this->DinorantMix(lpObj, TRUE);
		break;
	case CHAOS_MIX_FRUIT:
		this->FruitMix(lpObj, TRUE);
		break;
	case CHAOS_MIX_WING1:
		this->Wing2Mix(lpObj, 0, TRUE);
		break;
	case CHAOS_MIX_BLOOD_CASTLE:
		this->BloodCastleMix(lpObj, TRUE);
		break;
	case CHAOS_MIX_WING2:
		this->Wing1Mix(lpObj, TRUE);
		break;
	case CHAOS_MIX_PET1:
		this->PetMix(lpObj, 0, TRUE);
		break;
	case CHAOS_MIX_PET2:
		this->PetMix(lpObj, 1, TRUE);
		break;
	case CHAOS_MIX_SIEGE_POTION1:
		this->SiegePotionMix(lpObj, 0, TRUE);
		break;
	case CHAOS_MIX_SIEGE_POTION2:
		this->SiegePotionMix(lpObj, 1, TRUE);
		break;
	case CHAOS_MIX_LIFE_STONE:
		this->LifeStoneMix(lpObj, TRUE);
		break;
	case CHAOS_MIX_SENIOR:
		this->SeniorMix(lpObj, TRUE);
		break;
	case CHAOS_MIX_PLUS_ITEM_LEVEL3:
		this->PlusItemLevelMix(lpObj, 2, TRUE);
		break;
	case CHAOS_MIX_PLUS_ITEM_LEVEL4:
		this->PlusItemLevelMix(lpObj, 3, TRUE);
		break;
	case CHAOS_MIX_WING3:
		this->Wing2Mix(lpObj, 1, TRUE);
		break;
	case CHAOS_MIX_PIECE_OF_HORN:
		this->PieceOfHornMix(lpObj, TRUE);
		break;
	case CHAOS_MIX_BROKEN_HORN:
		this->BrokenHornMix(lpObj, TRUE);
		break;
	case CHAOS_MIX_HORN_OF_FENRIR:
		this->HornOfFenrirMix(lpObj, TRUE);
		break;
	case CHAOS_MIX_HORN_OF_FENRIR_UPGRADE:
		this->HornOfFenrirUpgradeMix(lpObj, TRUE);
		break;
	case CHAOS_MIX_SHIELD_POTION1:
		this->ShieldPotionMix(lpObj, 0, TRUE);
		break;
	case CHAOS_MIX_SHIELD_POTION2:
		this->ShieldPotionMix(lpObj, 1, TRUE);
		break;
	case CHAOS_MIX_SHIELD_POTION3:
		this->ShieldPotionMix(lpObj, 2, TRUE);
		break;
	case CHAOS_MIX_JEWEL_OF_HARMONY_ITEM_PURITY:
		this->JewelOfHarmonyItemPurityMix(lpObj, TRUE);
		break;
	case CHAOS_MIX_JEWEL_OF_HARMONY_ITEM_SMELT:
		this->JewelOfHarmonyItemSmeltMix(lpObj, TRUE);
		break;
	case CHAOS_MIX_JEWEL_OF_HARMONY_ITEM_RESTORE:
		this->JewelOfHarmonyItemRestoreMix(lpObj, TRUE);
		break;
	case CHAOS_MIX_ITEM_380:
		this->Item380Mix(lpObj, TRUE);
		break;
	case CHAOS_MIX_ILLUSION_TEMPLE:
		this->IllusionTempleMix(lpObj, TRUE);
		break;
	case CHAOS_MIX_FEATHER_OF_CONDOR:
		this->FeatherOfCondorMix(lpObj, TRUE);
		break;
	case CHAOS_MIX_WING4:
		this->Wing3Mix(lpObj, TRUE);
		break;
	case CHAOS_MIX_CHAOS_CARD:
		this->ChaosCardMix(lpObj, 0, TRUE);
		break;
	case CHAOS_MIX_CHERRY_BLOSSOM:
		this->CherryBlossomMix(lpObj, 0, TRUE);
		break;
	case CHAOS_MIX_SOCKET_ITEM_CREATE_SEED:
		this->SocketItemCreateSeedMix(lpObj, TRUE);
		break;
	case CHAOS_MIX_SOCKET_ITEM_CREATE_SEED_SPHERE:
		this->SocketItemCreateSeedSphereMix(lpObj, TRUE);
		break;
	case CHAOS_MIX_SOCKET_ITEM_MOUNT_SEED_SPHERE:
		this->SocketItemMountSeedSphereMix(lpObj, ChaosMixInfo, TRUE);
		break;
	case CHAOS_MIX_SOCKET_ITEM_UN_MOUNT_SEED_SPHERE:
		this->SocketItemUnMountSeedSphereMix(lpObj, ChaosMixInfo, TRUE);
		break;
	case CHAOS_MIX_IMPERIAL_GUARDIAN:
		this->ImperialGuardianMix(lpObj, TRUE);
		break;
	case CHAOS_MIX_CHEST:
		this->ChestMix(lpObj, TRUE);
		break;
	//case CHAOS_MIX_SUMMON_SCROLL:
	//	this->SummonScrollMix(lpObj, 0, TRUE);
	//	break;
	case CHAOS_MIX_PLUS_ITEM_LEVEL5:
		this->PlusItemLevelMix(lpObj, 4, TRUE);
		break;
	case CHAOS_MIX_PLUS_ITEM_LEVEL6:
		this->PlusItemLevelMix(lpObj, 5, TRUE);
		break;
	case CHAOS_MIX_LUCKY_ITEM_CREATE:
		this->LuckyItemCreateMix(lpObj, TRUE);
		break;
	case CHAOS_MIX_LUCKY_ITEM_REFINE:
		this->LuckyItemRefineMix(lpObj, TRUE);
		break;
	//case CHAOS_MIX_MONSTER_WING:
	//	this->MonsterWingMix(lpObj, TRUE);
	//	break;
	//case CHAOS_MIX_SOCKET_WEAPON:
	//	this->SocketWeaponMix(lpObj, TRUE);
	//	break;
	}

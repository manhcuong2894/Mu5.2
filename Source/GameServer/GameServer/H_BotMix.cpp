...
char* H_BotMix::GetMessage(int index) // OK
{
...
void H_BotMix::LoadXML(char* FilePath)
{
	H_BotMix::ClearObjBot();
	...
	for (pugi::xml_node ItemsBot = bItemsBot.child("Items"); ItemsBot; ItemsBot = ItemsBot.next_sibling())
	{
		...
		std::map<int, H_BotMix::DATA_CREATE_BOT>::iterator it = this->m_DataCreateBot.find(bIndexBot);
	...
	for (pugi::xml_node BotAsociation = bBotAsociation.child("Bot"); BotAsociation; BotAsociation = BotAsociation.next_sibling())
	{
		...
		std::map<int, H_BotMix::DATA_CREATE_BOT>::iterator it = this->m_DataCreateBot.find(cIndexBot);
		if (it == this->m_DataCreateBot.end())
		{
			continue;
		}
	...
	for (pugi::xml_node NeedItem = bNeedItem.child("Mix"); NeedItem; NeedItem = NeedItem.next_sibling())
	{
		...
		std::map<int, H_BotMix::DATA_MIX_DATA>::iterator it = this->m_DataMix.find(nMixIndex);
	...
	for (pugi::xml_node DataResult = bDataResult.child("ItemResult"); DataResult; DataResult = DataResult.next_sibling())
	{
		...
		std::map<int, H_BotMix::DATA_MIX_DATA>::iterator it = this->m_DataMix.find(rMixIndex);
		if (it == this->m_DataMix.end())
		{
			continue;
		}
...
BYTE H_BotMix::gObjInventoryInsertItemPos(int aIndex, CItem item, int pos, BOOL RequestCheck) //OK
{
...
H_BotMix::DATA_CREATE_BOT* H_BotMix::GetBotIndex(int aIndex)
{
	for (auto it = m_DataCreateBot.begin(); it != m_DataCreateBot.end(); ++it)
	{
		if (it->second.Enable == 1)
...
BOOL H_BotMix::IsInTrade(int aIndex)
{
	H_BotMix::DATA_CREATE_BOT* DataBot = this->GetBotIndex(gObj[aIndex].TargetNumber);
	if (DataBot)
		return 1;
	else
		return 0;
}
int H_BotMix::ItemCount(int aIndex)
{
	int Count = 0;
...
int H_BotMix::GetItemCountMixIndex(int MixIndex)
{
	std::map<int, H_BotMix::DATA_MIX_DATA>::iterator it = this->m_DataMix.find(MixIndex);
...
BYTE H_BotMix::BoxExcOptions(int maxOptions)
{
...
BOOL H_BotMix::XuLyItemTrade(int aIndex, int BotNum)
{
	char sbuf[512] = { 0 };
	int fitem = -1;
	int count = 0;
	...
	std::map<int, H_BotMix::DATA_CREATE_BOT>::iterator it = this->m_DataCreateBot.find(BotNum);
	...
	{
		int foundItem = 0;
		std::map<int, H_BotMix::DATA_MIX_DATA>::iterator itDBMix = this->m_DataMix.find(i);
	...
	GCChatTargetSend(&gObj[aIndexBot], aIndexBot, gMessage.GlobalText(906));
	goto Cancel;
OK:
	if (mIndex != -1)
	{
		std::map<int, H_BotMix::DATA_MIX_DATA>::iterator itDBMixResult = this->m_DataMix.find(mIndex);

		if (itDBMixResult == this->m_DataMix.end())
		{
			GCChatTargetSend(&gObj[aIndexBot], aIndexBot, gMessage.GlobalText(907));
			goto Cancel;
		}
		if (itDBMixResult->second.PriceType == 0)
		{
			...
			{
				GCChatTargetSend(&gObj[aIndexBot], aIndexBot, gMessage.GlobalText(908), itDBMixResult->second.PriceCount);
				goto Cancel;
			}
		}
		else if (itDBMixResult->second.PriceType == 1)
		{
			...
			{
				GCChatTargetSend(&gObj[aIndexBot], aIndexBot, gMessage.GlobalText(909), itDBMixResult->second.PriceCount);
				goto Cancel;
			}
		}
		else if (itDBMixResult->second.PriceType == 2)
		{
			...
			{
				GCChatTargetSend(&gObj[aIndexBot], aIndexBot, gMessage.GlobalText(910), itDBMixResult->second.PriceCount);
				goto Cancel;
			}
		}
		else if (itDBMixResult->second.PriceType == 3)
		{
			...
			{
				GCChatTargetSend(&gObj[aIndexBot], aIndexBot, gMessage.GlobalText(911), itDBMixResult->second.PriceCount);
				goto Cancel;
			}
		}
		else
		{
			GCChatTargetSend(&gObj[aIndexBot], aIndexBot, gMessage.GlobalText(912));
			goto Cancel;
		}
		...
		if (random <= itDBMixResult->second.SuccessRate)
		{
			...
			{
				GCChatTargetSend(&gObj[aIndexBot], aIndexBot, gMessage.GlobalText(913));
				goto Cancel;
			}
		...
		// Reset NPC Bot interface
		{
			int botIdx = lpObj->TargetNumber;
			if (OBJMAX_RANGE(botIdx)) {
				gObj[botIdx].Interface.use = 0;
				gObj[botIdx].Interface.type = INTERFACE_NONE;
				gObj[botIdx].Interface.state = 0;
				gObj[botIdx].TargetNumber = -1;
			}
		}
...
void H_BotMix::TradeOk(int aIndex)
{
	int MixNum = -1;
	int MixItem = -1;
	...
	if (!DataBot)
	{
		gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, "Bot trade failed: invalid bot configuration.");
	...
	LogAdd(LOG_RED, "H_BotMix::TradeOk %s", DataBot->BotName);
	this->XuLyItemTrade(aIndex, DataBot->IndexBot);
}

BOOL H_BotMix::TradeOpen(int index, int nindex)
{
	...
	H_BotMix::DATA_CREATE_BOT* DataBot = this->GetBotIndex(nindex);
	if (!DataBot) return 0;
	...
	if (DataBot->OnylVip == 1 && gObj[index].AccountLevel == 0)
	{
		...
		GCChatTargetSend(&gObj[DataBot->Cache_aIndex], index, this->GetMessage(2));
	...
	if (this->Enabled == TRUE)
	{
		...
		else
		{
			...
			// Set NPC Bot interface state so CGTradeOkButtonRecv doesn't reject
			lpBot->Interface.state = 1;
			lpBot->Interface.use = 1;
			lpBot->Interface.type = INTERFACE_TRADE;
			...
			GCChatTargetSend(&gObj[DataBot->Cache_aIndex], index, this->GetMessage(0));
...
void H_BotMix::TradeCancel(int aIndex)
{
	LPOBJ lpObj = &gObj[aIndex];
...
void H_BotMix::SendDataIsTrade(int aIndex, int IndexBot, int TypeSend, int MixIndex)
{
	...
	switch (TypeSend)
	{
		case H_BotMix::eSendRecvListDataMix:
		{
			std::map<int, H_BotMix::DATA_CREATE_BOT>::iterator itBot = this->m_DataCreateBot.find(IndexBot);
			...
			for (int i = itBot->second.MixIndexMin; i <= itBot->second.MixIndexMax; i++)
			{
				...
				if (itDBMix != this->m_DataMix.end())
				{
					...
					memset(info.NameMix, 0, sizeof(info.NameMix));
					memcpy(info.NameMix, itDBMix->second.MixName, (sizeof(info.NameMix) - 1 < sizeof(itDBMix->second.MixName)) ? (sizeof(info.NameMix) - 1) : sizeof(itDBMix->second.MixName));
		...
		break;
		case H_BotMix::eSendRecvListItemMix:
		{
			if (MixIndex == -1) return;
			std::map<int, H_BotMix::DATA_MIX_DATA>::iterator itDBMix = this->m_DataMix.find(MixIndex);

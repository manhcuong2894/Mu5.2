...
void CCustomMonster::Load(char* path) // OK
{
	...
	try
	{
		while(true)
		{
			...
			info.DameBossCount = lpMemScript->GetAsNumber();
			info.KillBossCount = lpMemScript->GetAsNumber();
...
void CCustomMonster::SetCustomMonsterInfo(LPOBJ lpObj) // OK
{
	...
	double maxLifeRate = (double)((CustomMonsterInfo.MaxLife == -1) ? 100 : CustomMonsterInfo.MaxLife);

	lpObj->Life = (lpObj->Life * maxLifeRate) / 100.0;

	lpObj->MaxLife = (lpObj->MaxLife * maxLifeRate) / 100.0;

	lpObj->ScriptMaxLife = (lpObj->ScriptMaxLife * maxLifeRate) / 100.0;
...
void CCustomMonster::MonsterDieProc(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	{
		char bossKillInfo[128];
		char* monsterName = gMonsterManager.GetMonsterName(lpObj->Class);
		char* mapName = gMapManager.GetMapName(lpObj->Map);

		if (monsterName == 0 || monsterName[0] == 0)
		{
			monsterName = lpObj->Name;
		}

		if (mapName == 0 || mapName[0] == 0)
		{
			mapName = "Unknown";
		}

		//sprintf_s(bossKillInfo, "%s%s - %s (%d, %d)", lpTarget-> Name, monsterName, mapName, lpObj->X, lpObj->Y);
		gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GlobalText(CustomMonsterInfo.KillMessage), mapName, lpObj->X, lpObj->Y, lpTarget->Name, monsterName);
	}

	if(CustomMonsterInfo.InfoMessage != -1)
	{
		gNotice.GCNoticeSend(lpTarget->Index,1,0,0,0,0,0,gMessage.GlobalText(CustomMonsterInfo.InfoMessage),CustomMonsterInfo.WCoinC,CustomMonsterInfo.WCoinP);
	}

	if(CustomMonsterInfo.WCoinC != -1 && CustomMonsterInfo.WCoinP != -1)
	{
		GDCustomMonsterRewardSaveSend(lpTarget->Index,lpObj->Class,lpObj->Map,CustomMonsterInfo.WCoinC,CustomMonsterInfo.WCoinP);
	}

	if (CustomMonsterInfo.KillBossCount > 0)
	{
		GDCustomBossKillSaveSend(lpTarget->Index, CustomMonsterInfo.KillBossCount);
	}
...
bool CCustomMonster::GetShowBXHDmg(int index, int map)
{
	CUSTOM_MONSTER_INFO CustomMonsterInfo;
	...
	return (CustomMonsterInfo.DameBossCount > 0);
}

...
bool CHackSkillCheck::CheckSpeedHack(LPOBJ lpObj,int skill) // OK
{
	...
	if(lpObj->SpeedHackCount > HackSkillInfo.MaxCount)
	{
		...
		if( lpObj->SpeedHackDialog == 0)
		{
			...
			{
				gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GlobalText(843));	
			}
		...
		{
			LogAdd(LOG_RED,"[SpeedHackDetected][%s][%s] BYPASS CloseClient Skill: %d Count: %d MaxCount: %d",lpObj->Account,lpObj->Name,skill,lpObj->SpeedHackCount,HackSkillInfo.MaxCount);
			return 1;
		}

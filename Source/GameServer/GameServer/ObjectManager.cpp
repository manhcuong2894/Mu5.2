...
void CObjectManager::ObjectStateProc(LPOBJ lpObj, int MessageCode, int aIndex, int SubCode) // OK
{
	...
	case 16:
		...
		GCLifeSend(lpObj->Index, 0xFF, lpObj->Life, lpObj->Shield);
		break;
	}
}
...
static bool CharacterGameCloseInventoryHasSerial(LPOBJ lpObj, DWORD serial)
{
	if (serial == 0)
	{
		return false;
...
static void CharacterGameCloseRestoreChaosItems(LPOBJ lpObj)
{
	...
	for (int n = 0; n < CHAOS_BOX_SIZE; n++)
	{
		...
		if (CharacterGameCloseInventoryHasSerial(lpObj, lpObj->ChaosBox[n].m_Serial) != false)
		{
			continue;
		}
		...
		{
			LogAdd(LOG_RED, "[ChaosReconnectRollback] Cannot restore item Account:%s Name:%s Slot:%d Index:%d Serial:%u", lpObj->Account, lpObj->Name, n, lpObj->ChaosBox[n].m_Index, lpObj->ChaosBox[n].m_Serial);
		}
	}
}

static void CharacterGameCloseChaosRollback(LPOBJ lpObj)
{
	if (lpObj == 0)
	{
		return;
	...
	if ((lpObj->Interface.use == 0 || (lpObj->Interface.type != INTERFACE_CHAOS_BOX && lpObj->Interface.type != INTERFACE_TRAINER)) && chaosItemCount == 0)
	{
		return;
	}

	LogAdd(LOG_RED, "[ChaosReconnectRollback] Account:%s Name:%s Transaction:%d Interface:%d/%d ChaosItem:%d", lpObj->Account, lpObj->Name, lpObj->Transaction, lpObj->Interface.use, lpObj->Interface.type, chaosItemCount);
	...
	else
	{
		...
		CharacterGameCloseRestoreChaosItems(lpObj);
	}
...
bool CObjectManager::CharacterGameClose(int aIndex) // OK
{
	...
	CharacterGameCloseChaosRollback(lpObj);

#if(CUSTOM_CHOTROI)
	gBCustomChoTroi.RollbackCacheItem(aIndex, false, false, "CharacterGameClose");
#endif

#if(CUSTOM_CHANGEITEM)
	if (lpObj->Interface.use != 0 && lpObj->Interface.type == INTERFACE_CHANGEITEM)
	{
		gCustomChangeItem.SetStateInterface(lpObj->Index);
	}
#endif
...
bool CObjectManager::CharacterGetRespawnLocation(LPOBJ lpObj) // OK
{
	...
	if (lpObj->IsFakeOnline != 0 && lpObj->Socket == INVALID_SOCKET && lpObj->KillerType == 0 && info != 0 && info->ReturnAfterDie != 0 && info->ReturnAfterDieTimeMax <= 0)
	{
		if (gGate.GetGate(info->GateNumber, &gate, &map, &x, &y, &dir, &level) != 0)
		{
			if (s_FakeOnline.GetReturnAfterDiePosition(info, map, &x, &y) != 0)
			{
				lpObj->Map = map;
				lpObj->X = x;
				lpObj->Y = y;
				...
				LogAdd(LOG_BLUE, "[FakeOnline][%s] ReturnAfterDie Map:%d X:%d Y:%d ConfigX:%d ConfigY:%d", lpObj->Name, map, x, y, info->MapX, info->MapY);
				return 1;
			}
...
bool CObjectManager::CharacterUseCustomJewel(LPOBJ lpObj, int SourceSlot, int TargetSlot) // OK
{
	...
	if ((GetLargeRand() % 100) < gCustomJewel.GetCustomJewelSuccessRate(lpObj->Inventory[SourceSlot].m_Index, lpObj->AccountLevel))
	{
		...
		int NewItemIndex = gCustomJewel.GetUpdateInfo(lpObj->Inventory[SourceSlot].m_Index, lpItem->m_Index);

		if (NewItemIndex > 0)
		{
			lpItem->m_Index = NewItemIndex;
		}
...
GCLifeSend(aIndex, 0xFE, (lpObj->MaxLife + lpObj->AddLife), (lpObj->MaxShield + lpObj->AddShield));
GCLifeSend(aIndex, 0xFF, lpObj->Life, lpObj->Shield);
...
bool CObjectManager::CharacterInfoSet(BYTE* aRecv, int aIndex) // OK
{
	...
	lpObj->DiemTichLuyVQ = lpMsg->DiemTichLuyVQ;
	lpObj->NhanThuongTichLuyVQ = lpMsg->NhanThuongTichLuyVQ;
	lpObj->MocResetTichLuyVQ = lpMsg->MocResetTichLuyVQ;
	...
	for (int n = 0; n < INVENTORY_SIZE; n++)
	{
		...
		{
			LogAdd(LOG_RED, "[CharacterInfoSet] Skip invalid inventory item Account:%s Name:%s Slot:%d Index:%d", lpObj->Account, lpObj->Name, n, item.m_Index);
			continue;
		}

		if (gItemManager.InventoryAddItem(aIndex, item, n) == 0xFF)
		{
			if (INVENTORY_WEAR_RANGE(n) != 0)
			{
				lpObj->Inventory[n] = item;
				...
				LogAdd(LOG_RED, "[CharacterInfoSet] Force load wear item Account:%s Name:%s Slot:%d Index:%d", lpObj->Account, lpObj->Name, n, item.m_Index);
			}
			else
			{
				LogAdd(LOG_RED, "[CharacterInfoSet] Cannot load inventory item Account:%s Name:%s Slot:%d Index:%d", lpObj->Account, lpObj->Name, n, item.m_Index);
			}
		}
	}
...
#if (ANTIHACK_GGNEW)
	gAPIGameGuard.ClearCache(aIndex);
#endif

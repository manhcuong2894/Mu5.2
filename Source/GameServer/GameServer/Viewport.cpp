...
static DWORD GetViewportDwordValue(double value)
{
	if (value <= 0.0)
	{
		return 0;
...
#endif
		info.MaxHP = GetViewportDwordValue(lpTarget->MaxLife + lpTarget->AddLife);
		info.CurHP = GetViewportDwordValue(lpTarget->Life);
...
#endif
	info.MaxHP = GetViewportDwordValue(lpObj->MaxLife + lpObj->AddLife);
	info.CurHP = GetViewportDwordValue(lpObj->Life);

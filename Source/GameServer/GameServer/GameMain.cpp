...
{
  if (CreateMutex(0, 0, gServerInfo.m_ServerMutex) == 0 ||
...
void ConnectServerInfoSend() // OK
{
  ...
  pMsg.UserTotal =
      (BYTE)((gObjTotalUser == 0)
                 ? 0
                 : ((gObjTotalUser * 100) / gServerInfo.m_ServerMaxUserNumber));
...
{
  if (gJoinServerConnection.Connect(gServerInfo.m_JoinServerAddress,
                                    (WORD)gServerInfo.m_JoinServerPort,
                                    wMsg) == 0) {
    return 0;
  }
...
{
  if (gDataServerConnection.Connect(gServerInfo.m_DataServerAddress,
                                    (WORD)gServerInfo.m_DataServerPort,
                                    wMsg) == 0) {
    return 0;
  }
...
void CALLBACK QueueTimerCallback(PVOID lpParameter,
                                 BOOLEAN TimerOrWaitFired) // OK
{
  PROTECT_START
  ...
  case QUEUE_TIMER_VIEWPORT:
    ...
    for (int n = OBJECT_START_USER; n < MAX_OBJECT; n++) {
      s_FakeOnline.PickupItem(n);
      s_FakeOnline.ReturnOriginalPosition(n);
    }
#endif
    break;

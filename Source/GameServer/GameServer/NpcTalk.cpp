...
bool CNpcTalk::NpcTalk(LPOBJ lpNpc,LPOBJ lpObj) // OK
{
	...
	if (gEventFindPath.NpcTalk(lpNpc, lpObj))
	{
		return 1;
	}
	...
	if (gCustomChangeItem.Dialog(lpObj, lpNpc))
	{
		return 1;
	}
#endif
	...
	if (GoblinMixExpansion->TalkNpc(lpObj, lpNpc->Class, lpNpc->Map, lpNpc->X, lpNpc->Y))
	{
		return true;
	}
...
void CNpcTalk::NpcCastleSiegeCrownSwitch(LPOBJ lpNpc,LPOBJ lpObj) // OK
{
	...
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GlobalText(427));
		return;
	}
...
void CNpcTalk::NpcCastleSiegeSenior(LPOBJ lpNpc,LPOBJ lpObj) // OK
{
	...
	GCChatTargetSend(lpObj,lpNpc->Index,gMessage.GlobalText(240));
	...
	if(gCastleSiege.GetCastleState() == CASTLESIEGE_STATE_STARTSIEGE)
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GlobalText(241));
		return;
	}

	if(gCastleSiege.CheckGuildOwnCastle(lpObj->GuildName) == 0)
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GlobalText(242));
		return;
	}
	...
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GlobalText(243));
		return;
	}
...
{
	GCChatTargetSend(lpObj,lpNpc->Index,gMessage.GlobalText(514));
...
void CNpcTalk::NpcCastleSiegeGuardsman(LPOBJ lpNpc,LPOBJ lpObj) // OK
{
	...
	if(gCastleSiege.GetCastleState() == CASTLESIEGE_STATE_STARTSIEGE)
	{
		GCChatTargetSend(lpObj,lpNpc->Index,gMessage.GlobalText(244));
		return;
	}
...
void CNpcTalk::NpcGuildMaster(LPOBJ lpNpc,LPOBJ lpObj) // OK
{
	if(lpObj->GuildNumber > 0)
	{
		if (gServerInfo.m_GuildWarehouseEnable == 1)
		{
			...
			{
				gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GlobalText(840));
			}
			else
			{
		...
		{
			gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GlobalText(245));
		}
		return;
	}
	...
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GlobalText(246));
		return;
	}
...
void CNpcTalk::CGNpcTalkRecv(PMSG_NPC_TALK_RECV* lpMsg,int aIndex) // OK
{
	...
	{
		GCChatTargetSend(lpObj,lpNpc->Index,gMessage.GlobalText(861));
	...
	{
		GCChatTargetSend(lpObj,lpNpc->Index,gMessage.GlobalText(247));
		return;
	}

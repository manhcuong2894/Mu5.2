...
void WriteTxt(LPSTR FileName, LPSTR String)
{
	...
	{
		LogAdd(LOG_RED, "Error: Cant Write File: %s", FileName);
	}
	fclose(f);
}
...
CLogToFile::CLogToFile(const LPSTR LogFileName, const LPSTR LogDirectoryName, BOOL bWithDate)
{
	...
	{
		strcpy_s(this->m_szLogFileName, "LOGDATA");
	}
	else
	{
		strcpy_s(this->m_szLogFileName, LogFileName);
	}


	if (!strcmp(LogDirectoryName, ""))
	{
		strcpy_s(this->m_szLogDirectoryName, "LOG");
	}
	else
	{
		strcpy_s(this->m_szLogDirectoryName, LogDirectoryName);
	}

...
void CItemManager::Load(char* filename) // OK
{
	...
	for (pugi::xml_node child = root.child("Section"); child; child = child.next_sibling())
	{
		...
		for (pugi::xml_node Item = child.child("Item"); Item; Item = Item.next_sibling())
		{
			...
			memset(&Item_info, 0, offsetof(ITEM_INFO, Name));
			memset(((char*)&Item_info) + offsetof(ITEM_INFO, Name) + sizeof(std::string), 0,
				sizeof(ITEM_INFO) - offsetof(ITEM_INFO, Name) - sizeof(std::string));
			Item_info.Name.clear();
...
char* CItemManager::GetItemName(int index) // OK
{
	...
	if (it == this->m_ItemInfo.end())
	{
		return const_cast<char*>("No Name");
	}
	else
	{
		return const_cast<char*>(it->second.Name.c_str());
	}
}
...
bool CItemManager::ConvertItemByte(CItem* lpItem, BYTE* lpMsg) // OK
{
	if (lpItem == 0 || lpMsg == 0)
	{
		LogAdd(LOG_RED, "[ConvertItemByte] Invalid pointer lpItem=0x%08X lpMsg=0x%08X", (DWORD)lpItem, (DWORD)lpMsg);
		return 0;
	}
...
void CItemManager::GCItemListSend(int aIndex, bool SendTime) // OK
{
	...
	const int MaxItemListChunkCount = 120;
	const BYTE ItemListFirstSubCode = 0x10;
	const BYTE ItemListAppendSubCode = 0x12;
	bool appendChunk = false;
	bool sentPacket = false;

	#define GC_ITEM_LIST_FLUSH(forceSend) \
		do { \
			if (pMsg.count > 0 || ((forceSend) != false && sentPacket == false)) \
			{ \
				pMsg.header.setE(0xF3, ((appendChunk != false) ? ItemListAppendSubCode : ItemListFirstSubCode), 0); \
	...
	int InventoryMaxValue = INVENTORY_SIZE;

	for (int n = 0; n < InventoryMaxValue; n++)
	{
		CItem* lpItem = &lpObj->Inventory[n];
		...
		if (lpItem->IsItem() != 0 && lpItem->m_ItemExist != 0)
		{
			...
			if (pMsg.count >= MaxItemListChunkCount || (size + sizeof(info)) > sizeof(send))
			{
				GC_ITEM_LIST_FLUSH(false);
			}
	...
	GC_ITEM_LIST_FLUSH(true);

	#undef GC_ITEM_LIST_FLUSH

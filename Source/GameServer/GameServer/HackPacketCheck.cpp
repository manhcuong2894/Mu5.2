...
bool CHackPacketCheck::CheckPacketHack(int aIndex,int index,int value,int encrypt,int serial) // OK
{
	...
	if(lpInfo == 0)
	{
		...
		gLog.Output(LOG_HACK,"[HackPacketCheck][%s][%s] BYPASS CloseClient unknown packet (Index: %d, Value: %d)",gObj[aIndex].Account,gObj[aIndex].Name,index,value);
		return 0;
	}
	...
	if(lpInfo->Encrypt >= 0 && lpInfo->Encrypt != encrypt)
	{
		...
		gLog.Output(LOG_HACK,"[HackPacketCheck][%s][%s] BYPASS CloseClient encryption packet (Index: %d, Value: %d)",gObj[aIndex].Account,gObj[aIndex].Name,index,lpInfo->Value);
		return 0;
	}
	...
	if(serial >= 0 && gSerialCheck[aIndex].CheckSerial(serial) == 0)
	{
		...
		gLog.Output(LOG_HACK,"[HackPacketCheck][%s][%s] BYPASS CloseClient serial packet (Index: %d, Value: %d)",gObj[aIndex].Account,gObj[aIndex].Name,index,lpInfo->Value);
		return 0;
	}
	...
	if(this->CheckPacketHackMinCount(aIndex,index,lpInfo->MinCount) == 0)
	{
		...
		{
			gLog.Output(LOG_HACK,"[HackPacketCheck][%s][%s] BYPASS CloseClient count packet (Index: %d, Value: %d, Count: [%d][%d])",gObj[aIndex].Account,gObj[aIndex].Name,index,lpInfo->Value,gObj[aIndex].HackPacketCount[index],lpInfo->MaxCount);
		}
		return 0;
	}

...
#define INVENTORY_WEAR_RANGE(x)			(((x)<0)?0:((x)>=INVENTORY_WEAR_SIZE)?(((x)>=EQUIPMENT_PENTAGRAM && (x)<=EQUIPMENT_FLAG_NAT)?1:0):1)
#define INVENTORY_FULL_RANGE(x)			(((x)<0)?0:((x)>=INVENTORY_EXT4_SIZE)?(((x)>=EQUIPMENT_PENTAGRAM && (x)<=EQUIPMENT_FLAG_NAT)?1:0):1)
#else
...
struct PMSG_ITEM_MOVE_RECV
{
	...
	// Must match Main wsclientinline.h SendRequestEquipmentItem packet order.
	BYTE SourceFlag;
	BYTE SourceSlot;

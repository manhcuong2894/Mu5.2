...
PART_t &PART_t::operator=(BYTE *value) {
  if (!(value[0] == 0xFF && (value[2] & 0x80) == 0x80 &&
        (value[4] & 0xF0) == 0xF0)) {
...
BYTE OBJECTSTRUCT::GetX() { return X; }

BYTE OBJECTSTRUCT::GetY() { return Y; }

BYTE OBJECTSTRUCT::GetTX() { return TX; }

BYTE OBJECTSTRUCT::GetTY() { return TY; }
...
BYTE OBJECTSTRUCT::GetState() { return State; }

BYTE OBJECTSTRUCT::GetDirection() { return Dir; }

BYTE OBJECTSTRUCT::GetViewState() { return ViewState; }

const char *OBJECTSTRUCT::GetName() { return Name; }
...
void gObjFirstProc() // OK
{
  ...
  for (int n = OBJECT_START_USER; n < MAX_OBJECT; n++) {
    ...
    if ((GetTickCount() - lpObj->OnlineRewardCoin1) >=
        ((DWORD)gServerInfo.m_OnlineRewardCoin1Delay * 60000)) {
      lpObj->OnlineRewardCoin1 = GetTickCount();
      Coin1 = gServerInfo.m_OnlineRewardCoin1Value[lpObj->AccountLevel];
    }

    if ((GetTickCount() - lpObj->OnlineRewardCoin2) >=
        ((DWORD)gServerInfo.m_OnlineRewardCoin2Delay * 60000)) {
      lpObj->OnlineRewardCoin2 = GetTickCount();
      Coin2 = gServerInfo.m_OnlineRewardCoin2Value[lpObj->AccountLevel];
    }

    if ((GetTickCount() - lpObj->OnlineRewardCoin3) >=
        ((DWORD)gServerInfo.m_OnlineRewardCoin3Delay * 60000)) {
  ...
  if (gCloseMsg != 0) {
    if ((--gCloseMsgTime) <= 1) {
      ...
      if ((gCloseMsgTime % 10) == 0) {
        gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GlobalText(481),
                                  gCloseMsgTime);
...
//**************************************************************************//
#if (CUSTOM_CHOTROI)
static void gObjInitChoTroiCache(LPOBJ lpObj) {
  if (lpObj == 0) {
    return;
  }
  ...
  for (int n = 0; n < 3; n++) {
    lpObj->CH_IndexItem[n] = -1;
  }

  for (int n = 0; n < 4; n++) {
    lpObj->CH_InfoItem[n].Clear();
  }
}
#endif
void gObjInit() // OK
...
#if (CUSTOM_CHOTROI)
  gObjInitChoTroiCache(gObj.CommonStruct);
#endif

  gMemoryAllocator.m_TempMemoryAllocatorInfo.Alloc();

  gMemoryAllocator.BindMemoryAllocatorInfo(
      OBJECT_START_USER, gMemoryAllocator.m_TempMemoryAllocatorInfo);
}
...
void gObjSetExperienceTable() // OK
{
  ...
  for (int Character_Level = 1; Character_Level <= MAX_CHARACTER_LEVEL;
       Character_Level++) {
    if (MAX_CHARACTER_LEVEL > 400) {
      float normalized_level =
          static_cast<float>(Character_Level) / MAX_CHARACTER_LEVEL * 400.0f;

      float baseExperience = (9.0f + normalized_level) * normalized_level *
                             normalized_level * 10.0f;

      if (normalized_level > 255.0f) {
        float LevelOver_N = normalized_level - 255.0f;
        baseExperience +=
            (9.0f + LevelOver_N) * LevelOver_N * LevelOver_N * 1000.0f;
      }
      gLevelExperience[Character_Level] = static_cast<DWORD>(baseExperience);
    } else {
      DWORD LevelExperience =
          (9.0f + Character_Level) * Character_Level * Character_Level * 10.0f;
...
void gObjCharZeroSet(int aIndex) // OK
{
...
#if (ANTIHACK_GGNEW)
  lpObj->StatusNotCheck = 0;
  //gAPIGameGuard.ClearCache(lpObj->Index);
#endif
  ...
  memset(lpObj->CommandManagerTransaction, 0,
         sizeof(lpObj->CommandManagerTransaction));
  ...
  lpObj->CItem_StatusMix = 0;
  lpObj->CItem_IndexMix = -1;
...
void gObjCalcExperience(LPOBJ lpObj) // OK
{
  lpObj->Experience =
      ((lpObj->Experience < gLevelExperience[(lpObj->Level - 1)])
           ? gLevelExperience[(lpObj->Level - 1)]
           : lpObj->Experience);

  lpObj->NextExperience = gLevelExperience[(
      (lpObj->Level >= MAX_CHARACTER_LEVEL) ? MAX_CHARACTER_LEVEL
                                            : lpObj->Level)];

  gMasterSkillTree.CalcMasterLevelNextExperience(lpObj);
}

bool gObjGetRandomFreeLocation(int map, int *ox, int *oy, int tx, int ty,
                               int count) // OK
{
  int x = (*ox);
  int y = (*oy);
  ...
  for (int n = 0; n < count; n++) {
    (*ox) =
        ((GetLargeRand() % (tx + 1)) * ((GetLargeRand() % 2 == 0) ? -1 : 1)) +
        x;
    (*oy) =
        ((GetLargeRand() % (ty + 1)) * ((GetLargeRand() % 2 == 0) ? -1 : 1)) +
        y;
...
bool gObjAllocData(int aIndex) // OK
{
  ...
  if (gMemoryAllocator.GetMemoryAllocatorInfo(&MemoryAllocatorInfo, aIndex) ==
      0) {
...
#if (CUSTOM_CHOTROI)
    gObjInitChoTroiCache(gObj.ObjectStruct[aIndex]);
#endif
    ...
    if (OBJECT_MONSTER_RANGE(aIndex) != 0) {
      gObjMonCount =
          (((++gObjMonCount) >= MAX_OBJECT_MONSTER) ? OBJECT_START_MONSTER
                                                    : gObjMonCount);
    }

    if (OBJECT_BOTS_RANGE(aIndex) != 0) {
      gObjBotCount = (((++gObjBotCount) >= MAX_OBJECT_BOTS) ? OBJECT_START_BOTS
                                                            : gObjBotCount);
    }

    if (OBJECT_SUMMON_RANGE(aIndex) != 0) {
      gObjCallMonCount =
          (((++gObjCallMonCount) >= OBJECT_START_USER) ? MAX_OBJECT_MONSTER
                                                       : gObjCallMonCount);
    }

    if (OBJECT_USER_RANGE(aIndex) != 0) {
      gObjCount =
          (((++gObjCount) >= MAX_OBJECT) ? OBJECT_START_USER : gObjCount);
    }
  } else {
    MemoryAllocatorInfo.m_Index = aIndex;
...
short gObjAddSearch(SOCKET socket, char *IpAddress) // OK
{
  ...
  if (gMemoryAllocator.GetMemoryAllocatorFree(&index, OBJECT_START_USER,
                                              MAX_OBJECT, 10000) != 0) {
    return index;
  }
...
short gObjDel(int aIndex) // OK
{
  ...
  if (lpObj->Type == OBJECT_USER) {
    ...
    if (lpObj->Account[0] != 0) {
      LogAddConnect(LOG_ALERT, "[Obj][%d] DelAccount (%s)", aIndex,
                    lpObj->Account);
    #if(ANTIHACK_GGNEW)
      gAPIGameGuard.ClearCache(aIndex, 1);
    #endif
...
{
  return (int)sqrt(pow(((float)lpObj->X - (float)lpTarget->X), 2) +
                   pow(((float)lpObj->Y - (float)lpTarget->Y), 2));
}
//**************************************************************************//
...
bool gObjIsConnectedGP(int aIndex) // OK
{
  ...
  if (lpObj->Type != OBJECT_USER && lpObj->Type != OBJECT_BOTS ||
      (lpObj->CloseCount > 0 || lpObj->MapServerMoveQuit != 0)) {
    return 0;
  }
...
bool gObjIsChangeSkin(int aIndex) // OK
{
  ...
  if (gObj[aIndex].Change == gServerInfo.m_TransformationRing1 ||
      gObj[aIndex].Change == gServerInfo.m_TransformationRing2 ||
      gObj[aIndex].Change == gServerInfo.m_TransformationRing3 ||
      gObj[aIndex].Change == gServerInfo.m_TransformationRing4 ||
      gObj[aIndex].Change == gServerInfo.m_TransformationRing5 ||
      gObj[aIndex].Change == gServerInfo.m_TransformationRing6) {
    return ((gObj[aIndex].ChangeSkin == 0) ? 1 : 0);
  }

  if (gObj[aIndex].Change == 372 || gObj[aIndex].Change == 373 ||
      gObj[aIndex].Change == 374 || gObj[aIndex].Change == 378 ||
      gObj[aIndex].Change == 477 || gObj[aIndex].Change == 503 ||
      gObj[aIndex].Change == 548 || gObj[aIndex].Change == 616 ||
      gObj[aIndex].Change == 617 || gObj[aIndex].Change == 625 ||
      gObj[aIndex].Change == 626 || gObj[aIndex].Change == 642) {
    return ((gObj[aIndex].ChangeSkin == 0) ? 1 : 0);
  }
...
bool gObjCheckResistance(LPOBJ lpObj, int type) // OK
{
  ...
  if ((lpObj->Authority & 32) != 0 &&
      (lpObj->Inventory[10].m_Index == GET_ITEM(13, 42) ||
       lpObj->Inventory[11].m_Index == GET_ITEM(13, 42))) {
    return 1;
  }

  if (gEffectManager.CheckEffect(lpObj, EFFECT_IRON_DEFENSE) != 0 ||
      gEffectManager.CheckEffect(lpObj, EFFECT_IRON_DEFENSE_IMPROVED) != 0) {
    return 1;
  }

  if ((type == 0 || type == 2) &&
      gEffectManager.CheckEffect(lpObj, EFFECT_SOUL_POTION) != 0) {
...
bool gObjCheckTeleportArea(int aIndex, int x, int y) // OK
{
  ...
  if (x < (lpObj->X - 8) || x > (lpObj->X + 8) || y < (lpObj->Y - 8) ||
      y > (lpObj->Y + 8)) {
    return 0;
  }

  if (gMap[lpObj->Map].CheckAttr(lpObj->X, lpObj->Y, 1) != 0 ||
      gMap[lpObj->Map].CheckAttr(x, y, 1) != 0) {
    return 0;
  }
...
bool gObjCheckMapTile(LPOBJ lpObj, int type) // OK
{
  ...
  if (CC_MAP_RANGE(lpObj->Map) != 0 &&
      gChaosCastle.GetState(GET_CC_LEVEL(lpObj->Map)) == CC_STATE_START) {
    return 0;
  }

  for (int x = 0; x < 3; x++) {
    for (int y = 0; y < 3; y++) {
      if (gMap[lpObj->Map].CheckAttr((lpObj->X + x), (lpObj->Y + y), 4) == 0 &&
          gMap[lpObj->Map].CheckAttr((lpObj->X + x), (lpObj->Y + y), 8) == 0) {
        return 0;
      }
    }
  }
...
#if (GAMESERVER_UPDATE >= 802)
  memcpy(lpObj->EventInventoryMap2, lpObj->EventInventoryMap1,
         EVENT_INVENTORY_SIZE);
#endif
...
bool gObjInventoryCommit(int aIndex) // OK
{
  ...
  memcpy(lpObj->EventInventoryMap1, lpObj->EventInventoryMap2,
         EVENT_INVENTORY_SIZE);
...
void gObjInterfaceCheckTime(LPOBJ lpObj) // OK
{
  ...
  if (lpObj->Interface.type == INTERFACE_TRADE && lpObj->Interface.state == 1) {
    if (OBJECT_RANGE(lpObj->TargetNumber) != 0) {
      LPOBJ lpTarget = &gObj[lpObj->TargetNumber];

      if (lpTarget->Interface.type == INTERFACE_TRADE &&
          lpTarget->Interface.state == 1) {
        if (lpObj->Map != lpTarget->Map || lpObj->X < (lpTarget->X - 5) ||
            lpObj->X > (lpTarget->X + 5) || lpObj->Y < (lpTarget->Y - 5) ||
            lpObj->Y > (lpTarget->Y + 5)) {
          gTrade.CloseTrade(lpObj->Index, 0);
        }
      }
    }
  }
...
void ConvertSecondsToTime(int index, int total_seconds, int type) {
  ...
  if (days > 0) {
    if (hours > 0 && minutes > 0 && seconds > 0) {
      sprintf_s(TimePenalty, "%s: %d day(s), %d hour(s), %d m, %d s",
                gMessage.GlobalText(58), days, hours, minutes, seconds);
    } else if (hours > 0 && minutes > 0) {
      sprintf_s(TimePenalty, "%s: %d day(s), %d hour(s), %d m",
                gMessage.GlobalText(58), days, hours, minutes);
    } else if (hours > 0) {
      sprintf_s(TimePenalty, "%s: %d day(s), %d hour(s)",
                gMessage.GlobalText(58), days, hours);
    }
  } else if (hours > 0) {
    if (minutes > 0 && seconds > 0) {
      sprintf_s(TimePenalty, "%s: %d hour(s), %d m, %d s",
                gMessage.GlobalText(58), hours, minutes, seconds);
    } else if (minutes > 0) {
      sprintf_s(TimePenalty, "%s: %d hour(s), %d m", gMessage.GlobalText(58),
                hours, minutes);
    } else if (hours > 0) {
      sprintf_s(TimePenalty, "%s: %d hour(s)", gMessage.GlobalText(58), hours);
    }
  } else if (minutes > 0) {
    sprintf_s(TimePenalty, "%s: %d m, %d s", gMessage.GlobalText(58), minutes,
              seconds);
  } else {
...
void gObjUserDie(LPOBJ lpObj, LPOBJ lpTarget) // OK
{
  ...
  //// 1. L?y rand() % 255 cho t?ng h? màu (C?ng thêm 100 d? màu sáng hon, không
  /// ra màu den sì)
  // int r = 100 + (rand() % 155);
  // int g = 100 + (rand() % 155);
  // int b = 100 + (rand() % 155);
  // DWORD randColor = RGBA(r, g, b, 255);

  // 1. T?o m?ng ch?a 5 màu d?p b?n mu?n (có th? t? blend RGB theo ý thích)
  DWORD colorList[5] = {
      RGBA(255, 0, 0, 255),   // Ð?
      RGBA(0, 255, 0, 255),   // Xanh lá cây (M?c d?nh)
      RGBA(0, 255, 255, 255), // Xanh ng?c (Cyan)
      RGBA(255, 255, 0, 255), // Vàng
      RGBA(255, 128, 0, 255)  // Cam
  };
  // 2. Ch?y ng?u nhiên t? 0 -> 4 d? l?y ra 1 màu
  DWORD randColor = colorList[rand() % 5];

  if (CA_MAP_RANGE(lpObj->Map) != 0) {
    gCustomArena.UserDieProc(lpObj, lpTarget);
    return;
  } else if (DS_MAP_RANGE(lpObj->Map) != 0) {
    if (gServerInfo.m_ShowDSKillLog && lpObj->Type == OBJECT_USER &&
        lpTarget->Type == OBJECT_USER) {
      char Text[200];

      sprintf(Text, gMessage.GlobalText(515), lpTarget->Name, lpObj->Name,
              gMapManager.GetMapName(lpObj->Map));

      gNotice.GCNoticeSendToAll(0xFE, 0, 0, 0, randColor, 0, Text);
    }
    gDevilSquare.UserDieProc(lpObj, lpTarget);
    return;
  } else if (BC_MAP_RANGE(lpObj->Map) != 0) {
    if (gServerInfo.m_ShowBCKillLog && lpObj->Type == OBJECT_USER &&
        lpTarget->Type == OBJECT_USER) {
      char Text[200];

      sprintf(Text, gMessage.GlobalText(517), lpObj->Name);
    ...
    if (gServerInfo.m_ShowCCKillLog && lpObj->Type == OBJECT_USER &&
        lpTarget->Type == OBJECT_USER) {
      char Text[200];
  ...
  if (lpTarget->Type == OBJECT_USER && lpObj->IsFakeOnline == true) {
    ...
    if (info != 0 && info->ReturnAfterDie != 0 && info->ReturnAfterDieTimeMax > 0) {
      int minDelay = info->ReturnAfterDieTimeMin;
      int maxDelay = info->ReturnAfterDieTimeMax;
      DWORD delay = (DWORD)minDelay;

      if (maxDelay > minDelay) {
        delay += (DWORD)(GetLargeRand() % ((maxDelay - minDelay) + 1));
      }

      lpObj->IsFakeReturnAfterDie = true;
      lpObj->IsFakeReturnAfterDieTick = GetTickCount() + delay;

      LogAdd(LOG_BLUE, "[FakeOnline][%s] ReturnAfterDie scheduled in %d ms", lpObj->Name, delay);
    }
  }
#endif
  ...
  if (lpObj->Type == OBJECT_USER && lpTarget->Type == OBJECT_USER) {
    ...
    LogAdd(LOG_BLACK, "[%s][%s] Kill [%s][%s]", lpTarget->Account,
           lpTarget->Name, lpObj->Account, lpObj->Name);
  }

  // Die System User
  if (gServerInfo.m_DieUserSwitch == 1 && lpObj->Type == OBJECT_USER &&
      lpTarget->Type == OBJECT_USER) {
    char Text[200];
    sprintf(Text, gMessage.GlobalText(515), lpObj->Name, lpTarget->Name,
            gMapManager.GetMapName(lpObj->Map));
    // 2. Gán bi?n màu randColor vào hàm Notice
    gNotice.GCNoticeSendToAll(0xFE, 0, 0, 0, randColor, 0, Text);

    // PostMessagePK(lpTarget->Name, gMessage.GlobalText(515), lpObj->Name);
  }
  // Die System Monster
  if (gServerInfo.m_DieMonsterSwitch == 1 && lpObj->Type == OBJECT_USER &&
      lpTarget->Type == OBJECT_MONSTER) {

    char Text[200];
    sprintf(Text, gMessage.GlobalText(516), lpObj->Name, lpTarget->Name,
            gMapManager.GetMapName(lpObj->Map));
      ...
      while (count-- != 0) {
        ...
        if (lpObj->Inventory[number].IsItem() == 1) {
          ...
          if (gItemManager.CGPkDrop(&lpMsg, lpObj->Index) == 1) {
            LogAdd(LOG_USER, "[%s][%s] PK Drop, ItemName: %s", lpObj->Account,
                   lpObj->Name,
        ...
        while (count-- != 0) {
          ...
          if (lpObj->Inventory[number].IsItem() == 1) {
            ...
            if (gItemManager.CGPkDrop(&lpMsg, lpObj->Index) == 1) {
              dropresult = 1;
              LogAdd(
                  LOG_USER, "[%s][%s] Pk Drop, ItemName: %s", lpObj->Account,
                  lpObj->Name,
                  gItemManager.GetItemName(lpObj->Inventory[number].m_Index));
              break;
            }
...
void gObjPlayerKiller(LPOBJ lpObj, LPOBJ lpTarget) // OK
{
  ...
  if (gTvTEvent.CheckPlayerTarget(lpObj) &&
      gTvTEvent.CheckPlayerTarget(lpTarget)) {
    return;
  }

  if (gGvGEvent.CheckPlayerTarget(lpObj) &&
      gGvGEvent.CheckPlayerTarget(lpTarget)) {
    return;
  }
  ...
  if (lpObj->PKLevel > PVP_NEUTRAL) {
    if (lpTarget->PKLevel >
        PVP_CAUTION) //-- si el enemigo es mayor PVP_MURDERER1
    {
      return;
    }
    ...
    if (lpTarget->PKLevel < PVP_MURDERER1) {
      //-- matar a un personaje PVP_CAUTION, PVP_NEUTRAL o PVP_HERO te vuelve
      // PVP_CAUTION
      lpObj->PKCount = 1;
...
BOOL gObjMoveGate(int aIndex, int gate) // OK
{
  ...
  if (lpObj->PShopOpen != 0 &&
      ((lpObj->Map == MAP_CASTLE_SIEGE && map == MAP_LORENCIA) ||
       (lpObj->Map == MAP_LORENCIA && map == MAP_CASTLE_SIEGE))) {
    goto ERROR_JUMP;
  }

  if (lpObj->GensFamily == 0 && gMapManager.GetMapGensBattle(map) != 0) {
    gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0,
                         gMessage.GlobalText(229));
    goto ERROR_JUMP;
  }

  if (BC_MAP_RANGE(map) != 0 || CC_MAP_RANGE(map) != 0 ||
      DS_MAP_RANGE(map) != 0 || IT_MAP_RANGE(map) != 0 ||
      DG_MAP_RANGE(map) != 0 || IG_MAP_RANGE(map) != 0) {
  ...
  if (TargetGate == 97) {
    if (gCastleSiege.GetCastleState() == CASTLESIEGE_STATE_STARTSIEGE) {
      if (lpObj->CsJoinSide != 1 &&
          gCastleSiege.GetCastleTowerAccessable() == 0) {
        goto ERROR_JUMP;
      }
      ...
      if (gCastleSiege.CheckCastleOwnerMember(lpObj->Index) == 0 &&
          gCastleSiege.CheckCastleOwnerUnionMember(lpObj->Index) == 0) {
        goto ERROR_JUMP;
      }
    }
  }
  ...
  if (map == MAP_ATLANS && lpObj->Inventory[8].IsItem() != 0 &&
      (lpObj->Inventory[8].m_Index == GET_ITEM(13, 2) ||
       lpObj->Inventory[8].m_Index == GET_ITEM(13, 3))) // Uniria,Dinorant
  {
    gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, gMessage.GlobalText(274));
    goto ERROR_JUMP;
  }

  if ((map == MAP_ICARUS || map == MAP_KANTURU3) &&
      lpObj->Inventory[7].IsItem() == 0 &&
      lpObj->Inventory[8].m_Index != GET_ITEM(13, 3) &&
  ...
  if (map == MAP_RAKLION2) {
    if (gRaklion.GetRaklionState() == RAKLION_STATE_CLOSE_DOOR ||
        gRaklion.GetRaklionState() == RAKLION_STATE_ALL_USER_DIE ||
        gRaklion.GetRaklionState() == RAKLION_STATE_NOTIFY4 ||
        gRaklion.GetRaklionState() == RAKLION_STATE_END) {
  ...
  if (map == MAP_RAKLION1) {
    if (gate == 292) {
      ...
      gRaklionUtil.NotifyRaklionState(aIndex, gRaklion.GetRaklionState(),
                                      gRaklion.GetRaklionDetailState());
    }
  }
  ...
  if (lpObj->Interface.use != 0 || lpObj->DieRegen != 0) {
    lpObj->State = OBJECT_DELCMD;
    lpObj->RegenOk = 1;
    gMove.GCTeleportSend(aIndex, gate, lpObj->Map, (BYTE)lpObj->X,
                         (BYTE)lpObj->Y, lpObj->Dir);
    return 0;
  }
...
#if(CUSTOM_CHANGEITEM)
  if (lpObj->Interface.use != 0 && lpObj->Interface.type == INTERFACE_CHANGEITEM) {
    gCustomChangeItem.SetStateInterface(aIndex, 0);
  }
#endif

  if (lpObj->Map != map) {
    short NextServerCode = gMapServerManager.CheckMapServerMove(
        aIndex, map, lpObj->LastServerCode);
  ...
  gMove.GCTeleportSend(aIndex, gate, lpObj->Map, (BYTE)lpObj->X, (BYTE)lpObj->Y,
                       lpObj->Dir);
...
void gObjTeleport(int aIndex, int map, int x, int y) // OK
{
  ...
  gMove.GCTeleportSend(aIndex, 1, lpObj->Map, (BYTE)lpObj->X, (BYTE)lpObj->Y,
                       lpObj->Dir);
...
void gObjSummonAlly(LPOBJ lpObj, int map, int x, int y) // OK
{
  ...
  gMove.GCTeleportSend(lpObj->Index, -1, lpObj->Map, (BYTE)lpObj->X,
                       (BYTE)lpObj->Y, lpObj->Dir);
...
void gObjSkillUseProc(LPOBJ lpObj) // OK
{
  ...
  if (lpObj->Type == OBJECT_USER && lpObj->SkillSummonPartyTime > 0) {
    if ((--lpObj->SkillSummonPartyTime) == 0) {
      if (gMoveSummon.CheckMoveSummon(lpObj, lpObj->SkillSummonPartyMap,
                                      lpObj->SkillSummonPartyX,
                                      lpObj->SkillSummonPartyY) == 0) {
        gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0,
                             gMessage.GlobalText(272));
      } else {
        gObjSummonAlly(lpObj, lpObj->SkillSummonPartyMap,
                       lpObj->SkillSummonPartyX, lpObj->SkillSummonPartyY);
      }
    } else {
      gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0,
                           gMessage.GlobalText(273),
...
bool gObjInventorySearchSerialNumber(LPOBJ lpObj, DWORD serial) // OK
{
  ...
  for (int n = 0; n < INVENTORY_SIZE; n++) {
    if (lpObj->Inventory[n].m_Serial != 0 &&
        lpObj->Inventory[n].m_Serial == serial && (count++) > 0) {
      gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0,
                           gMessage.GlobalText(482));
...
bool gObjWarehouseSearchSerialNumber(LPOBJ lpObj, DWORD serial) // OK
{
  ...
  for (int n = 0; n < WAREHOUSE_SIZE; n++) {
    if (lpObj->Warehouse[n].m_Serial != 0 &&
        lpObj->Warehouse[n].m_Serial == serial && (count++) > 0) {
      gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0,
                           gMessage.GlobalText(482));
      gObjUserKill(lpObj->Index);
      return 0;
...
void gObjAddMsgSend(LPOBJ lpObj, int MsgCode, int SendUser, int SubCode) // OK
{
  for (int n = 0; n < MAX_MONSTER_SEND_MSG; n++) {
    if (gSMMsg[lpObj->Index][n].MsgCode == -1) {
      ...
      if (gSMMsg.ObjectStruct[lpObj->Index]->ObjectStruct[n] ==
          gSMMsg.ObjectStruct[lpObj->Index]->CommonStruct) {
        gSMMsg.ObjectStruct[lpObj->Index]->ObjectStruct[n] =
            new MESSAGE_STATE_MACHINE;
      }
...
void gObjAddMsgSendDelay(LPOBJ lpObj, int MsgCode, int SendUser,
                         int MsgTimeDelay, int SubCode) // OK
{
  for (int n = 0; n < MAX_MONSTER_SEND_MSG; n++) {
    if (gSMMsg[lpObj->Index][n].MsgCode == -1) {
      if (gSMMsg.ObjectStruct[lpObj->Index] == gSMMsg.CommonStruct) {
...
void gObjAddAttackProcMsgSendDelay(LPOBJ lpObj, int MsgCode, int SendUser,
                                   int MsgTimeDelay, int SubCode,
                                   int SubCode2) // OK
{
  for (int n = 0; n < MAX_MONSTER_SEND_ATTACK_MSG; n++) {
    if (gSMAttackProcMsg[lpObj->Index][n].MsgCode == -1) {
      if (gSMAttackProcMsg.ObjectStruct[lpObj->Index] ==
          gSMAttackProcMsg.CommonStruct) {
        gSMAttackProcMsg.ObjectStruct[lpObj->Index] =
            new MESSAGE_STATE_ATTACK_MACHINE_COMMON;
      }

      if (gSMAttackProcMsg.ObjectStruct[lpObj->Index]->ObjectStruct[n] ==
          gSMAttackProcMsg.ObjectStruct[lpObj->Index]->CommonStruct) {
        gSMAttackProcMsg.ObjectStruct[lpObj->Index]->ObjectStruct[n] =
            new MESSAGE_STATE_ATTACK_MACHINE;
      }
...
void gObjSecondProc() {
  ...
  for (n = 0; n < MAX_OBJECT; n++) {
...
#endif
        if (lpObj->MonsterDeleteTime != 0 &&
            GetTickCount() >= lpObj->MonsterDeleteTime) {
          gObjDel(lpObj->Index);
          continue;
        }
      }
...
#if (GAMESERVER_TYPE == 1)
        if ((lpObj->Class < 204)   ? FALSE
            : (lpObj->Class > 209) ? FALSE
                                   : TRUE) {
...
#if (GAMESERVER_TYPE == 1)
        if (lpObj->Map == MAP_CASTLE_SIEGE &&
            gCastleSiege.GetCastleState() == CASTLESIEGE_STATE_STARTSIEGE) {
          if ((lpObj->X < 150 || lpObj->X > 200) ||
              (lpObj->Y < 175 || lpObj->Y > 225)) {
            if (lpObj->AccumulatedCrownAccessTime > 0) {
              lpObj->AccumulatedCrownAccessTime =
                  (((lpObj->AccumulatedCrownAccessTime -
                     gServerInfo.m_CastleSiegeDecayAccumulatedTimeValue) < 0)
                       ? 0
                       : (lpObj->AccumulatedCrownAccessTime -
                          gServerInfo.m_CastleSiegeDecayAccumulatedTimeValue));
            }
          }
        }
#endif
        ...
        if (lpObj->PartyNumber >= 0) {
          ...
          if (OBJECT_RANGE(lpObj->CurAutoJoinParty) != 0) {
            if (gParty.IsParty(lpObj->CurAutoJoinParty) != 0) {
              if (gParty.PartyPasswordCheck(lpObj->CurAutoJoinParty,
                                            lpObj->CurAutoJoinPartyPassword) !=
                  0) {
    ...
    if (lpObj->Connected >= OBJECT_LOGGED && lpObj->Type == OBJECT_USER &&
      ...
      if (lpObj->CloseCount == 1) {
        if (lpObj->CloseType == 1) {
          ...
          if (lpObj->Connected == OBJECT_ONLINE) {
            gObjectManager.CharacterGameClose(lpObj->Index);
          }

          if (lpObj->Account[0] != 0) {
            GJDisconnectAccountSend(lpObj->Index, lpObj->Account,
                                    lpObj->IpAddr);
            LogAddConnect(LOG_ALERT, "[Obj][%d] LogoutAccount (%s)",
                          lpObj->Index, lpObj->Account);
            GMAntiSpam->ClearHistory(lpObj->Account);
          }

          if (gServerInfo.m_MaxHWIDConnection > 0 &&
              lpObj->m_HardwareId[0] != '\0') {
            gmHardwareId.RemoveHardwareId(lpObj->m_HardwareId);
            memset(lpObj->m_HardwareId, 0, sizeof(lpObj->m_HardwareId));
          }
        ...
        if (lpObj->CloseType == 3) {
          gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0,
                               gMessage.GlobalText(859),
                               (lpObj->CloseCount - 1));
        } else if (lpObj->CloseType == 4) {
          gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0,
                               gMessage.GlobalText(858),
                               (lpObj->CloseCount - 1));
        } else {
          gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0,
                               gMessage.GlobalText(484),
                               (lpObj->CloseCount - 1));
        }
      }
      lpObj->CloseCount--;
    ...
    if (lpObj->Connected > OBJECT_LOGGED && lpObj->Type == OBJECT_USER) {
      if (GetTickCount() - lpObj->AutoSaveTime > 120000) {
      ...
      if (lpObj->CheckSumTime > 0 &&
          GetTickCount() - lpObj->CheckSumTime > 5000 &&
          lpObj->IsFakeOnline != true) {
#else
      if (lpObj->CheckSumTime > 0 &&
          GetTickCount() - lpObj->CheckSumTime > 5000) {
#endif
        LogAdd(LOG_BLACK, "[%s][%s] CheckSumTime Error", lpObj->Account,
               lpObj->Name);
      ...
      if (lpObj->Type == OBJECT_USER) {
        if (lpObj->Connected >= OBJECT_LOGGED) {
          if (GetTickCount() - lpObj->ConnectTickCount > 60000) {
            ...
            CloseClient(n);
            LogAdd(LOG_BLACK,
                   "Game response error causes conclusion [%d][%s][%s][%s]",
                   lpObj->Index, lpObj->Account, lpObj->Name, lpObj->IpAddr);
          ...
          if (GetTickCount() - lpObj->ConnectTickCount > 90000) {
            CloseClient(n);
            LogAdd(LOG_BLACK,
  ...
  if (lpObj->CheckLifeTime > 0) {
    ...
    if (lpObj->CheckLifeTime <= 0) {
      ...
      if (lpObj->AttackObj != 0) {
        gObjectManager.CharacterLifeCheck(lpObj->AttackObj, lpObj, 0, 1, 0, 0,
                                          0, 0);
      }
    }
  }
...
BOOL gObjBackSpring(LPOBJ lpObj, LPOBJ lpTarget) // OK
{
...
#if (GAMESERVER_TYPE == 1)
  if (lpObj->Class == 277 || lpObj->Class == 283 || lpObj->Class == 288 ||
      lpObj->Class == 278 || lpObj->Class == 215 || lpObj->Class == 216 ||
      lpObj->Class == 217 || lpObj->Class == 218 || lpObj->Class == 219) {
    return FALSE;
  }
  ...
  if (lpObj->Class == 459 || lpObj->Class == 460 || lpObj->Class == 461 ||
      lpObj->Class == 462) {
    return FALSE;
  }

  if (lpObj->Class == 524 || lpObj->Class == 525 || lpObj->Class == 527 ||
      lpObj->Class == 528) {
    return FALSE;
  }
  // GAMBI FIX
  /*
  if(gEffectManager.CheckEffect(lpObj,EFFECT_ICE_ARROW) != 0 ||
  gEffectManager.CheckEffect(lpObj,EFFECT_STERN) != 0 ||
  gEffectManager.CheckEffect(lpObj,EFFECT_SLEEP) != 0)
  {
          return FALSE;
  }
  */
  ...
  {
      //===BProtectMove
      CBAntihackResetCache(lpObj->Index);
  }
#endif
...
#if (GAMESERVER_TYPE == 1)
    tdir = GetPathPacketDirPos(lpObj->X - lpTargetObj->X,
        lpObj->Y - lpTargetObj->Y) *
        2;
#else
    tdir = GetPathPacketDirPos(lpObj->X - lpTargetObj->X,
        lpObj->Y - lpTargetObj->Y) *
...
void gObjCheckSelfDefense(LPOBJ lpObj, int aTargetIndex) {
  ...
  for (n = 0; n < MAX_SELF_DEFENSE; n++) {
    if (gObj[aTargetIndex].SelfDefense[n] >= 0) {
      ...
      if (iPartyNumber >= 0) {
        if (gObj[gObj[aTargetIndex].SelfDefense[n]].PartyNumber ==
            iPartyNumber) {
          return;
        }
      }
  ...
  wsprintf(szTemp, gMessage.GlobalText(485), lpObj->Name,
           gObj[aTargetIndex].Name);
  ...
  // LogAdd(LOG_BLACK,"[%s][%s] Set SelfDefence
  // [%s][%s]",lpObj->Account,lpObj->Name,gObj[aTargetIndex].Account,gObj[aTargetIndex].Name);
}
  ...
  for (int n = 0; n < MAX_SELF_DEFENSE; n++) {
    if (lpObj->SelfDefense[n] >= 0) {
      if (GetTickCount() > lpObj->SelfDefenseTime[n]) {
        ...
        // lpObj->Account, lpObj->Name, gObj[lpObj->SelfDefense[n]].Account,
        // gObj[lpObj->SelfDefense[n]].Name);
        lpObj->SelfDefense[n] = -1;
      }
    }
  }
...
void gObjGuildWarEndSend(LPOBJ lpObj, BYTE Result1, BYTE Result2) {
  ...
  for (int n = 0; n < MAX_GUILD_USER; n++) {
    if (lpObj->Guild->Use[n] && lpObj->Guild->Index[n] >= 0) {
      ...
      gGuild.GCGuildWarEndSend(lpObj->Guild->Index[n], Result1,
                               lpObj->Guild->TargetGuildNode->Name);
    }
  }

  for (int n = 0; n < MAX_GUILD_USER; n++) {
    if (lpObj->Guild->TargetGuildNode->Use[n] &&
        lpObj->Guild->TargetGuildNode->Index[n] >= 0) {
      gObj[lpObj->Guild->TargetGuildNode->Index[n]].IsInBattleGround = 0;
      gGuild.GCGuildWarEndSend(lpObj->Guild->TargetGuildNode->Index[n], Result2,
                               lpObj->Guild->Name);
    }
  }
}

void gObjGuildWarEndSend(GUILD_INFO_STRUCT *lpGuild1,
                         GUILD_INFO_STRUCT *lpGuild2, BYTE Result1,
                         BYTE Result2) {
...
void gObjGuildWarEnd(GUILD_INFO_STRUCT *lpGuild,
...
BOOL gObjGuildWarProc(GUILD_INFO_STRUCT *lpGuild1, GUILD_INFO_STRUCT *lpGuild2,
                      int score) {
  int totalscore;
  int n;
  int maxscore;
  ...
  wsprintf(szTemp, "%s ( %d ) VS %s ( %d )", lpGuild1->Name,
           lpGuild1->PlayScore, lpGuild2->Name, lpGuild2->PlayScore);
  LogAdd(LOG_BLACK, szTemp);

  if (lpGuild1->WarType == 1) {
    SetBattleTeamScore(lpGuild1->BattleGroundIndex, lpGuild1->BattleTeamCode,
                       lpGuild1->PlayScore);
    maxscore = 100;
  } else {
    maxscore = 20;
...
int gObjGuildWarItemGive(GUILD_INFO_STRUCT *lpWinGuild,
                         GUILD_INFO_STRUCT *lpLoseGuild) {
  return true;
  int n;
  ...
  while (count--) {
    ...
    if (lpObj->Inventory[number].IsItem() == 1) {
      if (gItemManager.InventoryInsertItem(win_user,
                                           lpObj->Inventory[number]) != 0xFF) {
        return true;
      }
      break;
    }
...
int gObjGetGuildUnionNumber(LPOBJ lpObj) {
  ...
  if (lpObj->Guild != NULL) {
    iUnion = (lpObj->Guild->GuildUnion == 0) ? lpObj->Guild->Number
                                             : lpObj->Guild->GuildUnion;
  }
...
BOOL gObjCheckRival(LPOBJ lpObj, LPOBJ lpTargetObj) {
  ...
  int iUnion = !lpObj->Guild->GuildUnion ? lpObj->Guild->Number
                                         : lpObj->Guild->GuildUnion;

  if (gUnionManager.GetGuildRelationShip(iUnion, lpTargetObj->Guild->Number) ==
      2) {
    return true;
  }
  return false;
...
void gObjNotifyUpdateUnionV1(LPOBJ lpObj) {
  ...
  PMSG_UNION_VIEWPORT_NOTIFY_COUNT *lpMsg =
      (PMSG_UNION_VIEWPORT_NOTIFY_COUNT *)&cBUFFER_V1;
  PMSG_UNION_VIEWPORT_NOTIFY *lpMsgBody =
      (PMSG_UNION_VIEWPORT_NOTIFY
           *)&cBUFFER_V1[sizeof(PMSG_UNION_VIEWPORT_NOTIFY_COUNT)];
  ...
  for (int n = 0; n < MAX_VIEWPORT; n++) {
    ...
    if (lpObj->VpPlayer[n].state == 2 &&
        lpObj->VpPlayer[n].type == OBJECT_USER) {
      ...
      gObjGetGuildUnionName(lpTargetObj, lpMsgBody[iVp1Count].szUnionName,
                            sizeof(lpMsgBody[iVp1Count].szUnionName));

      lpMsgBody[iVp1Count].btGuildRelationShip =
          gObjGetRelationShip(lpObj, lpTargetObj);
  ...
  if (iVp1Count > 0 && iVp1Count <= MAX_VIEWPORT) {
    ...
    lpMsg->h.set(0x67, iVp1Count * sizeof(PMSG_UNION_VIEWPORT_NOTIFY) +
                           sizeof(PMSG_UNION_VIEWPORT_NOTIFY_COUNT));

    DataSend(lpObj->Index, (LPBYTE)lpMsg,
             ((lpMsg->h.size[1] & 0xFF) & 0xFF |
              ((lpMsg->h.size[0] & 0xFF) & 0xFF) << 8) &
                 0xFFFF);
  } else {
    if (iVp1Count != 0) {
      LogAdd(LOG_BLACK,
...
void gObjNotifyUpdateUnionV2(LPOBJ lpObj) {
  ...
  PMSG_UNION_VIEWPORT_NOTIFY_COUNT *lpMsg2 =
      (PMSG_UNION_VIEWPORT_NOTIFY_COUNT *)&cBUFFER_V2;
  PMSG_UNION_VIEWPORT_NOTIFY *lpMsgBody2 =
      (PMSG_UNION_VIEWPORT_NOTIFY
           *)&cBUFFER_V2[sizeof(PMSG_UNION_VIEWPORT_NOTIFY_COUNT)];

  gObjGetGuildUnionName(lpObj, lpMsgBody2->szUnionName,
                        sizeof(lpMsgBody2->szUnionName));
  ...
  DataSend(lpObj->Index, (LPBYTE)lpMsg2,
           ((lpMsg2->h.size[1] & 0xFF) & 0xFF |
            ((lpMsg2->h.size[0] & 0xFF) & 0xFF) << 8) &
               0xFFFF);
  ...
  for (int n = 0; n < MAX_VIEWPORT; n++) {
    if (lpObj->VpPlayer2[n].type == OBJECT_USER &&
      ...
      if (lpTargetObj->Guild != 0) {
        lpMsgBody2->btGuildRelationShip =
            gObjGetRelationShip(lpTargetObj, lpObj);
      }

      if (lpMsgBody2->btGuildRelationShip != 1) {
        DataSend(lpObj->VpPlayer2[n].index, (LPBYTE)lpMsg2,
                 ((lpMsg2->h.size[1] & 0xFF) & 0xFF |
                  ((lpMsg2->h.size[0] & 0xFF) & 0xFF) << 8) &
                     0xFFFF);
      }
...
bool gObjRebuildMasterSkillTree(LPOBJ lpObj) {
  ...
  for (int n = 0; n < MAX_SKILL_LIST; n++) {
    if (lpObj->Skill[n].IsMasterSkill() != 0) {
      ...
      lpObj->Skill[n].m_DamageMin =
          gSkillManager.GetSkillDamage(lpObj->Skill[n].m_skill);

      lpObj->Skill[n].m_DamageMax =
          lpObj->Skill[n].m_DamageMin + (lpObj->Skill[n].m_DamageMin / 2);
    }
  }
  ...
  if (gServerInfo.m_AnnounceChaosMix != 0) {
    ...
    if (mod == 0) {
      if (gItemManager.GetInfo(index, &ItemInfo) != 0 &&
          gServerInfo.m_AnnounceChaosMixSuccess[0] != '\0') {
        wsprintf(buff, gServerInfo.m_AnnounceChaosMixSuccess, lpObj->Name,
                 ItemInfo.GetName(), 10 + type);
      }
    } else if (mod == 1) {
      if (gItemManager.GetInfo(index, &ItemInfo) != 0 &&
          gServerInfo.m_AnnounceChaosMixFailed[0] != '\0') {
        wsprintf(buff, gServerInfo.m_AnnounceChaosMixFailed, lpObj->Name,
                 ItemInfo.GetName(), 10 + type);
      }
    }
...
bool gObjCheckAutoParty(LPOBJ lpObj, LPOBJ lpTarget) {
  ...
  if (lpTarget->AutoPartyPassword[0] != 0 &&
      strcmp(lpObj->AutoPartyPassword, lpTarget->AutoPartyPassword) != 0) {
    return 0;
  }
...
bool gObjCheckEventJoin(LPOBJ lpObj) {
  if (KALIMA_MAP_RANGE(lpObj->Map) != 0 || CA_MAP_RANGE(lpObj->Map) != 0 ||
      DS_MAP_RANGE(lpObj->Map) != 0 || BC_MAP_RANGE(lpObj->Map) != 0 ||
      CC_MAP_RANGE(lpObj->Map) != 0 || IT_MAP_RANGE(lpObj->Map) != 0 ||
      DA_MAP_RANGE(lpObj->Map) != 0 || DG_MAP_RANGE(lpObj->Map) != 0 ||
      IG_MAP_RANGE(lpObj->Map) != 0 || lpObj->Map == MAP_CASTLE_SIEGE ||
      lpObj->Map == MAP_LAND_OF_TRIALS) {
    return 1;
  }

...
void CQuestWorld::CGQuestWorldDetailRecv(PMSG_QUEST_WORLD_DETAIL_RECV* lpMsg,int aIndex) // OK
{
	...
	{
		QUEST_WORLD_OBJECTIVE obj;
		if(gQuestWorldObjective.GetQuestWorldObjective(lpObj,lpInfo->Index,lpInfo->Group,&obj,n) != 0)
		{
			memcpy(&pMsg.QuestObjective[pMsg.ObjectiveCount],&obj,sizeof(obj));
			pMsg.ObjectiveCount++;
		}
	}
	for(int n=pMsg.ObjectiveCount;n < MAX_QUEST_WORLD_OBJECTIVE_COUNT;n++)
	{
	...
	for(int n=0;n < MAX_QUEST_WORLD_OBJECTIVE_COUNT;n++)
	{
		QUEST_WORLD_REWARD reward;
		if(gQuestWorldReward.GetQuestWorldReward(lpObj,lpInfo->Index,lpInfo->Group,&reward,n) != 0)
		{
			memcpy(&pMsg.QuestReward[pMsg.RewardCount],&reward,sizeof(reward));
			pMsg.RewardCount++;
		}
	}
	for(int n=pMsg.RewardCount;n < MAX_QUEST_WORLD_OBJECTIVE_COUNT;n++)
	{
		memset(&pMsg.QuestReward[n],0,sizeof(pMsg.QuestReward[n]));
	}

	LogAdd(LOG_BLUE, "[DebugUI] UI Req! Packed %d Rewards for Index %d Group %d", pMsg.RewardCount, pMsg.QuestIndex, pMsg.QuestGroup);
...
void CQuestWorld::CGQuestWorldDetailSend(int aIndex, int Index, int Group)
{
	...
	{
		QUEST_WORLD_OBJECTIVE obj;
		if (gQuestWorldObjective.GetQuestWorldObjective(lpObj, Index, Group, &obj, n) != 0)
		{
			memcpy(&pMsg.QuestObjective[pMsg.ObjectiveCount], &obj, sizeof(obj));
			pMsg.ObjectiveCount++;
		}
	...
	{
		QUEST_WORLD_REWARD reward;
		if (gQuestWorldReward.GetQuestWorldReward(lpObj, Index, Group, &reward, n) != 0)
		{
			memcpy(&pMsg.QuestReward[pMsg.RewardCount], &reward, sizeof(reward));
			pMsg.RewardCount++;
		}
...
void CQuestWorld::GCQuestWorldDetailSend(LPOBJ lpObj,int QuestIndex,int QuestGroup,int QuestOption) // OK
{
	...
	{
		QUEST_WORLD_OBJECTIVE obj;
		if(gQuestWorldObjective.GetQuestWorldObjective(lpObj,QuestIndex,QuestGroup,&obj,n) != 0)
		{
			memcpy(&pMsg.QuestObjective[pMsg.ObjectiveCount],&obj,sizeof(obj));
			pMsg.ObjectiveCount++;
		}
	...
	//LogAdd(LOG_BLUE, "[DebugUI] UI Req! Packed %d Objectives for Index %d Group %d", pMsg.ObjectiveCount, QuestIndex, QuestGroup);

	for(int n=0;n < MAX_QUEST_WORLD_OBJECTIVE_COUNT;n++)
	{
		QUEST_WORLD_REWARD reward;
		if(gQuestWorldReward.GetQuestWorldReward(lpObj,QuestIndex,QuestGroup,&reward,n) != 0)
		{
			memcpy(&pMsg.QuestReward[pMsg.RewardCount],&reward,sizeof(reward));
			pMsg.RewardCount++;
		}

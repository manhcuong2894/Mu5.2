...
void CDevilSquare::ProcState_EMPTY(DEVIL_SQUARE_LEVEL* lpLevel) // OK
{
	if (lpLevel->RemainTime > 0 && lpLevel->RemainTime <= (this->m_WarningTime * 60))
	{
		...
		if (lpLevel->MinutesLeft != minutes)
		{
			...
			if (lpLevel->Level == 0)
			{
				gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GlobalText(144), (lpLevel->MinutesLeft + 1));
			}
		}
	...
	if (lpLevel->RemainTime <= 0)
	{
		if (lpLevel->Level == 0)
		{
			gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GlobalText(145));
		}
		this->NoticeSendToAll(lpLevel, 1, gMessage.GlobalText(146), (lpLevel->Level + 1), this->m_NotifyTime);
...
void CDevilSquare::ProcState_STAND(DEVIL_SQUARE_LEVEL* lpLevel) // OK
{
	...
	if (lpLevel->RemainTime <= 0)
	{
		this->NoticeSendToAll(lpLevel, 0, gMessage.GlobalText(147));
...
void CDevilSquare::ProcState_START(DEVIL_SQUARE_LEVEL* lpLevel) // OK
{
	...
	if (lpLevel->RemainTime <= 0)
	{
		this->NoticeSendToAll(lpLevel, 0, gMessage.GlobalText(148));
...
int CDevilSquare::GetUserAbleLevel(LPOBJ lpObj) // OK
{
	...
	if (lpObj->Class == CLASS_MG || lpObj->Class == CLASS_DL || lpObj->Class == CLASS_RF)
	{
		if (lpObj->ChangeUp < 2)
		{
			if (lpObj->Level >= 10 && lpObj->Level <= NORMALIZE_LEVEL(111) - 1)
			{
				level = 0;
			}
	...
	else
	{
		if (lpObj->ChangeUp < 2)
		{
			if (lpObj->Level >= 15 && lpObj->Level <= NORMALIZE_LEVEL(131) - 1)
			{
				level = 0;
			}
...
void CDevilSquare::NpcCharon(LPOBJ lpNpc, LPOBJ lpObj) // OK
{
	...
	if (DS_LEVEL_RANGE(level) == 0)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GlobalText(149));
		return;
	}

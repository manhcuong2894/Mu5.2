		...
		for (int botNum = 0; botNum < MAX_BOTBUFFER; botNum++)
		{
			if (this->bot[botNum].Enabled == 1)
			{
				...
				if (result >= 0)
				{
					...
					gObj[result].Mana = gObj[result].MaxMana;
...
BOOL ObjBotBuffer::TradeOpen(int index, int nindex)
{
	...
	{
		gNotice.NewMessageDevTeam(lpObj->Index, gMessage.GlobalText(521), this->bot[number].Name);
		return 1;
	}
	if (this->bot[number].OnlyVip != 0 && lpObj->AccountLevel == 0)
	{
		gNotice.NewMessageDevTeam(gObj[nindex].Index, gMessage.GlobalText(522), this->bot[number].Name);
		return 1;
	}
	if ((DWORD)this->bot[number].Zen > gObj[index].Money)
	{
		gNotice.NewMessageDevTeam(gObj[index].Index, gMessage.GlobalText(523), this->bot[number].Name, this->bot[number].Zen);
		return 1;
	}
	if (this->bot[number].PCPoints > gObj[index].PCPoint)
	{
		gNotice.NewMessageDevTeam(gObj[index].Index, gMessage.GlobalText(524), this->bot[number].Name, this->bot[number].PCPoints);
		return 1;
	}
	if (this->bot[number].ActiveGensFamily == 1)
	{
		...
		{
			gNotice.NewMessageDevTeam(gObj[index].Index, gMessage.GlobalText(525), this->bot[number].Name);
			return 1;
		}
		if (this->bot[number].GensFamily != GENS_FAMILY_DUPRIAN && gObj[index].GensFamily == GENS_FAMILY_NONE)
		{
			gNotice.NewMessageDevTeam(gObj[index].Index, gMessage.GlobalText(526), this->bot[number].Name);
			return 1;
		}
		if (this->bot[number].GensFamily == GENS_FAMILY_VARNERT && gObj[index].GensFamily == GENS_FAMILY_DUPRIAN)
		{
			gNotice.NewMessageDevTeam(gObj[index].Index, gMessage.GlobalText(527), this->bot[number].Name);
			return 1;
		}
		if (this->bot[number].GensFamily == GENS_FAMILY_DUPRIAN && gObj[index].GensFamily == GENS_FAMILY_VARNERT)
		{
			gNotice.NewMessageDevTeam(gObj[index].Index, gMessage.GlobalText(528), this->bot[number].Name);
			return 1;
		}
		if (this->bot[number].ContributionGens > gObj[index].GensContribution)
		{
			gNotice.NewMessageDevTeam(gObj[index].Index, gMessage.GlobalText(529), this->bot[number].Name, this->bot[number].ContributionGens);
	...
	for (int x = 0; x < this->bot[number].SkillCount; x++)
	{
		...
		case SKILL_MANA_SHIELD:
		{
			if (lpObj->Class == CLASS_DW)
			{
				...
				gSkillManager.GCSkillAttackSend(lpBot, SKILL_GREATER_MANA, index, 1);
			}
		...
		case SKILL_GREATER_LIFE:
		{
			if (lpObj->Class == CLASS_DK)
			{
				...
				gSkillManager.GCSkillAttackSend(lpBot, SKILL_GREATER_LIFE, index, 1);
			}
		...
		case SKILL_GREATER_DEFENSE:
		{
			if (lpObj->Class == CLASS_FE)
			{
				...
				gSkillManager.GCSkillAttackSend(lpBot, SKILL_GREATER_DEFENSE, index, 1);
		...
		case SKILL_GREATER_DAMAGE:
		{
			if (lpObj->Class == CLASS_FE)
			{
				...
				gSkillManager.GCSkillAttackSend(lpBot, SKILL_GREATER_DAMAGE, index, 1);
			}
		...
		case SKILL_GREATER_CRITICAL_DAMAGE:
		{
			if (lpObj->Class == CLASS_DL)
			{
				...
				gSkillManager.GCSkillAttackSend(lpBot, SKILL_GREATER_CRITICAL_DAMAGE, index, 1);
			}
		...
		case SKILL_DAMAGE_REFLECT:
		{
			...
			{
				GCActionSend(lpBot, ACTION_RESPECT1, lpBot->Index, nindex);
				gSkillManager.GCSkillAttackSend(lpBot, SKILL_DAMAGE_REFLECT, index, 1);
		...
		case EFFECT_GREATER_DEFENSE_SUCCESS_RATE:
		{
			...
			gEffectManager.AddEffect(lpObj, 1, EFFECT_GREATER_DEFENSE_SUCCESS_RATE, this->bot[number].skill[x].time * 60, this->bot[number].skill[x].power, 0, 0, 0);

			//LogAdd(LOG_BLACK, "[BotBuffer] CharName: %s - EFFECT_GREATER_DEFENSE_SUCCESS_RATE SecondSkillTime: %d", lpObj->Name, this->bot[number].skill[x].time);
		}
		break;
		}

		gNotice.NewNoticeSend(lpObj->Index, 0, 0, 0, 0, 0, gMessage.GlobalText(530), this->bot[number].Name, this->bot[number].skill[x].time);
	}
	...
	if (this->bot[number].Zen > 0)
	{
		...
		gNotice.NewNoticeSend(gObj[index].Index, 0, 0, 0, 0, 0, gMessage.GlobalText(531), this->bot[number].Name, this->bot[number].Zen);
	}
	...
	if (this->bot[number].PCPoints > 0)
	{
		...
		gNotice.NewNoticeSend(gObj[index].Index, 0, 0, 0, 0, 0, gMessage.GlobalText(532), this->bot[number].Name, gObj[index].PCPoint);
	}

	gNotice.NewMessageDevTeam(lpObj->Index, gMessage.GlobalText(533), this->bot[number].Name);
	return 1;
}

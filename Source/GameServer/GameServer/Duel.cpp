...
bool CDuel::AddDuelArena(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	for(int n=0;n < MAX_DUEL_ARENA;n++)
	{
		...
		if(lpInfo->State == DA_STATE_EMPTY)
		{
			...
			if(gServerInfo.m_DuelArenaAnnounceSwitch != 0){gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GlobalText(174),lpObj->Name,lpTarget->Name);}
...
void CDuel::CheckDuelScore(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	if(lpObj->DuelScore >= gServerInfo.m_DuelMaxScore)
	{
		...
		if(gServerInfo.m_DuelArenaAnnounceSwitch != 0){gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GlobalText(175),lpObj->Name,lpTarget->Name);}
...
GCLifeSend(lpTarget->Index,0xFF,lpTarget->Life,lpTarget->Shield);
...
void CDuel::CGDuelStartRecv(PMSG_DUEL_START_RECV* lpMsg,int aIndex) // OK
{
	...
	{
		gNotice.GCNoticeSend(aIndex,1,0,0,0,0,0,gMessage.GlobalText(160));
		return;
	}
	...
	{
		gNotice.GCNoticeSend(aIndex,1,0,0,0,0,0,gMessage.GlobalText(161));
		return; 
	}

	if((lpTarget->Option & 2) == 0)
	{
		gNotice.GCNoticeSend(aIndex,1,0,0,0,0,0,gMessage.GlobalText(162));
		return;
	}

	if(OBJECT_RANGE(lpObj->DuelUserReserved) != 0 || OBJECT_RANGE(lpObj->DuelUserRequested) != 0 || OBJECT_RANGE(lpObj->DuelUser) != 0)
	{
		gNotice.GCNoticeSend(aIndex,1,0,0,0,0,0,gMessage.GlobalText(163));
		return;
	}

	if(OBJECT_RANGE(lpTarget->DuelUserReserved) != 0 || OBJECT_RANGE(lpTarget->DuelUserRequested) != 0 || OBJECT_RANGE(lpTarget->DuelUser) != 0)
	{
		gNotice.GCNoticeSend(aIndex,1,0,0,0,0,0,gMessage.GlobalText(164));
		return;
	}

	if(lpObj->Guild != 0 && lpObj->Guild->WarState == 1)
	{
		gNotice.GCNoticeSend(aIndex,1,0,0,0,0,0,gMessage.GlobalText(165));
		return;
	}

	if(lpTarget->Guild != 0 && lpTarget->Guild->WarState == 1)
	{
		gNotice.GCNoticeSend(aIndex,1,0,0,0,0,0,gMessage.GlobalText(166));
		return;
	}
	...
	{
		gNotice.GCNoticeSend(aIndex,1,0,0,0,0,0,gMessage.GlobalText(632), 50);
		return;
	}
	...
	{
		gNotice.GCNoticeSend(aIndex,1,0,0,0,0,0,gMessage.GlobalText(167));
		return;
	}
	...
	{
		gNotice.GCNoticeSend(aIndex,1,0,0,0,0,0,gMessage.GlobalText(168));
		return;
	}
	...
	{
		gNotice.GCNoticeSend(aIndex,1,0,0,0,0,0,gMessage.GlobalText(169));
		return;
	}
	...
	for(int n=0;n < MAX_SELF_DEFENSE;n++)
	{
		...
		{
			gNotice.GCNoticeSend(aIndex,1,0,0,0,0,0,gMessage.GlobalText(170));
			return;
		}

		if(lpTarget->SelfDefense[n] >= 0)
		{
			gNotice.GCNoticeSend(aIndex,1,0,0,0,0,0,gMessage.GlobalText(171));
			return;
		}
	}
...
void CDuel::CGDuelOkRecv(PMSG_DUEL_OK_RECV* lpMsg,int aIndex) // OK
{
	...
	if(lpMsg->flag == 0)
	{
		gNotice.GCNoticeSend(aIndex,1,0,0,0,0,0,gMessage.GlobalText(172));
		this->ResetDuel(lpObj);
		this->ResetDuel(lpTarget);
	...
	{
		gNotice.GCNoticeSend(aIndex,1,0,0,0,0,0,gMessage.GlobalText(173));
		this->ResetDuel(lpObj);
		this->ResetDuel(lpTarget);
		this->GCDuelStartSend(bIndex,0,aIndex);
		return;
...
void CDuel::StartDuelBit(LPOBJ lpObj, LPOBJ lpTarget)
{
	...
	if(this->AddDuelArena(lpObj,lpTarget) == 0)
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GlobalText(173));
		this->ResetDuel(lpObj);
		this->ResetDuel(lpTarget);

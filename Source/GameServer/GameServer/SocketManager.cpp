...
bool CSocketManager::DataRecv(int index, IO_MAIN_BUFFER* lpIoBuffer) // OK
{
	...
	while (true)
	{
		...
		{
		#if (ANTIHACK_GGNEW)
			BYTE subcode = ((header == 0xC1 || header == 0xC3) ? ((size > 3) ? lpMsg[count + 3] : 0) : ((size > 4) ? lpMsg[count + 4] : 0));
			if (gAPIGameGuard.Tracking(index, header, head, subcode))
			{
				return 0;
			}
		#endif
...
DWORD WINAPI CSocketManager::ServerAcceptThread(CSocketManager* lpSocketManager) // OK
{
	...
	while (true)
	{
		...
		lpSocketManager->m_critical.lock();
	#if(ANTIHACK_GGNEW) 
		CBAntihackAddIpAddress(inet_ntoa(SocketAddr.sin_addr));

		if (CBAntihackCheckFlood(inet_ntoa(SocketAddr.sin_addr)) == 0)
		{
			closesocket(socket);
			lpSocketManager->m_critical.unlock();
			continue;
		...
		gLog.Output(LOG_CONNECT, "[SocketManager] Client accepted (Index: %d, IP: %s)", index, inet_ntoa(SocketAddr.sin_addr));

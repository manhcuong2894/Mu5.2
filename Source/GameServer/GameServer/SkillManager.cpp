...
void CSkillManager::Load(char* path) // OK
{
	...
	try
	{
		while (true)
		{
			...
			info.MasterType = lpMemScript->GetAsNumber();
...
void CSkillManager::ExportBMD(std::string filename)
{
	int MaxLine = 650;
	int Size = sizeof(SKILL_ATTRIBUTE);

	SKILL_ATTRIBUTE* m_SkillAttribute = new SKILL_ATTRIBUTE[MaxLine];

	memset(m_SkillAttribute, 0, (sizeof(SKILL_ATTRIBUTE) * MaxLine));

	for (int i = 0; i < MaxLine; i++)
	{
		SKILL_INFO info;
		
		if (this->GetInfo(i, &info) == false)
			continue;

		//strcpy_s(m_SkillAttribute[i].Name, info.Name);

		m_SkillAttribute[i].Damage = info.Damage;

		m_SkillAttribute[i].Mana = info.Mana;

		m_SkillAttribute[i].AbilityGuage = info.BP;

		m_SkillAttribute[i].Distance = info.Range;

		m_SkillAttribute[i].Delay = info.Delay;

		m_SkillAttribute[i].Energy = info.RequireEnergy;

		m_SkillAttribute[i].Charisma = info.RequireLeadership;

		m_SkillAttribute[i].MasteryType = info.MasterType;

		m_SkillAttribute[i].KillCount = info.RequireKillCount;

		//m_SkillAttribute[i].Level = info.Level;

		m_SkillAttribute[i].SkillUseType;

		m_SkillAttribute[i].SkillBrand;

		m_SkillAttribute[i].SkillRank;

		m_SkillAttribute[i].Magic_Icon;

		m_SkillAttribute[i].TypeSkill;

		m_SkillAttribute[i].Strength = 0;

		m_SkillAttribute[i].Dexterity = 0;

		m_SkillAttribute[i].ItemSkill;

		m_SkillAttribute[i].IsDamage;

		m_SkillAttribute[i].Effect = info.Effect;

		if (info.RequireGuildStatus == G_MASTER)
			m_SkillAttribute[i].RequireDutyClass[0] = 1;

		if (info.RequireGuildStatus == G_SUB_MASTER)
			m_SkillAttribute[i].RequireDutyClass[1] = 1;

		if (info.RequireGuildStatus == G_BATTLE_MASTER)
			m_SkillAttribute[i].RequireDutyClass[2] = 1;

		for (int j = 0; j < 7; j++)
		{
			m_SkillAttribute[i].RequireClass[j] = info.RequireClass[j];
		}
	}

	PackFileEncrypt(filename.c_str(), (BYTE*)m_SkillAttribute, MaxLine, Size, 0xA2F1, false);

	if (m_SkillAttribute)
	{
		delete[] m_SkillAttribute;
	}
}
...
{
	if (index == GET_ITEM(12, 11))
	{
		return (SKILL_SUMMON1 + level);
	}

	return gItemManager.AdquireSkill(index);
...
bool CSkillManager::SkillHeal(int aIndex, int bIndex, CSkill* lpSkill) // OK
{
	...
	{
		GCLifeSend(bIndex, 0xFF, lpTarget->Life, lpTarget->Shield);
	}
...
bool CSkillManager::SkillDecay(int aIndex, int bIndex, CSkill* lpSkill, BYTE x, BYTE y, bool combo) // OK
{
	...
	if (OBJECT_RANGE(bIndex) != 0 && (this->CheckSkillRange(lpSkill->m_index, lpObj->X, lpObj->Y, x, y) == 0 || this->CheckSkillRadio(lpSkill->m_index, x, y, gObj[bIndex].X, gObj[bIndex].Y) == 0))
	{
		x = gObj[bIndex].X;
		y = gObj[bIndex].Y;
	}
...
bool CSkillManager::SkillSummonParty(int aIndex, int bIndex, CSkill* lpSkill) // OK
{
	...
	if (OBJECT_RANGE(lpObj->PartyNumber) != 0)
	{
		...
		for (int n = 0; n < MAX_PARTY_USER; n++)
		{
			...
			{
				gNotice.GCNoticeSend(index, 1, 0, 0, 0, 0, 0, gMessage.GlobalText(272));
				continue;
			}
			...
			if (gObjGetRandomFreeLocation(lpObj->Map, &px, &py, 4, 4, 10) == 0)
			{
				...
				gNotice.GCNoticeSend(index, 1, 0, 0, 0, 0, 0, gMessage.GlobalText(273), gObj[index].SkillSummonPartyTime);
			}
			else
			{
				gObj[index].SkillSummonPartyTime = 5;

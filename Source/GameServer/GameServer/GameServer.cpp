...
#if(ANTIHACK_GGNEW)
static void LogAPIGameGuardModulePath()
{
	char modulePath[MAX_PATH] = { 0 };
	char currentDirectory[MAX_PATH] = { 0 };
	HMODULE module = GetModuleHandleA("APIGameGuard.dll");

	if (module == 0)
	{
		LogAdd(LOG_RED, "[XShield AntiCheat] APIGameGuard.dll is not loaded");
		return;
	}

	if (GetModuleFileNameA(module, modulePath, sizeof(modulePath)) == 0)
	{
		LogAdd(LOG_RED, "[XShield AntiCheat] APIGameGuard.dll path read failed: %d", GetLastError());
		return;
	}

	LogAdd(LOG_BLUE, "[XShield AntiCheat] APIGameGuard.dll loaded: %s", modulePath);

	if (GetCurrentDirectoryA(sizeof(currentDirectory), currentDirectory) != 0)
	{
		LogAdd(LOG_BLUE, "[XShield AntiCheat] CurrentDirectory: %s", currentDirectory);
		LogAdd(LOG_BLUE, "[XShield AntiCheat] XML relative path: %s\\..\\Data\\Hack\\GameGuard.xml", currentDirectory);
	}
}
#endif
...
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) // OK
{
	...
	if (WSAStartup(MAKEWORD(2, 2), &wsa) == 0)
	{
		...
		#if(ANTIHACK_GGNEW)
			CBAntihackLoadXML();
			XShieldServerInit(hWnd);
			LogAPIGameGuardModulePath();
			LogAdd(LOG_BLUE, "[XShield AntiCheat] Connect start");
			if (XShieldServerConnect(WM_XSHIELD_SERVER_MSG_PROC) == 0)
			{
				LogAdd(LOG_RED, "[XShield AntiCheat] Connect failed or disabled. Check Data\\Hack\\GameGuard.xml and XShield 127.0.0.1:55112");
			}
			else
			{
				LogAdd(LOG_BLUE, "[XShield AntiCheat] Connect requested");
			}
		#endif
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) // OK
{
	...
	case WM_COMMAND:
		...
		case IDM_FILE_1MINUTESERVERCLOSE:
			if (gCloseMsg == 0)
			{
				gCloseMsg = 1;
				gCloseMsgTime = 60;
				gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GlobalText(487));
			}
			break;
		...
		case IDM_FILE_5MINUTESERVERCLOSE:
			if (gCloseMsg == 0)
			{
				gCloseMsg = 1;
				gCloseMsgTime = 300;
				gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GlobalText(489));
			}
			break;
		...
		case IDM_RELOAD_RELOADBOTS: //MC bots
			...
			break;
		case IDM_RELOAD_GHRS:
		#if (CUSTOM_GHRS)
			gCustomGHRS.Load(gPath.GetFullPath("Custom\\CustomGHRS.txt"));
			LogAdd(LOG_BLUE, "[ServerInfo] CustomGHRS loaded successfully");
		#else
			gServerInfo.ReloadGHRS();
			LogAdd(LOG_BLUE, "[ServerInfo] GHRS loaded successfully");
		#endif
			break;
		case IDM_RELOAD_RELOADALL:
			gServerInfo.ReloadAll();
			break;
		case IDM_FAKEONLINE_MAKE:
#if (FAKE_ONLINE == TRUE)
		...
		case IDM_STARTLD:
			gCastleDeep.StartLD();
			break;
		case ID_TOOLS_EXPORTITEM:
			gServerInfo.ExportItemBMD();
			break;
		case ID_TOOLS_EXPORTXML:
			gServerInfo.ExportItemXML();
			break;
		default:
...
#if(ANTIHACK_GGNEW)
			XShieldServerReconnect(hWnd, WM_XSHIELD_SERVER_MSG_PROC);
#endif
			break;
		}
		break;
	...
	case WM_DATA_SERVER_MSG_PROC:
		...
		break;
#if(ANTIHACK_GGNEW)
	case WM_XSHIELD_SERVER_MSG_PROC:
		XShieldServerMsgProc(wParam, lParam);
		break;
#endif
	case WM_DESTROY:
		PostQuitMessage(0);
...
#if(ANTIHACK_GGNEW)
LRESULT CALLBACK ViewLogHack(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam) // OK
{
	if (message == WM_TIMER && wParam == WM_TIMER_VIEWHACKLOG)
	{
		SetClassLongPtr(hDlg, GCLP_HICON, (LONG_PTR)LoadIcon(0, IDI_APPLICATION));
		char WindowName[100];
		sprintf(WindowName, "%s", CBAntihackGetWinTitle().c_str());
		SetWindowText(hDlg, WindowName);
		gServerDisplayer.LogTextPaintHack(hDlg);
	}
	switch (message)
	{
	case WM_INITDIALOG:
	{
		SetTimer(hDlg, WM_TIMER_VIEWHACKLOG, 2000, 0);
	...
	case WM_COMMAND:
		...
		else if (LOWORD(wParam) == IDC_HACK_RELOAD)
		{

...
#if(ANTIHACK_GGNEW)
#include "LogToFile.h"
CLogToFile g_AntiCheatLog("GameGuard", ".\\LOGS\\HACK_LOG\\GameGuard", FALSE);

static void EnsureAntiCheatLogDirectory()
{
	CreateDirectory(".\\LOGS", 0);
	CreateDirectory(".\\LOGS\\HACK_LOG", 0);
	CreateDirectory(".\\LOGS\\HACK_LOG\\GameGuard", 0);
}

void LogHackAdd(eLogColor color, char* text, ...) // OK
{
	tm today;
	time_t ltime;
	time(&ltime);
	...
	if (LOG_ALERT == color || color == LOG_RED)
	{
		EnsureAntiCheatLogDirectory();
		g_AntiCheatLog.Output(log);
	}
}
#endif

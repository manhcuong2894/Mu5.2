...
bool CEffectManager::AddEffect(LPOBJ lpObj, bool type, int index, int count, WORD value1, WORD value2, WORD value3, WORD value4) // OK
{
	...
	if (lpEffect == 0)
	{
		for (int n = 0; n < MAX_EFFECT_LIST; n++)
		{
			...
			{
				this->GCPeriodicEffectSend(lpObj, lpEffect->m_group, 0, 0, lpEffect->m_count, lpEffect->m_index, lpEffect->m_value[0]);
...
int CEffectManager::GetActiveBuffCount(LPOBJ lpObj, BYTE* lpMsg, int* size) // OK
{
	int count = 0;
...
void CEffectManager::GCPeriodicEffectSend(LPOBJ lpObj, WORD group, WORD value, BYTE state, DWORD time, BYTE effect, WORD power) // OK
{

...
bool CCustomAttack::CommandCustomAttack(LPOBJ lpObj, char* arg) // OK
{
	...
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GlobalText(791));
		return 0;
	}

	if (gMapManager.GetMapCustomAttack(lpObj->Map) == 0)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GlobalText(36));
		return 0;
	}
	...
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GlobalText(763));
		return 0;
	}
	...
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GlobalText(792));
		return 0;
	}
	...
	gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GlobalText(795));

	if (this->m_CustomAttackTime[lpObj->AccountLevel] > 0)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GlobalText(793), this->m_CustomAttackTime[lpObj->AccountLevel]);
	}
	return 1;
}
...
bool CCustomAttack::CommandCustomAttackOffline(LPOBJ lpObj, char* arg) // OK
{
	if (lpObj->AttackCustom == 0)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GlobalText(796));
		return 0;
	}

	if (gMap[lpObj->Map].CheckAttr(lpObj->X, lpObj->Y, 1) != 0)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GlobalText(797));
		return 0;
	}
...
void CCustomAttack::OnAttackClose(LPOBJ lpObj) // OK
{
	if (lpObj->AttackCustom != 0)
	{
		...
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GlobalText(790));
	...
	{
//#if (ANTIHACK_GGNEW)
//		lpObj->StatusNotCheck = 1; //Đoạn này thêm để khi treo offline antihack không cần check !, nếu src không có thì thôi
//#endif
...
void CCustomAttack::OnAttackMonsterAndMsgProc(LPOBJ lpObj) // OK
{
	...
	if (lpObj->AttackCustomTime > 0)
	{
		if ((--lpObj->AttackCustomTime) == 0)
		{
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GlobalText(794));
			this->OnAttackClose(lpObj);
			return;
		}
	}

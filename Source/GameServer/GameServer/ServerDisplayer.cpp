...
CServerDisplayer::CServerDisplayer() // OK
{
	for (int n = 0; n < MAX_LOG_TEXT_LINE; n++)
	{
		...
		memset(&this->m_logHack[n], 0, sizeof(this->m_logHack[n]));
	}

	for (int n = 0; n < MAX_LOGGLOBAL_TEXT_LINE; n++)
	{
		memset(&this->m_logGlobal[n], 0, sizeof(this->m_logGlobal[n]));
	}

	for (int n = 0; n < MAX_LOGCONNECT_TEXT_LINE; n++)
	{
		memset(&this->m_logConnect[n], 0, sizeof(this->m_logConnect[n]));
	}

	this->m_count = 0;
	this->m_countHack = 0;
	this->m_countConnect = 0;
	this->m_countGlobal = 0;
...
void CServerDisplayer::LogTextPaintHack(HWND hDlg) // OK
{
	RECT rect;
	...
	HWND hWndButton = GetDlgItem(hDlg, IDC_HACK_RELOAD);

	if (hWndButton != 0)
	{
		int newX = rect.left + 5;
		int newY = rect.bottom - 50;

		SetWindowPos(hWndButton, NULL, newX, newY, 0, 0, SWP_NOSIZE | SWP_NOZORDER);
	}
	...
	int line = 0;
	int count = (((this->m_countHack - 1) >= 0) ? (this->m_countHack - 1) : (MAX_LOG_TEXT_LINE - 1));
	int lineHeight = 15;
	int textMargin = 18;

	for (int n = 0; n < MAX_LOG_TEXT_LINE; n++)
	{
		switch (this->m_logHack[count].color)
		{
		case LOG_BLACK:
		...
		case LOG_RED:
			...
			break;
		case LOG_GREEN:
			SetTextColor(hdc, RGB(255, 79, 135));
			break;
		case LOG_BLUE:
			SetTextColor(hdc, RGB(177, 25, 247));
			break;
		case LOG_ALERT:
			SetTextColor(hdc, RGB(247, 173, 25));
			break;
		case LOG_YELOW:
			SetTextColor(hdc, RGB(255, 204, 25));
			break;
		...
		int size = strlen(this->m_logHack[count].text);

		if (size > 1)
		{
			WCHAR text_unicode[255] = { 0 };
			int nn = MultiByteToWideChar(CP_UTF8, 0, this->m_logHack[count].text, size, text_unicode, 255);

			if (nn == 0)
			{
				nn = MultiByteToWideChar(CP_ACP, 0, this->m_logHack[count].text, size, text_unicode, 255);
			}
			...
			if (nn > 1)
			{
				...
				textRect.top = rect.bottom - textMargin - (line * lineHeight);
				textRect.right = rect.right;
				textRect.bottom = textRect.top + lineHeight;

				DrawTextW(hdc, text_unicode, nn, &textRect, DT_LEFT | DT_SINGLELINE);
				line++;
			}
		}
...
void CServerDisplayer::LogAddTextHack(eLogColor color, char* text, int size) // OK
{
	...
	memset(&this->m_logHack[this->m_countHack].text, 0, sizeof(this->m_logHack[this->m_countHack].text));

	memcpy(&this->m_logHack[this->m_countHack].text, text, size);

	this->m_logHack[this->m_countHack].color = color;

	this->m_countHack = (((++this->m_countHack) >= MAX_LOG_TEXT_LINE) ? 0 : this->m_countHack);

	gLog.Output(LOG_HACK, "%s", ((size > 9) ? &text[9] : text));

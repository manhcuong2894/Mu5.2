...
#define SUB_GM_ACCESS_DENIED_RESULT 0x12
...
void JGConnectAccountRecv(SDHP_CONNECT_ACCOUNT_RECV* lpMsg) // OK
{
	...
	if (gServerInfo.m_IsSubGM != 0 &&
		gGameMaster.CheckGameMasterAccount(lpMsg->account) == 0)
	{
		GJDisconnectAccountSend(lpMsg->index, lpMsg->account, lpObj->IpAddr);
		GCConnectAccountSend(lpMsg->index, SUB_GM_ACCESS_DENIED_RESULT);
		return;
	}
...
void JGMapServerMoveAuthRecv(SDHP_MAP_SERVER_MOVE_AUTH_RECV* lpMsg) // OK
{
	...
	{
		GJDisconnectAccountSend(lpMsg->index, lpMsg->account, lpObj->IpAddr);
		GCMapServerMoveAuthSend(lpMsg->index, SUB_GM_ACCESS_DENIED_RESULT);
		return;
	}

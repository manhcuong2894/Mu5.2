...
void CGMHolyItem::DGHolyInventorySave(int aIndex)
{
	...
	for (int n = 0; n < HOLY_INVENTORY_SIZE; n++)
	{
		ItemByteConvert(n, pMsg.Inventory[n], lpObj->HolyInventory[n], -1);
	}
...
void CGMHolyItem::GCInventoryListSend(int aIndex)
{
	...
	for (size_t n = 0; n < HOLY_INVENTORY_SIZE; n++)
	{
		if (lpObj->HolyInventory[n].bItemindex != -1)
		{
			...
			this->ItemByteConvert(n, info.Inventory, lpObj->HolyInventory[n], (n < m_bItems.size()) ? m_bItems[n].BundledIndex : -1);
...
void CGMHolyItem::ItemByteConvert(int index, BYTE* btData, HOLY_INVENTORY lpItem, int BundledIndex)
{
	...
	btData[6] = (BYTE)(BundledIndex & 0xFF);

	btData[7] = (BYTE)((BundledIndex >> 8) & 0xFF);

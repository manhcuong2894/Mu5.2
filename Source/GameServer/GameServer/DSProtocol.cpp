...
#if (FAKE_ONLINE == TRUE)
static bool FakeOnlineForceAddSkill(LPOBJ lpObj, int skillId)
{
  if (lpObj == 0 || skillId <= 0 || skillId >= 300) {
    return 0;
  }

  SKILL_INFO SkillInfo;

  if (gSkillManager.GetInfo(skillId, &SkillInfo) == 0) {
    LogAdd(LOG_RED, "[FakeOnline][%s] SkillID %d invalid or not loaded in Skill.txt", lpObj->Name, skillId);
    return 0;
  }

  if (gSkillManager.GetSkill(lpObj, skillId) != 0) {
    return 1;
  }

  for (int n = 0; n < MAX_SKILL_LIST; n++) {
    if (lpObj->Skill[n].IsSkill() == 0 && lpObj->Skill[n].Set(skillId, 0) != 0) {
      LogAdd(LOG_RED, "[FakeOnline][%s] Force add SkillID %d to skill slot %d", lpObj->Name, skillId, n);
      return 1;
    }
  }

  LogAdd(LOG_RED, "[FakeOnline][%s] SkillID %d cannot be added, skill list is full", lpObj->Name, skillId);
  return 0;
}
#endif
...
void DataServerProtocolCore(BYTE HeadCode, BYTE *ReceiveBuffer, int Size) // OK
{
  ...
  case 0x05:
    switch (
        ((ReceiveBuffer[0] == 0xC1) ? ReceiveBuffer[3] : ReceiveBuffer[4])) {
    case 0x00:
    ...
    case 0x01:
      ...
      break;
    case 0x70:
      gWarehouse.DGWarehouseGuildItemRecv(
          (SDHP_WAREHOUSE_ITEM_RECV *)ReceiveBuffer);
      break;
    case 0x71:
      gWarehouse.DGWarehouseGuildFreeRecv(
          (SDHP_WAREHOUSE_FREE_RECV *)ReceiveBuffer);
      break;
    case 0x75:
      gWarehouse.GDWarehouseGuildClose(
          (SDHP_WAREHOUSEGUILD_CLOSE_RECV *)ReceiveBuffer);
      break;
    }
    break;
  case 0x07:
  ...
  case 0x0D:
    ...
    case 0x00:
#if (GAMESERVER_UPDATE >= 401)
      gMasterSkillTree.DGMasterSkillTreeRecv(
          (SDHP_MASTER_SKILL_TREE_RECV *)ReceiveBuffer);
#endif
      break;
    }
    break;
  case 0x0E:
    ...
    case 0x00:
      ...
      gNpcTalk.DGNpcLeoTheHelperRecv(
          (SDHP_NPC_LEO_THE_HELPER_RECV *)ReceiveBuffer);
#endif
      break;
    case 0x01:
#if (GAMESERVER_UPDATE >= 401)
  ...
  case 0x0F:
    switch (
        ((ReceiveBuffer[0] == 0xC1) ? ReceiveBuffer[3] : ReceiveBuffer[4])) {
    case 0x00:
      gCommandManager.DGCommandResetRecv(
          (SDHP_COMMAND_RESET_RECV *)ReceiveBuffer);
      break;
    case 0x01:
      gCommandManager.DGCommandMasterResetRecv(
          (SDHP_COMMAND_MASTER_RESET_RECV *)ReceiveBuffer);
      break;
    case 0x02:
      gCommandManager.DGCommandMarryRecv(
          (SDHP_COMMAND_MARRY_RECV *)ReceiveBuffer);
      break;
    case 0x05:
      gCommandManager.DGCommandRenameRecv(
          (SDHP_COMMAND_RENAME_RECV *)ReceiveBuffer);
      break;
    case 0x06:
      gCommandManager.CommandBlocAccRecv(
          (SDHP_COMMAND_BLOC_RECV *)ReceiveBuffer);
      break;
    case 0x07:
      gCommandManager.CommandBlocCharRecv(
          (SDHP_COMMAND_BLOC_RECV *)ReceiveBuffer);
      break;
    case 0x08:
  ...
  case 0x11:
    ...
    case 0x00:
#if (GAMESERVER_UPDATE >= 501)
      gGensSystem.DGGensSystemInsertRecv(
          (SDHP_GENS_SYSTEM_INSERT_RECV *)ReceiveBuffer);
#endif
      break;
    case 0x01:
#if (GAMESERVER_UPDATE >= 501)
      gGensSystem.DGGensSystemDeleteRecv(
          (SDHP_GENS_SYSTEM_DELETE_RECV *)ReceiveBuffer);
#endif
      break;
    case 0x02:
#if (GAMESERVER_UPDATE >= 501)
      gGensSystem.DGGensSystemMemberRecv(
          (SDHP_GENS_SYSTEM_MEMBER_RECV *)ReceiveBuffer);
#endif
      break;
    case 0x03:
#if (GAMESERVER_UPDATE >= 501)
      gGensSystem.DGGensSystemUpdateRecv(
          (SDHP_GENS_SYSTEM_UPDATE_RECV *)ReceiveBuffer);
#endif
      break;
    case 0x04:
#if (GAMESERVER_UPDATE >= 501)
      gGensSystem.DGGensSystemRewardRecv(
          (SDHP_GENS_SYSTEM_REWARD_RECV *)ReceiveBuffer);
#endif
      break;
    case 0x70:
#if (GAMESERVER_UPDATE >= 501)
      gGensSystem.DGGensSystemCreateRecv(
          (SDHP_GENS_SYSTEM_CREATE_RECV *)ReceiveBuffer);
#endif
      break;
    }
    break;
  ...
  case 0x18:
    ...
    case 0x01:
#if (GAMESERVER_UPDATE >= 501)
      gCashShop.DGCashShopItemBuyRecv(
          (SDHP_CASH_SHOP_ITEM_BUY_RECV *)ReceiveBuffer);
#endif
      break;
    case 0x02:
#if (GAMESERVER_UPDATE >= 501)
      gCashShop.DGCashShopItemGifRecv(
          (SDHP_CASH_SHOP_ITEM_GIF_RECV *)ReceiveBuffer);
#endif
      break;
    case 0x03:
#if (GAMESERVER_UPDATE >= 501)
      gCashShop.DGCashShopItemNumRecv(
          (SDHP_CASH_SHOP_ITEM_NUM_RECV *)ReceiveBuffer);
#endif
      break;
    case 0x04:
#if (GAMESERVER_UPDATE >= 501)
      gCashShop.DGCashShopItemUseRecv(
          (SDHP_CASH_SHOP_ITEM_USE_RECV *)ReceiveBuffer);
#endif
      break;
    case 0x05:
#if (GAMESERVER_UPDATE >= 501)
      gCashShop.DGCashShopPeriodicItemRecv(
          (SDHP_CASH_SHOP_PERIODIC_ITEM_RECV *)ReceiveBuffer);
#endif
      break;
    case 0x06:
#if (GAMESERVER_UPDATE >= 501)
      gCashShop.DGCashShopRecievePointRecv(
          (SDHP_CASH_SHOP_RECIEVE_POINT_RECV *)ReceiveBuffer);
#endif
      break;
    }
    break;
  case 0x19:
    ...
    case 0x01:
#if (GAMESERVER_UPDATE <= 402)
      gPcPoint.DGPcPointItemBuyRecv(
          (SDHP_PC_POINT_ITEM_BUY_RECV *)ReceiveBuffer);
#endif
      break;
    case 0x02:
#if (GAMESERVER_UPDATE <= 402)
      gPcPoint.DGPcPointRecievePointRecv(
          (SDHP_PC_POINT_RECIEVE_POINT_RECV *)ReceiveBuffer);
#endif
      break;
    }
    break;
  case 0x1A:
    ...
    case 0x00:
#if (GAMESERVER_UPDATE >= 402)
      gLuckyCoin.DGLuckyCoinCountRecv(
          (SDHP_LUCKY_COIN_COUNT_RECV *)ReceiveBuffer);
#endif
      break;
    case 0x01:
#if (GAMESERVER_UPDATE >= 402)
      gLuckyCoin.DGLuckyCoinRegisterRecv(
          (SDHP_LUCKY_COIN_REGISTER_RECV *)ReceiveBuffer);
#endif
      break;
    case 0x02:
#if (GAMESERVER_UPDATE >= 402)
      gLuckyCoin.DGLuckyCoinExchangeRecv(
          (SDHP_LUCKY_COIN_EXCHANGE_RECV *)ReceiveBuffer);
#endif
      break;
    }
    break;
  ...
  case 0x23:
    ...
    case 0x00:
      ...
      gPentagramSystem.DGPentagramJewelInfoRecv(
          (SDHP_PENTAGRAM_JEWEL_INFO_RECV *)ReceiveBuffer);
#endif
      break;
    }
    break;
  ...
  case 0x25:
    ...
    case 0x00:
#if (GAMESERVER_UPDATE >= 802)
      gPersonalShop.DGPShopItemValueRecv(
          (SDHP_PSHOP_ITEM_VALUE_RECV *)ReceiveBuffer);
#endif
      break;
    }
    break;
  case 0x26:
    ...
    case 0x00:
#if (GAMESERVER_UPDATE >= 802)
      gEventInventory.DGEventInventoryRecv(
          (SDHP_EVENT_INVENTORY_RECV *)ReceiveBuffer);
#endif
      break;
    }
    break;
  case 0x27:
    ...
    case 0x00:
      ...
      gMuunSystem.DGMuunInventoryRecv(
          (SDHP_MUUN_INVENTORY_RECV *)ReceiveBuffer);
#endif
      break;
    }
    break;
  case 0x28:
    ...
    case 0x00:
#if (GAMESERVER_UPDATE >= 801)
      gGuildMatching.DGGuildMatchingListRecv(
          (SDHP_GUILD_MATCHING_LIST_RECV *)ReceiveBuffer);
#endif
      break;
    case 0x02:
#if (GAMESERVER_UPDATE >= 801)
      gGuildMatching.DGGuildMatchingInsertRecv(
          (SDHP_GUILD_MATCHING_INSERT_RECV *)ReceiveBuffer);
#endif
      break;
    case 0x03:
#if (GAMESERVER_UPDATE >= 801)
      gGuildMatching.DGGuildMatchingCancelRecv(
          (SDHP_GUILD_MATCHING_CANCEL_RECV *)ReceiveBuffer);
#endif
      break;
    case 0x04:
#if (GAMESERVER_UPDATE >= 801)
      gGuildMatching.DGGuildMatchingJoinInsertRecv(
          (SDHP_GUILD_MATCHING_JOIN_INSERT_RECV *)ReceiveBuffer);
#endif
      break;
    case 0x05:
#if (GAMESERVER_UPDATE >= 801)
      gGuildMatching.DGGuildMatchingJoinCancelRecv(
          (SDHP_GUILD_MATCHING_JOIN_CANCEL_RECV *)ReceiveBuffer);
#endif
      break;
    case 0x06:
#if (GAMESERVER_UPDATE >= 801)
      gGuildMatching.DGGuildMatchingJoinAcceptRecv(
          (SDHP_GUILD_MATCHING_JOIN_ACCEPT_RECV *)ReceiveBuffer);
#endif
      break;
    case 0x07:
#if (GAMESERVER_UPDATE >= 801)
      gGuildMatching.DGGuildMatchingJoinListRecv(
          (SDHP_GUILD_MATCHING_JOIN_LIST_RECV *)ReceiveBuffer);
#endif
      break;
    case 0x08:
#if (GAMESERVER_UPDATE >= 801)
      gGuildMatching.DGGuildMatchingJoinInfoRecv(
          (SDHP_GUILD_MATCHING_JOIN_INFO_RECV *)ReceiveBuffer);
#endif
      break;
    case 0x09:
#if (GAMESERVER_UPDATE >= 801)
      gGuildMatching.DGGuildMatchingNotifyRecv(
          (SDHP_GUILD_MATCHING_NOTIFY_RECV *)ReceiveBuffer);
#endif
      break;
    case 0x0A:
#if (GAMESERVER_UPDATE >= 801)
      gGuildMatching.DGGuildMatchingNotifyMasterRecv(
          (SDHP_GUILD_MATCHING_NOTIFY_MASTER_RECV *)ReceiveBuffer);
#endif
      break;
    }
    break;
  case 0x29:
    ...
    case 0x00:
#if (GAMESERVER_UPDATE >= 801)
      gPartyMatching.DGPartyMatchingInsertRecv(
          (SDHP_PARTY_MATCHING_INSERT_RECV *)ReceiveBuffer);
#endif
      break;
    case 0x01:
#if (GAMESERVER_UPDATE >= 801)
      gPartyMatching.DGPartyMatchingListRecv(
          (SDHP_PARTY_MATCHING_LIST_RECV *)ReceiveBuffer);
#endif
      break;
    case 0x02:
#if (GAMESERVER_UPDATE >= 801)
      gPartyMatching.DGPartyMatchingJoinInsertRecv(
          (SDHP_PARTY_MATCHING_JOIN_INSERT_RECV *)ReceiveBuffer);
#endif
      break;
    case 0x03:
#if (GAMESERVER_UPDATE >= 801)
      gPartyMatching.DGPartyMatchingJoinInfoRecv(
          (SDHP_PARTY_MATCHING_JOIN_INFO_RECV *)ReceiveBuffer);
#endif
      break;
    case 0x04:
#if (GAMESERVER_UPDATE >= 801)
      gPartyMatching.DGPartyMatchingJoinListRecv(
          (SDHP_PARTY_MATCHING_JOIN_LIST_RECV *)ReceiveBuffer);
#endif
      break;
    case 0x05:
#if (GAMESERVER_UPDATE >= 801)
      gPartyMatching.DGPartyMatchingJoinAcceptRecv(
          (SDHP_PARTY_MATCHING_JOIN_ACCEPT_RECV *)ReceiveBuffer);
#endif
      break;
    case 0x06:
#if (GAMESERVER_UPDATE >= 801)
      gPartyMatching.DGPartyMatchingJoinCancelRecv(
          (SDHP_PARTY_MATCHING_JOIN_CANCEL_RECV *)ReceiveBuffer);
#endif
      break;
    case 0x07:
#if (GAMESERVER_UPDATE >= 801)
      gPartyMatching.DGPartyMatchingNotifyRecv(
          (SDHP_PARTY_MATCHING_NOTIFY_RECV *)ReceiveBuffer);
#endif
      break;
    case 0x08:
#if (GAMESERVER_UPDATE >= 801)
      gPartyMatching.DGPartyMatchingNotifyLeaderRecv(
          (SDHP_PARTY_MATCHING_NOTIFY_LEADER_RECV *)ReceiveBuffer);
#endif
      break;
    }
    break;
  case 0x41:
    switch (
        ((ReceiveBuffer[0] == 0xC1) ? ReceiveBuffer[3] : ReceiveBuffer[4])) {
    case 0x00:
      GMHolyItem->DGHolyInventoryRecv(
          (SDHP_INVENTORY_HOLY_RECV *)ReceiveBuffer);
      break;
    }
    break;
  case 0x59:
#if (CUSTOM_MOCNAP)
    gMocNap.DGMocNapAutoRewardRecv(
        (SDHP_MOCNAP_AUTO_REWARD_RECV *)ReceiveBuffer);
#endif
    break;
  case 0x5A:
#if (CUSTOM_MOCNAP)
    gMocNap.DGMocNapPaymentCreateRecv(
        (SDHP_MOCNAP_PAYMENT_CREATE_RECV *)ReceiveBuffer);
#endif
    break;
  case 0x72:
  ...
  case 0xD3:
    switch (
        ((ReceiveBuffer[0] == 0xC1) ? ReceiveBuffer[3] : ReceiveBuffer[4])) {
    case 0x40:
      gCustomRanking.RecvInfoCharTop((DATA_VIEWTOPRANKING *)ReceiveBuffer);
      break;
#if (CB_QUAPHUCLOI)
    case 0x6C:
      gCB_QuaPhucLoi.DSGetInfoQuaPhucLoi(ReceiveBuffer);
      break;
#endif
#if (CB_CUSTOMGIFTCODE)
    case 0x6A:
      gCB_GiftCode.DSRecvProtocol(ReceiveBuffer);
      break;
#endif
    }
    break;
...
#if (CUSTOM_GHRS)
  case 0xF6:
    gCustomGHRS.GDCustomGHRSRecv((SDHP_CUSTOM_GHRS_RECV *)ReceiveBuffer);
    break;
#endif
  case 0xF7:
    switch (
        ((ReceiveBuffer[0] == 0xC1) ? ReceiveBuffer[3] : ReceiveBuffer[4])) {
    case 0x00:
      gCustomNpcQuest.DGCustomNpcQuestRecv(
          (SDHP_CUSTOMNPCQUEST_RECV *)ReceiveBuffer);
      break;
    }
    break;
#ifdef CHAOS_MACHINE_EXTENSION
    // case 0xF8:
    //	switch (((ReceiveBuffer[0] == 0xC1) ? ReceiveBuffer[3] :
    // ReceiveBuffer[4]))
    //	{
    //	case 0x00:
  ...
  case 0xFE:
    ...
    case 0x02:
      ...
      break;
    case 0x03:
      gBCustomChoTroi.DGAnsItemStatus((CUSTOM_LOAD_COUNT *)ReceiveBuffer);
      break;
    }
    break;
#endif
...
void DGCharacterListRecv(SDHP_CHARACTER_LIST_RECV *lpMsg) // OK
{
  if (gObjIsAccountValid(lpMsg->index, lpMsg->account) == 0) {
    LogAdd(LOG_RED, "[DGCharacterListRecv] Invalid Account [%d](%s)",
  ...
  for (int n = 0; n < lpMsg->count; n++) {
    SDHP_CHARACTER_LIST *lpInfo =
        (SDHP_CHARACTER_LIST *)(((BYTE *)lpMsg) +
                                sizeof(SDHP_CHARACTER_LIST_RECV) +
                                (sizeof(SDHP_CHARACTER_LIST) * n));
...
#pragma region CHAR_SET_CALC
    ...
    bool IsFullSet =
        gSetItemOption.IsFullSet(player_body, EQUIPMENT_NEW_LENGTH);
...
#pragma endregion
  ...
  if (gServerInfo.m_MGCreateType == 0 &&
      CharacterCreationLevel >=
          (WORD)gServerInfo.m_MGCreateLevel[gObj[lpMsg->index].AccountLevel] &&
      CharacterCreationReset >=
          (DWORD)gServerInfo.m_MGCreateReset[gObj[lpMsg->index].AccountLevel] &&
      CharacterCreationMasterReset >=
          (DWORD)gServerInfo
              .m_MGCreateMasterReset[gObj[lpMsg->index].AccountLevel]) {
    pMsg.ClassCode |= 4;
  }

  if (gServerInfo.m_DLCreateType == 0 &&
      CharacterCreationLevel >=
          (WORD)gServerInfo.m_DLCreateLevel[gObj[lpMsg->index].AccountLevel] &&
      CharacterCreationReset >=
          (DWORD)gServerInfo.m_DLCreateReset[gObj[lpMsg->index].AccountLevel] &&
      CharacterCreationMasterReset >=
          (DWORD)gServerInfo
              .m_DLCreateMasterReset[gObj[lpMsg->index].AccountLevel]) {
    pMsg.ClassCode |= 2;
  }

  if (gServerInfo.m_SUCreateType == 0 &&
      CharacterCreationLevel >=
          (WORD)gServerInfo.m_SUCreateLevel[gObj[lpMsg->index].AccountLevel] &&
      CharacterCreationReset >=
          (DWORD)gServerInfo.m_SUCreateReset[gObj[lpMsg->index].AccountLevel] &&
      CharacterCreationMasterReset >=
          (DWORD)gServerInfo
              .m_SUCreateMasterReset[gObj[lpMsg->index].AccountLevel]) {
    pMsg.ClassCode |= 1;
  }

  if (gServerInfo.m_RFCreateType == 0 &&
      CharacterCreationLevel >=
          (WORD)gServerInfo.m_RFCreateLevel[gObj[lpMsg->index].AccountLevel] &&
      CharacterCreationReset >=
          (DWORD)gServerInfo.m_RFCreateReset[gObj[lpMsg->index].AccountLevel] &&
      CharacterCreationMasterReset >=
          (DWORD)gServerInfo
              .m_RFCreateMasterReset[gObj[lpMsg->index].AccountLevel]) {
    pMsg.ClassCode |= 8;
  }

  gObj[lpMsg->index].ClassCode = pMsg.ClassCode;

  gObj[lpMsg->index].ClassFlag =
      ((CharacterCreationLevel >=
        gServerInfo.m_MGCreateLevel[gObj[lpMsg->index].AccountLevel])
           ? (gObj[lpMsg->index].ClassFlag | 4)
           : gObj[lpMsg->index].ClassFlag);

  gObj[lpMsg->index].ClassFlag =
      ((CharacterCreationLevel >=
        gServerInfo.m_DLCreateLevel[gObj[lpMsg->index].AccountLevel])
           ? (gObj[lpMsg->index].ClassFlag | 2)
           : gObj[lpMsg->index].ClassFlag);

  gObj[lpMsg->index].ClassFlag =
      ((CharacterCreationLevel >=
        gServerInfo.m_SUCreateLevel[gObj[lpMsg->index].AccountLevel])
           ? (gObj[lpMsg->index].ClassFlag | 1)
           : gObj[lpMsg->index].ClassFlag);

  gObj[lpMsg->index].ClassFlag =
      ((CharacterCreationLevel >=
        gServerInfo.m_RFCreateLevel[gObj[lpMsg->index].AccountLevel])
           ? (gObj[lpMsg->index].ClassFlag | 8)
           : gObj[lpMsg->index].ClassFlag);
...
{
  if (gObjIsAccountValid(lpMsg->index, lpMsg->account) == 0) {
    LogAdd(LOG_RED, "[DGCharacterCreateRecv] Invalid Account [%d](%s)",
           lpMsg->index, lpMsg->account);
    CloseClient(lpMsg->index);
    return;
  }
...
{
  if (gObjIsAccountValid(lpMsg->index, lpMsg->account) == 0) {
    LogAdd(LOG_RED, "[DGCharacterDeleteRecv] Invalid Account [%d](%s)",
           lpMsg->index, lpMsg->account);
    CloseClient(lpMsg->index);
    return;
  }
...
{
  if (gObjIsAccountValid(lpMsg->index, lpMsg->account) == 0) {
    LogAdd(LOG_RED, "[DGCharacterInfoRecv] Invalid Account [%d](%s)",
           lpMsg->index, lpMsg->account);
    CloseClient(lpMsg->index);
    return;
  }
  ...
  if (info != 0 && lpObj->Socket == INVALID_SOCKET) {
    ...
    lpObj->IsFakeReturnAfterDieTick = 0;
    lpObj->IsFakeTargetIndex = -1;
    lpObj->IsFakeSkillVisualTick = 0;
    ...
    lpObj->Map = gGate.GetGateMap(info->GateNumber);
    lpObj->X = info->MapX;
    lpObj->Y = info->MapY;
    ...
    FakeOnlineForceAddSkill(lpObj, info->SkillID);
  ...
  if (lpObj->MapServerMoveRequest == 0) {
    if ((lpObj->NextServerCode = gMapServerManager.CheckMapServerMove(
             lpObj->Index, lpObj->Map, lpObj->LastServerCode)) !=
        gServerInfo.m_ServerCode) {
      if (lpObj->NextServerCode == -1) {
        CloseClient(lpObj->Index);
        return;
      } else {
        GJMapServerMoveSend(lpObj->Index, lpObj->NextServerCode, lpObj->Map,
                            (BYTE)lpObj->X, (BYTE)lpObj->Y);
        return;
      }
    }
  }
  ...
  if (gMasterSkillTree.CheckMasterLevel(lpObj) == 0) {
    pMsg.NextExperience[0] =
        SET_NUMBERHB(SET_NUMBERHW(SET_NUMBERHDW(lpObj->NextExperience)));
    pMsg.NextExperience[1] =
        SET_NUMBERLB(SET_NUMBERHW(SET_NUMBERHDW(lpObj->NextExperience)));
    pMsg.NextExperience[2] =
        SET_NUMBERHB(SET_NUMBERLW(SET_NUMBERHDW(lpObj->NextExperience)));
    pMsg.NextExperience[3] =
        SET_NUMBERLB(SET_NUMBERLW(SET_NUMBERHDW(lpObj->NextExperience)));
    pMsg.NextExperience[4] =
        SET_NUMBERHB(SET_NUMBERHW(SET_NUMBERLDW(lpObj->NextExperience)));
    pMsg.NextExperience[5] =
        SET_NUMBERLB(SET_NUMBERHW(SET_NUMBERLDW(lpObj->NextExperience)));
    pMsg.NextExperience[6] =
        SET_NUMBERHB(SET_NUMBERLW(SET_NUMBERLDW(lpObj->NextExperience)));
    pMsg.NextExperience[7] =
        SET_NUMBERLB(SET_NUMBERLW(SET_NUMBERLDW(lpObj->NextExperience)));
  } else {
    pMsg.NextExperience[0] =
        SET_NUMBERHB(SET_NUMBERHW(SET_NUMBERHDW(lpObj->MasterNextExperience)));
    pMsg.NextExperience[1] =
        SET_NUMBERLB(SET_NUMBERHW(SET_NUMBERHDW(lpObj->MasterNextExperience)));
    pMsg.NextExperience[2] =
        SET_NUMBERHB(SET_NUMBERLW(SET_NUMBERHDW(lpObj->MasterNextExperience)));
    pMsg.NextExperience[3] =
        SET_NUMBERLB(SET_NUMBERLW(SET_NUMBERHDW(lpObj->MasterNextExperience)));
    pMsg.NextExperience[4] =
        SET_NUMBERHB(SET_NUMBERHW(SET_NUMBERLDW(lpObj->MasterNextExperience)));
    pMsg.NextExperience[5] =
        SET_NUMBERLB(SET_NUMBERHW(SET_NUMBERLDW(lpObj->MasterNextExperience)));
    pMsg.NextExperience[6] =
        SET_NUMBERHB(SET_NUMBERLW(SET_NUMBERLDW(lpObj->MasterNextExperience)));
    pMsg.NextExperience[7] =
        SET_NUMBERLB(SET_NUMBERLW(SET_NUMBERLDW(lpObj->MasterNextExperience)));
  }
  ...
  if (lpObj->ExtInventory < 4 && gServerInfo.m_SwitchBuyInventoryExt) {
    pMsg.PriceInventoryExt =
        gServerInfo.m_PriceInventoryExt[lpObj->ExtInventory];
  } else {
...
#if (GAMESERVER_UPDATE >= 701)
  gPentagramSystem.GDPentagramJewelInfoSend(lpObj->Index,
                                            PENTAGRAM_JEWEL_TYPE_INVENTORY);
#endif
  ...
  gNotice.GCNoticeSend(lpObj->Index, 0, 0, 0, 0, 0, 0, gMessage.GlobalText(256),
                       lpObj->Name);

  gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0,
                       gMessage.GlobalText((248 + lpObj->AccountLevel)),
                       lpObj->AccountExpireDate);

  GJAccountServerInfoSend(lpObj->Index, lpObj->AccountLevel,
                          lpObj->AccountExpireDate, lpObj->Lock);
  ...
  if (lpObj->Authority == 32) {
    if (gServerInfo.m_OnlineGmSwitch == 1) {
      ...
      gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GlobalText(639),
                                lpObj->Name);
#endif
    }
  } else {
  ...
  if (lpObj->CloseCount <= 0) {
    if (gServerInfo.m_ServerMinLevel != 0 &&
        lpObj->Level < gServerInfo.m_ServerMinLevel) {
      lpObj->CloseCount = 6;
      lpObj->CloseType = 3;
      return;
    }

    if (gServerInfo.m_ServerMinReset != 0 &&
        lpObj->Reset < gServerInfo.m_ServerMinReset) {
      lpObj->CloseCount = 6;
      lpObj->CloseType = 3;
      return;
    }

    if (gServerInfo.m_ServerMinMasterReset != 0 &&
        lpObj->MasterReset < gServerInfo.m_ServerMinMasterReset) {
      lpObj->CloseCount = 6;
      lpObj->CloseType = 3;
      return;
    }

    if (gServerInfo.m_ServerMaxLevel != 0 &&
        lpObj->Level > gServerInfo.m_ServerMaxLevel) {
      lpObj->CloseCount = 6;
      lpObj->CloseType = 4;
      return;
    }

    if (gServerInfo.m_ServerMaxReset != 0 &&
        lpObj->Reset > gServerInfo.m_ServerMaxReset) {
      lpObj->CloseCount = 6;
      lpObj->CloseType = 4;
      return;
    }

    if (gServerInfo.m_ServerMaxMasterReset != 0 &&
        lpObj->MasterReset > gServerInfo.m_ServerMaxMasterReset) {
      lpObj->CloseCount = 6;
      lpObj->CloseType = 4;
      return;
    }
...
#if (CUSTOM_GHRS)
  gNotice.GCNoticeSend(lpObj->Index, 0, 0, 0, 0, 0, 0, gMessage.GlobalText(916),
                       lpObj->Name, gCustomGHRS.maxResets);
#else
  gNotice.GCNoticeSend(lpObj->Index, 0, 0, 0, 0, 0, 0, gMessage.GlobalText(916),
                       lpObj->Name, gServerInfo.GHRSMax);
#endif
#if (CUSTOM_MOCNAP)
  gMocNap.UserSendClientInfo(lpObj->Index);
#endif

  LogAddConnect(LOG_BLUE, "[Obj][%d] LogInCharacter (%s)", lpObj->Index,
                lpObj->Name);
}
...
{
  if (gObjIsAccountValid(lpMsg->index, lpMsg->account) == 0) {
    LogAdd(LOG_RED, "[DGCreateItemRecv] Invalid Account [%d](%s)", lpMsg->index,
           lpMsg->account);
    CloseClient(lpMsg->index);
    return;
  }
  ...
  if (lpMsg->Map != 0xFF && lpMsg->Map != 0xFE && lpMsg->Map != 0xED &&
      lpMsg->Map != 0xEC && lpMsg->Map != 0xEB && MAP_RANGE(lpMsg->Map) == 0) {
    return;
  }

  if (lpMsg->Map == 0xFE || lpMsg->Map == 0xFF) {
    if ((lpMsg->Map == 0xFE && lpObj->Interface.type != INTERFACE_TRAINER) ||
        (lpMsg->Map == 0xFF && lpObj->Interface.type != INTERFACE_CHAOS_BOX)) {
      return;
    }
    ...
    if (lpMsg->ItemIndex == GET_ITEM(13, 18) ||
        lpMsg->ItemIndex == GET_ITEM(13, 37) ||
        lpMsg->ItemIndex == GET_ITEM(13, 51) ||
        lpMsg->ItemIndex == GET_ITEM(14, 7) ||
        lpMsg->ItemIndex == GET_ITEM(14, 19) ||
        lpMsg->ItemIndex == GET_ITEM(14, 109)) {
      item.m_Durability = (float)lpMsg->Dur;
    } else {
      item.m_Durability = (float)gItemManager.GetItemDurability(
          lpMsg->ItemIndex, lpMsg->Level, lpMsg->NewOption, lpMsg->SetOption);
    }

    item.Convert(lpMsg->ItemIndex, lpMsg->Option1, lpMsg->Option2,
                 lpMsg->Option3, lpMsg->NewOption, lpMsg->SetOption,
                 lpMsg->JewelOfHarmonyOption, lpMsg->ItemOptionEx,
                 lpMsg->SocketOption, lpMsg->SocketOptionBonus);

    if (lpMsg->ItemIndex == GET_ITEM(13, 4) ||
        lpMsg->ItemIndex == GET_ITEM(13, 5)) {
      ...
      if (gItemManager.GetInfo(lpMsg->ItemIndex, &item_info) &&
          item_info.Kind3 == ItemKind3::PET_GAIN_EXPERIENCE) {
        item.SetPetItemInfo(1, 0);
      }
    }
    ...
    gChaosBox.GCChaosMixSend(lpObj->Index, ((lpMsg->Map == 0xFF) ? 1 : 100),
                             &item);
    ...
    if (slot != 0xFF) {
      ...
      if (lpMsg->Duration > 0) {
        gCashShop.GCCashShopPeriodicItemSend(lpObj->Index, item.m_Index, slot,
                                             item.m_PeriodicItemTime);
      }
    }
    ...
    if (gMap[lpMsg->Map].MonsterItemDrop(
            lpMsg->ItemIndex, lpMsg->Level, lpMsg->Dur, lpMsg->X, lpMsg->Y,
            lpMsg->Option1, lpMsg->Option2, lpMsg->Option3, lpMsg->NewOption,
            lpMsg->SetOption, lpMsg->LootIndex, lpMsg->Serial,
            lpMsg->JewelOfHarmonyOption, lpMsg->ItemOptionEx,
            lpMsg->SocketOption, lpMsg->SocketOptionBonus,
            lpMsg->Duration) != 0) {
      if (BC_MAP_RANGE(lpMsg->Map) != 0) {
        gBloodCastle.SetEventItemSerial(lpMsg->Map, lpMsg->ItemIndex,
                                        lpMsg->Serial);
      }

      if (IT_MAP_RANGE(lpMsg->Map) != 0) {
        gIllusionTemple.SetEventItemSerial(lpMsg->Map, lpMsg->ItemIndex,
                                           lpMsg->Serial);
      }
    }
  }
...
{
  if (gObjIsAccountValid(lpMsg->index, lpMsg->account) == 0) {
    LogAdd(LOG_RED, "[DGPetItemInfoRecv] Invalid Account [%d](%s)",
           lpMsg->index, lpMsg->account);
    CloseClient(lpMsg->index);
    return;
  }
  ...
  if (lpMsg->type == 0) {
    for (int n = 0; n < lpMsg->count; n++) {
      ...
      if (lpObj->Inventory[lpInfo->slot].m_Serial == lpInfo->serial) {
        lpObj->Inventory[lpInfo->slot].SetPetItemInfo(lpInfo->level,
                                                      lpInfo->experience);
  ...
  if (lpMsg->type == 1) {
    for (int n = 0; n < lpMsg->count; n++) {
      ...
      if (lpObj->Warehouse[lpInfo->slot].m_Serial == lpInfo->serial) {
        lpObj->Warehouse[lpInfo->slot].SetPetItemInfo(lpInfo->level,
                                                      lpInfo->experience);
      }
    }
  }
...
{
  if (gObjIsAccountValid(lpMsg->index, lpMsg->account) == 0) {
    LogAdd(LOG_RED, "[DGOptionDataRecv] Invalid Account [%d](%s)", lpMsg->index,
           lpMsg->account);
    CloseClient(lpMsg->index);
    return;
  }
...
void DGCharacterNameCheckRecv(SDHP_CHARACTER_NAME_CHECK_RECV *lpMsg) // OK
{
  ...
  if (gObjIsAccountValid(lpMsg->index, lpMsg->account) == 0) {
    LogAdd(LOG_RED, "[DGCharacterNameCheckRecv] Invalid Account [%d](%s)",
           lpMsg->index, lpMsg->account);
    CloseClient(lpMsg->index);
    return;
  }
...
void DGCharacterNameChangeRecv(SDHP_CHARACTER_NAME_CHANGE_RECV *lpMsg) // OK
{
  ...
  if (gObjIsAccountValid(lpMsg->index, lpMsg->account) == 0) {
    LogAdd(LOG_RED, "[DGCharacterNameChangeRecv] Invalid Account [%d](%s)",
           lpMsg->index, lpMsg->account);
    CloseClient(lpMsg->index);
    return;
  }
...
void DGCrywolfSyncRecv(SDHP_CRYWOLF_SYNC_RECV *lpMsg) // OK
{
  ...
  if (gCrywolfSync.GetCrywolfState() == lpMsg->CrywolfState &&
      gCrywolfSync.GetOccupationState() == lpMsg->OccupationState) {
    return;
  }
  ...
  LogAdd(LOG_BLUE, "[ CRYWOLF ] SYNC Occupation: %d, State: %d",
         lpMsg->OccupationState, lpMsg->CrywolfState);
}
...
void DGCrywolfInfoRecv(SDHP_CRYWOLF_INFO_RECV *lpMsg) // OK
{
  ...
  if (lpMsg->OccupationState == 0 || lpMsg->OccupationState == 1 ||
      lpMsg->OccupationState == 2) {
...
void DGGlobalNoticeRecv(SDHP_GLOBAL_NOTICE_RECV *lpMsg) // OK
{
  ...
  gNotice.GCNoticeSendToAll(lpMsg->type, lpMsg->count, lpMsg->opacity,
                            lpMsg->delay, lpMsg->color, lpMsg->speed, "%s",
                            lpMsg->message);
}
...
void DGSNSDataRecv(SDHP_SNS_DATA_RECV *lpMsg) // OK
{
  ...
  if (gObjIsAccountValid(lpMsg->index, lpMsg->account) == 0) {
    LogAdd(LOG_RED, "[DGSNSDataRecv] Invalid Account [%d](%s)", lpMsg->index,
           lpMsg->account);
    CloseClient(lpMsg->index);
    return;
  }
...
{
  if (gObjIsAccountValid(lpMsg->index, lpMsg->account) == 0) {
    LogAdd(LOG_RED, "[DGGlobalWhisperRecv] Invalid Account [%d](%s)",
           lpMsg->index, lpMsg->account);
    CloseClient(lpMsg->index);
    return;
  }
  ...
  lpMsg->message[(sizeof(lpMsg->message) - 1)] =
      ((strlen(lpMsg->message) > (sizeof(lpMsg->message) - 1))
           ? 0
           : lpMsg->message[(sizeof(lpMsg->message) - 1)]);

  gLog.Output(LOG_CHAT, "[Whisper][%s][%s] - (Name: %s, Message: %s)",
              lpMsg->account, lpMsg->name, lpMsg->TargetName, lpMsg->message);
}

void DGGlobalWhisperEchoRecv(SDHP_GLOBAL_WHISPER_ECHO_RECV *lpMsg) // OK
{
  if (gObjIsAccountValid(lpMsg->index, lpMsg->account) == 0) {
    LogAdd(LOG_RED, "[DGGlobalWhisperEchoRecv] Invalid Account [%d](%s)",
           lpMsg->index, lpMsg->account);
    CloseClient(lpMsg->index);
    return;
  }
...
void GDCreateItemSend(int aIndex, BYTE map, BYTE x, BYTE y, int index,
                      BYTE level, BYTE dur, BYTE Option1, BYTE Option2,
                      BYTE Option3, int LootIndex, BYTE NewOption,
                      BYTE SetOption, BYTE JewelOfHarmonyOption,
                      BYTE ItemOptionEx, BYTE SocketOption[MAX_SOCKET_OPTION],
                      BYTE SocketOptionBonus, DWORD duration) // OK
{
  if (gObjIsAccountValid(aIndex, gObj[aIndex].Account) == 0) {
    return;
  }
  ...
  pMsg.Map =
      ((map == 0xEB)
           ? ((gEventInventory.IsEventItem(index) == 0)
                  ? ((gMuunSystem.IsMuunItem(index) == 0)
                         ? ((gMuunSystem.IsMuunUtil(index) == 0) ? map : 0xED)
                         : 0xED)
                  : 0xEC)
           : map);
  pMsg.ItemIndex = index;
  pMsg.Level = level;
  pMsg.Dur = ((dur == 0) ? gItemManager.GetItemDurability(index, level,
                                                          NewOption, SetOption)
                         : dur);
  ...
  ITEM_INFO item_info;
  if (index == GET_ITEM(13, 4) || index == GET_ITEM(13, 5) ||
      (gItemManager.GetInfo(index, &item_info) &&
       item_info.Kind3 == ItemKind3::PET_GAIN_EXPERIENCE)) {
...
#if (GAMESERVER_UPDATE >= 701)
  if (gPentagramSystem.IsPentagramItem(index) != 0 ||
      gPentagramSystem.IsPentagramJewel(index) != 0 ||
      gPentagramSystem.IsPentagramMithril(index) != 0) {
    pMsg.SocketOptionBonus =
...
#if (GAMESERVER_UPDATE >= 803)
  gMuunSystem.GetMuunItemSocketOption(index, &pMsg.Level, &pMsg.SocketOption[0],
                                      &pMsg.SocketOptionBonus);

  gMuunSystem.GetMuunUtilSocketOption(index, &pMsg.Level, &pMsg.SocketOption[0],
                                      &pMsg.SocketOptionBonus);
#endif
...
void GDCrywolfSyncSend(int MapServerGroup, int CrywolfState,
                       int OccupationState) // OK
{
...
void GDGlobalPostSend(int MapServerGroup, BYTE type, char *name,
                      char *message) // OK
{
...
void GDGlobalNoticeSend(int MapServerGroup, BYTE type, BYTE count, BYTE opacity,
                        WORD delay, DWORD color, BYTE speed,
                        char *message) // OK
{
...
void GDCharacterInfoSaveSend(int aIndex) // OK
{
  ...
  pMsg.DiemTichLuyVQ = lpObj->DiemTichLuyVQ;
  pMsg.NhanThuongTichLuyVQ = lpObj->NhanThuongTichLuyVQ;
  pMsg.MocResetTichLuyVQ = lpObj->MocResetTichLuyVQ;
  pMsg.Power = lpObj->PowerATTK;

  // Transaction buffer can be missing items currently moved into chaos/trade UI.
  CItem *lpSaveInventory =
      (lpObj->Transaction == 1) ? lpObj->Inventory1 : lpObj->Inventory;

  for (int n = 0; n < INVENTORY_SIZE; n++) {
    gItemManager.DBItemByteConvert(pMsg.Inventory[n], &lpSaveInventory[n]);
  }
...
#if (GAMESERVER_UPDATE >= 701)
  gPentagramSystem.GDPentagramJewelInfoSaveSend(aIndex,
                                                PENTAGRAM_JEWEL_TYPE_INVENTORY);

  gPentagramSystem.GDPentagramJewelInfoSaveSend(aIndex,
                                                PENTAGRAM_JEWEL_TYPE_WAREHOUSE);
#endif
...
void GDOptionDataSaveSend(int aIndex, BYTE *SkillKey, BYTE GameOption,
                          BYTE QKey, BYTE WKey, BYTE EKey, BYTE ChatWindow,
                          BYTE RKey, DWORD QWERLevel) // OK
{
  LPOBJ lpObj = &gObj[aIndex];
...
void GDResetInfoSaveSend(int aIndex, int ResetDay, int ResetWek,
                         int ResetMon) // OK
{
  LPOBJ lpObj = &gObj[aIndex];
...
void GDMasterResetInfoSaveSend(int aIndex, int MasterResetDay,
                               int MasterResetWek, int MasterResetMon) // OK
{
  LPOBJ lpObj = &gObj[aIndex];
...
void GDCrywolfInfoSaveSend(int MapServerGroup, int CrywolfState,
                           int OccupationState) // OK
{
...
void GDCustomMonsterRewardSaveSend(int aIndex, int MonsterClass, int MapNumber,
                                   int RewardValue1, int RewardValue2) // OK
{
...
void GDCustomBossKillSaveSend(int aIndex, DWORD points) {
  SDHP_CUSTOM_BOSS_KILL_SAVE_SEND pMsg;

  pMsg.header.set(0x57, sizeof(pMsg));
...
void GDVongQuayTichLuySaveSend(int aIndex) {
  if (OBJECT_RANGE(aIndex) == 0 || gObjIsConnected(aIndex) == false ||
      gObj[aIndex].Type != OBJECT_USER || gObj[aIndex].Account[0] == 0 ||
      gObj[aIndex].Name[0] == 0 || gObj[aIndex].MocResetTichLuyVQ == 0) {
    return;
  }

  SDHP_VONGQUAY_TICHLUY_SAVE_SEND pMsg;

  pMsg.header.set(0x58, sizeof(pMsg));
  pMsg.index = aIndex;
  memcpy(pMsg.account, gObj[aIndex].Account, sizeof(pMsg.account));
  memcpy(pMsg.name, gObj[aIndex].Name, sizeof(pMsg.name));
  pMsg.DiemTichLuyVQ = gObj[aIndex].DiemTichLuyVQ;
  pMsg.NhanThuongTichLuyVQ = gObj[aIndex].NhanThuongTichLuyVQ;
  pMsg.MocResetTichLuyVQ = gObj[aIndex].MocResetTichLuyVQ;

  gDataServerConnection.DataSend((BYTE *)&pMsg, sizeof(pMsg));
}

void GDRankingCustomArenaSaveSend(int aIndex, DWORD number, DWORD score,
                                  DWORD rank) // OK
{
  LPOBJ lpObj = &gObj[aIndex];
...
void GS_GDReqCastleNpcBuy(int iMapSvrGroup, int aIndex, int iNpcNumber,
                          int iNpcIndex, int iNpcDfLevel, int iNpcRgLevel,
                          int iNpcMaxHP, int iNpcHP, BYTE btNpcX, BYTE btNpcY,
                          BYTE btNpcDIR, int iBuyCost) {
  if (iMapSvrGroup < 0) {
    return;
  }
...
void GS_GDReqCastleNpcRepair(int iMapSvrGroup, int aIndex, int iNpcNumber,
                             int iNpcIndex, int iRepairCost) {
  if (iMapSvrGroup < 0) {
    return;
  }
...
void GS_GDReqCastleNpcUpgrade(int iMapSvrGroup, int aIndex, int iNpcNumber,
                              int iNpcIndex, int iNpcUpType, int iNpcUpValue,
                              int iNpcUpIndex) {
  if (iMapSvrGroup < 0) {
    return;
  }
...
void GS_GDReqTaxRateChange(int iMapSvrGroup, int aIndex, int iTaxType,
                           int iTaxRate) {
  if (iMapSvrGroup < 0) {
    return;
  }
...
void GS_GDReqSiegeDateChange(int iMapSvrGroup, int aIndex, WORD wStartYear,
                             BYTE btStartMonth, BYTE btStartDay, WORD wEndYear,
                             BYTE btEndMonth, BYTE btEndDay) {
  if (iMapSvrGroup < 0) {
    return;
  }
...
void GS_GDReqCastleOwnerChange(int iMapSvrGroup, BOOL bIsCastleOccupied,
                               char *lpszGuildName) {
  if (iMapSvrGroup < 0) {
    return;
  }
...
void GS_GDReqCastleStateSync(int iMapSvrGroup, int iCastleState,
                             int iTaxRateChaos, int iTaxRateStore,
                             int iTaxHuntZone, char *lpszOwnerGuild) {
  if (iMapSvrGroup < 0) {
    return;
  }
...
void GS_DGAnsCastleTotalInfo(LPBYTE lpRecv) {
  ...
  if (lpMsg->wMapSvrNum != gMapServerManager.GetMapServerGroup())
    LogAdd(LOG_RED,
           "[CastleSiege] PACKET-ERROR [0x80][0x00] GS_DGAnsCastleTotalInfo() "
           "- lpMsg->wMapSvrNum != g_MapServerManager.GetMapServerGroup()");
#else
  return;
#endif
}
...
void GS_DGAnsOwnerGuildMaster(LPBYTE lpRecv) {
  ...
  GCAnsCastleSiegeState(lpMsg->iIndex, lpMsg->iResult, lpMsg->szCastleOwnGuild,
...
void GS_DGAnsCastleNpcBuy(LPBYTE lpRecv) {
  ...
  if (lpMsg->wMapSvrNum != gMapServerManager.GetMapServerGroup()) {
    LogAdd(LOG_RED,
           "[CastleSiege] PACKET-ERROR [0x80][0x03] GS_DGAnsCastleNpcBuy() - "
           "lpMsg->wMapSvrNum != g_MapServerManager.GetMapServerGroup()");
    return;
  }
  ...
  if (lpMsg->iResult == 1) {
    BOOL bAddResult =
        gCastleSiege.AddDbNPC(lpMsg->iNpcNumber, lpMsg->iNpcIndex);
    ...
    if (bAddResult == TRUE) {
      ...
      LogAdd(LOG_BLACK,
             "[CastleSiege] GS_DGAnsCastleNpcBuy() - CCastleSiege::AddDbNPC() "
             "OK - Npc:(CLS:%d, IDX:%d)",
             lpMsg->iNpcNumber, lpMsg->iNpcIndex);
    } else {
      LogAdd(LOG_BLACK,
             "[CastleSiege] GS_DGAnsCastleNpcBuy() - CCastleSiege::AddDbNPC() "
             "FAILED - Npc:(CLS:%d, IDX:%d)",
             lpMsg->iNpcNumber, lpMsg->iNpcIndex);
    }
  } else {
    LogAdd(LOG_BLACK,
           "[CastleSiege] GS_DGAnsCastleNpcBuy() FAILED - Result:(%d), "
           "Npc:(CLS:%d, IDX:%d)",
           lpMsg->iResult, lpMsg->iNpcNumber, lpMsg->iNpcIndex);
  }

  GCAnsNpcBuy(lpMsg->iIndex, lpMsg->iResult, lpMsg->iNpcNumber,
              lpMsg->iNpcIndex);
#else
  return;
#endif
...
void GS_DGAnsCastleNpcRepair(LPBYTE lpRecv) {
  ...
  if (lpMsg->wMapSvrNum != gMapServerManager.GetMapServerGroup()) {
    LogAdd(LOG_RED,
           "[CastleSiege] PACKET-ERROR [0x80][0x04] GS_DGAnsCastleNpcRepair() "
           "- lpMsg->wMapSvrNum != g_MapServerManager.GetMapServerGroup()");
    return;
  }

  if (lpMsg->iResult == 1) {
    BOOL bRepairResult = gCastleSiege.RepairDbNPC(
        lpMsg->iNpcNumber, lpMsg->iNpcIndex, lpMsg->iNpcHp, lpMsg->iNpcMaxHp);
    if (bRepairResult == TRUE) {
      ...
      LogAdd(LOG_BLACK,
             "[CastleSiege] GS_DGAnsCastleNpcRepair() - "
             "CCastleSiege::RepairDbNPC() OK - Npc:(CLS:%d, IDX:%d)",
             lpMsg->iNpcNumber, lpMsg->iNpcIndex);
    } else {
      LogAdd(LOG_BLACK,
             "[CastleSiege] GS_DGAnsCastleNpcRepair() - "
             "CCastleSiege::RepairDbNPC() FAILED - Npc:(CLS:%d, IDX:%d)",
             lpMsg->iNpcNumber, lpMsg->iNpcIndex);
    }
  } else {
    LogAdd(LOG_BLACK,
           "[CastleSiege] GS_DGAnsCastleNpcRepair() FAILED - Result:(%d), "
           "Npc:(CLS:%d, IDX:%d)",
           lpMsg->iResult, lpMsg->iNpcNumber, lpMsg->iNpcIndex);
  }

  GCAnsNpcRepair(lpMsg->iIndex, lpMsg->iResult, lpMsg->iNpcNumber,
                 lpMsg->iNpcIndex, lpMsg->iNpcHp, lpMsg->iNpcMaxHp);
#else
  return;
#endif
}
...
void GS_DGAnsCastleNpcUpgrade(LPBYTE lpRecv) {
  ...
  if (lpMsg->wMapSvrNum != gMapServerManager.GetMapServerGroup()) {
    LogAdd(LOG_RED,
           "[CastleSiege] PACKET-ERROR [0x80][0x05] GS_DGAnsCastleNpcUpgrade() "
           "- lpMsg->wMapSvrNum != g_MapServerManager.GetMapServerGroup()");
    return;
  }
  ...
  if (lpMsg->iResult == 0) {
    LogAdd(LOG_RED,
           "[CastleSiege] ERROR - Castle NPC Upgrade Fail() (CLS:%d, IDX:%d, "
           "UPTYPE:%d, UPVAL:%d)",
           lpMsg->iNpcNumber, lpMsg->iNpcIndex, lpMsg->iNpcUpType,
           lpMsg->iNpcUpValue);
  } else {
    gCastleSiege.UpgradeDbNPC(lpMsg->iIndex, lpMsg->iNpcNumber,
                              lpMsg->iNpcIndex, lpMsg->iNpcUpType,
                              lpMsg->iNpcUpValue, lpMsg->iNpcUpIndex);
    LogAdd(LOG_BLACK,
           "[CastleSiege] [0x80][0x06] GS_DGAnsTaxInfo() - Npc Upgrade OK "
           "(CLS:%d, IDX:%d, UPTYPE:%d, UPVAL:%d)",
           lpMsg->iNpcNumber, lpMsg->iNpcIndex, lpMsg->iNpcUpType,
           lpMsg->iNpcUpValue);
  }

  GCAnsNpcUpgrade(lpMsg->iIndex, lpMsg->iResult, lpMsg->iNpcNumber,
...
void GS_DGAnsTaxInfo(LPBYTE lpRecv) {
  ...
  if (lpMsg->wMapSvrNum != gMapServerManager.GetMapServerGroup()) {
    LogAdd(LOG_RED,
           "[CastleSiege] PACKET-ERROR [0x80][0x06] GS_DGAnsTaxInfo() - "
           "lpMsg->wMapSvrNum != g_MapServerManager.GetMapServerGroup()");
    return;
  }

  if (lpMsg->iResult == TRUE) {
    GCAnsTaxMoneyInfo(lpMsg->iIndex, lpMsg->iResult, lpMsg->iTaxRateChaos,
                      lpMsg->iTaxRateStore, lpMsg->i64CastleMoney);
...
void GS_DGAnsTaxRateChange(LPBYTE lpRecv) {
  ...
  if (lpMsg->wMapSvrNum != gMapServerManager.GetMapServerGroup()) {
    LogAdd(LOG_RED,
           "[CastleSiege] PACKET-ERROR [0x80][0x07] GS_DGAnsTaxRateChange() - "
           "lpMsg->wMapSvrNum != g_MapServerManager.GetMapServerGroup()");
    return;
  }
  ...
  if (lpMsg->iResult == 1) {
    ...
    GCAnsTaxRateChange(lpMsg->iIndex, lpMsg->iResult, lpMsg->iTaxKind,
                       lpMsg->iTaxRate);
  }
#else
  return;
...
void GS_DGAnsCastleMoneyChange(LPBYTE lpRecv) {
  ...
  if (lpMsg->wMapSvrNum != gMapServerManager.GetMapServerGroup()) {
    LogAdd(
        LOG_RED,
        "[CastleSiege] PACKET-ERROR [0x80][0x08] GS_DGAnsCastleMoneyChange() - "
        "lpMsg->wMapSvrNum != g_MapServerManager.GetMapServerGroup()");
    return;
  }
  ...
  if (lpMsg->iResult == TRUE) {
    if (gObjIsConnected(lpMsg->iIndex)) {
      if (lpMsg->iMoneyChanged < 0) {
        ...
        LogAdd(LOG_BLACK,
               "[CastleSiege] [0x80][0x08] GS_DGAnsCastleMoneyChange() - "
               "Withdraw Request OK [%s][%s] (ReqMoney:%d, TotMoney:%I64d)",
               gObj[lpMsg->iIndex].Account, gObj[lpMsg->iIndex].Name,
               lpMsg->iMoneyChanged, lpMsg->i64CastleMoney);
      }
    }
    gCastleSiege.SetCastleMoney(lpMsg->i64CastleMoney);
  }
...
void GS_DGAnsSiegeDateChange(LPBYTE lpRecv) {
  ...
  if (lpMsg->wMapSvrNum != gMapServerManager.GetMapServerGroup()) {
    LogAdd(LOG_RED,
           "[CastleSiege] PACKET-ERROR [0x80][0x09] GS_DGAnsSiegeDateChange() "
           "- lpMsg->wMapSvrNum != g_MapServerManager.GetMapServerGroup()");
    return;
  }
#else
...
void GS_DGAnsGuildMarkRegInfo(LPBYTE lpRecv) {
  ...
  if (lpMsg->wMapSvrNum != gMapServerManager.GetMapServerGroup()) {
    LogAdd(LOG_RED,
           "[CastleSiege] PACKET-ERROR [0x80][0x0A] GS_DGAnsGuildMarkRegInfo() "
           "- lpMsg->wMapSvrNum != g_MapServerManager.GetMapServerGroup()");
    return;
  }
...
void GS_DGAnsSiegeEndedChange(LPBYTE lpRecv) {
  ...
  if (lpMsg->wMapSvrNum != gMapServerManager.GetMapServerGroup()) {
    LogAdd(LOG_RED,
           "[CastleSiege] PACKET-ERROR [0x80][0x0B] GS_DGAnsSiegeEndedChange() "
           "- lpMsg->wMapSvrNum != g_MapServerManager.GetMapServerGroup()");
    return;
  }
#else
...
void GS_DGAnsCastleOwnerChange(LPBYTE lpRecv) {
  ...
  if (lpMsg->wMapSvrNum != gMapServerManager.GetMapServerGroup()) {
    LogAdd(
        LOG_RED,
        "[CastleSiege] PACKET-ERROR [0x80][0x0C] GS_DGAnsCastleOwnerChange() - "
        "lpMsg->wMapSvrNum != g_MapServerManager.GetMapServerGroup()");
    return;
  }
#else
...
void GS_DGAnsRegAttackGuild(LPBYTE lpRecv) {
  ...
  if (lpMsg->wMapSvrNum != gMapServerManager.GetMapServerGroup()) {
    LogAdd(LOG_RED,
           "[CastleSiege] PACKET-ERROR [0x80][0x0D] GS_DGAnsRegAttackGuild() - "
           "lpMsg->wMapSvrNum != g_MapServerManager.GetMapServerGroup()");
    return;
  }
...
void GS_DGAnsRestartCastleState(LPBYTE lpRecv) {
  ...
  if (lpMsg->wMapSvrNum != gMapServerManager.GetMapServerGroup()) {
    LogAdd(
        LOG_RED,
        "[CastleSiege] PACKET-ERROR [0x80][0x0E] GS_DGAnsRestartCastleState() "
        "- lpMsg->wMapSvrNum != g_MapServerManager.GetMapServerGroup()");
    return;
  } else {
...
void GS_DGAnsMapSvrMsgMultiCast(LPBYTE lpRecv) {
  ...
  if (lpMsg->wMapSvrNum != gMapServerManager.GetMapServerGroup()) {
    LogAdd(
        LOG_RED,
        "[CastleSiege] PACKET-ERROR [0x80][0x0F] GS_DGAnsMapSvrMsgMultiCast() "
        "- lpMsg->wMapSvrNum != g_MapServerManager.GetMapServerGroup()");
    return;
  }
...
void GS_DGAnsRegGuildMark(LPBYTE lpRecv) {
  ...
  if (lpMsg->wMapSvrNum != gMapServerManager.GetMapServerGroup()) {
    LogAdd(LOG_RED,
           "[CastleSiege] PACKET-ERROR [0x80][0x10] GS_DGAnsRegGuildMark() - "
           "lpMsg->wMapSvrNum != g_MapServerManager.GetMapServerGroup()");
    return;
  } else {
...
void GS_DGAnsGuildMarkReset(LPBYTE lpRecv) {
  ...
  if (lpMsg->wMapSvrNum != gMapServerManager.GetMapServerGroup()) {
    LogAdd(LOG_RED,
           "[CastleSiege] PACKET-ERROR [0x80][0x11] GS_DGAnsGuildMarkReset() - "
           "lpMsg->wMapSvrNum != g_MapServerManager.GetMapServerGroup()");
    return;
  }
#else
...
void GS_DGAnsGuildSetGiveUp(LPBYTE lpRecv) {
  ...
  if (lpMsg->wMapSvrNum != gMapServerManager.GetMapServerGroup()) {
    LogAdd(LOG_RED,
           "[CastleSiege] PACKET-ERROR [0x80][0x12] GS_DGAnsGuildSetGiveUp() - "
           "lpMsg->wMapSvrNum != g_MapServerManager.GetMapServerGroup()");
    return;
  }

  GCAnsGiveUpCastleSiege(lpMsg->iIndex, lpMsg->iResult, lpMsg->bIsGiveUp,
                         lpMsg->iRegMarkCount, lpMsg->szGuildName);
#else
  return;
#endif
}
...
void GS_DGAnsNpcRemove(LPBYTE lpRecv) {
  ...
  if (lpMsg->wMapSvrNum != gMapServerManager.GetMapServerGroup()) {
    LogAdd(LOG_RED,
           "[CastleSiege] PACKET-ERROR [0x80][0x16] GS_DGAnsNpcRemove() - "
           "lpMsg->wMapSvrNum != g_MapServerManager.GetMapServerGroup()");
    return;
  }
#else
...
void GS_DGAnsCastleTributeMoney(LPBYTE lpRecv) {
  ...
  if (lpMsg->wMapSvrNum != gMapServerManager.GetMapServerGroup()) {
    LogAdd(
        LOG_RED,
        "[CastleSiege] PACKET-ERROR [0x80][0x18] GS_DGAnsCastleTributeMoney() "
        "- lpMsg->wMapSvrNum != g_MapServerManager.GetMapServerGroup()");
    return;
  }

  LogAdd(LOG_BLACK,
         "[CastleSiege] [0x80][0x18] GS_DGAnsCastleTributeMoney() - Money "
         "Tribute OK (%d)",
         gCastleSiegeSync.GetTributeMoney());
...
void GS_DGAnsResetCastleTaxInfo(LPBYTE lpRecv) {
  ...
  if (lpMsg->wMapSvrNum != gMapServerManager.GetMapServerGroup()) {
    LogAdd(
        LOG_RED,
        "[CastleSiege] PACKET-ERROR [0x80][0x19] GS_DGAnsResetCastleTaxInfo() "
        "- lpMsg->wMapSvrNum != g_MapServerManager.GetMapServerGroup()");
    return;
  } else {
    if (lpMsg->iResult == 1) {
...
void GS_DGAnsResetSiegeGuildInfo(LPBYTE lpRecv) {
  ...
  if (lpMsg->wMapSvrNum != gMapServerManager.GetMapServerGroup()) {
    LogAdd(
        LOG_RED,
        "[CastleSiege] PACKET-ERROR [0x80][0x1A] GS_DGAnsResetSiegeGuildInfo() "
        "- lpMsg->wMapSvrNum != g_MapServerManager.GetMapServerGroup()");
    return;
  }
#else
...
void GS_DGAnsResetRegSiegeInfo(LPBYTE lpRecv) {
  ...
  if (lpMsg->wMapSvrNum != gMapServerManager.GetMapServerGroup()) {
    LogAdd(
        LOG_RED,
        "[CastleSiege] PACKET-ERROR [0x80][0x1B] GS_DGAnsResetRegSiegeInfo() - "
        "lpMsg->wMapSvrNum != g_MapServerManager.GetMapServerGroup()");
    return;
  }
#else
...
void GS_DGAnsCastleInitData(LPBYTE lpRecv) {
  ...
  CSP_CSINITDATA *lpMsgBody =
      (CSP_CSINITDATA *)(lpRecv + sizeof(CSP_ANS_CSINITDATA));
  ...
  if (lpMsg->wMapSvrNum != gMapServerManager.GetMapServerGroup()) {
    LogAdd(LOG_RED,
           "[CastleSiege] PACKET-ERROR [0x81] GS_DGAnsCastleInitData() - "
           "lpMsg->wMapSvrNum != g_MapServerManager.GetMapServerGroup()");
    return;
  }
  ...
  if (gCastleSiege.GetDataLoadState() != 2) {
    LogAdd(LOG_RED,
           "[CastleSiege] CASTLE SIEGE DATA SETTING FAILED [0x81] - "
           "m_iCastleDataLoadState != CASTLESIEGE_DATALOAD_2 (%d)",
           gCastleSiege.GetDataLoadState());
    return;
  }
  ...
  if (lpMsg->iResult == FALSE) {
    LogAdd(LOG_RED, "[CastleSiege] CASTLE SIEGE DATA SETTING FAILED [0x81] - "
                    "lpMsg->iResult == 0");
    return;
  }
  ...
  if (bRET_VAL == FALSE) {
    LogAdd(LOG_RED, "[CastleSiege] CASTLE SIEGE DATA SETTING FAILED [0x81] - "
                    "g_CastleSiege.SetCastleInitData() == FALSE");
    return;
  }
  ...
  if (bRET_VAL == FALSE) {
    LogAdd(LOG_RED, "[CastleSiege] CASTLE SIEGE DATA SETTING FAILED [0x81] - "
                    "g_CastleSiege.SetCastleNpcData() == FALSE");
    return;
  }
...
void GS_DGAnsCastleNpcInfo(LPBYTE lpRecv) {
  ...
  if (lpMsg->wMapSvrNum != gMapServerManager.GetMapServerGroup()) {
    LogAdd(LOG_RED,
           "[CastleSiege] PACKET-ERROR [0x82] GS_DGAnsCastleNpcInfo() - "
           "lpMsg->wMapSvrNum != g_MapServerManager.GetMapServerGroup()");
    return;
  }
#else
...
void GS_DGAnsAllGuildMarkRegInfo(LPBYTE lpRecv) {
  ...
  CSP_GUILDREGINFO *lpMsgBody =
      (CSP_GUILDREGINFO *)(lpRecv + sizeof(CSP_ANS_ALLGUILDREGINFO));
  char cBUFFER[1668];
  ...
  if (lpMsg->wMapSvrNum != gMapServerManager.GetMapServerGroup()) {
    LogAdd(LOG_RED,
           "[CastleSiege] PACKET-ERROR [0x83] GS_DGAnsAllGuildMarkRegInfo() - "
           "lpMsg->wMapSvrNum != g_MapServerManager.GetMapServerGroup()");
    return;
  }
  ...
  if (lpMsg->iResult == TRUE) {
    ...
    for (int i = 0; i < lpMsg->iCount; i++) {
      ...
      lpMsgSendBody[i].btRegMarks1 =
          SET_NUMBERHB(SET_NUMBERHW(lpMsgBody[i].iRegMarkCount));
      lpMsgSendBody[i].btRegMarks2 =
          SET_NUMBERLB(SET_NUMBERHW(lpMsgBody[i].iRegMarkCount));
      lpMsgSendBody[i].btRegMarks3 =
          SET_NUMBERHB(SET_NUMBERLW(lpMsgBody[i].iRegMarkCount));
      lpMsgSendBody[i].btRegMarks4 =
          SET_NUMBERLB(SET_NUMBERLW(lpMsgBody[i].iRegMarkCount));
      memcpy(&lpMsgSendBody[i].szGuildName, lpMsgBody[i].szGuildName,
             sizeof(lpMsgSendBody[i].szGuildName));
    }
  }

  lpMsgSend->h.set(0xB4, (lpMsgSend->iCount * sizeof(PMSG_CSREGGUILDLIST) +
                          sizeof(PMSG_ANS_CSREGGUILDLIST)));

  DataSend(lpMsg->iIndex, (LPBYTE)lpMsgSend,
           (lpMsgSend->iCount * sizeof(PMSG_CSREGGUILDLIST) +
            sizeof(PMSG_ANS_CSREGGUILDLIST)));
#else
  return;
#endif
...
void GS_DGAnsFirstCreateNPC(LPBYTE lpRecv) {
  ...
  if (lpMsg->wMapSvrNum != gMapServerManager.GetMapServerGroup()) {
    LogAdd(LOG_RED,
           "[CastleSiege] PACKET-ERROR [0x84] GS_DGAnsFirstCreateNPC() - "
           "lpMsg->wMapSvrNum != g_MapServerManager.GetMapServerGroup()");
    return;
  }
...
void GS_DGAnsCalcREgGuildList(LPBYTE lpRecv) {
  ...
  CSP_CALCREGGUILDLIST *lpMsgBody =
      (CSP_CALCREGGUILDLIST *)(lpRecv + sizeof(CSP_ANS_CALCREGGUILDLIST));
  ...
  if (lpMsg->wMapSvrNum != gMapServerManager.GetMapServerGroup()) {
    LogAdd(LOG_RED,
           "[CastleSiege] PACKET-ERROR [0x85] GS_DGAnsCalcREgGuildList() - "
           "lpMsg->wMapSvrNum != g_MapServerManager.GetMapServerGroup()");
    return;
  }
...
void GS_DGAnsCsGulidUnionInfo(LPBYTE lpRecv) {
  ...
  CSP_CSGUILDUNIONINFO *lpMsgBody =
      (CSP_CSGUILDUNIONINFO *)(lpRecv + sizeof(CSP_ANS_CSGUILDUNIONINFO));
  ...
  if (lpMsg->wMapSvrNum != gMapServerManager.GetMapServerGroup()) {
    LogAdd(LOG_RED,
           "[CastleSiege] PACKET-ERROR [0x86] GS_DGAnsCsGulidUnionInfo() - "
           "lpMsg->wMapSvrNum != g_MapServerManager.GetMapServerGroup()");
    return;
  }
...
void GS_DGAnsCsSaveTotalGuildInfo(LPBYTE lpRecv) {
  ...
  if (lpMsg->wMapSvrNum != gMapServerManager.GetMapServerGroup()) {
    LogAdd(LOG_RED,
           "[CastleSiege] PACKET-ERROR [0x87] GS_DGAnsCsSaveTotalGuildInfo() - "
           "lpMsg->wMapSvrNum != g_MapServerManager.GetMapServerGroup()");
    return;
  }
...
void GS_DGAnsCsLoadTotalGuildInfo(LPBYTE lpRecv) {
  ...
  CSP_CSLOADTOTALGUILDINFO *lpMsgBody =
      (CSP_CSLOADTOTALGUILDINFO *)(lpRecv +
  ...
  if (lpMsg->wMapSvrNum != gMapServerManager.GetMapServerGroup()) {
    LogAdd(LOG_RED,
           "[CastleSiege] PACKET-ERROR [0x88] GS_DGAnsCsLoadTotalGuildInfo() - "
           "lpMsg->wMapSvrNum != g_MapServerManager.GetMapServerGroup()");
    return;
  }
...
void GS_DGAnsCastleNpcUpdate(LPBYTE lpRecv) {
  ...
  if (lpMsg->wMapSvrNum != gMapServerManager.GetMapServerGroup()) {
    LogAdd(LOG_RED,
           "[CastleSiege] PACKET-ERROR [0x89] GS_DGAnsCastleNpcUpdate() - "
           "lpMsg->wMapSvrNum != g_MapServerManager.GetMapServerGroup()");
    return;
  }
  ...
  LogAdd(LOG_BLACK,
         "[CastleSiege] [0x89] GS_DGAnsCastleNpcUpdate() - Npc Data Update "
         "Result : '%s'",
         szResult);
#else
  return;
...
// Set Coin System
void GDSetCoinSend(int aIndex, int value1, int value2, int value3,
                   char *LogName) // OK
{
  LPOBJ lpObj = &gObj[aIndex];
  ...
  if (value1 > 0) {
    lpObj->Coin1 += value1;
    gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0,
                         gMessage.GlobalText(730), value1);
  }
  if (value2 > 0) {
    lpObj->Coin2 += value2;
    gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0,
                         gMessage.GlobalText(731), value2);
  }
  if (value3 > 0) {
    lpObj->Coin3 += value3;
    gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0,
                         gMessage.GlobalText(732), value3);
  }

  if (value1 < 0) {
    lpObj->Coin1 += value1;
    gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0,
                         gMessage.GlobalText(760), value1);
  }
  if (value2 < 0) {
    lpObj->Coin2 += value2;
    gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0,
                         gMessage.GlobalText(761), value2);
  }
  if (value3 < 0) {
    lpObj->Coin3 += value3;
    gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0,
                         gMessage.GlobalText(762), value3);
  }
  ...
  LogAdd(LOG_ALERT, "[%s][%s] WZ_SetCoin(%s) Val1: %d, Val2: %d, Val3: %d",
         lpObj->Account, lpObj->Name, LogName, value1, value2, value3);
}

void GDMocNapAutoRewardSend(int aIndex) {
...
void GDMocNapPaymentCreateSend(int aIndex, int amount) {
  if (OBJECT_RANGE(aIndex) == 0 || gObjIsConnectedGP(aIndex) == 0) {
    return;
  }
...
void GDKillSystemSend(int aIndex, int bIndex) // OK
{
  return;
  if (gObjIsAccountValid(aIndex, gObj[aIndex].Account) == 0 ||
      gObjIsAccountValid(bIndex, gObj[bIndex].Account) == 0) {
    return;
  }

...
void CTrade::ResetTrade(int aIndex) // OK
{
	...
	lpObj->TradeCoin2 = 0;
	lpObj->TradeCoin3 = 0;
	GCCoinSend(aIndex, lpObj->Coin1, lpObj->Coin2, lpObj->Coin3);
...
void CTrade::CGTradeRequestRecv(PMSG_TRADE_REQUEST_RECV* lpMsg, int aIndex) // OK
{
	...
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GlobalText(861));
		return;
	}

	if (gServerInfo.m_PKDisableTrade == 1 && lpTarget->PKLevel >= 5)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GlobalText(862));
		return;
	}
	...
	if (gH_BotMix.TradeOpen(aIndex, bIndex) == 1)
	{
		return;
	}
...
#ifdef TRADE_SYSTEM_COIN
	if (lpMsg->coin1 >= MAX_TRADE_MONEY || lpObj->Coin1 < lpMsg->coin1)
	{
		return;
	}

	if (lpMsg->coin2 >= MAX_TRADE_MONEY || lpObj->Coin2 < lpMsg->coin2)
	{
		return;
	}

	if (lpMsg->coin3 >= MAX_TRADE_MONEY || lpObj->Coin3 < lpMsg->coin3)
	{
		return;
	}
#endif // TRADE_SYSTEM_COIN
...
#ifdef TRADE_SYSTEM_COIN
	GCCoinSend(aIndex, (lpObj->Coin1 - lpMsg->coin1), (lpObj->Coin2 - lpMsg->coin2), (lpObj->Coin3 - lpMsg->coin3));
#endif // TRADE_SYSTEM_COIN
...
#ifdef TRADE_SYSTEM_COIN
	lpObj->TradeCoin1 = lpMsg->coin1;
	lpObj->TradeCoin2 = lpMsg->coin2;
	lpObj->TradeCoin3 = lpMsg->coin3;

	this->GCTradeMoneySend(bIndex, lpMsg->money, lpMsg->coin1, lpMsg->coin2, lpMsg->coin3);
#else
...
void CTrade::CGTradeOkButtonRecv(PMSG_TRADE_OK_BUTTON_RECV* lpMsg, int aIndex) // OK
{
	...
	if ((GetTickCount() - lpObj->TradeOkTime) < 2000)
	{
		return;
	}
	...
	// BotMix: NPC bot never sets TradeOk, so check BotMix BEFORE dual-confirm check
	if (gH_BotMix.IsInTrade(aIndex) == 1 && lpMsg->flag == 1 && lpObj->TradeOk == 1)
	{
		gH_BotMix.TradeOk(aIndex);
		return;
	}
	...
	{
		gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, "Trade failed: pentagram inventory check failed.");
		gNotice.GCNoticeSend(bIndex, 1, 0, 0, 0, 0, 0, "Trade failed: pentagram inventory check failed.");
	...
	{
		gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, "Trade failed: max Zen or Coin limit reached.");
		gNotice.GCNoticeSend(bIndex, 1, 0, 0, 0, 0, 0, "Trade failed: max Zen or Coin limit reached.");
		this->ResetTrade(aIndex);
...
#ifdef TRADE_SYSTEM_COIN
	if (gObjCheckMaxCoin(aIndex, lpTarget->TradeCoin1, 1) == 0 || gObjCheckMaxCoin(bIndex, lpObj->TradeCoin1, 1) == 0)
	{
		gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, "Trade failed: max Zen or Coin limit reached.");
		gNotice.GCNoticeSend(bIndex, 1, 0, 0, 0, 0, 0, "Trade failed: max Zen or Coin limit reached.");
		this->ResetTrade(aIndex);
	...
	if (gObjCheckMaxCoin(aIndex, lpTarget->TradeCoin2, 2) == 0 || gObjCheckMaxCoin(bIndex, lpObj->TradeCoin2, 2) == 0)
	{
		gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, "Trade failed: max Zen or Coin limit reached.");
		gNotice.GCNoticeSend(bIndex, 1, 0, 0, 0, 0, 0, "Trade failed: max Zen or Coin limit reached.");
		this->ResetTrade(aIndex);
	...
	if (gObjCheckMaxCoin(aIndex, lpTarget->TradeCoin3, 3) == 0 || gObjCheckMaxCoin(bIndex, lpObj->TradeCoin3, 3) == 0)
	{
		gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, "Trade failed: max Zen or Coin limit reached.");
		gNotice.GCNoticeSend(bIndex, 1, 0, 0, 0, 0, 0, "Trade failed: max Zen or Coin limit reached.");
		this->ResetTrade(aIndex);
...
#ifdef TRADE_SYSTEM_COIN
	int MyCoin1 = lpTarget->TradeCoin1 - lpObj->TradeCoin1;
	int MyCoin2 = lpTarget->TradeCoin2 - lpObj->TradeCoin2;
	int MyCoin3 = lpTarget->TradeCoin3 - lpObj->TradeCoin3;
#endif // TRADE_SYSTEM_COIN
...
#ifdef TRADE_SYSTEM_COIN
	int YourCoin1 = lpObj->TradeCoin1 - lpTarget->TradeCoin1;
	int YourCoin2 = lpObj->TradeCoin2 - lpTarget->TradeCoin2;
	int YourCoin3 = lpObj->TradeCoin3 - lpTarget->TradeCoin3;
#endif // TRADE_SYSTEM_COIN
...
#ifdef TRADE_SYSTEM_COIN
	GDSetCoinSend(lpObj->Index, (MyCoin1), (MyCoin2), (MyCoin3), "Trade Coin");

	GDSetCoinSend(lpTarget->Index, (YourCoin1), (YourCoin2), (YourCoin3), "Trade Coin");
...
void CTrade::CGTradeCancelButtonRecv(int aIndex) // OK
{
	...
	if (gH_BotMix.IsInTrade(aIndex) == 1)
	{
		gH_BotMix.TradeCancel(aIndex);
		return;
	}
...
#ifdef TRADE_SYSTEM_COIN
void CTrade::GCTradeMoneySend(int aIndex, DWORD money, DWORD coin1, DWORD coin2, DWORD coin3) // OK
{
...
void CTrade::CloseTrade(int aIndex, BYTE result) // OK
{
	...
	this->GCTradeResultSend(aIndex, result);
	...
	if (lpTarget->TargetNumber != aIndex)
	{
		return;
	}
	...
	this->GCTradeResultSend(bIndex, result);
}

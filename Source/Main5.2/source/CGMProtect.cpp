...
{
	template<typename T>
	void readTrailingVector(FILE* fp, std::vector<T>& value)
	{
		...
		if (fseek(fp, currentPosition, SEEK_SET) != 0)
		{
			value.clear();
			return;
		}

		if (endPosition < 0 || (endPosition - currentPosition) < static_cast<long>(sizeof(DWORD)))
		{
			value.clear();
			return;
		}
...
bool CGMProtect::IsXShieldEnabled()
{
	return (m_Kernel.AntihackEnable != 0);
}

WORD CGMProtect::GetXShieldAntiHackPort()
{
	return ((m_Kernel.AntiHackPort == 0) ? 55111 : m_Kernel.AntiHackPort);
}

char* CGMProtect::GetXShieldHackVersion()
{
	return m_Kernel.AntihackVersion;
}
...
bool CGMProtect::LoadMainFileInfo(MAIN_FILE_INFO& mainInfo, std::string& filePath)
{
	...
	if (fp != nullptr)
	{
		...
		else
		{
			...
			/*+022*/ readTrailingVector(fp, CustomBow);
			fclose(fp);
		}
	}
	else

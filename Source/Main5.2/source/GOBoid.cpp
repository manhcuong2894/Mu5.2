...
bool RenderBug(OBJECT* o, bool bForceRender)
{
	if (o->Live)
	{
		...
		if (o->Visible && o->Alpha > 0)
		{
			...
			float ownerScale = to->Scale;
			if (ownerScale > 1.0f) // If owner is unusually large (e.g., Rage Fighter's 1.03f or 1.35f scale) 
			{
				ownerScale = (SceneFlag == CHARACTER_SCENE) ? 1.0f : 0.93f; // Normalize mount scale so it doesn't inflate
			}

			if (bForceRender)
			{
				o->Scale = o->Scale;
				o->Scale = (float)((ownerScale / 0.01) * (o->BackupScale * 0.01));
			}

...
int g_iCrowdVisiblePlayerCount = 0;
int g_iEquipmentFxRenderLevelCap = -1;
...
static bool IsCrowdPlayerObject(const CHARACTER* c, const OBJECT* o)
{
	return (c != NULL
		&& o != NULL
		&& c != Hero
		&& (Hero == NULL || o != &Hero->Object)
		&& o->Kind == KIND_PLAYER
		&& o->Type == MODEL_PLAYER);
}

static float GetDistance2ToHero(const OBJECT* o)
{
	if (o == NULL || Hero == NULL)
	{
		return 0.0f;
	...
	const float dx = Hero->Object.Position[0] - o->Position[0];
	const float dy = Hero->Object.Position[1] - o->Position[1];
	return (dx * dx) + (dy * dy);
}

static int GetCrowdAdaptivePerformanceLevel()
{
	int level = 0;
	...
	if (FPS > 1.0f && gmProtect != NULL && gmProtect->ajust_fps_render > 0)
	{
		...
		if (g_iCrowdVisiblePlayerCount >= 40)
		{
			...
			else if (fpsRatio < 0.58f && level < 2)
			{
				level = 2;
			}
			else if (fpsRatio < 0.75f && level < 1)
			{
				level = 1;
			}
		}
...
static bool IsCrowdAdaptiveLodActive()
{
	...
	return (g_iCrowdVisiblePlayerCount >= 40
		|| (performanceLevel > 0 && g_iCrowdVisiblePlayerCount >= 35));
}
...
enum ECrowdEquipmentFxZone
{
	...
	CROWD_EQUIPMENT_FX_MID = 1,
	CROWD_EQUIPMENT_FX_FAR = 2,
};

struct CrowdEquipmentFxThresholds
{
	float FullEnterDist2;
	float FullExitDist2;
	float FarEnterDist2;
	float FarExitDist2;
};
...
{
	float fullEnterDist = 260.0f;
	float fullExitDist = 320.0f;
	float farEnterDist = 460.0f;
	float farExitDist = 540.0f;

	if (g_iCrowdVisiblePlayerCount >= 115)
	{
		fullEnterDist = 150.0f;
		fullExitDist = 190.0f;
		farEnterDist = 280.0f;
		farExitDist = 340.0f;
	}
	...
	thresholds.FullEnterDist2 = fullEnterDist * fullEnterDist;
	thresholds.FullExitDist2 = fullExitDist * fullExitDist;
	thresholds.FarEnterDist2 = farEnterDist * farEnterDist;
	thresholds.FarExitDist2 = farExitDist * farExitDist;
	return thresholds;
}
...
static int GetCrowdEquipmentFxZone(const CHARACTER* c, const OBJECT* o)
{
	...
	if (Hero == NULL || !IsCrowdAdaptiveLodActive())
	{
...
static int GetCrowdEquipmentFxUpdateStep(const OBJECT* o)
{
	...
	if (g_iCrowdVisiblePlayerCount >= 90)
	{
		if (dist2 > (480.0f * 480.0f))
		{
			step = 4;
		}
...
{
	const int step = GetCrowdEquipmentFxUpdateStep(o);
	if (step <= 1)
	{
		return true;
	}
...
{
	if (!IsCrowdPlayerObject(c, o) || !IsCrowdAdaptiveLodActive())
	{
		return true;
	}
...
int GetCrowdAdaptivePoseUpdateStep(const OBJECT* o)
{
	...
	{
	case 3:
		if (dist2 > (400.0f * 400.0f) && step < 4)
		{
			step = 4;
		}
		break;
	case 2:
		if (dist2 > (520.0f * 520.0f) && step < 3)
		{
			step = 3;
		}
		break;
...
bool ShouldUpdateCrowdPoseThisFrame(const OBJECT* o, unsigned int salt)
{
	...
	if (o->AnimationFrame <= 0.0f && o->PriorAnimationFrame <= 0.0f)
	{
		return true;
	}
...
static bool ShouldUpdateCrowdClothThisFrame(const CHARACTER* c, const OBJECT* o)
{
	...
	const unsigned int seed = g_uiCrowdAnimationFrameId + 131u + (unsigned int)(((UINT_PTR)o) >> 5);
	return (seed % (unsigned int)step) == 0;
}
...
bool ShouldUseCrowdSimplifiedRender(const OBJECT* o)
{
	...
	if (g_iCrowdVisiblePlayerCount < 40 && GetCrowdAdaptivePerformanceLevel() == 0)
	{
		return false;
	}
...
bool ShouldCullCrowdLinkedObject(const CHARACTER* c, const OBJECT* o, int Type)
{
	...
	if (g_iCrowdVisiblePlayerCount < 75 && performanceLevel < 2)
	{
		return false;
	}
	...
	if (performanceLevel >= 3)
	{
		return !isMainEquipment && dist2 > (500.0f * 500.0f);
	}

	if (performanceLevel >= 2)
	{
		return !isMainEquipment && dist2 > (620.0f * 620.0f);
	}

	if (g_iCrowdVisiblePlayerCount >= 110)
	{
		return !isMainEquipment && dist2 > (480.0f * 480.0f);
	}

	if (g_iCrowdVisiblePlayerCount >= 100)
	{
		return !isMainEquipment && dist2 > (560.0f * 560.0f);
	}
...
void SetPlayerStop(CHARACTER* c)
{
	...
	if (o->animation_hide_traditional() && gCharacterManager.GetBaseClass(c->Class) != CLASS_DARK_LORD)
	{
...
void SetPlayerHighBowAttack(CHARACTER* sc)
{
	...
	bool movement_normal_hide = true;
	if (so->animation_hide_traditional() && gCharacterManager.GetBaseClass(sc->Class) != CLASS_DARK_LORD)
		movement_normal_hide = false;
	...
	if (so->Type == MODEL_PLAYER)
	{
		...
		{
			if ((sc_type_helper == MODEL_HELPER + 37 || so->movementType == Movement::Fenrir) && !sc->SafeZone && movement_normal_hide)
			{
				SetAction(so, PLAYER_HIT_RIDE_BOW_UP, true);
			}
			else if ((sc_type_helper == MODEL_HELPER + 2 || so->movementType == Movement::Uniria) && !sc->SafeZone && movement_normal_hide)
			{
				SetAction(so, PLAYER_HIT_RIDE_BOW_UP, true);
			}
			else if ((sc_type_helper == MODEL_HELPER + 3 || so->movementType == Movement::Dinorant) && !sc->SafeZone && movement_normal_hide)
			{
				SetAction(so, PLAYER_HIT_RIDE_BOW_UP, true);
			}
			else if ((sc_type_helper == MODEL_HELPER + 4 || so->movementType == Movement::Horse) && !sc->SafeZone && movement_normal_hide)
			{
				SetAction(so, PLAYER_HIT_RIDE_BOW_UP, true);
			}
			else
...
void SetPlayerAttack(CHARACTER* c)
{
	...
	if (o->Type == MODEL_PLAYER)
	{
		...
		else if ((sc_type_helper == MODEL_HELPER + 2 || o->movementType == Movement::Uniria || sc_type_helper == MODEL_HELPER + 3 || o->movementType == Movement::Dinorant) && !c->SafeZone && movement_normal_hide)
		{
			if (sc_type_weapon01 < MODEL_SPEAR || sc_type_weapon01 >= MODEL_SPEAR + 5)
			{
...
void MoveCharacterClient(CHARACTER* cc)
{
	...
	if (co->Live)
	{
		...
		bool bIsHero = (cc == Hero);
		if (!bIsHero && !co->Visible)
		{
...
void RenderLinkObject(float x, float y, float z, CHARACTER* c, PART_t* f, int Type, int Level, int Option1, bool Link, bool Translate, int RenderType, bool bRightHandItem)
{
	...
	const bool isQuiver = (WeaponKind == ItemKind2::QUIVER);
	...
	if (Link)
	{
						...
						else
						{
							...
							default:
								...
								else
								{
									if (isQuiver)
									{
										// Give every quiver a dedicated diagonal back pose instead of the generic weapon fallback.
										Angle[0] = 60.0;
										Angle[1] = 0.0;
										Angle[2] = -80.0;
										AngleMatrix(Angle, Matrix);
									...
									else if (Type > (MODEL_BOW + 25) && WeaponKind == ItemKind2::BOW)
									{
										Angle[0] = -60.0;
										Angle[1] = 0.0;
										Angle[2] = -80.0;
		...
		if (!isQuiver && !bRightHandItem && (Type < MODEL_SHIELD || Type >= MODEL_HELM) && Type != (MODEL_BOW + 20))
		{
	...
	pModel->PrepareGpuAssist(pObject, Type, RenderType, false, false, Translate);
	...
	if (g_pOption->GetRenderEffect() == true)
	{
		if (o->Alpha >= 0.5f && c->HideShadow == false)
		{
			if (World != WD_10HEAVEN && (o->Type == MODEL_PLAYER) && (!(MODEL_HELPER + 2 <= c->Helper.Type && c->Helper.Type <= MODEL_HELPER + 3) || c->SafeZone)
				...
				&& ShouldRenderCrowdPlayerShadow(c, o))
			{
...
{
	g_iCrowdVisiblePlayerCount = 0;

	for (int i = 0; i < MAX_CHARACTERS_CLIENT; ++i)
	{
		CHARACTER* visibleCharacter = gmCharacters->GetCharacter(i);
	...
	{
		g_iRemotePlayerSpriteEffectBudget = 1100;
		g_iRemotePlayerJointEffectBudget = 550;
	}
	...
	if (performanceLevel > 0)
	{
		...
		g_iRemotePlayerJointEffectBudget = (g_iRemotePlayerJointEffectBudget * budgetPercent) / 100;
	}
...
void SetCharacterClass(CHARACTER* c)
{
	...
	bool bSetStop = true;
	if (gMapManager->InChaosCastle() == true) bSetStop = false;
	if (c->Object.CurrentAction >= PLAYER_SIT1 && c->Object.CurrentAction <= PLAYER_POSE_FEMALE1) bSetStop = false;
	if (c->Object.CurrentAction >= PLAYER_HIT_FIST && c->Object.CurrentAction <= PLAYER_RIDE_SKILL) bSetStop = false;
	if (bSetStop) SetPlayerStop(c);
...
void runtime_physical_cloth(CHARACTER* pCharacter, OBJECT* o, vec3_t CloakLight)
{
	...
	if (!g_isCharacterBuff(o, eBuff_Cloaking) && pCloth != NULL)
	{
		...
		const bool shouldUpdateCloth = ShouldUpdateCrowdClothThisFrame(pCharacter, o);

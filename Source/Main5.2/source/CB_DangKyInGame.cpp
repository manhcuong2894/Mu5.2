...
void OnClickRegisterSuccess(LPVOID pClass)
{
...
bool CB_DangKyInGame::HandleConfirmSubmit()
{
	...
	{
		gInterface->OpenMessageBox("Error", "Sai Ma Captcha" );
		return false;
	}
...
bool CB_DangKyInGame::RenderWindow(int X, int Y)
{
	...
	if (gInterface->DrawButton(StartX + 100, StartY + 45, 100, 12, "Xác Nhận", 80, false) || submitByEnter)
	{
...
void CB_DangKyInGame::RecvKQRegInGame(XULY_CGPACKET* lpMsg)
{
	...
	{
		gInterface->OpenMessageBoxOkCancel(&OnClickRegisterSuccess, "Kết Quả", "Chúc mừng bạn đã đăng ký thành công ! \n ID : %s", szID);
		CUIMng& rUIMng = CUIMng::Instance();
		rUIMng.m_LoginWin.GetIDInputBox()->SetText(szID);
		rUIMng.m_LoginWin.GetPassInputBox()->SetText(szPass);
// removed inline close
//
//
//

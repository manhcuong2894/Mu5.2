...
static void CreateDeathStabMainEffect(CHARACTER* sc, OBJECT* so, OBJECT* to)
{
  ...
  {
    vec3_t Position;
    float fDistance = 1210.0f + (float)(j * 9);
...
extern BOOL Util_CheckOption(char *lpszCommandLine, unsigned char cOption,
                             char *lpszString);
...
BOOL CreateSocket(char *IpAddr, unsigned short Port) {
  ...
  g_ConsoleDebug->Write(MCD_NORMAL,
                        "[Connect to Server] ip address = %s, port = %d",
                        IpAddr, Port);
...
void DeleteSocket() { SocketClient.Close(); }
...
void ReceiveServerList(BYTE *ReceiveBuffer) {
  LPPHEADER_DEFAULT_SUBCODE_WORD Data =
      (LPPHEADER_DEFAULT_SUBCODE_WORD)ReceiveBuffer;
  ...
  for (int i = 0; i < TotalServer; i++) {
    LPPRECEIVE_SERVER_LIST Data2 =
        (LPPRECEIVE_SERVER_LIST)(ReceiveBuffer + Offset);

    g_ServerListManager->InsertServerGroup(Data2->Index, Data2->Percent,
                                           Data2->ServerName);

    g_ConsoleDebug->Write(MCD_RECEIVE, "0xF4 [ReceiveServerList(%d %d %d): %s]",
                          i, Data2->Index, Data2->Percent, Data2->ServerName);
...
void ReceiveServerConnect(BYTE *ReceiveBuffer) {
  ...
  g_ErrorReport.Write("[ReceiveServerConnect] ip=%s port=%d\r\n", MyAddressIp, Data->Port);
  SocketClient.Close();

  if (g_pReconnectUI->ReconnectCreateConnection(MyAddressIp, Data->Port)) {
    g_bGameServerConnected = TRUE;
    g_ErrorReport.Write("[ReceiveServerConnect] GameServer socket connected\r\n");
  } else {
    g_ErrorReport.Write("[ReceiveServerConnect] GameServer socket connect failed\r\n");
  }

  char Text[100];
  sprintf(Text, GlobalText[481], MyAddressIp, Data->Port);
  g_pChatListBox->AddText(
      "", Text, SEASON3B::TYPE_SYSTEM_MESSAGE); // Đã kết nối với máy chủ
}
...
void ReceiveServerVersion(BYTE *ReceiveBuffer) {
  LPPHEADER_SERVER_UPDATE_VERSION Data =
      (LPPHEADER_SERVER_UPDATE_VERSION)ReceiveBuffer;
  ...
  if (Data->state == 0) //-- continen la misma version
  {
    ...
    GMConnectHex->SetUpdateVersion(false);
             ...
             1) //-- tienen diferente version y hay que descargar el archivo
  {
    GMConnectHex->write_received_file();

    GMConnectHex->SetUpdateVersion(true);
  } else if (Data->state == 3) //-- no existe actualizacion pero el cliente
                               // tiene un numero diferente
  {
    DeleteSocket();
...
void ReceiveServerUpdate(BYTE *ReceiveBuffer) {
  ...
  float percentage =
      (static_cast<float>(currentProgress) / static_cast<float>(totalSize));

  GMConnectHex->download_information(fileVersion, offset, totalSize,
                                     percentage);

  GMConnectHex->write_data_current(
      fileVersion, offset, chunkSize, totalSize,
      (ReceiveBuffer + sizeof(PHEADER_SERVER_VERSION)));
}
...
void ReceiveConfirmPassword2(BYTE *ReceiveBuffer) {
  LPPRECEIVE_CONFIRM_PASSWORD2 Data =
      (LPPRECEIVE_CONFIRM_PASSWORD2)ReceiveBuffer;
...
void ReceiveCharacterList(BYTE *ReceiveBuffer) {
  LPPHEADER_DEFAULT_CHARACTER_LIST Data =
      (LPPHEADER_DEFAULT_CHARACTER_LIST)ReceiveBuffer;
  ...
  for (int i = 0; i < Data->MaxClass; i++) {
    LPPRECEIVE_CHARACTER_LIST Data2 =
        (LPPRECEIVE_CHARACTER_LIST)(ReceiveBuffer + Offset);

    int iClass =
        gCharacterManager.ChangeServerClassTypeToClientClassType(Data2->Class);
    ...
    CHARACTER *c =
        CreateHero(Data2->Index, iClass, 0, fPos[0], fPos[1], fAngle);
...
void ReceiveCreateCharacter(BYTE *ReceiveBuffer) {
  ...
  if (Data->Result == 1) {
    ...
    CHARACTER *pCharacter =
        CreateHero(Data->Index, CharacterView.Class, CharacterView.Skin,
                   fPos[0], fPos[1], fAngle);
...
BOOL ReceiveLogOut(BYTE *ReceiveBuffer, BOOL bEncrypted) {
  ...
  g_ConsoleDebug->Write(MCD_RECEIVE, "0x02 [ReceiveServerList(%d)]",
                        Data->Value);
...
BOOL ReceiveJoinMapServer(BYTE *ReceiveBuffer, BOOL bEncrypted) {
  ...
  if (pCharacter) {
    CreateCharacterPointer(pCharacter, MODEL_PLAYER, Data->PositionX,
                           Data->PositionY, ((float)Data->Angle - 1.f) * 45.f);
  ...
  if (gCharacterManager.IsMasterLevel(Hero->Class) == true) {
    ...
    g_pMasterSkillTree->SetMasterType(
  ...
  if (!(gMapManager->currentMap >= WD_65DOPPLEGANGER1 &&
        gMapManager->currentMap <= WD_68DOPPLEGANGER4)) {
    char Text[256];
    sprintf(Text, "%s%s", GlobalText[484],
            gMapManager->GetMapName(gMapManager->currentMap));

    g_pChatListBox->AddText(
        "", Text, SEASON3B::TYPE_SYSTEM_MESSAGE); // Chào mừng bạn đến với
  }
  ...
  if (gMapManager->currentMap < WD_69EMPIREGUARDIAN1 ||
      WD_72EMPIREGUARDIAN4 < gMapManager->currentMap) {
...
void ReceiveRevival(BYTE *ReceiveBuffer) {
  ...
  if (gMapManager->currentMap >= WD_45CURSEDTEMPLE_LV1 &&
      gMapManager->currentMap <= WD_45CURSEDTEMPLE_LV6) {
    if (!(Data->Map >= WD_45CURSEDTEMPLE_LV1 &&
          Data->Map <= WD_45CURSEDTEMPLE_LV6)) {
  ...
  if (gMapManager->currentMap != Data->Map) {
    ...
    if ((gMapManager->InChaosCastle(OldWorld) == true &&
         OldWorld != gMapManager->currentMap) ||
    ...
    if (pCharacter) {
      if (gMapManager->currentMap == -1 ||
          pCharacter->Helper.Type != MODEL_HELPER + 3 || pCharacter->SafeZone) {
        o->Position[2] = RequestTerrainHeight(o->Position[0], o->Position[1]);
      } else {
        if (gMapManager->currentMap == WD_8TARKAN ||
            gMapManager->currentMap == WD_10HEAVEN)
          o->Position[2] =
              RequestTerrainHeight(o->Position[0], o->Position[1]) + 90.f;
        else
          o->Position[2] =
              RequestTerrainHeight(o->Position[0], o->Position[1]) + 30.f;
      }
    }
  }
...
void ReceiveMagicList(BYTE *ReceiveBuffer) {
  ...
  if (Data->Value == 0xFF) {
    LPPRECEIVE_MAGIC_LIST Data2 =
        (LPPRECEIVE_MAGIC_LIST)(ReceiveBuffer + Offset);
...
BOOL ReceiveInventory(BYTE *ReceiveBuffer, BOOL bEncrypted, bool appendPacket) {
  if (bEncrypted) {
    LPPHEADER_DEFAULT_SUBCODE_WORD Data = (LPPHEADER_DEFAULT_SUBCODE_WORD)
        ReceiveBuffer; // LPPHEADER_DEFAULT_SUBCODE_WORD 6byte

    const BYTE LegacyItemListAppendFlag = 0x80;
    const int ItemCount = (Data->Value & ~LegacyItemListAppendFlag);
    appendPacket =
        (appendPacket || ((Data->Value & LegacyItemListAppendFlag) != 0));
    ...
    for (int i = 0; i < ItemCount; i++) {
      LPPRECEIVE_INVENTORY Data2 =
          (LPPRECEIVE_INVENTORY)(ReceiveBuffer +
                                 Offset); // LPPRECEIVE_INVENTORY 8byte
      ...
      if (CharacterMachine->CheckEquipment(target_index) == false) {
        if (target_index < MAX_EQUIPMENT_INDEX ||
            target_index >= MAX_MY_INVENTORY_INDEX) {
          if (target_index >= MAX_MY_INVENTORY_INDEX &&
              target_index < MAX_MY_SHOP_INVENTORY_INDEX) {
            g_pMyShopInventory->InsertItem(
                target_index - MAX_MY_INVENTORY_INDEX, Data2->Item);
          }
        } else {
          g_pMyInventory->InsertItem(target_index - MAX_EQUIPMENT_INDEX,
                                     Data2->Item);
        }
      } else {
  ...
  g_ConsoleDebug->Write(MCD_RECEIVE, appendPacket
                                         ? "0x12 [ReceiveInventoryAppend]"
...
void ReceiveDeleteInventory(BYTE *ReceiveBuffer) {
  ...
  if (Data->SubCode != 0xff) {
    ...
    if (CharacterMachine->CheckEquipment(itemindex) == true) {
      ...
      if (itemindex < MAX_EQUIPMENT_INDEX ||
          itemindex >= MAX_MY_INVENTORY_INDEX) {
        if (itemindex >= MAX_MY_INVENTORY_INDEX &&
            itemindex < MAX_MY_SHOP_INVENTORY_INDEX) {
  ...
  g_ConsoleDebug->Write(MCD_RECEIVE, "0x28 [ReceiveDeleteInventory(%d %d)]",
                        Data->SubCode, Data->Value);
}
...
void ReceiveTradeInventory(BYTE *ReceiveBuffer) {
  ...
  if (Data->SubCode == 3) {
    ...
    g_pChatListBox->AddText(
        "", GlobalText[1208],
        SEASON3B::TYPE_ERROR_MESSAGE); // Quá trình tái tạo đã thất bại
    PlayBuffer(SOUND_MIX01);
    PlayBuffer(SOUND_BREAK01);
    g_pMixInventory->SetMixState(SEASON3B::CNewUIMixInventory::MIX_FINISHED);
    g_pMixInventory->DeleteAllItems();
...
void ReceiveChatWhisperResult(BYTE *ReceiveBuffer) {
  ...
  switch (Data->Value) {
  case 0: {
    g_pChatListBox->AddText(ChatWhisperID, GlobalText[482],
...
void ReceiveNotice(BYTE *ReceiveBuffer) {
  ...
  if (Data->Result == 0) {
    ...
    if (CHARACTER_SCENE != SceneFlag) {
      g_pChatListBox->AddText("", (char *)Data->Notice,
    ...
    if (Data->Notice != NULL && Data->Notice[0] != '\0') {
      g_pSlideHelpMgr->AddSlide(Data->Count, Data->Delay, (char *)Data->Notice,
                                Data->Result - 10, Data->Speed / 10.0f,
                                Data->Color);
    }
  }
#if (NOTICE_PKSYSTEM)
  else if (Data->Result == 0xFE) {
    if (Data->Notice != NULL && Data->Notice[0] != '\0') {
      g_pItemEnduranceInfo->AddNoticePK(Data->Color, Data->Delay,
                                        (char *)Data->Notice);
    }
  }
#endif
...
void ReceiveMoveCharacter(BYTE *ReceiveBuffer) {
  ...
  if (pCharacter && pCharacter->Dead == 0) {
    ...
    if (Key == HeroKey) {
      ...
      if (o->Type == MODEL_MONSTER01 + 52) {
        ...
        o->Angle[2] = CreateAngle(o->Position[0], o->Position[1],
                                  pCharacter->PositionX * 100.f,
                                  pCharacter->PositionY * 100.f);
        ...
        if (PathFinding2(pCharacter->PositionX, pCharacter->PositionY,
                         pCharacter->TargetX, pCharacter->TargetY,
                         &pCharacter->Path, 0.0f, iDefaultWall)) {
    ...
    g_ConsoleDebug->Write(
        MCD_RECEIVE, "ID : %s | sX : %d | sY : %d | tX : %d | tY : %d",
        pCharacter->ID, pCharacter->PositionX, pCharacter->PositionY,
        pCharacter->TargetX, pCharacter->TargetY);
  }
}
...
BOOL ReceiveTeleport(BYTE *ReceiveBuffer, BOOL bEncrypted) {
  ...
  LPPRECEIVE_TELEPORT_POSITION Data =
      (LPPRECEIVE_TELEPORT_POSITION)ReceiveBuffer;
  ...
  if (gMapManager->currentMap == -1 || Hero->Helper.Type != MODEL_HELPER + 3 ||
      Hero->SafeZone) {
    o->Position[2] = RequestTerrainHeight(o->Position[0], o->Position[1]);
  } else {
    if (gMapManager->currentMap == WD_8TARKAN ||
  ...
  if (Data->Flag == 0) {
    ...
    if (gMapManager->currentMap != Data->Map) {
      ...
      if (gMapManager->currentMap == -1 ||
          Hero->Helper.Type != MODEL_HELPER + 3 || Hero->SafeZone) {
        o->Position[2] = RequestTerrainHeight(o->Position[0], o->Position[1]);
      } else {
        if (gMapManager->currentMap == WD_8TARKAN ||
            gMapManager->currentMap == WD_10HEAVEN)
  ...
  if (pCharacter) {
    ...
    case 0:
      if (Type == 0x1FFF) {
        ...
        pCharacter->Weapon[0].Level = Level;
    ...
    case 1:
      if (Type == 0x1FFF) {
        ...
        pCharacter->Weapon[1].Level = Level;
    ...
    case 2:
      if (Type == 0x1FFF) {
        pCharacter->BodyPart[BODYPART_HELM].Type =
            MODEL_BODY_HELM +
            gCharacterManager.GetSkinModelIndex(pCharacter->Class);
    ...
    case 3:
      if (Type == 0x1FFF) {
        pCharacter->BodyPart[BODYPART_ARMOR].Type =
            MODEL_BODY_ARMOR +
            gCharacterManager.GetSkinModelIndex(pCharacter->Class);
    ...
    case 4:
      if (Type == 0x1FFF) {
        pCharacter->BodyPart[BODYPART_PANTS].Type =
            MODEL_BODY_PANTS +
            gCharacterManager.GetSkinModelIndex(pCharacter->Class);
    ...
    case 5:
      if (Type == 0x1FFF) {
        pCharacter->BodyPart[BODYPART_GLOVES].Type =
            MODEL_BODY_GLOVES +
            gCharacterManager.GetSkinModelIndex(pCharacter->Class);
    ...
    case 6:
      if (Type == 0x1FFF) {
        pCharacter->BodyPart[BODYPART_BOOTS].Type =
            MODEL_BODY_BOOTS +
            gCharacterManager.GetSkinModelIndex(pCharacter->Class);
    ...
    case 8: {
      ...
      if (Type == 0x1FFF) {
        ...
        case ITEM_HELPER + 64:
        case ITEM_HELPER + 65: {
          ThePetProcess().CreatePet(Type, pCharacter->Helper.Type, o->Position,
                                    pCharacter);
        } break;
        case ITEM_HELPER + 67:
          ThePetProcess().CreatePet(Type, pCharacter->Helper.Type, o->Position,
        ...
        default:
          if (ThePetProcess().IsPet(Type)) {
            ThePetProcess().CreatePet(Type, pCharacter->Helper.Type,
                                      o->Position, pCharacter);
            break;
          } else {
...
void ReceiveCreatePlayerViewport(BYTE *ReceiveBuffer, int Size) {
  ...
  for (int i = 0; i < Data->Value; i++) {
    ...
    if (!FindText(Temp, "webzen", false)) {
      ...
      CHARACTER *c = CreateCharacter(Key, MODEL_PLAYER, Data2->PositionX,
                                     Data2->PositionY, 0);
      ...
      if (CreateFlag) {
        ...
        CreateEffect(BITMAP_MAGIC + 2, o->Position, o->Angle, o->Light, 0, o);
        c->Object.Alpha = 0.f;
      } else if (PathFinding2(c->PositionX, c->PositionY, c->TargetX,
                              c->TargetY, &c->Path)) {
        c->Movement = true;
      }

      if (gMapManager->InHellas()) {
        CreateJoint(BITMAP_FLARE + 1, o->Position, o->Position, o->Angle, 8, o,
                    20.f);
      }
      ...
      for (int j = 0; j < Data2->s_BuffCount; ++j) {
        ...
        battleCastle::SettingBattleFormation(
            c, static_cast<eBuffState>(Data2->s_BuffEffectState[j]));
      }

      if (gMapManager->InBattleCastle() &&
          battleCastle::IsBattleCastleStart()) {
        g_pSiegeWarfare->InitSkillUI();
      }
    }

    Offset += (sizeof(PCREATE_CHARACTER) -
               (sizeof(BYTE) * (MAX_BUFF_SLOT_INDEX - Data2->s_BuffCount)));
  }

  gmAIController->SetActiveCharacter(Hero);

  g_ConsoleDebug->Write(MCD_RECEIVE, "0x12 [ReceiveCreatePlayerViewport(%d)]",
                        Data->Value);
}
...
void ReceiveCreateTransformViewport(BYTE *ReceiveBuffer) {
  ...
  for (int i = 0; i < Data->Value; i++) {
    ...
    if (FindText(Temp, "webzen") == false) {
      int Class = gCharacterManager.ChangeServerClassTypeToClientClassType(
          Data2->ServerClass);

      WORD Type = ((WORD)(Data2->TypeH) << 8) + Data2->TypeL;

      CHARACTER *c =
          CreateMonster(Type, Data2->PositionX, Data2->PositionY, Key);
      ...
      for (int j = 0; j < Data2->s_BuffCount; ++j) {
        ...
        g_ConsoleDebug->Write(MCD_RECEIVE, "ID : %s, Buff : %d", c->ID,
      ...
      if (CreateFlag) {
        ...
        c->Object.Position[1] = ((c->PositionY) + 0.5f) * TERRAIN_SCALE;
        c->Object.Alpha = 0.f;
        CreateEffect(MODEL_MAGIC_CIRCLE1, o->Position, o->Angle, o->Light, 0,
                     o);
        CreateParticle(BITMAP_LIGHTNING + 1, o->Position, o->Angle, o->Light, 2,
                       1.f, o);
      } else if (PathFinding2((c->PositionX), (c->PositionY), Data2->TX,
                              Data2->TY, &c->Path)) {
        c->Movement = true;
      }
    ...
    Offset += (sizeof(PCREATE_TRANSFORM) -
               (sizeof(BYTE) * (MAX_BUFF_SLOT_INDEX - Data2->s_BuffCount)));
  }
  ...
  g_ConsoleDebug->Write(
      MCD_RECEIVE, "0x45 [ReceiveCreateTransformViewport(%d)]", Data->Value);
}
...
void AppearMonster(CHARACTER *c) {
  ...
  case 345: {
    if (g_Direction.m_CMVP.m_iCryWolfState == CRYWOLF_STATE_READY)
      ...
      CreateJoint(BITMAP_JOINT_THUNDER + 1, vPos, vPos, c->Object.Angle, 7,
                  NULL, 60.f + rand() % 10);
      CreateJoint(BITMAP_JOINT_THUNDER + 1, vPos, vPos, c->Object.Angle, 7,
                  NULL, 50.f + rand() % 10);
      CreateJoint(BITMAP_JOINT_THUNDER + 1, vPos, vPos, c->Object.Angle, 7,
      ...
      CreateParticle(BITMAP_SMOKE + 4, c->Object.Position, c->Object.Angle,
                     c->Object.Light, 1, 5.0f);
      CreateParticle(BITMAP_SMOKE + 4, c->Object.Position, c->Object.Angle,
                     c->Object.Light, 1, 5.0f);
      CreateParticle(BITMAP_SMOKE + 4, c->Object.Position, c->Object.Angle,
                     c->Object.Light, 1, 5.0f);

      Vector(c->Object.Position[0], c->Object.Position[1],
             c->Object.Position[2] + 120.0f, vPos);
      CreateJoint(BITMAP_JOINT_THUNDER, c->Object.Position, vPos,
                  c->Object.Angle, 17);
      CreateJoint(BITMAP_JOINT_THUNDER, c->Object.Position, vPos,
                  c->Object.Angle, 17);
      CreateJoint(BITMAP_JOINT_THUNDER, c->Object.Position, vPos,
  ...
  for (int i = 0; i < Data->Value; i++) {
    ...
    if (pCharacter != NULL) {
      ...
      for (int j = 0; j < Data2->s_BuffCount; ++j) {
        ...
        g_ConsoleDebug->Write(MCD_RECEIVE, "ID : %s, Buff : %d", pCharacter->ID,
                              static_cast<int>(Data2->s_BuffEffectState[j]));
      }
      ...
      if (o->Type >= MODEL_LITTLESANTA + 1 &&
          o->Type <= MODEL_LITTLESANTA + 4) {
        pCharacter->Object.Angle[2] = 90.f;
      } else if (o->Type >= MODEL_LITTLESANTA + 6 &&
                 o->Type <= MODEL_LITTLESANTA + 7) {
        pCharacter->Object.Angle[2] = 90.f;
      ...
      if (gMapManager->InBattleCastle() && pCharacter->MonsterIndex == 277) {
        if (g_isCharacterBuff((&pCharacter->Object), eBuff_CastleGateIsOpen)) {
          ...
          battleCastle::SetCastleGate_Attribute(Data2->PositionX,
                                                Data2->PositionY, 0);
        } else {
      ...
      if (CreateFlag) {
        AppearMonster(pCharacter);
                 ...
                 o->Type == MODEL_MONSTER01 + 121 && TeleportFlag) {
        vec3_t Light;
      ...
      if (gMapManager->currentMap >= WD_65DOPPLEGANGER1 &&
          gMapManager->currentMap <= WD_68DOPPLEGANGER4 && Key != HeroKey) {
        iDefaultWall = TW_NOMOVE;
      }

      else if (PathFinding2(pCharacter->PositionX, pCharacter->PositionY,
                            Data2->TargetX, Data2->TargetY, &pCharacter->Path,
                            0.0f, iDefaultWall)) {
        pCharacter->Movement = true;
      } else {
        pCharacter->Movement = false;
    ...
    g_ConsoleDebug->Write(
        MCD_RECEIVE,
        "0x13 [ReceiveCreateMonsterViewport(Type : %d | Key : %d)]", Type, Key);

    Offset += (sizeof(PCREATE_MONSTER) -
               (sizeof(BYTE) * (MAX_BUFF_SLOT_INDEX - Data2->s_BuffCount)));
  }
}
...
void ReceiveCreateSummonViewport(BYTE *ReceiveBuffer) {
  ...
  for (int i = 0; i < Data->Value; i++) {
    ...
    if (Type >= 152 && Type <= 158) {
      c = CreateHellGate(&Data2->ID[0], Key, Type, Data2->PositionX,
                         Data2->PositionY, CreateFlag);
    } else {
    ...
    } else if (PathFinding2(c->PositionX, c->PositionY, Data2->TargetX,
                            Data2->TargetY, &c->Path)) {
      c->Movement = true;
    }
    // c->AddUIInfo();

    Offset += (sizeof(PCREATE_SUMMON) -
               (sizeof(BYTE) * (MAX_BUFF_SLOT_INDEX - Data2->s_BuffCount)));
  }

  g_ConsoleDebug->Write(MCD_RECEIVE, "0x1F [ReceiveCreateSummonViewport(%d)]",
                        Data->Value);
}
  ...
  for (int i = 0; i < Data->Value; i++) {
    ...
    if (pCharacter) {
      ...
      for (int k = 0; k < buffSize; k++) {
        UnRegisterBuff(g_CharacterBuff((&pCharacter->Object), k),
                       &pCharacter->Object);
      }
    }
    DeleteCharacter(Key);
...
void ReceiveDamage(BYTE *ReceiveBuffer) {
  ...
  DWORD ShieldDamage = ((DWORD)(Data->ShieldDamageH) << 8) +
  Data->ShieldDamageL;
...
static int GetClampedSignedDamage(DWORD value) {
  return (value > 0x7FFFFFFFUL) ? 0x7FFFFFFF : (int)value;
}

static WORD GetClampedWordDamage(DWORD value) {
  return (value > 0xFFFFUL) ? 0xFFFF : (WORD)value;
}
...
void ProcessDamageCastle(LPPRECEIVE_ATTACK Data) {
  ...
  if (pCharacter) {
...
#else
    DWORD Damage = (((WORD)(Data->DamageH) << 8) + Data->DamageL);
    DWORD ShieldDamage =
        (((WORD)(Data->ShieldDamageH) << 8) + Data->ShieldDamageL);
#endif
    int DamageType = (Data->DamageType) & 0x3f;
    bool bDoubleEnable = (Data->DamageType >> 6) & 0x01;
    bool bComboEnable = (Data->DamageType >> 7) & 0x01;

    unsigned __int64 accumDamage =
        ((unsigned __int64)ShieldDamage + (unsigned __int64)Damage);
    __int64 rstDamage = -1;
    float rstScale = 0.8f;
    int ShockDamage = GetClampedSignedDamage(Damage);
    ...
    if (Success) {
      SetPlayerShock(pCharacter, ShockDamage);
      ...
      if (Key == HeroKey) {
        ...
        if (g_isCharacterBuff(o, eBuff_PhysDefense) &&
          ...
          if (cm) {
            OBJECT *om = &cm->Object;
            float fAngle = CreateAngle(om->Position[0], om->Position[1],
                                       o->Position[0], o->Position[1]);
            if (fabs((double)fAngle - om->Angle[2]) < 10.f) {
              vec3_t Angle = {0.0f, 0.0f, fAngle + 180.f};
              CreateEffect(MODEL_MAGIC_CAPSULE2, o->Position, Angle, o->Light,
                           0, o);
            }
          }
        }
    ...
    pCharacter->Hit = GetClampedWordDamage(Damage);
  }
}
...
void ReceiveAttackDamage(BYTE *ReceiveBuffer) {
  ...
  if (pCharacter) {
    ...
    if (Success) {
      SetPlayerShock(pCharacter, GetClampedSignedDamage(Damage));
      ...
      if (bRepeatedly || bEndRepeatedly) {
        g_CMonkSystem.SetRepeatedly(GetClampedSignedDamage(Damage), DamageType,
                                    bDoubleEnable, bEndRepeatedly);
        if (bEndRepeatedly) {
    ...
    pCharacter->Hit = GetClampedWordDamage(Damage);
    g_ConsoleDebug->Write(MCD_RECEIVE, "0x15 [ReceiveAttackDamage(%d %u)]",
                          AttackPlayer, Damage);
  }
}
  ...
  if (pCharacter) {
    ...
    if (pCharacter->Helper.Type != MODEL_HELPER + 4) {
      ...
      pCharacter->Object.Position[0] =
          pCharacter->TargetX * TERRAIN_SCALE + TERRAIN_SCALE * 0.5f;
      pCharacter->Object.Position[1] =
          pCharacter->TargetY * TERRAIN_SCALE + TERRAIN_SCALE * 0.5f;
  ...
  if (pCharacter) {
    ...
    if (Data->State == 1) // add
    {
      ...
      if (g_isCharacterBuff(o, bufftype)) {
        if ((o->Type >= MODEL_CRYWOLF_ALTAR1 &&
             o->Type <= MODEL_CRYWOLF_ALTAR5) ||
        ...
        if (bufftype == eBuff_CastleRegimentDefense ||
            ...
            bufftype == eBuff_CastleRegimentAttack2 ||
            bufftype == eBuff_CastleRegimentAttack3) {
        ...
        g_ConsoleDebug->Write(MCD_RECEIVE, "RegisterBuff ID : %s, Buff : %d",
                              pCharacter->ID, static_cast<int>(bufftype));
      }
    } else // clear
    {
      eBuffState bufftype = static_cast<eBuffState>(Data->BuffIndex);
      ...
      if (bufftype == eBuff_CastleRegimentDefense ||
        ...
        g_ConsoleDebug->Write(MCD_RECEIVE, "UnRegisterBuff ID : %s, Buff : %d",
                              pCharacter->ID, static_cast<int>(bufftype));
      } else if (bufftype == eBuff_GMEffect) {
        DeleteParts(pCharacter);
      }
  ...
  if (so->Type == MODEL_PLAYER) {
    if (gCharacterManager.GetBaseClass(c->Class) == CLASS_ELF && !c->SafeZone) {
      ...
      if (EquipedBowType == BOWTYPE_BOW) {
        if ((sc_type_helper == MODEL_HELPER + 37 ||
             so->movementType == Movement::Fenrir) &&
            !c->SafeZone) {
          SetAction(so, PLAYER_FENRIR_ATTACK_BOW, true);
        } else if ((sc_type_helper == MODEL_HELPER + 2 ||
                    so->movementType == Movement::Uniria) &&
                   !c->SafeZone) {
          SetAction(so, PLAYER_HIT_RIDE_BOW, true);
        } else if ((sc_type_helper == MODEL_HELPER + 3 ||
                    so->movementType == Movement::Dinorant) &&
                   !c->SafeZone) {
          SetAction(so, PLAYER_HIT_RIDE_BOW, true);
        } else if ((sc_type_helper == MODEL_HELPER + 4 ||
                    so->movementType == Movement::Horse) &&
                   !c->SafeZone) {
          SetAction(so, PLAYER_HIT_RIDE_BOW, true);
        } else {
          if (c->Wing.Type != -1)
...
BOOL ReceiveMonsterSkill(BYTE *ReceiveBuffer, int Size, BOOL bEncrypted) {
  ...
  if (sc && tc) {
    ...
    if (SkillNumber > ATMON_SKILL_BIGIN && SkillNumber < ATMON_SKILL_END) {
      sc->Object.Angle[2] = CreateAngle(so->Position[0], so->Position[1],
    ...
    g_ConsoleDebug->Write(
        MCD_RECEIVE,
        "0x69 [ReceiveMonsterSkill(Skill : %d | SKey : %d |TKey : %d)]",
    ...
    if (MagicNumber != 28 && MagicNumber != 27 && MagicNumber != 417 &&
      ...
      if (sc && tc) {
        ...
        if (MagicNumber != Skill_Combo) {
          if (sc != Hero && MagicNumber != Skill_Teleport &&
              MagicNumber != Skill_Teleport_Ally && to->Visible)
            so->Angle[2] = CreateAngle(so->Position[0], so->Position[1],
                                       to->Position[0], to->Position[1]);
          ...
          if ((signed int)MagicNumber > Skill_Fire_Burst_Strengthener) {
            ...
            LABEL_321:
              ...
              }
              {
                ...
                Vector(0.5f, 0.5f, 0.5f, Light);
                for (int i = 0; i < 3; ++i) {
                  CreateEffect(BITMAP_SHINY + 6, Position, o->Angle, Light, 3,
                               o, -1, 0, 0, 0, 0.5f);
                }
                ...
                for (int i = 0; i < 4; i++) {
                  Position[0] =
                      (o->Position[0] - 119.f) + (float)(rand() % 240);
                  Position[2] = (o->Position[2] + 49.f) + (float)(rand() % 60);
                  CreateJoint(BITMAP_2LINE_GHOST, Position, o->Position,
                              o->Angle, 0, o, 20.f, o->PKKey, 0,
            ...
            LABEL_343:
              if (sc == Hero) {
                if (so->CurrentAction == PLAYER_SKILL_ATT_UP_OURFORCES) {
                  SendRequestAction(
                      AT_RAGEBUFF_1,
                      ((BYTE)((so->Angle[2] + 22.5f) / 360.f * 8.f + 1.f) % 8));
                } else {
                  SendRequestAction(
                      AT_RAGEBUFF_2,
                      ((BYTE)((so->Angle[2] + 22.5f) / 360.f * 8.f + 1.f) % 8));
              ...
              }
              {
                ...
                if (MagicNumber != Skill_Increase_Health &&
                    MagicNumber != Skill_Stamina_Increase_Strengthener) {
                  if (MagicNumber != Skill_Increase_Block &&
                      MagicNumber != Skill_Defense_Success_Rate_Increase_P &&
                      MagicNumber != Skill_DefSuccessRate_Increase_Mastery) {
                    if (MagicNumber == Skill_Ignore_Defense ||
                        MagicNumber == 575) {
                      DeleteEffect(BITMAP_LIGHT_RED, _pObj, 1);
                      CreateEffect(BITMAP_LIGHT_RED, _pObj->Position,
                                   _pObj->Angle, _pObj->Light, 1, _pObj, -1, 0,
                                   0, 0, 1.5f);
                    }
                  } else {
                    DeleteEffect(BITMAP_LIGHT_RED, _pObj, 2);
                    CreateEffect(BITMAP_LIGHT_RED, _pObj->Position,
                                 _pObj->Angle, _pObj->Light, 2, _pObj, -1, 0, 0,
                                 0, 1.5f);

                    Vector(0.7f, 0.7f, 1.0f, vLight);
                    DeleteEffect(MODEL_WINDFOCE, _pObj, 1);
                    CreateEffect(MODEL_WINDFOCE, _pObj->Position, _pObj->Angle,
                                 vLight, 1, _pObj, -1, 0, 0, 0, 1.0f);
                  }
                } else {
                  DeleteEffect(BITMAP_LIGHT_RED, _pObj, 0);
                  CreateEffect(BITMAP_LIGHT_RED, _pObj->Position, _pObj->Angle,
                               _pObj->Light, 0, _pObj, -1, 0, 0, 0, 1.5f);
                }
                if ((signed int)MagicNumber >
                    Skill_Defense_Success_Rate_Increase_P) {
                  switch (MagicNumber) {
                  case Skill_DefSuccessRate_Increase_Mastery:
                    g_CharacterRegisterBuff(
                        _pObj, EFFECT_GREATER_DEFENSE_SUCCESS_RATE_ENHANCED);
                    break;
                  case Skill_Stamina_Increase_Strengthener:
                  ...
                  case 0x23Fu:
                    g_CharacterRegisterBuff(
                        _pObj, EFFECT_IGNORE_DEFENSE_RATE_IMPROVED);
                    break;
                  }
                } else {
                  switch (MagicNumber) {
                  case Skill_Defense_Success_Rate_Increase_P:
                    g_CharacterRegisterBuff(
                        _pObj, EFFECT_GREATER_DEFENSE_SUCCESS_RATE_IMPROVED);
                    break;
            ...
            LABEL_382:
              ...
              if (g_CMonkSystem.SetLowerEffEct()) {
                CreateEffect(MODEL_VOLCANO_OF_MONK, to->Position, to->Angle,
                             vLight, g_CMonkSystem.GetLowerEffCnt(), to, -1, 0,
                             0, 0, 0.8f);
              }

              CreateEffect(MODEL_TARGETMON_EFFECT, to->Position, to->Angle,
                           vLight, 0, to, -1, 0, 0, 0, 1.0f);
          ...
          LABEL_80:
            ...
            LABEL_112:
              if (sc->SkillSuccess) {
                ...
                CreateEffect(BITMAP_SKULL, to->Position, to->Angle, vLight, 0,
                             to);
            ...
            LABEL_274:
              if (!g_isCharacterBuff(to, eBuff_HpRecovery)) {
                ...
                CreateEffect(BITMAP_LIGHT, to->Position, to->Angle, to->Light,
                             1, to);
              }
            ...
            LABEL_303:
              ...
              CreateEffect(MODEL_INFINITY_ARROW, so->Position, so->Angle,
                           so->Light, 0, so);
            ...
            LABEL_305:
              ...
              if (MagicNumber != Skill_Sleep &&
                  MagicNumber != Skill_Sleep_Strengthener) {
                if (MagicNumber == 220)
            ...
            LABEL_319:
              ...
              CreateEffect(MODEL_SWELL_OF_MAGICPOWER, so->Position, so->Angle,
                           vLight, 0, so);
            ...
            case Skill_Phoenix_Shot:
              ...
              if (sc != Hero) {
                ...
                g_CMonkSystem.RageCreateEffect(so, MagicNumber);
                /*sub_511360((int)&unk_EBCD98, a1, (int)a2, (int)a3, so,
                 * MagicNumber);*/
              }
              Vector(1.0, 1.0, 1.0, vLight);
              CreateEffect(BITMAP_SUPER0001, to->Position, to->Angle, vLight, 0,
                           to, -1, 0, 0, 0, 0.5, -1);
              break;
            default:
              break;
        ...
        LABEL_340:
          ...
          case Skill_Recovery: {
            ...
            VectorCopy(o->Position, Position);
            for (int i = 0; i < 19; ++i) {
              CreateJoint(BITMAP_FLARE, Position, Position, o->Angle, 47, o, 40,
                          2);
            }
            ...
            if (pModels) {
              ...
              for (int i = 0; i < 2; ++i) {
                pModels->TransformByObjectBone(vFirePosition, so,
                                               rand() % iNumBones);
          ...
          case Skill_Invisibility:
            ...
            if (sc->SkillSuccess && !g_isCharacterBuff(to, eBuff_Cloaking) &&
                so != to) {
          ...
          case Skill_Improve_ag:
            if (!g_isCharacterBuff(to, eBuff_AddAG)) {
              ...
              CreateEffect(BITMAP_LIGHT, to->Position, to->Angle, to->Light, 2,
                           to);
            }
      ...
      g_pChatListBox->AddText("", GlobalText[2249],
                              SEASON3B::TYPE_SYSTEM_MESSAGE);
    }
  } else {
        ...
        if (sc != Hero) {
          if ((signed int)MagicNumber > Skill_Phoenix_Shot) {
            ...
            LABEL_26:
              ...
              /*if (sc->Helper.Type == (MODEL_HELPER + 37) || so->movementType
              == 5) SetAction(so, PLAYER_FENRIR_ATTACK_DARKLORD_FLASH); else
            ...
            LABEL_43:
              ...
              Vector(0.5f, 0.5f, 0.5f, Light);
              for (int i = 0; i < 3; ++i) {
                CreateEffect(BITMAP_SHINY + 6, Position, so->Angle, Light, 3,
                             so, -1, 0, 0, 0, 0.5f);
              }
              ...
              for (int i = 0; i < 4; i++) {
                ...
                CreateJoint(BITMAP_2LINE_GHOST, Position, so->Position,
                            so->Angle, 0, so, 20.f, so->PKKey, 0,
                            so->m_bySkillSerialNum);
              }
              {
                CHARACTER *st = gmCharacters->GetCharacter(SelectedCharacter);
            ...
            case Skill_Multi_Shot: {
              ...
              CreateEffect(MODEL_BLADE_SKILL, Position, so->Angle, Light, 1, so,
                           Key);
              goto LABEL_109;
            }
            case Skill_Recovery: {
              ...
              VectorCopy(so->Position, Position);
              for (int i = 0; i < 19; ++i) {
                CreateJoint(BITMAP_FLARE, Position, Position, so->Angle, 47, so,
                            40, 2);
              }
              Vector(0.3f, 0.2f, 0.1f, Light);
              ...
              if (pModels) {
                for (int i = 0; i < 2; ++i) {
                  ...
                  pModels->TransformByObjectBone(vFirePosition, so,
                                                 rand() % iNumBones);
                  CreateEffect(BITMAP_FLARE, vFirePosition, so->Angle, Light,
                               3);
                }
                PlayBuffer(SOUND_SKILL_RECOVER);
              }
            ...
            case Skill_Pollution:
              g_SummonSystem.CastSummonSkill(MagicNumber, sc, so,
...
void ReceiveChainMagic(BYTE *ReceiveBuffer) {
  ...
  CHARACTER *pSourceChar =
      gmCharacters->GetCharacter(FindCharacterIndex(pPacketData->wUserIndex));
  ...
  if (pSourceChar) {
    ...
    pSourceChar->Skill =
        ((WORD)(pPacketData->MagicH) << 8) + pPacketData->MagicL;

    pTempObject = pSourceObject;

    int iOffset = sizeof(PRECEIVE_CHAIN_MAGIC);
    for (int i = 0; i < (int)(pPacketData->byCount); i++) {
      LPPRECEIVE_CHAIN_MAGIC_OBJECT pPacketData2 =
          (LPPRECEIVE_CHAIN_MAGIC_OBJECT)(ReceiveBuffer + iOffset);
      CHARACTER *pTargetChar = gmCharacters->GetCharacter(
          FindCharacterIndex(pPacketData2->wTargetIndex));
      ...
      if (pTempObject != pTargetObject && pTargetObject != NULL &&
          pTargetObject->Live == true) {
        vec3_t vAngle;
...
void ReceiveMagicPosition(BYTE *ReceiveBuffer, int Size) {
  ...
  if (pCharacter) {
    ...
    for (int i = 0; i < Data->Count; i++) {
      LPPRECEIVE_MAGIC_POSITION Data2 =
          (LPPRECEIVE_MAGIC_POSITION)(ReceiveBuffer + Offset);
...
BOOL ReceiveDieExp(BYTE *ReceiveBuffer, BOOL bEncrypted) {
  ...
  if (pCharacter) {
    ...
    if (Success) {
      ...
      Hero->Damage = GetClampedWordDamage(Damage);
...
void ReceiveGetItem(BYTE *ReceiveBuffer) {
  ...
  if (Data->Result != NOT_GET_ITEM) {
    if (Data->Result == GET_ITEM_ZEN) {
      ...
      CharacterMachine->Gold = (Data->Item[0] << 24) + (Data->Item[1] << 16) +
                               (Data->Item[2] << 8) + (Data->Item[3]);

      int getGold = CharacterMachine->Gold - backupGold;

      if (getGold > 0) {
        sprintf(szMessage, "%d %s %s", getGold, GlobalText[224],
                GlobalText[918]); // zen ** đã được nhặt
        g_pChatListBox->AddText("", szMessage, SEASON3B::TYPE_SYSTEM_MESSAGE);
      }
    } else {
      if (Data->Result != GET_ITEM_MULTI) {
        if (Data->Result >= MAX_EQUIPMENT_INDEX &&
            Data->Result < MAX_MY_INVENTORY_INDEX)
          g_pMyInventory->InsertItem(Data->Result - MAX_EQUIPMENT_INDEX,
                                     Data->Item);
      }
      ...
      char szMessage[128];
      sprintf(szMessage, "%s %s", szItem,
              GlobalText[918]); // tên item đã được nhặt
    ...
    int Type = ConvertItemType(Data->Item);
    if (Type == ITEM_POTION + 13 || Type == ITEM_POTION + 14 ||
        Type == ITEM_POTION + 16 || Type == ITEM_WING + 15 ||
        Type == ITEM_POTION + 22 || Type == INDEX_COMPILED_CELE ||
        Type == INDEX_COMPILED_SOUL || Type == ITEM_POTION + 31 ||
...
BOOL ReceiveEquipmentItem(BYTE *ReceiveBuffer, BOOL bEncrypted) {
  LPPHEADER_DEFAULT_SUBCODE_ITEM Data =
      (LPPHEADER_DEFAULT_SUBCODE_ITEM)ReceiveBuffer;
  ...
  if (bEncrypted) {
    ...
    if (Data->SubCode == 0xFF) {
      ...
      SEASON3B::CNewUIPickedItem *pPickedItem =
          SEASON3B::CNewUIInventoryCtrl::GetPickedItem();
      ...
      if (pPickedItem) {
        if (g_pMyInventory->IsInvenctrlSame(pPickedItem->GetOwnerInventory()) ==
            -1) {
          if (pPickedItem->GetOwnerInventory() ==
      ...
      if (iSourceIndex >= MAX_MY_INVENTORY_INDEX &&
          iSourceIndex < INVENTORY_FULL_SIZE) {
      ...
      if (Data->SubCode) {
        if (Data->SubCode == REQUEST_EQUIPMENT_TRADE) {
          ...
          g_pStorageInventory->ProcessToReceiveStorageItems(Data->Index,
                                                            Data->Item);
        ...
        if (CharacterMachine->CheckEquipment(itemindex) == true) {
          ...
          if (itemindex < MAX_EQUIPMENT_INDEX ||
            ...
            if (itemindex >= MAX_MY_INVENTORY_INDEX &&
                itemindex < MAX_MY_SHOP_INVENTORY_INDEX) {
              g_pMyShopInventory->InsertItem(itemindex - MAX_MY_INVENTORY_INDEX,
                                             Data->Item);
            }
          } else {
            ...
            g_pMyInventory->InsertItem(itemindex - MAX_EQUIPMENT_INDEX,
                                       Data->Item);
          }
        }
      }
      if (Data->SubCode != REQUEST_EQUIPMENT_CHAOS_MIX &&
          (Data->SubCode < REQUEST_EQUIPMENT_TRAINER_MIX ||
           Data->SubCode > REQUEST_EQUIPMENT_JERRIDON_MIX) &&
          (Data->SubCode != REQUEST_EQUIPMENT_CHAOS_CARD_MIX) &&
          (Data->SubCode != REQUEST_EQUIPMENT_CHERRYBLOSSOM_MIX) &&
          (Data->SubCode < REQUEST_EQUIPMENT_EXTRACT_SEED_MIX ||
           Data->SubCode > REQUEST_EQUIPMENT_DETACH_SOCKET_MIX)) {
        if (REQUEST_EQUIPMENT_LUCKY_SET_MIX == Data->SubCode ||
            REQUEST_EQUIPMENT_LUCKY_ITEM_MIX == Data->SubCode) {
    ...
    g_ConsoleDebug->Write(MCD_RECEIVE, "0x24 [ReceiveEquipmentItem(%d %d)]",
                          Data->SubCode, Data->Index);
  }
  return TRUE;
}
...
void ReceivePostChatItem(BYTE *ReceiveBuffer) {
  LPPHEADER_DEFAULT_SUBCODE_ITEM_ID Data =
      (LPPHEADER_DEFAULT_SUBCODE_ITEM_ID)ReceiveBuffer;
  ...
  if (NULL != pNewItem) {
    ...
    if (TextName[0] != 0) {
      ...
      g_pChatListBox->AddText(CharacterName, TextChat,
                              MESSAGE_TYPE::TYPE_WHISPER_MESSAGE,
                              MESSAGE_TYPE::TYPE_ALL_MESSAGE, pNewItem);
    }
...
BOOL ReceiveTalk(BYTE *ReceiveBuffer, BOOL bEncrypted) {
  ...
  case 7:
    SEASON3B::CreateMessageBox(
        MSGBOX_LAYOUT_CLASS(SEASON3B::CTrainerMenuMsgBoxLayout));
    break;
  case INDEX_NPC_LAHAP: {
    if (COMGEM::isAble()) {
      g_pNewUISystem->HideAll();
      SEASON3B::CreateMessageBox(
          MSGBOX_LAYOUT_CLASS(SEASON3B::CGemIntegrationMsgBoxLayout));
    }
  } break;
  case 0x0C:
  ...
  case 0x11: {
    SEASON3B::CreateMessageBox(
        MSGBOX_LAYOUT_CLASS(SEASON3B::CElpisMsgBoxLayout));
  } break;
  case 0x12: {
    SEASON3B::CreateMessageBox(
        MSGBOX_LAYOUT_CLASS(SEASON3B::COsbourneMsgBoxLayout));
  } break;
  case 0x13: {
  ...
  case 0x17: {
    SEASON3B::CreateMessageBox(
        MSGBOX_LAYOUT_CLASS(SEASON3B::CSeedMasterMenuMsgBoxLayout));
  } break;
  case 0x18: {
    SEASON3B::CreateMessageBox(
        MSGBOX_LAYOUT_CLASS(SEASON3B::CSeedInvestigatorMenuMsgBoxLayout));
  } break;
  case 0x19: {
    SEASON3B::CreateMessageBox(
        MSGBOX_LAYOUT_CLASS(SEASON3B::CResetCharacterPointMsgBoxLayout));
  } break;
  case 0x20: {
    SEASON3B::CreateMessageBox(
        MSGBOX_LAYOUT_CLASS(SEASON3B::CDelgardoMainMenuMsgBoxLayout));
  } break;
  case 0x21: {
  ...
  case 0x26: {
    SEASON3B::CreateMessageBox(
        MSGBOX_LAYOUT_CLASS(SEASON3B::CLuckyTradeMenuMsgBoxLayout));
  } break;
  case 0x28: {
...
void ReceiveBuy(BYTE *ReceiveBuffer) {
  ...
  if (Data->Index != 255) {
    if (Data->Index >= MAX_EQUIPMENT_INDEX &&
        Data->Index < MAX_MY_INVENTORY_INDEX) {
  ...
  if (Data->Index == 0xfe) {
    ...
    g_pChatListBox->AddText(Hero->ID, GlobalText[732],
                            SEASON3B::TYPE_ERROR_MESSAGE);
  }
  BuyCost = 0;
...
void ReceiveSell(BYTE *ReceiveBuffer) {
  ...
  if (Data->Flag != 0) {
    if (Data->Flag == 0xff) {
      ...
      g_pChatListBox->AddText(Hero->ID, GlobalText[733],
                              SEASON3B::TYPE_ERROR_MESSAGE);
...
void ReceiveLevelUp(BYTE *ReceiveBuffer) {
  ...
  DWORD iExp =
      CharacterAttribute->NextExperince - CharacterAttribute->Experience;
  ...
  if (gCharacterManager.IsMasterLevel(Hero->Class) == true &&
    ...
    for (int i = 0; i < 19; ++i) {
      CreateJoint(BITMAP_FLARE, o->Position, o->Position, o->Angle, 46, o, 80,
                  2);
    }
  } else {
    for (int i = 0; i < 15; ++i) {
      CreateJoint(BITMAP_FLARE, o->Position, o->Position, o->Angle, 0, o, 40,
                  2);
    }
...
void ReceiveLife(BYTE *ReceiveBuffer) {
  ...
  switch (Data->Index) {
  case 0xff:
    CharacterAttribute->Life = (CharacterAttribute->Life == 0)
...
void ReceivePK(BYTE *ReceiveBuffer) {
  ...
  if (pCharacter) {
    ...
    case PVP_HERO2:
    case PVP_HERO1:
      g_pChatListBox->AddText(pCharacter->ID, GlobalText[487],
                              SEASON3B::TYPE_SYSTEM_MESSAGE);
      break;
    case PVP_NEUTRAL:
      g_pChatListBox->AddText(pCharacter->ID, GlobalText[488],
                              SEASON3B::TYPE_ERROR_MESSAGE);
      break;
    case PVP_CAUTION:
      g_pChatListBox->AddText(pCharacter->ID, GlobalText[489],
                              SEASON3B::TYPE_SYSTEM_MESSAGE);
      break;
    case PVP_MURDERER1:
      sprintf(szTemp, "%s %d%s", GlobalText[490], 1, GlobalText[491]);
      g_pChatListBox->AddText(pCharacter->ID, szTemp,
                              SEASON3B::TYPE_ERROR_MESSAGE);
      break;
    case PVP_MURDERER2:
...
void ReceiveDurability(BYTE *ReceiveBuffer) {
  ...
  if (CharacterMachine->CheckEquipment(Data->Value)) {
    ...
    if (pPickedItem) {
      int InvenIndex =
          g_pMyInventory->IsInvenctrlSame(pPickedItem->GetOwnerInventory());
      ...
      if (InvenIndex != -1) {
        ...
        if (pPickItem) {
          ...
          iSourceIndex = g_pMyInventory->SlotCntSendSrcInvenIndex(iSourceIndex,
                                                                  InvenIndex);

          if (iSourceIndex == iLinealPos) {
            CUSTOM_ITEM_STACK *StackItem = GMProtect->GetItemStack(
                pPickItem->Type, ((pPickItem->Level >> 3) & 15));
  ...
  g_ConsoleDebug->Write(MCD_RECEIVE, "0x2A [ReceiveDurability(%d %d)]",
                        Data->Value, Data->KeyL);
}
...
void ReceiveTradeYourGold(BYTE *ReceiveBuffer) {
  ...
  g_pTrade->SetYourTradeGold(int(Data->nGold), int(Data->nCoin1),
                             int(Data->nCoin2), int(Data->nCoin3));
}
...
void ReceiveTradeExit(BYTE *ReceiveBuffer) {
  if (EquipmentItem) {
    if (!g_bPacketAfter_EquipmentItem) {
      memcpy(g_byPacketAfter_EquipmentItem, ReceiveBuffer,
             sizeof(PHEADER_DEFAULT));
...
void ReceiveStorageGold(BYTE *ReceiveBuffer) {
  ...
  g_ConsoleDebug->Write(MCD_RECEIVE, "0x81 [ReceiveStorageGold(%d %d %d)]",
                        Data->Result, Data->StorageGold, Data->Gold);
}
...
void ReceivePartyResult(BYTE *ReceiveBuffer) {
  ...
  switch (Data->Value) {
  case 0:
    g_pChatListBox->AddText("", GlobalText[497],
                            SEASON3B::TYPE_ERROR_MESSAGE); // Không thể Tạo nhóm
    break;
  case 1:
    g_pChatListBox->AddText(
        "", GlobalText[498],
        SEASON3B::TYPE_ERROR_MESSAGE); // Yêu cầu Tạo nhóm bị từ chối
    break;
  case 2:
    g_pChatListBox->AddText(
        "", GlobalText[499],
        SEASON3B::TYPE_ERROR_MESSAGE); // Thành viên đã rời khỏi Nhóm
    break;
  case 3:
    g_pChatListBox->AddText(
        "", GlobalText[500],
        SEASON3B::TYPE_ERROR_MESSAGE); //"Đối phương đã thoát khỏi trò chơi
    break;
  case 4:
    g_pChatListBox->AddText(
        "", GlobalText[501],
        SEASON3B::TYPE_ERROR_MESSAGE); // Đối phương đã gia nhập Nhóm khác
    break;
  case 5:
    g_pChatListBox->AddText("", GlobalText[502],
                            SEASON3B::TYPE_ERROR_MESSAGE); // Giải tán Nhóm
    break;
  case 6:
    g_pChatListBox->AddText(
        "", GlobalText[2990],
        SEASON3B::TYPE_ERROR_MESSAGE); // Không thể tạo nhóm với quân Genus của
                                       // đối thủ
    break;
  case 7:
    g_pChatListBox->AddText(
        "", GlobalText[2997],
        SEASON3B::TYPE_ERROR_MESSAGE); // Trong khu vực giao đấu không thể giải
                                       // tán Nhóm
    break;
  case 8:
    g_pChatListBox->AddText(
        "", GlobalText[2998],
        SEASON3B::TYPE_ERROR_MESSAGE); // Trong khu vực giao đấu, Nhóm không
                                       // được kích hoạt
    break;
  }
}
...
void ReceivePartyList(BYTE *ReceiveBuffer) {
  ...
  for (int i = 0; i < Data->Count; i++) {
    LPPRECEIVE_PARTY_LIST Data2 =
        (LPPRECEIVE_PARTY_LIST)(ReceiveBuffer + Offset);
    PARTY_t *p = &Party[i];
  ...
  g_ConsoleDebug->Write(MCD_RECEIVE, "0x42 [ReceivePartyList(partynum : %d)]",
                        Data->Count);
}
...
void ReceivePartyInfo(BYTE *ReceiveBuffer) {
  ...
  for (int i = 0; i < Data->Count; i++) {
    LPPRECEIVE_PARTY_INFO Data2 =
        (LPPRECEIVE_PARTY_INFO)(ReceiveBuffer + Offset);
  ...
  if (g_iFollowCharacter >= 0) {
    ...
    if (c) {
      for (int i = 0; i < PartyNumber; ++i) {
        if (strcmp(Party[i].Name, c->ID) == NULL &&
            strlen(Party[i].Name) == strlen(c->ID)) {
          IsParty = true;
        }
      }
    }
...
{
  LPPRECEIVE_GETITEMINFO_FOR_PARTY Data =
      (LPPRECEIVE_GETITEMINFO_FOR_PARTY)ReceiveBuffer;
  int Key = ((int)(Data->KeyH) << 8) + Data->KeyL;
  int Index = FindCharacterIndex(Key);
  CHARACTER *c = gmCharacters->GetCharacter(Index);
...
void ReceiveGuild(BYTE *ReceiveBuffer) {
  ...
  CHARACTER *pCharacter =
      gmCharacters->GetCharacter(FindCharacterIndex(GuildPlayerKey));

  if (pCharacter) {
    SEASON3B::CNewUICommonMessageBox *pMsgBox;
    SEASON3B::CreateMessageBox(
        MSGBOX_LAYOUT_CLASS(SEASON3B::CGuildRequestMsgBoxLayout), &pMsgBox);
...
void ReceiveGuildResult(BYTE *ReceiveBuffer) {
  ...
  case 0:
    g_pChatListBox->AddText(
        "", GlobalText[503],
        SEASON3B::TYPE_ERROR_MESSAGE); // Chủ Hội đã từ chối yêu cầu xin gia
                                       // nhập của bạn!
    break;
  case 1:
    g_pChatListBox->AddText(
        "", GlobalText[504],
        SEASON3B::TYPE_ERROR_MESSAGE); // Chúc mừng bạn đã gia nhập Hội
    break;
  case 2:
    g_pChatListBox->AddText(
        "", GlobalText[505],
        SEASON3B::TYPE_ERROR_MESSAGE); // Hội đã vượt quá số lượng thành viên
                                       // cho phép!
    break;
  case 3:
    g_pChatListBox->AddText(
        "", GlobalText[506],
        SEASON3B::TYPE_ERROR_MESSAGE); // Đã thoát khỏi trò chơi!
    break;
  case 4:
    g_pChatListBox->AddText(
        "", GlobalText[507],
        SEASON3B::TYPE_ERROR_MESSAGE); // Người này không phải là Chủ Hội!
    break;
  case 5:
    g_pChatListBox->AddText(
        "", GlobalText[508],
        SEASON3B::TYPE_ERROR_MESSAGE); // Không thể gia nhập Hội vào lúc này!
    break;
  case 6:
    g_pChatListBox->AddText(
        "", GlobalText[509],
        SEASON3B::TYPE_ERROR_MESSAGE); // Không thể gửi yêu cầu gia nhập!
    break;
  case 7:
    g_pChatListBox->AddText(
        "", GlobalText[510],
        SEASON3B::TYPE_ERROR_MESSAGE); // Từ cấp độ 6 trở lên mới có thể gia
                                       // nhập Hội
    break;
  case 0xA1:
    g_pChatListBox->AddText(
        "", GlobalText[2992],
        SEASON3B::TYPE_ERROR_MESSAGE); // Chủ Hội không được gia nhập vào Genus
    break;
  case 0xA2:
    g_pChatListBox->AddText(
        "", GlobalText[2995],
        SEASON3B::TYPE_ERROR_MESSAGE); // Guild Master có thể gia nhập vào Genus
                                       // khác.
    break;
  case 0xA3:
    g_pChatListBox->AddText(
        "", GlobalText[2996],
        SEASON3B::TYPE_ERROR_MESSAGE); // Phải gia nhập vào Genus cùng với chủ
                                       // Hội thì mới có thể đăng ký gia nhập
                                       // Hội
    break;
  }
}
...
void ReceiveGuildList(BYTE *ReceiveBuffer) {
  ...
  for (int i = 0; i < Data->Count; i++) {
    LPPRECEIVE_GUILD_LIST Data2 =
        (LPPRECEIVE_GUILD_LIST)(ReceiveBuffer + Offset);
    ...
    // Sync GuildStatus to character in viewport
    for (int j = 0; j < MAX_CHARACTERS_CLIENT; j++) {
      CHARACTER *c = gmCharacters->GetCharacter(j);
      if (c && c->Object.Live && c->Object.Kind == KIND_PLAYER) {
        if (strcmp(c->ID, p->Name) == NULL) {
          {
...
void ReceiveGuildLeave(BYTE *ReceiveBuffer) {
  ...
  switch (Data->Value) {
  case 0:
    g_pChatListBox->AddText("", GlobalText[511], SEASON3B::TYPE_ERROR_MESSAGE);
    break; // Thông tin ID không đúng
  case 1:
    g_pChatListBox->AddText("", GlobalText[512], SEASON3B::TYPE_ERROR_MESSAGE);
    break; // Đã rời khỏi Hội
  case 2:
    g_pChatListBox->AddText("", GlobalText[513], SEASON3B::TYPE_ERROR_MESSAGE);
    break; // Chỉ có Chủ Hội mới có thể giải tán Hội
  case 3:
    g_pChatListBox->AddText("", GlobalText[514], SEASON3B::TYPE_ERROR_MESSAGE);
    break; // Không thể rời khỏi Hội
  case 4:
    g_pChatListBox->AddText("", GlobalText[515], SEASON3B::TYPE_ERROR_MESSAGE);
    break; // Hội đã bị giải tán
  case 5:
    g_pChatListBox->AddText("", GlobalText[568], SEASON3B::TYPE_ERROR_MESSAGE);
    break; //"Bạn đã tham gia Hội
  }
...
void ReceiveCreateGuildResult(BYTE *ReceiveBuffer) {
  ...
  case 0:
    g_pChatListBox->AddText(
        "", GlobalText[516],
        SEASON3B::TYPE_ERROR_MESSAGE); // Tên này đã được sử dụng
    break;
  case 2:
    g_pChatListBox->AddText(
        "", GlobalText[517],
        SEASON3B::TYPE_ERROR_MESSAGE); // Tên Hội ít nhất phải có 4 ký tự
    break;
  case 3:
    g_pChatListBox->AddText(
        "", GlobalText[518],
        SEASON3B::TYPE_ERROR_MESSAGE); // Đối phương đang là thành viên của Hội
                                       // khác
    break;
  case 4:
    g_pChatListBox->AddText(
        "", GlobalText[940],
        SEASON3B::TYPE_ERROR_MESSAGE); // Bạn không phải là thành viên của Hội
    break;
  case 5:
    g_pChatListBox->AddText(
        "", GlobalText[941],
        SEASON3B::TYPE_ERROR_MESSAGE); // Tên Guild không được có khoảng trống
    break;
  case 6:
    g_pChatListBox->AddText(
        "", GlobalText[942],
        SEASON3B::TYPE_ERROR_MESSAGE); // Không được bỏ trống
    break;
  case 1:
...
void ReceiveDeclareWar(BYTE *ReceiveBuffer) {
  ...
  if (Data->Type == 1) {
    SEASON3B::CreateMessageBox(
        MSGBOX_LAYOUT_CLASS(SEASON3B::CBattleSoccerMsgBoxLayout));
  } else {
    SEASON3B::CreateMessageBox(
        MSGBOX_LAYOUT_CLASS(SEASON3B::CGuildWarMsgBoxLayout));
  }
}
...
void ReceiveDeclareWarResult(BYTE *ReceiveBuffer) {
  ...
  switch (Data->Value) {
  case 0:
    g_pChatListBox->AddText("", GlobalText[519], SEASON3B::TYPE_ERROR_MESSAGE);
    break; // Bạn không phải là thành viên của Hội
  case 1:
    g_pChatListBox->AddText("", GlobalText[520], SEASON3B::TYPE_ERROR_MESSAGE);
    break; // Tuyên chiến với Hội
  case 2:
    g_pChatListBox->AddText("", GlobalText[521], SEASON3B::TYPE_ERROR_MESSAGE);
    break; // Chủ Hội đã thoát khỏi trò chơi
  case 3:
    g_pChatListBox->AddText("", GlobalText[522], SEASON3B::TYPE_ERROR_MESSAGE);
    break; // Hội này không tồn tại
  case 4:
    g_pChatListBox->AddText("", GlobalText[523], SEASON3B::TYPE_ERROR_MESSAGE);
    break; // Không được tuyên chiến
  case 5:
    g_pChatListBox->AddText("", GlobalText[524], SEASON3B::TYPE_ERROR_MESSAGE);
    break; // Chỉ có chủ Hội mới có thể tuyên chiến
  case 6:
    g_pChatListBox->AddText("", GlobalText[525], SEASON3B::TYPE_ERROR_MESSAGE);
    break; // Lời khiêu chiến bị từ chối
  }
...
void ReceiveGuildBeginWar(BYTE *ReceiveBuffer) {
  ...
  g_ConsoleDebug->Write(MCD_RECEIVE, "0x62 [ReceiveGuildBeginWar(%d)]",
                        Data->Team);
}
...
void ReceiveGuildEndWar(BYTE *ReceiveBuffer) {
  ...
  case 0:
    ...
    break;
  case 1:
    sprintf(Text, GlobalText[528]);
    Win = 2;
    break;
  case 2:
    sprintf(Text, GlobalText[529]);
    Win = 2;
    break;
  case 3:
...
void ReceiveGuildInfo(BYTE *ReceiveBuffer) {
  ...
  int Index = g_GuildCache.SetGuildMark(Data->GuildKey, Data->UnionName,
                                        Data->GuildName, Data->Mark);
}
...
void ReceiveGuildAssign(BYTE *ReceiveBuffer) {
  ...
  if (pData->byResult == 0x01) {
    ...
    // Update GuildStatus for the target character in viewport
    char szTargetName[MAX_ID_SIZE + 1] = {0};
    memcpy(szTargetName, pData->szTargetName, MAX_ID_SIZE);
    szTargetName[MAX_ID_SIZE] = 0;

    for (int i = 0; i < MAX_CHARACTERS_CLIENT; i++) {
      CHARACTER *c = gmCharacters->GetCharacter(i);
      if (c && c->Object.Live && c->Object.Kind == KIND_PLAYER) {
        if (strcmp(c->ID, szTargetName) == NULL) {
          if (pData->byType == 0x03) {
            // Kick: remove guild
            c->GuildStatus = -1;
            c->GuildMarkIndex = -1;
          }
...
void ReceiveGuildRelationShip(BYTE *ReceiveBuffer) {
  ...
  g_pGuildInfoWindow->ReceiveGuildRelationShip(
      pData->byRelationShipType, pData->byRequestType,
...
void ReceiveGuildRelationShipResult(BYTE *ReceiveBuffer) {
  ...
  LPPMSG_GUILD_RELATIONSHIP_RESULT pData =
      (LPPMSG_GUILD_RELATIONSHIP_RESULT)ReceiveBuffer;
  ...
  if (nCharKey == HeroKey && pData->byResult == 0x01 &&
      pData->byRelationShipType == 0x01 && pData->byRequestType == 0x02)
    GuildMark[Hero->GuildMarkIndex].UnionName[0] = NULL;
}
...
void ReceiveBanUnionGuildResult(BYTE *ReceiveBuffer) {
  ...
  if (pData->byResult == 0x01) {
    ...
    g_pChatListBox->AddText("", GlobalText[1328],
                            SEASON3B::TYPE_SYSTEM_MESSAGE);
  }
}

void ReceiveUnionViewportNotify(BYTE *ReceiveBuffer) {
  LPPMSG_UNION_VIEWPORT_NOTIFY_COUNT pData =
      (LPPMSG_UNION_VIEWPORT_NOTIFY_COUNT)ReceiveBuffer;
  int Offset = sizeof(PMSG_UNION_VIEWPORT_NOTIFY_COUNT);
  for (int i = 0; i < pData->byCount; ++i) {
    LPPMSG_UNION_VIEWPORT_NOTIFY pData2 =
        (LPPMSG_UNION_VIEWPORT_NOTIFY)(ReceiveBuffer + Offset);

    int nGuildMarkIndex = g_GuildCache.GetGuildMarkIndex(pData2->nGuildKey);
    memcpy(GuildMark[nGuildMarkIndex].UnionName, pData2->szUnionName,
           sizeof(char) * MAX_GUILDNAME);
...
void ReceiveUnionList(BYTE *ReceiveBuffer) {
  ...
  if (pData->byResult == 1) {
    ...
    for (int i = 0; i < pData->byCount; ++i) {
      ...
      g_pGuildInfoWindow->AddUnionList(tmp, pData2->szGuildName,
                                       pData2->byMemberCount);
...
void Receive_Master_Level_Exp(BYTE *ReceiveBuffer) {
  ...
  Master_Level_Data.lNext_MasterLevel_Experince =
      ConvertByteToLong(Data->btMNextExp);
  ...
  // Reinitialize master skill tree for current class
  if (gCharacterManager.IsMasterLevel(Hero->Class) == true) {
    g_pMasterSkillTree->SetMasterType(
        gCharacterManager.GetCharacterClass(Hero->Class));
  }
...
void Receive_Master_LevelUp(BYTE *ReceiveBuffer) {
  ...
  WORD iExp = Master_Level_Data.lNext_MasterLevel_Experince -
...
#if MAIN_UPDATE > 303
  LPPMSG_ANS_MASTERLEVEL_SKILL Data =
      (LPPMSG_ANS_MASTERLEVEL_SKILL)ReceiveBuffer;

  switch (Data->btResult) {
  case 1:
    g_pMasterSkillTree->RegisterSkillInCharacterAttribute(Data->nSkillNum);
    g_pMasterSkillTree->SkillUpgrade(Data->nSkillNum, Data->nSkillLevel,
                                     Data->fSkillValue, Data->fSkillNextValue);
    break;
  case 6:
    break;
  case 7:
...
void Receive_Master_LevelSkillTree(BYTE *ReceiveBuffer) {
  ...
  LPPMSG_ANS_MASTERLEVEL_SKILL_LIST_COUNT Data =
      (LPPMSG_ANS_MASTERLEVEL_SKILL_LIST_COUNT)ReceiveBuffer;
  int lOfs = sizeof(PMSG_ANS_MASTERLEVEL_SKILL_LIST_COUNT);
  for (int i = 0; i < Data->nCount; i++) {
    LPPMSG_ANS_MASTERLEVEL_SKILL_LIST pElementMsg =
        (LPPMSG_ANS_MASTERLEVEL_SKILL_LIST)(ReceiveBuffer + lOfs);
    g_pMasterSkillTree->SetMasterSkillTreeInfo(
        pElementMsg->byIndex, pElementMsg->byPoint, pElementMsg->fValue,
        pElementMsg->fNextValue);
...
BOOL ReceiveNewJoinMapServer(BYTE *ReceiveBuffer) {
  LPPRECEIVE_NEW_JOIN_MAP_SERVER Data =
      (LPPRECEIVE_NEW_JOIN_MAP_SERVER)ReceiveBuffer;
...
BOOL ReceiveNewCalculate(BYTE *ReceiveBuffer) {
  LPPRECEIVE_DEFAULT_CHARACTER Data =
      (LPPRECEIVE_DEFAULT_CHARACTER)ReceiveBuffer;
  ...
  CharacterAttribute->Work_DefenseSuccessRatePvP =
      Data->btDefenseSuccessRatePvP;
  ...
  CharacterAttribute->Work_DarkSpiritAttackDamageMin =
      Data->btDarkSpiritAttackDamageMin;
  CharacterAttribute->Work_DarkSpiritAttackDamageMax =
      Data->btDarkSpiritAttackDamageMax;
  CharacterAttribute->Work_DarkSpiritAttackSpeed =
      Data->btDarkSpiritAttackSpeed;
  CharacterAttribute->Work_DarkSpiritAttackSuccessRate =
      Data->btDarkSpiritAttackSuccessRate;
...
void GCBuyConfirmRecv(BYTE *ReceiveBuffer) // OK
{
  ...
  SEASON3B::CreateMessageBox(
      MSGBOX_LAYOUT_CLASS(SEASON3B::CConfirmBuyMsgBoxLayout));
}

void GCCharacterServerInfoRecv(BYTE *ReceiveBuffer) {
  LPPRECEIVE_CHARACTER_SERVER_INFO Data =
      (LPPRECEIVE_CHARACTER_SERVER_INFO)ReceiveBuffer;
...
void ReceiveServerCommand(BYTE *ReceiveBuffer) {
  ...
  case 5: {
    ...
    SEASON3B::CreateMessageBox(
        MSGBOX_LAYOUT_CLASS(SEASON3B::CDialogMsgBoxLayout), &pMsgBox);
    if (pMsgBox) {
  ...
  case 14: {
    ...
    case 0:
      SEASON3B::CreateMessageBox(
          MSGBOX_LAYOUT_CLASS(SEASON3B::CWhiteAngelEventLayout));
      break;
  ...
  case 15: {
    ...
    case 1:
      SEASON3B::CreateMessageBox(
          MSGBOX_LAYOUT_CLASS(SEASON3B::CHarvestEventLayout));
      break;
    case 2:
      SEASON3B::CreateOkMessageBox(GlobalText[2023]);
      break;
  ...
  case 16: {
    ...
    case 0:
      SEASON3B::CreateMessageBox(
          MSGBOX_LAYOUT_CLASS(SEASON3B::CSantaTownSantaMsgBoxLayout), &pMsgBox);
  ...
  case 17:
    SEASON3B::CreateMessageBox(
        MSGBOX_LAYOUT_CLASS(SEASON3B::CSantaTownLeaveMsgBoxLayout));
    break;
  case 47:
  case 48:
  case 49:
  ...
  case 58: {
    ...
    if (pCharacter) {
      ...
      if (to != NULL) {
        CreateEffect(MODEL_EFFECT_SKURA_ITEM, to->Position, to->Angle,
                     to->Light, 0, to);
...
void ReceiveMix(BYTE *ReceiveBuffer) {
  ...
  case 0: {
    ...
    case SEASON3A::MIXTYPE_SEED_SPHERE:
      ...
      // 				unicode::_sprintf(szText,
      // GlobalText[1208]);	// ºÎÈ° ½ÇÆÐ
      // g_pChatListBox->AddText("", szText, SEASON3B::TYPE_ERROR_MESSAGE);
      // break;
  ...
  case 1: {
    ...
    case SEASON3A::MIXTYPE_SEED_SPHERE:
      ...
      // 			case SEASON3A::MIXTYPE_TRAINER:
      // 				unicode::_sprintf(szText,
      // GlobalText[1209]); g_pChatListBox->AddText("", szText,
      // SEASON3B::TYPE_SYSTEM_MESSAGE); 				break;
    case SEASON3A::MIXTYPE_OSBOURNE:
...
void ReceiveGemMixResult(BYTE *ReceiveBuffer) {
  ...
  case 2:
  case 3: {
    sprintf(sBuf, "%s%s %s", GlobalText[1801], GlobalText[1816],
            GlobalText[868]);
  ...
  case 1: {
    SEASON3B::CreateMessageBox(
        MSGBOX_LAYOUT_CLASS(SEASON3B::CGemIntegrationUnityResultMsgBoxLayout));
  } break;
  case 4: {
    g_pChatListBox->AddText("", GlobalText[1817],
                            SEASON3B::TYPE_SYSTEM_MESSAGE);
    COMGEM::GetBack();
  } break;
  case 5: {
    g_pChatListBox->AddText("", GlobalText[1811],
                            SEASON3B::TYPE_SYSTEM_MESSAGE);
    COMGEM::GetBack();
  } break;
  }
...
void ReceiveGemUnMixResult(BYTE *ReceiveBuffer) {
  ...
  case 0:
  case 5: {
    sprintf(sBuf, "%s%s %s", GlobalText[1800], GlobalText[1816],
            GlobalText[868]);
    g_pChatListBox->AddText("", sBuf, SEASON3B::TYPE_SYSTEM_MESSAGE);
    COMGEM::GetBack();
  } break;
  case 1: {
    SEASON3B::CreateMessageBox(MSGBOX_LAYOUT_CLASS(
        SEASON3B::CGemIntegrationDisjointResultMsgBoxLayout));
  } break;
  case 2:
  case 3:
  case 4:
  case 6: {
    g_pChatListBox->AddText("", GlobalText[1812],
                            SEASON3B::TYPE_SYSTEM_MESSAGE);
    COMGEM::GetBack();
  } break;
  case 7: {
    g_pChatListBox->AddText("", GlobalText[1815],
                            SEASON3B::TYPE_SYSTEM_MESSAGE);
    COMGEM::GetBack();
  } break;
  case 8: {
...
void ReceiveDevilSquareRank(BYTE *ReceiveBuffer) {
  ...
  matchEvent::SetMatchResult(Data->m_Count, Data->m_MyRank,
                             (MatchResult *)&Data->m_byRank, Data->m_MyRank);
}
...
void ReceiveMoveToEventMatchResult(BYTE *ReceiveBuffer) {
  ...
  case 5: {
    unicode::t_char strText[128];
    unicode::_sprintf(strText, GlobalText[853], GlobalText[1146],
                      MAX_BLOOD_CASTLE_MEN);
    SEASON3B::CreateOkMessageBox(strText);
  } break;
  ...
  case 9: {
    unicode::t_char strText[128];
    unicode::_sprintf(strText, GlobalText[853], GlobalText[1147],
                      MAX_CHAOS_CASTLE_MEN);
    SEASON3B::CreateOkMessageBox(strText);
  } break;
  }
}
...
void ReceiveEventZoneOpenTime(BYTE *ReceiveBuffer) {
  ...
  if (Data->Value == 1) {
    ...
    if (0 == Data->KeyH) {
      ...
      unicode::_sprintf(strText, GlobalText[851], (int)Data->KeyH,
                        GlobalText[1146]);
    }
    SEASON3B::CreateOkMessageBox(strText);
    ...
    if (0 == time) {
      char szOpenTime1[256] = {
          0,
      };
      char szOpenTime2[256] = {
          0,
      };
...
void ReceiveSetAttribute(BYTE *ReceiveBuffer) {
  ...
  case 0: {
    ...
    for (int k = 0; k < Data->m_byCount; k++) {
      int dx = Data->m_vAttribute[(k * 2) + 1].m_byX -
               Data->m_vAttribute[(k * 2)].m_byX + 1;
      int dy = Data->m_vAttribute[(k * 2) + 1].m_byY -
               Data->m_vAttribute[(k * 2)].m_byY + 1;

      g_ErrorReport.Write("count:%d, x:%d, y:%d \r\n", Data->m_byCount, dx, dy);

      AddTerrainAttributeRange(Data->m_vAttribute[(k * 2)].m_byX,
                               Data->m_vAttribute[(k * 2)].m_byY, dx, dy,
  ...
  case 1:
    for (int i = 0; i < Data->m_byCount; ++i) {
      if (Data->m_byMapSetType) {
        ...
        SubTerrainAttribute(Data->m_vAttribute[i].m_byX,
                            Data->m_vAttribute[i].m_byY, Data->m_byMapAttr);
      } else {
        g_ErrorReport.Write("AddTerrainAttribute - count:%d, x:%d, y:%d \r\n",
                            Data->m_byCount, Data->m_vAttribute[i].m_byX,
                            Data->m_vAttribute[i].m_byY);
        AddTerrainAttribute(Data->m_vAttribute[i].m_byX,
                            Data->m_vAttribute[i].m_byY, Data->m_byMapAttr);
      }
    }
    break;
...
void ReceiveDuelRequest(BYTE *ReceiveBuffer) {
  ...
  g_DuelMgr.SetDuelPlayer(DUEL_ENEMY, MAKEWORD(Data->bIndexL, Data->bIndexH),
                          Data->szID);
...
void ReceiveDuelStart(BYTE *ReceiveBuffer) {
  ...
  if (Data->nResult == 0) {
    ...
    SEASON3B::CreateMessageBox(
        MSGBOX_LAYOUT_CLASS(SEASON3B::CDuelCreateErrorMsgBoxLayout));
...
void ReceiveDuelScore(BYTE *ReceiveBuffer) {
  ...
  if (g_DuelMgr.IsDuelPlayer(MAKEWORD(Data->bIndexL1, Data->bIndexH1),
                             DUEL_HERO)) {
    g_DuelMgr.SetScore(DUEL_HERO, Data->btDuelScore1);
    g_DuelMgr.SetScore(DUEL_ENEMY, Data->btDuelScore2);
  } else if (g_DuelMgr.IsDuelPlayer(MAKEWORD(Data->bIndexL2, Data->bIndexH2),
                                    DUEL_HERO)) {
...
void ReceiveDuelChannelList(BYTE *ReceiveBuffer) {
  ...
  for (int i = 0; i < 4; ++i) {
    g_DuelMgr.SetDuelChannel(i, Data->channel[i].bStart,
                             Data->channel[i].bWatch, Data->channel[i].szID1,
                             Data->channel[i].szID2);
  }
}
...
void ReceiveDuelWatchRequestReply(BYTE *ReceiveBuffer) {
  ...
  if (Data->nResult == 0) {
    ...
    g_DuelMgr.SetDuelPlayer(DUEL_HERO, MAKEWORD(Data->bIndexL1, Data->bIndexH1),
                            Data->szID1);
    g_DuelMgr.SetDuelPlayer(
        DUEL_ENEMY, MAKEWORD(Data->bIndexL2, Data->bIndexH2), Data->szID2);
  } else if (Data->nResult == 16) {
    SEASON3B::CreateMessageBox(
        MSGBOX_LAYOUT_CLASS(SEASON3B::CDuelCreateErrorMsgBoxLayout));
  } else if (Data->nResult == 27) {
    SEASON3B::CreateMessageBox(
        MSGBOX_LAYOUT_CLASS(SEASON3B::CDuelWatchErrorMsgBoxLayout));
  } else {
  }
}

void ReceiveDuelWatcherJoin(BYTE *ReceiveBuffer) {
  LPPMSG_DUEL_JOINCNANNEL_BROADCAST Data =
      (LPPMSG_DUEL_JOINCNANNEL_BROADCAST)ReceiveBuffer;
  g_DuelMgr.AddDuelWatchUser(Data->szID);
}

void ReceiveDuelWatchEnd(BYTE *ReceiveBuffer) {
  LPPMSG_ANS_DUEL_LEAVECNANNEL Data =
      (LPPMSG_ANS_DUEL_LEAVECNANNEL)ReceiveBuffer;
  if (Data->nResult == 0) {
...
void ReceiveDuelWatcherQuit(BYTE *ReceiveBuffer) {
  LPPMSG_DUEL_LEAVECNANNEL_BROADCAST Data =
      (LPPMSG_DUEL_LEAVECNANNEL_BROADCAST)ReceiveBuffer;
...
void ReceiveDuelWatcherList(BYTE *ReceiveBuffer) {
  ...
  LPPMSG_DUEL_OBSERVERLIST_BROADCAST Data =
      (LPPMSG_DUEL_OBSERVERLIST_BROADCAST)ReceiveBuffer;
  for (int i = 0; i < Data->nCount; ++i) {
...
void ReceiveDuelResult(BYTE *ReceiveBuffer) {
  LPPMSG_DUEL_RESULT_BROADCAST Data =
      (LPPMSG_DUEL_RESULT_BROADCAST)ReceiveBuffer;
  ...
  SEASON3B::CreateMessageBox(
      MSGBOX_LAYOUT_CLASS(SEASON3B::CDuelResultMsgBoxLayout), &lpMsgBox);
  if (lpMsgBox) {
...
void ReceiveDuelRound(BYTE *ReceiveBuffer) {
  LPPMSG_DUEL_ROUNDSTART_BROADCAST Data =
      (LPPMSG_DUEL_ROUNDSTART_BROADCAST)ReceiveBuffer;
...
void ReceiveCreateShopTitleViewport(BYTE *ReceiveBuffer) {
  ...
  for (int i = 0; i < Header->byCount; i++) {
    PSHOPTITLE_DATAINFO *pShopTitle =
        (PSHOPTITLE_DATAINFO *)(ReceiveBuffer + Offset);
...
void ReceiveSetPriceResult(BYTE *ReceiveBuffer) {
  ...
  if (Header->byResult != 0x01 && g_IsPurchaseShop == PSHOPWNDTYPE_SALE) {
    ...
    RemovePersonalItemPrice(MAX_MY_INVENTORY_INDEX +
                                g_pMyShopInventory->GetTargetIndex(),
                            PSHOPWNDTYPE_SALE);
...
void ReceivePersonalShopItemList(BYTE *ReceiveBuffer) {
  LPGETPSHOPITEMLIST_HEADERINFO Header =
      (LPGETPSHOPITEMLIST_HEADERINFO)ReceiveBuffer;
  ...
  if (Header->byResult == 0x01) {
    ...
    RemoveAllPerosnalItemPrice(
        PSHOPWNDTYPE_PURCHASE); //. clear item price table
    LPGETPSHOPITEM_DATAINFO pShopItem =
        (LPGETPSHOPITEM_DATAINFO)(ReceiveBuffer +
                                  sizeof(GETPSHOPITEMLIST_HEADERINFO));

    for (int i = 0; i < Header->byCount; i++, pShopItem++) {
      if (pShopItem->iItemPrice > 0 || pShopItem->JoBValue > 0 ||
          pShopItem->JoSValue > 0 || pShopItem->JoCValue > 0) {
        int itemindex = pShopItem->byPos - (MAX_MY_INVENTORY_INDEX);
        g_pPurchaseShopInventory->InsertItem(itemindex, pShopItem->byItemInfo);
        AddPersonalItemPrice(pShopItem->byPos, pShopItem->PersonalShopType,
                             pShopItem->iItemPrice, pShopItem->JoBValue,
                             pShopItem->JoSValue, pShopItem->JoCValue,
                             PSHOPWNDTYPE_PURCHASE);
      } else {
        g_ConsoleDebug->Write(MCD_ERROR,
                              "[ReceivePersonalShopItemList]Item Cound : %d, "
                              "Item Index : %d, Item Price : %d",
                              Header->byCount, i, pShopItem->iItemPrice);

        g_ErrorReport.Write(
            "@ ReceivePersonalShopItemList - item price less than zero(%d)\n",
    ...
    case 0x03: {
      g_pChatListBox->AddText("", GlobalText[1120],
                              SEASON3B::TYPE_ERROR_MESSAGE);
    } break;
    case 0x04:
    default:
      g_ErrorReport.Write(
          "@ [Fault] ReceivePersonalShopItemList (result : %d)\n",
          Header->byResult);
    }
  }
...
void ReceiveRefreshItemList(BYTE *ReceiveBuffer) {
  ...
  if (Header->byResult == 0x01 && g_IsPurchaseShop == PSHOPWNDTYPE_PURCHASE) {
    ...
    if (Header->byResult == 0x01) {
      ...
      g_ErrorReport.Write(
          "@ [Notice] ReceiveRefreshItemList (InventoryCtrl Count Items(%d))\n",
          uiCntInvenCtrl);
    } else {
...
void ReceivePurchaseItem(BYTE *ReceiveBuffer) {
  ...
  if (Header->byResult == 0x01) {
    if (g_pNewUISystem->IsVisible(SEASON3B::INTERFACE_PURCHASESHOP_INVENTORY)) {
      RemovePersonalItemPrice(g_pPurchaseShopInventory->GetSourceIndex(),
                              PSHOPWNDTYPE_PURCHASE);
      g_pPurchaseShopInventory->DeleteItem(
    ...
    switch (Header->byResult) {
    case 0x07: {
      g_pChatListBox->AddText("", GlobalText[423],
                              SEASON3B::TYPE_ERROR_MESSAGE);
    } break;
    case 0x08: {
      g_pChatListBox->AddText("", GlobalText[375],
                              SEASON3B::TYPE_ERROR_MESSAGE);
    ...
    case 0x0D:
      ...
      break;
    case 0x23:
      g_pChatListBox->AddText("", GlobalText[495],
                              SEASON3B::TYPE_ERROR_MESSAGE);
      break;
    case 0x09:
    default:
...
void ReceiveDisplayEffectViewport(BYTE *ReceiveBuffer) {
  LPDISPLAYEREFFECT_NOTIFYINFO Header =
      (LPDISPLAYEREFFECT_NOTIFYINFO)ReceiveBuffer;
    ...
    if (o->Kind == KIND_PLAYER) {
      ...
      case 0x10: //. Level up
      {
        if (gCharacterManager.IsMasterLevel(pCharacter->Class) == true) {
          CreateJoint(BITMAP_FLARE, o->Position, o->Position, o->Angle, 45, o,
                      80, 2);
          for (int i = 0; i < 19; ++i) {
            CreateJoint(BITMAP_FLARE, o->Position, o->Position, o->Angle, 46, o,
                        80, 2);
          }
        } else {
          for (int i = 0; i < 15; ++i) {
      ...
      case 0x11: {
        if (!gMapManager->InChaosCastle()) {
          CreateEffect(MODEL_SHIELD_CRASH, o->Position, o->Angle, o->Light, 0,
                       o);
...
void ReceiveFriendList(BYTE *ReceiveBuffer) {
  ...
  for (int i = 0; i < Header->Count; ++i) {
    LPFS_FRIEND_LIST_DATA Data =
        (LPFS_FRIEND_LIST_DATA)(ReceiveBuffer + iMoveOffset);
...
void ReceiveAddFriendResult(BYTE *ReceiveBuffer) {
  ...
  case 0x00:
    ...
    break;
  case 0x01: {
    g_pChatListBox->AddText("", GlobalText[1075],
                            SEASON3B::TYPE_SYSTEM_MESSAGE);
    g_pFriendList->AddFriend(szName, 0, Data->Server);
    g_pFriendList->Sort();
    g_pWindowMgr->RefreshMainWndPalList();
...
void ReceiveRequestAcceptAddFriend(BYTE *ReceiveBuffer) {
  LPFS_ACCEPT_ADD_FRIEND_RESULT Data =
      (LPFS_ACCEPT_ADD_FRIEND_RESULT)ReceiveBuffer;
  char szName[MAX_ID_SIZE + 1] = {0};
  strncpy(szName, (const char *)Data->Name, MAX_ID_SIZE);
  szName[MAX_ID_SIZE] = '\0';
  char szText[MAX_TEXT_LENGTH + 1] = {0};
  ...
  DWORD dwWindowID = g_pWindowMgr->AddWindow(
      UIWNDTYPE_QUESTION_FORCE, UIWND_DEFAULT, UIWND_DEFAULT, szText, -1);
  if (dwWindowID != 0) {
...
void ReceiveFriendStateChange(BYTE *ReceiveBuffer) {
  ...
  if (dwChatRoomUIID > 0) {
    CUIChatWindow *pWindow =
        (CUIChatWindow *)g_pWindowMgr->GetWindow(dwChatRoomUIID);
    if (pWindow == NULL)
      ;
...
void ReceiveLetter(BYTE *ReceiveBuffer) {
  ...
  case 0x02:
    ...
    g_pChatListBox->AddText("", GlobalText[1062],
                            SEASON3B::TYPE_SYSTEM_MESSAGE);
    g_pLetterList->AddLetter(Data->Index, szName, szSubject, szDate, szTime,
                             0x00);
    g_pLetterList->Sort();
    break;
  case 0x00:
  ...
  if (g_pLetterList->GetLetterCount() >= g_iMaxLetterCount) {
    g_pChatListBox->AddText("", GlobalText[1073],
                            SEASON3B::TYPE_SYSTEM_MESSAGE);
  }
}
...
void ReceiveLetterText(BYTE *ReceiveBuffer) {
  ...
  if (g_iLetterReadNextPos_x == UIWND_DEFAULT) {
    ...
    dwUIID = g_pWindowMgr->AddWindow(
        UIWNDTYPE_READLETTER, g_iLetterReadNextPos_x, g_iLetterReadNextPos_y,
        tempTxt, 0, UIADDWND_FORCEPOSITION);
    g_iLetterReadNextPos_x = UIWND_DEFAULT;
  }

  CUILetterReadWindow *pWindow =
...
void ReceiveCreateChatRoomResult(BYTE *ReceiveBuffer) {
  ...
  case 0x01:
    if (Data->Type == 0) {
      g_pFriendMenu->RemoveRequestWindow(szName);
      DWORD dwUIID =
          g_pWindowMgr->AddWindow(UIWNDTYPE_CHAT, 100, 100, GlobalText[994]);
      ((CUIChatWindow *)g_pWindowMgr->GetWindow(dwUIID))
          ->ConnectToChatServer((char *)szIP, Data->RoomNumber, Data->Ticket);
    } else if (Data->Type == 1) {
      DWORD dwUIID = g_pFriendMenu->CheckChatRoomDuplication(szName);
      if (dwUIID == 0) {
        dwUIID = g_pWindowMgr->AddWindow(UIWNDTYPE_CHAT_READY, 100, 100,
                                         GlobalText[994]);
        ((CUIChatWindow *)g_pWindowMgr->GetWindow(dwUIID))
            ->ConnectToChatServer((char *)szIP, Data->RoomNumber, Data->Ticket);
        ...
        //				SendRequestCRDisconnectRoom(((CUIChatWindow
        //*)g_pWindowMgr->GetWindow(dwUIID))->GetCurrentSocket());
        // DWORD dwOldRoomNumber = ((CUIChatWindow
        //*)g_pWindowMgr->GetWindow(dwUIID))->GetRoomNumber();
        //				((CUIChatWindow
        //*)g_pWindowMgr->GetWindow(dwUIID))->ConnectToChatServer((char *)szIP,
        // Data->RoomNumber, Data->Ticket);
        //				g_pChatRoomSocketList->RemoveChatRoomSocket(dwOldRoomNumber);
      }
    } else if (Data->Type == 2) {
      DWORD dwUIID = g_pWindowMgr->AddWindow(UIWNDTYPE_CHAT_READY, 100, 100,
                                             GlobalText[994]);
      ((CUIChatWindow *)g_pWindowMgr->GetWindow(dwUIID))
          ->ConnectToChatServer((char *)szIP, Data->RoomNumber, Data->Ticket);
      g_pWindowMgr->GetWindow(dwUIID)->SetState(UISTATE_READY);
...
void ReceiveChatRoomInviteResult(BYTE *ReceiveBuffer) {
  ...
  CUIChatWindow *pChatWindow =
      (CUIChatWindow *)g_pWindowMgr->GetWindow(Data->WindowGuid);
  if (pChatWindow == NULL)
    return;
...
void ReceiveChatRoomUserList(DWORD dwWindowUIID, BYTE *ReceiveBuffer) {
  ...
  char szName[MAX_ID_SIZE + 1] = {0};
  for (int i = 0; i < Header->Count; ++i) {
    LPFS_CHAT_USERLIST_DATA Data =
        (LPFS_CHAT_USERLIST_DATA)(ReceiveBuffer + iMoveOffset);
    strncpy(szName, (const char *)Data->Name, MAX_ID_SIZE);
    szName[MAX_ID_SIZE] = '\0';
    ((CUIChatWindow *)g_pWindowMgr->GetWindow(dwWindowUIID))
...
void ReceiveChatRoomChatText(DWORD dwWindowUIID, BYTE *ReceiveBuffer) {
  ...
  if (pChatWindow->GetState() == UISTATE_READY) {
    ...
    g_pChatListBox->AddText("", GlobalText[1063],
                            SEASON3B::TYPE_SYSTEM_MESSAGE);
...
void ReceiveOption(BYTE *ReceiveBuffer) {
  ...
  g_pMainFrame->SetItemHotKey(SEASON3B::HOTKEY_Q, Data->KeyQWE[0] + ITEM_POTION,
                              byQLevel);
  g_pMainFrame->SetItemHotKey(SEASON3B::HOTKEY_W, Data->KeyQWE[1] + ITEM_POTION,
                              byWLevel);
  g_pMainFrame->SetItemHotKey(SEASON3B::HOTKEY_E, Data->KeyQWE[2] + ITEM_POTION,
                              byELevel);
  ...
  g_pMainFrame->SetItemHotKey(SEASON3B::HOTKEY_R, Data->KeyR + ITEM_POTION,
                              byRLevel);
}
...
void ReceiveEventChipInfomation(BYTE *ReceiveBuffer) {
  ...
  if (g_bEventChipDialogEnable == EVENT_LENA) {
    ...
    if (Data->m_shMutoNum[0] != -1 && Data->m_shMutoNum[1] != -1 &&
        Data->m_shMutoNum[2] != -1) {
...
void ReceiveBuffState(BYTE *ReceiveBuffer) {
  ...
  if (Data->byEffectOption == 0) {
    ...
    if (bufftype == eBuff_HelpNpc) {
      g_pChatListBox->AddText("", GlobalText[1828],
                              SEASON3B::TYPE_SYSTEM_MESSAGE);
    }
  } else {
...
void ReceiveServerImmigration(BYTE *ReceiveBuffer) {
  ...
  case 0:
    SEASON3B::CreateMessageBox(
        MSGBOX_LAYOUT_CLASS(SEASON3B::CServerImmigrationErrorMsgBoxLayout));
    break;
  case 1:
...
void ReceiveScratchResult(BYTE *ReceiveBuffer) {
  LPPRECEIVE_SCRATCH_TICKET_EVENT Data =
      (LPPRECEIVE_SCRATCH_TICKET_EVENT)ReceiveBuffer;
...
void ReceivePlaySoundEffect(BYTE *ReceiveBuffer) {
  LPPRECEIVE_PLAY_SOUND_EFFECT Data =
      (LPPRECEIVE_PLAY_SOUND_EFFECT)ReceiveBuffer;
...
void ReceiveQuestPrize(BYTE *ReceiveBuffer) {
  ...
  case 201: {
    ...
    if (c) {
      ...
      BYTE byClass = gCharacterManager.ChangeServerClassTypeToClientClassType(
          Data->m_byNumber);
  ...
  case 204: {
    ...
    if (pCharacter) {
      BYTE byClass = gCharacterManager.ChangeServerClassTypeToClientClassType(
          Data->m_byNumber);
      if (gCharacterManager.GetStepClass(byClass) == 3 ||
          gCharacterManager.GetStepClass(byClass) == 4) {
        pCharacter->Class = byClass;

        if (Hero == pCharacter) {
          CharacterAttribute->Class = byClass;

          if (gCharacterManager.IsMasterLevel(byClass)) {
            g_pMasterSkillTree->SetMasterType(
                gCharacterManager.GetCharacterClass(byClass));
            g_pMasterSkillTree->ClearSkillTreeInfo();
            g_pMasterSkillTree->InitMasterSkillPoint();
          }
        }

        CreateEffect(MODEL_CHANGE_UP_EFF, pCharacter->Object.Position,
                     pCharacter->Object.Angle, pCharacter->Object.Light, 0,
                     &pCharacter->Object);
        CreateEffect(MODEL_CHANGE_UP_NASA, pCharacter->Object.Position,
                     pCharacter->Object.Angle, pCharacter->Object.Light, 0,
                     &pCharacter->Object);
        CreateEffect(MODEL_CHANGE_UP_CYLINDER, pCharacter->Object.Position,
                     pCharacter->Object.Angle, pCharacter->Object.Light, 0,
                     &pCharacter->Object);
        CreateEffect(MODEL_INFINITY_ARROW3, pCharacter->Object.Position,
                     pCharacter->Object.Angle, pCharacter->Object.Light, 1,
                     &pCharacter->Object);
...
void ReceiveQuestMonKillInfo(BYTE *ReceiveBuffer) {
  LPPMSG_ANS_QUEST_MONKILL_INFO pData =
      (LPPMSG_ANS_QUEST_MONKILL_INFO)ReceiveBuffer;
...
void ReceiveQuestLimitResult(BYTE *ReceiveBuffer) {
  ...
  case QUEST_RESULT_CNT_LIMIT:
    SEASON3B::CreateMessageBox(
        MSGBOX_LAYOUT_CLASS(SEASON3B::CQuestCountLimitMsgBoxLayout));
    break;
  }
}
...
void ReceiveQuestByItemUseEP(BYTE *ReceiveBuffer) {
  DWORD *pdwQuestIndex =
      (DWORD *)(ReceiveBuffer + sizeof(PMSG_NPCTALK_QUESTLIST));
...
void ReceiveQuestByEtcEPList(BYTE *ReceiveBuffer) {
  ...
  g_QuestMng.SetQuestIndexByEtcList(
      (DWORD *)(ReceiveBuffer + sizeof(PMSG_NPCTALK_QUESTLIST)),
      pData->m_wQuestCount);
}
...
void ReceiveQuestCompleteResult(BYTE *ReceiveBuffer) {
  LPPMSG_ANS_QUESTEXP_COMPLETE pData =
      (LPPMSG_ANS_QUESTEXP_COMPLETE)ReceiveBuffer;
...
void ReceiveProgressQuestList(BYTE *ReceiveBuffer) {
  LPPMSG_ANS_QUESTEXP_PROGRESS_LIST pData =
      (LPPMSG_ANS_QUESTEXP_PROGRESS_LIST)ReceiveBuffer;
...
void ReceiveGensJoining(BYTE *ReceiveBuffer) {
  ...
  if (g_pNewUISystem->IsVisible(SEASON3B::INTERFACE_NPC_DIALOGUE))
    g_pNPCDialogue->ProcessGensJoiningReceive(pData->m_byResult,
                                              pData->m_byInfluence);
#endif
}
...
#if MAIN_UPDATE > 303
  LPPMSG_ANS_SECEDE_GENS_MEMBER pData =
      (LPPMSG_ANS_SECEDE_GENS_MEMBER)ReceiveBuffer;
  if (g_pNewUISystem->IsVisible(SEASON3B::INTERFACE_NPC_DIALOGUE))
...
#if MAIN_UPDATE > 303
  LPPMSG_SEND_GENS_MEMBER_VIEWPORT Data =
      (LPPMSG_SEND_GENS_MEMBER_VIEWPORT)ReceiveBuffer;
  int nOffset = sizeof(PMSG_SEND_GENS_MEMBER_VIEWPORT);
  int i;
  for (i = 0; i < Data->m_byCount; ++i) {
    LPPMSG_GENS_MEMBER_VIEWPORT_INFO Data2 =
        (LPPMSG_GENS_MEMBER_VIEWPORT_INFO)(ReceiveBuffer + nOffset);
    ...
    if (::IsStrifeMap(gMapManager->currentMap)) {
      ...
      if (Hero->m_byGensInfluence == c->m_byGensInfluence)
        CreateEffect(BITMAP_OUR_INFLUENCE_GROUND, c->Object.Position, vTemp,
                     vTemp, 0, &c->Object);
      else
        CreateEffect(BITMAP_ENEMY_INFLUENCE_GROUND, c->Object.Position, vTemp,
                     vTemp, 0, &c->Object);
    }
...
void ReceiveUseStateItem(BYTE *ReceiveBuffer) {
  ...
  case 0x00:
    if (fruit >= 0 && fruit <= 4) {
      ...
      if (index != 0) {
        unicode::_sprintf(strText, GlobalText[379], GlobalText[index], point,
                          GlobalText[1412]);
        SEASON3B::CreateOkMessageBox(strText);
      }
    }
  ...
  case 0x06:
    if (fruit >= 0 && fruit <= 4) {
      ...
      if (index != 0) {
        sprintf(Text, GlobalText[379], GlobalText[index], point,
                GlobalText[1903]);
...
void ReceiveWTMatchResult(BYTE *ReceiveBuffer) {
  ...
  if (Data->m_Type >= 0 && Data->m_Type < 3) {
    ...
    memcpy(g_wtMatchResult.m_MatchTeamName1, Data->m_MatchTeamName1,
           MAX_ID_SIZE);
    g_wtMatchResult.m_Score1 = Data->m_Score1;
    memset(g_wtMatchResult.m_MatchTeamName2, 0, MAX_ID_SIZE);
    memcpy(g_wtMatchResult.m_MatchTeamName2, Data->m_MatchTeamName2,
           MAX_ID_SIZE);
...
void ReceiveChangeMapServerInfo(BYTE *ReceiveBuffer) {
  LPPHEADER_MAP_CHANGESERVER_INFO Data =
      (LPPHEADER_MAP_CHANGESERVER_INFO)ReceiveBuffer;

  g_pReconnectUI->ReconnectOnMapServerMove(Data->m_vSvrInfo.m_szMapSvrIpAddress,
                                           Data->m_vSvrInfo.m_wMapSvrPort);
...
void ReceiveChangeMapServerResult(BYTE *ReceiveBuffer) {
  ...
  if (Data->Value == RESULT_SUB_GM_ACCESS_DENIED) {
    CUIMng::Instance().PopUpMsgWin(MESSAGE_SUB_GM_ACCESS_DENIED);
  }
...
void ReceiveBCStatus(BYTE *ReceiveBuffer) {
  ...
  switch (Data->btResult) {
  case 0x00:
    g_pChatListBox->AddText("", GlobalText[1500],
                            SEASON3B::TYPE_SYSTEM_MESSAGE);
    break;
  case 0x01:
  case 0x02:
    ...
    break;
  case 0x03:
    g_pChatListBox->AddText("", GlobalText[1501],
                            SEASON3B::TYPE_SYSTEM_MESSAGE);
    break;
  case 0x04:
    g_pChatListBox->AddText("", GlobalText[1502],
                            SEASON3B::TYPE_SYSTEM_MESSAGE);
    break;
  }
}
...
void ReceiveBCReg(BYTE *ReceiveBuffer) {
  ...
  switch (Data->btResult) {
  case 0x00:
    g_pChatListBox->AddText("", GlobalText[1503],
                            SEASON3B::TYPE_SYSTEM_MESSAGE);
    break;
  case 0x01:
    g_GuardsMan.SetRegStatus(1);
    g_pChatListBox->AddText("", GlobalText[1504],
                            SEASON3B::TYPE_SYSTEM_MESSAGE);
    break;
  case 0x02:
    g_pChatListBox->AddText("", GlobalText[1505],
                            SEASON3B::TYPE_SYSTEM_MESSAGE);
    break;
  case 0x03:
    g_pChatListBox->AddText("", GlobalText[1506],
                            SEASON3B::TYPE_SYSTEM_MESSAGE);
    break;
  case 0x04:
    g_pChatListBox->AddText("", GlobalText[1507],
                            SEASON3B::TYPE_SYSTEM_MESSAGE);
    break;
  case 0x05:
    g_pChatListBox->AddText("", GlobalText[1508],
                            SEASON3B::TYPE_SYSTEM_MESSAGE);
    break;
  case 0x06:
    g_pChatListBox->AddText("", GlobalText[1509],
                            SEASON3B::TYPE_SYSTEM_MESSAGE);
    break;
  case 0x07:
    g_pChatListBox->AddText("", GlobalText[1510],
                            SEASON3B::TYPE_SYSTEM_MESSAGE);
    break;
  case 0x08:
    g_pChatListBox->AddText("", GlobalText[1511],
                            SEASON3B::TYPE_SYSTEM_MESSAGE);
    break;
  default:
...
void ReceiveBCGiveUp(BYTE *ReceiveBuffer) {
  LPPMSG_ANS_GIVEUPCASTLESIEGE Data =
      (LPPMSG_ANS_GIVEUPCASTLESIEGE)ReceiveBuffer;

  switch (Data->btResult) {
  case 0x00:
    g_pChatListBox->AddText("", GlobalText[1512],
                            SEASON3B::TYPE_SYSTEM_MESSAGE);
    break;
  case 0x01:
    ...
    g_pChatListBox->AddText("", GlobalText[1513],
                            SEASON3B::TYPE_SYSTEM_MESSAGE);
    break;
  case 0x02:
    g_pChatListBox->AddText("", GlobalText[1514],
                            SEASON3B::TYPE_SYSTEM_MESSAGE);
    break;
  case 0x03:
    g_pChatListBox->AddText("", GlobalText[1515],
                            SEASON3B::TYPE_SYSTEM_MESSAGE);
    break;
  default:
...
void ReceiveBCRegMark(BYTE *ReceiveBuffer) {
  ...
  switch (Data->btResult) {
  case 0x00:
    g_pChatListBox->AddText("", GlobalText[1516],
                            SEASON3B::TYPE_SYSTEM_MESSAGE);
    break;
  case 0x01: {
    DWORD dwMarkCount;
  ...
  case 0x02:
    g_pChatListBox->AddText("", GlobalText[1517],
                            SEASON3B::TYPE_SYSTEM_MESSAGE);
    break;
  case 0x03:
    g_pChatListBox->AddText("", GlobalText[1518],
                            SEASON3B::TYPE_SYSTEM_MESSAGE);
    break;
  }
}
...
void ReceiveBCNPCBuy(BYTE *ReceiveBuffer) {
  ...
  switch (Data->btResult) {
  case 0:
    g_pChatListBox->AddText("", GlobalText[1519],
                            SEASON3B::TYPE_SYSTEM_MESSAGE);
    break;
  case 1:
    ...
    break;
  case 2:
    g_pChatListBox->AddText("", GlobalText[1386],
                            SEASON3B::TYPE_SYSTEM_MESSAGE);
    break;
  case 3:
    g_pChatListBox->AddText("", GlobalText[1520],
                            SEASON3B::TYPE_SYSTEM_MESSAGE);
    break;
  case 4:
    g_pChatListBox->AddText("", GlobalText[1616],
                            SEASON3B::TYPE_SYSTEM_MESSAGE);
    break;
  }
}
...
void ReceiveBCNPCUpgrade(BYTE *ReceiveBuffer) {
  ...
  case 0:
    ...
    break;
  case 1: {
    LPPMSG_NPCDBLIST pNPCInfo =
        g_SenatusInfo.GetNPCInfo(Data->iNpcNumber, Data->iNpcIndex);
  ...
  case 3:
    ...
    break;
  case 4:
    g_pChatListBox->AddText("", GlobalText[1521],
                            SEASON3B::TYPE_SYSTEM_MESSAGE);
    break;
  case 5:
    g_pChatListBox->AddText("", GlobalText[1522],
                            SEASON3B::TYPE_SYSTEM_MESSAGE);
    break;
  case 6:
    g_pChatListBox->AddText("", GlobalText[1523],
                            SEASON3B::TYPE_SYSTEM_MESSAGE);
    break;
  case 7:
    g_pChatListBox->AddText("", GlobalText[1524],
                            SEASON3B::TYPE_SYSTEM_MESSAGE);
    break;
  }
}
...
void ReceiveInfoCastleNpc(BYTE *ReceiveBuffer) {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    g_ConsoleDebug->Write(5,
                          "[iNpcUpMaxHP1: %d][iNpcUpMaxHP2: %d][iNpcDefense1: "
                          "%d][iNpcDefense2: %d][iNpcUpRecLevel: %d]",
                          Data->iNpcUpMaxHP1[i], Data->iNpcUpMaxHP2[i],
                          Data->iNpcDefense1[i], Data->iNpcDefense2[i],
                          Data->iNpcUpRecLevel[i]);
  }
}
...
void ReceiveBCGetTaxInfo(BYTE *ReceiveBuffer) {
  ...
  switch (Data->btResult) {
  case 0:
    g_pChatListBox->AddText("", GlobalText[1525],
                            SEASON3B::TYPE_SYSTEM_MESSAGE);
    break;
  case 1:
...
void ReceiveBCChangeTaxRate(BYTE *ReceiveBuffer) {
  ...
  switch (Data->btResult) {
  case 0:
    g_pChatListBox->AddText("", GlobalText[1526],
                            SEASON3B::TYPE_SYSTEM_MESSAGE);
    break;
  case 1:
    if (Data->btTaxType == 3) {
      g_pUIGateKeeper->SetEntranceFee(
          (Data->btTaxRate1 << 24) | (Data->btTaxRate2 << 16) |
...
void ReceiveBCWithdraw(BYTE *ReceiveBuffer) {
  ...
  switch (Data->btResult) {
  case 0:
    g_pChatListBox->AddText("", GlobalText[1527],
                            SEASON3B::TYPE_SYSTEM_MESSAGE);
    break;
  case 1:
...
void ReceiveBCDeclareGuildList(BYTE *ReceiveBuffer) {
  ...
  case 1: {
    ...
    for (int i = 0; i < Data->iCount; ++i) {
      LPPMSG_CSREGGUILDLIST pData2 =
          (LPPMSG_CSREGGUILDLIST)(ReceiveBuffer + Offset);
      ...
      *pMarkCount++ = pData2->btRegMarks1;
      ...
      g_pGuardWindow->AddDeclareGuildList(pData2->szGuildName, dwMarkCount,
                                          pData2->btIsGiveUp, pData2->btSeqNum);
...
void ReceiveBCGuildList(BYTE *ReceiveBuffer) {
  ...
  case 1: {
    ...
    for (int i = 0; i < Data->iCount; ++i) {
      LPPMSG_CSATTKGUILDLIST pData2 =
          (LPPMSG_CSATTKGUILDLIST)(ReceiveBuffer + Offset);

      g_pGuardWindow->AddGuildList(pData2->szGuildName, pData2->btCsJoinSide,
                                   pData2->btGuildInvolved,
                                   pData2->iGuildScore);
  ...
  case 2:
    g_pChatListBox->AddText("", GlobalText[1609],
                            SEASON3B::TYPE_SYSTEM_MESSAGE);
    break;
  case 3:
    g_pChatListBox->AddText("", GlobalText[1609],
...
void ReceiveGateCurrentState(BYTE *ReceiveBuffer) {
  LPPRECEIVE_GATE_CURRENT_STATE Data =
      (LPPRECEIVE_GATE_CURRENT_STATE)ReceiveBuffer;
  int Key = ((int)(Data->m_byKeyH) << 8) + Data->m_byKeyL;
  int Index = FindCharacterIndex(Key);
...
void ReceiveCrownSwitchState(BYTE *ReceiveBuffer) {
  ...
  case 0: {
    ...
    SEASON3B::CreateMessageBox(
        MSGBOX_LAYOUT_CLASS(SEASON3B::CCrownSwitchPopLayout));
  } break;

  case 1:
    SEASON3B::CreateMessageBox(
        MSGBOX_LAYOUT_CLASS(SEASON3B::CCrownSwitchPushLayout));
    break;
  ...
  case 2: {
    ...
    if (pCha) {
      ...
      SEASON3B::CreateMessageBox(
          MSGBOX_LAYOUT_CLASS(SEASON3B::CCrownSwitchOtherPushLayout), &pMsgBox);
      if (pMsgBox) {
...
void ReceiveCrownRegist(BYTE *ReceiveBuffer) {
  ...
  case 0: {
    SEASON3B::CProgressMsgBox *pMsgBox = NULL;
    SEASON3B::CreateMessageBox(
        MSGBOX_LAYOUT_CLASS(SEASON3B::CSealRegisterStartLayout), &pMsgBox);
    if (pMsgBox) {
      unicode::t_char strText[256];
  ...
  case 1:
    SEASON3B::CreateMessageBox(
        MSGBOX_LAYOUT_CLASS(SEASON3B::CSealRegisterSuccessLayout));
    break;

  case 2: {
    SEASON3B::CProgressMsgBox *pMsgBox = NULL;
    SEASON3B::CreateMessageBox(
        MSGBOX_LAYOUT_CLASS(SEASON3B::CSealRegisterFailLayout), &pMsgBox);
    if (pMsgBox) {
      unicode::t_char strText[256];
      int iTime = (pData->m_dwCrownAccessTime / 1000);
      if (iTime >= 59)
  ...
  case 3: {
    SEASON3B::CreateMessageBox(
        MSGBOX_LAYOUT_CLASS(SEASON3B::CSealRegisterOtherLayout));
  } break;
  case 4:
    SEASON3B::CreateMessageBox(
        MSGBOX_LAYOUT_CLASS(SEASON3B::CSealRegisterOtherCampLayout));
    break;
  }
}
...
void ReceiveCrownState(BYTE *ReceiveBuffer) {
  ...
  case 0: {
    SEASON3B::CreateMessageBox(
        MSGBOX_LAYOUT_CLASS(SEASON3B::CCrownDefenseRemoveLayout));
  ...
  case 1: {
    SEASON3B::CreateMessageBox(
        MSGBOX_LAYOUT_CLASS(SEASON3B::CCrownDefenseCreateLayout));
...
void ReceiveBattleCasleSwitchInfo(BYTE *ReceiveBuffer) {
  LPRECEIVE_CROWN_SWITCH_INFO pData =
      (LPRECEIVE_CROWN_SWITCH_INFO)ReceiveBuffer;
  Check_Switch(pData);
}
...
void ReceiveBuildTime(BYTE *ReceiveBuffer) {
  LPPRECEIVE_MONSTER_BUILD_TIME pData =
      (LPPRECEIVE_MONSTER_BUILD_TIME)ReceiveBuffer;
...
void ReceiveCastleHuntZoneInfo(BYTE *RecevieBuffer) {
  LPPRECEIVE_CASTLE_HUNTZONE_INFO pData =
      (LPPRECEIVE_CASTLE_HUNTZONE_INFO)RecevieBuffer;

  if (pData->m_byResult == 0) {
    SEASON3B::CreateMessageBox(
        MSGBOX_LAYOUT_CLASS(SEASON3B::CGatemanFailMsgBoxLayout));
  } else {
    g_pUIGateKeeper->SetInfo(pData->m_byResult, (bool)pData->m_byEnable,
                             pData->m_iCurrPrice, pData->m_iUnitPrice,
                             pData->m_iMaxPrice);
...
void ReceiveCastleHuntZoneResult(BYTE *ReceiveBuffer) {
  LPPRECEVIE_CASTLE_HUNTZONE_RESULT pData =
      (LPPRECEVIE_CASTLE_HUNTZONE_RESULT)ReceiveBuffer;
...
void ReceiveCatapultState(BYTE *ReceiveBuffer) {
  ...
  if (pData->m_byResult == 1) {
    ...
    g_pChatListBox->AddText("", "ReceiveCatapultState",
                            SEASON3B::TYPE_SYSTEM_MESSAGE);
  }
}
...
void ReceiveCatapultFire(BYTE *ReceiveBuffer) {
  ...
  if (pData->m_byResult) {
    ...
    g_pCatapultWindow->DoFire(Key, pData->m_byResult, pData->m_byWeaponType,
                              pData->m_byTargetX, pData->m_byTargetY);
  } else if (pData->m_byResult == 0) {
    g_pChatListBox->AddText("", "ReceiveCatapultFire",
                            SEASON3B::TYPE_SYSTEM_MESSAGE);
  }
}
...
void ReceiveCatapultFireToMe(BYTE *ReceiveBuffer) {
  ...
  g_pCatapultWindow->DoFireFixStartPosition(
      pData->m_byWeaponType, pData->m_byTargetX, pData->m_byTargetY);
}
...
void ReceivePreviewPort(BYTE *ReceiveBuffer) {
  ...
  for (int i = 0; i < pData->Value; i++) {
    LPPRECEIVE_PREVIEW_PORT pData2 =
        (LPPRECEIVE_PREVIEW_PORT)(ReceiveBuffer + Offset);
    WORD Key = ((WORD)(pData2->m_byKeyH) << 8) + pData2->m_byKeyL;
    Key &= 0x7FFF;

    switch (pData2->m_byObjType) {
    case 1: {
      CHARACTER *c = CreateCharacter(Key, MODEL_PLAYER, pData2->m_byPosX,
                                     pData2->m_byPosY, 0);
      OBJECT *o = &c->Object;
    ...
    case 3: //   NPC
    {
      ...
      CHARACTER *c =
          CreateMonster(Type, pData2->m_byPosX, pData2->m_byPosY, Key);
      if (c == NULL)
        break;
      OBJECT *o = &c->Object;
      ...
      if (gMapManager->InBattleCastle() && c->MonsterIndex == 277) {
        ...
        if (g_isCharacterBuff((&c->Object), eBuff_CastleGateIsOpen)) {
          ...
          battleCastle::SetCastleGate_Attribute(pData2->m_byPosX,
                                                pData2->m_byPosY, 0);
    ...
    Offset += (sizeof(PRECEIVE_PREVIEW_PORT) -
...
void ReceiveGuildCommand(BYTE *ReceiveBuffer) {
  ...
  GuildCommander GCmd = {pData->m_byTeam, pData->m_byX, pData->m_byY,
                         pData->m_byCmd};
...
void ReceiveGuildMemberLocation(BYTE *ReceiveBuffer) {
  ...
  for (int i = 0; i < pData->Value; i++) {
    LPPRECEIVE_MEMBER_LOCATION pData2 =
        (LPPRECEIVE_MEMBER_LOCATION)(ReceiveBuffer + Offset);
...
void ReceiveGuildNpcLocation(BYTE *ReceiveBuffer) {
  ...
  for (int i = 0; i < pData->Value; i++) {
    LPPRECEIVE_NPC_LOCATION pData2 =
        (LPPRECEIVE_NPC_LOCATION)(ReceiveBuffer + Offset);
    g_pSiegeWarfare->SetGuildMemberLocation(pData2->m_byType + 1, pData2->m_byX,
                                            pData2->m_byY);
...
void ReceiveCrywolfInfo(BYTE *ReceiveBuffer) {
  ...
  M34CryWolf1st::CheckCryWolf1stMVP(pData->btOccupationState,
...
void ReceiveCrywolStateAltarfInfo(BYTE *ReceiveBuffer) {
  LPPMSG_ANS_CRYWOLF_STATE_ALTAR_INFO pData =
      (LPPMSG_ANS_CRYWOLF_STATE_ALTAR_INFO)ReceiveBuffer;

  M34CryWolf1st::CheckCryWolf1stMVPAltarfInfo(
      pData->iCrywolfStatueHP, pData->btAltarState1, pData->btAltarState2,
      pData->btAltarState3, pData->btAltarState4, pData->btAltarState5);
}

void ReceiveCrywolfAltarContract(BYTE *ReceiveBuffer) {
  LPPPMSG_ANS_CRYWOLF_ALTAR_CONTRACT pData =
      (LPPPMSG_ANS_CRYWOLF_ALTAR_CONTRACT)ReceiveBuffer;
...
void ReceiveCrywolfLifeTime(BYTE *ReceiveBuffer) {
  LPPPMSG_ANS_CRYWOLF_LEFTTIME pData =
      (LPPPMSG_ANS_CRYWOLF_LEFTTIME)ReceiveBuffer;
...
void ReceiveCrywolfTankerHit(BYTE *ReceiveBuffer) {
  LPPMSG_NOTIFY_REGION_MONSTER_ATTACK pData =
      (LPPMSG_NOTIFY_REGION_MONSTER_ATTACK)ReceiveBuffer;

  M34CryWolf1st::DoTankerFireFixStartPosition(
      pData->btSourceX, pData->btSourceY, pData->btPointX, pData->btPointY);
}

void ReceiveCrywolfBenefitPlusChaosRate(BYTE *ReceiveBuffer) {
  LPPMSG_ANS_CRYWOLF_BENEFIT_PLUS_CHAOSRATE pData =
      (LPPMSG_ANS_CRYWOLF_BENEFIT_PLUS_CHAOSRATE)ReceiveBuffer;

  g_MixRecipeMgr.SetPlusChaosRate(pData->btPlusChaosRate);
}

void ReceiveCrywolfBossMonsterInfo(BYTE *ReceiveBuffer) {
  LPPMSG_ANS_CRYWOLF_BOSSMONSTER_INFO pData =
      (LPPMSG_ANS_CRYWOLF_BOSSMONSTER_INFO)ReceiveBuffer;

  M34CryWolf1st::Set_BossMonster(pData->btBossHP, pData->btMonster2);
}

void ReceiveCrywolfPersonalRank(BYTE *ReceiveBuffer) {
  LPPMSG_ANS_CRYWOLF_PERSONAL_RANK pData =
      (LPPMSG_ANS_CRYWOLF_PERSONAL_RANK)ReceiveBuffer;

  M34CryWolf1st::Set_MyRank(pData->btRank, pData->iGettingExp);
}

void ReceiveCrywolfHeroList(BYTE *ReceiveBuffer) {
  LPPMSG_ANS_CRYWOLF_HERO_LIST_INFO_COUNT pData =
      (LPPMSG_ANS_CRYWOLF_HERO_LIST_INFO_COUNT)ReceiveBuffer;
  int Offset = sizeof(PMSG_ANS_CRYWOLF_HERO_LIST_INFO_COUNT);
  for (int i = 0; i < pData->btCount; i++) {
    LPPMSG_ANS_CRYWOLF_HERO_LIST_INFO pData2 =
        (LPPMSG_ANS_CRYWOLF_HERO_LIST_INFO)(ReceiveBuffer + Offset);
    Offset += sizeof(PMSG_ANS_CRYWOLF_HERO_LIST_INFO);
    M34CryWolf1st::Set_WorldRank(pData2->iRank, pData2->btHeroClass,
                                 pData2->iHeroScore, pData2->szHeroName);
  }
}

void ReceiveKanturu3rdStateInfo(BYTE *ReceiveBuffer) {
  LPPMSG_ANS_KANTURU_STATE_INFO pData =
      (LPPMSG_ANS_KANTURU_STATE_INFO)ReceiveBuffer;

  g_pKanturu2ndEnterNpc->ReceiveKanturu3rdInfo(
      pData->btState, pData->btDetailState, pData->btEnter, pData->btUserCount,
      pData->iRemainTime);
}

void ReceiveKanturu3rdEnterBossMap(BYTE *ReceiveBuffer) {
  LPPMSG_ANS_ENTER_KANTURU_BOSS_MAP pData =
      (LPPMSG_ANS_ENTER_KANTURU_BOSS_MAP)ReceiveBuffer;
  g_pKanturu2ndEnterNpc->ReceiveKanturu3rdEnter(pData->btResult);
}

void ReceiveKanturu3rdCurrentState(BYTE *ReceiveBuffer) {
  LPPMSG_ANS_KANTURU_CURRENT_STATE pData =
      (LPPMSG_ANS_KANTURU_CURRENT_STATE)ReceiveBuffer;
  M39Kanturu3rd::Kanturu3rdState(pData->btCurrentState,
                                 pData->btCurrentDetailState);
  M39Kanturu3rd::CheckSuccessBattle(pData->btCurrentState,
                                    pData->btCurrentDetailState);
}

void ReceiveKanturu3rdState(BYTE *ReceiveBuffer) {
  LPPMSG_ANS_KANTURU_STATE_CHANGE pData =
      (LPPMSG_ANS_KANTURU_STATE_CHANGE)ReceiveBuffer;
...
void ReceiveKanturu3rdResult(BYTE *ReceiveBuffer) {
  LPPMSG_ANS_KANTURU_BATTLE_RESULT pData =
      (LPPMSG_ANS_KANTURU_BATTLE_RESULT)ReceiveBuffer;
  M39Kanturu3rd::Kanturu3rdResult(pData->btResult);
}

void ReceiveKanturu3rdTimer(BYTE *ReceiveBuffer) {
  LPPMSG_ANS_KANTURU_BATTLE_SCENE_TIMELIMIT pData =
      (LPPMSG_ANS_KANTURU_BATTLE_SCENE_TIMELIMIT)ReceiveBuffer;
  g_pKanturuInfoWindow->SetTime(pData->btTimeLimit);
}

void RecevieKanturu3rdMayaSKill(BYTE *ReceiveBuffer) {
  LPPMSG_NOTIFY_KANTURU_WIDE_AREA_ATTACK pData =
      (LPPMSG_NOTIFY_KANTURU_WIDE_AREA_ATTACK)ReceiveBuffer;
  M39Kanturu3rd::MayaSceneMayaAction(pData->btType);
}

void RecevieKanturu3rdLeftUserandMonsterCount(BYTE *ReceiveBuffer) {
  LPPMSG_NOTIFY_KANTURU_USER_MONSTER_COUNT pData =
      (LPPMSG_NOTIFY_KANTURU_USER_MONSTER_COUNT)ReceiveBuffer;
  M39Kanturu3rd::Kanturu3rdUserandMonsterCount(pData->bMonsterCount,
                                               pData->btUserCount);
}

void ReceiveCursedTempleEnterInfo(BYTE *ReceiveBuffer) {
  LPPMSG_CURSED_TEMPLE_USER_COUNT data =
      (LPPMSG_CURSED_TEMPLE_USER_COUNT)ReceiveBuffer;
...
void ReceiveCursedTempleEnterResult(BYTE *ReceiveBuffer) {
  LPPMSG_RESULT_ENTER_CURSED_TEMPLE data =
      (LPPMSG_RESULT_ENTER_CURSED_TEMPLE)ReceiveBuffer;
...
void ReceiveCursedTempleState(BYTE *ReceiveBuffer) {
  LPPMSG_ILLUSION_TEMPLE_STATE data =
      (LPPMSG_ILLUSION_TEMPLE_STATE)ReceiveBuffer;

  // 0: wait, 1: wait->ready, 2: ready->play, 3: play->end
  SEASON3A::eCursedTempleState cursedtemple =
      static_cast<SEASON3A::eCursedTempleState>(data->btIllusionTempleState +
                                                1);
...
void ReceiveRaklionStateInfo(BYTE *ReceiveBuffer) {
  LPPMSG_ANS_RAKLION_STATE_INFO pData =
      (LPPMSG_ANS_RAKLION_STATE_INFO)ReceiveBuffer;
  g_Raklion.SetState(pData->btState, pData->btDetailState);
}

void ReceiveRaklionCurrentState(BYTE *ReceiveBuffer) {
  LPPMSG_ANS_RAKLION_CURRENT_STATE pData =
      (LPPMSG_ANS_RAKLION_CURRENT_STATE)ReceiveBuffer;
  g_Raklion.SetState(pData->btCurrentState, pData->btCurrentDetailState);
}

void RecevieRaklionStateChange(BYTE *ReceiveBuffer) {
  LPPMSG_ANS_RAKLION_STATE_CHANGE pData =
      (LPPMSG_ANS_RAKLION_STATE_CHANGE)ReceiveBuffer;
  g_Raklion.SetState(pData->btState, pData->btDetailState);
}

void RecevieRaklionBattleResult(BYTE *ReceiveBuffer) {
  LPPMSG_ANS_RAKLION_BATTLE_RESULT pData =
      (LPPMSG_ANS_RAKLION_BATTLE_RESULT)ReceiveBuffer;
}

void RecevieRaklionWideAreaAttack(BYTE *ReceiveBuffer) {
  LPPMSG_NOTIFY_RAKLION_WIDE_AREA_ATTACK pData =
      (LPPMSG_NOTIFY_RAKLION_WIDE_AREA_ATTACK)ReceiveBuffer;
}

void RecevieRaklionUserMonsterCount(BYTE *ReceiveBuffer) {
  LPPMSG_NOTIFY_RAKLION_USER_MONSTER_COUNT pData =
      (LPPMSG_NOTIFY_RAKLION_USER_MONSTER_COUNT)ReceiveBuffer;
}
...
void TranslateChattingProtocol(DWORD dwWindowUIID, int HeadCode,
                               BYTE *ReceiveBuffer, int Size, BOOL bEcrypted);
    ...
    if (ReceiveBuffer == NULL) {
      ...
      if (ReceiveBuffer[0] == 0xC1) {
        ...
        if (ReceiveBuffer[0] == 0xC3) {
          ...
          Size = lpHeader->Size;
          iSize =
              g_SimpleModulusSC.Decrypt(byDec + 2, ReceiveBuffer + 2, Size - 2);
        } else {
          LPWBMSG_ENCRYPTED lpHeader = (LPWBMSG_ENCRYPTED)ReceiveBuffer;
          Size = ((int)(lpHeader->SizeH) << 8) + lpHeader->SizeL;
          iSize =
              g_SimpleModulusSC.Decrypt(byDec + 2, ReceiveBuffer + 3, Size - 3);
        }
        bEncrypted = TRUE;
        ...
        if ((g_byPacketSerialRecv) != byDec[2]) {
          ...
          g_ErrorReport.Write(
              "Decrypt error : g_byPacketSerialRecv(0x%02X), byDec(0x%02X)\r\n",
              g_byPacketSerialRecv, byDec[2]);
      ...
      DebugAngel_Write(PACKET_SAVE_FILE, "%s [%d.%d.%d.%d] Receive ",
                       timeString.c_str(),
                       (UCHAR)sockaddr.sin_addr.S_un.S_un_b.s_b1,
                       (UCHAR)sockaddr.sin_addr.S_un.S_un_b.s_b2,
                       (UCHAR)sockaddr.sin_addr.S_un.S_un_b.s_b3,
                       (UCHAR)sockaddr.sin_addr.S_un.S_un_b.s_b4);
      for (int i = 0; i < 6; i++) {
      ...
      switch (iTranslation) {
      case 1:
        TranslateChattingProtocol(iParam, HeadCode, ReceiveBuffer, Size,
                                  bEncrypted);
        break;
      default:
...
bool ReceiveRegistLuckyCoin(BYTE *ReceiveBuffer) {
  ...
  case 0:
    SEASON3B::CreateMessageBox(
        MSGBOX_LAYOUT_CLASS(SEASON3B::CUseRegistLuckyCoinMsgBoxLayout));
    break;
  case 1:
  ...
  case 100:
    SEASON3B::CreateMessageBox(
        MSGBOX_LAYOUT_CLASS(SEASON3B::CRegistOverLuckyCoinMsgBoxLayout));
    break;
  default:
    return false;
  }
...
bool ReceiveRequestExChangeLuckyCoin(BYTE *ReceiveBuffer) {
  ...
  case 0: {
    SEASON3B::CreateMessageBox(
        MSGBOX_LAYOUT_CLASS(SEASON3B::CExchangeLuckyCoinMsgBoxLayout));
  } break;
  case 1: {
    // g_pNewUISystem->Hide(SEASON3B::INTERFACE_EXCHANGE_LUCKYCOIN);
    g_pChatListBox->AddText(
        "", GlobalText[1888],
        SEASON3B::TYPE_SYSTEM_MESSAGE); // Trao đổi đã được hoàn thành
  } break;
  case 2: {
    SEASON3B::CreateMessageBox(
        MSGBOX_LAYOUT_CLASS(SEASON3B::CExchangeLuckyCoinInvenErrMsgBoxLayout));
  } break;
  default:
    return false;
  }
...
bool ReceiveEnterDoppelGangerEvent(BYTE *ReceiveBuffer) {
  LPPMSG_RESULT_ENTER_DOPPELGANGER Data =
      (LPPMSG_RESULT_ENTER_DOPPELGANGER)ReceiveBuffer;
...
bool ReceiveDoppelGangerMonsterPosition(BYTE *ReceiveBuffer) {
  LPPMSG_DOPPELGANGER_MONSTER_POSITION Data =
      (LPPMSG_DOPPELGANGER_MONSTER_POSITION)ReceiveBuffer;
...
bool ReceiveDoppelGangerState(BYTE *ReceiveBuffer) {
  ...
  case 2: // ready->play
  {
    ...
    SEASON3B::CNewUICommonMessageBox *pMsgBox;
    SEASON3B::CreateMessageBox(
        MSGBOX_LAYOUT_CLASS(SEASON3B::CDoppelGangerMsgBoxLayout), &pMsgBox);
    pMsgBox->AddMsg(GlobalText[2763], RGBA(255, 255, 255, 255),
                    SEASON3B::MSGBOX_FONT_NORMAL);
    pMsgBox->AddMsg(" ");
    pMsgBox->AddMsg(GlobalText[2764], RGBA(255, 255, 255, 255),
                    SEASON3B::MSGBOX_FONT_NORMAL);
    pMsgBox->AddMsg(" ");
    pMsgBox->AddMsg(GlobalText[2765], RGBA(255, 255, 255, 255),
                    SEASON3B::MSGBOX_FONT_NORMAL);
  } break;
  case 3: // play->end
    break;
...
bool ReceiveDoppelGangerIcewalkerState(BYTE *ReceiveBuffer) {
  LPPMSG_DOPPELGANGER_ICEWORKER_STATE Data =
      (LPPMSG_DOPPELGANGER_ICEWORKER_STATE)ReceiveBuffer;

  switch (Data->btIceworkerState) {
  case 0:
    g_pDoppelGangerFrame->SetIceWalkerMap(TRUE, (float)(22 - Data->btPosIndex) /
                                                    22.0f);
    break;
  case 1:
...
bool ReceiveDoppelGangerTimePartyState(BYTE *ReceiveBuffer) {
  LPPMSG_DOPPELGANGER_PLAY_INFO Data =
      (LPPMSG_DOPPELGANGER_PLAY_INFO)ReceiveBuffer;

  g_pDoppelGangerFrame->SetRemainTime(Data->wRemainSec);
  g_pDoppelGangerFrame->SetPartyMemberRcvd();
  LPPMSG_DOPPELGANGER_USER_POS pUserPos =
      (LPPMSG_DOPPELGANGER_USER_POS)&Data->UserPosData;
  for (int i = 0; i < Data->btUserCount; ++i) {
    g_pDoppelGangerFrame->SetPartyMemberInfo(
        pUserPos[i].wUserIndex, (float)(22 - pUserPos[i].btPosIndex) / 22.0f);
  }
...
bool ReceiveDoppelGangerResult(BYTE *ReceiveBuffer) {
  ...
  case 0: {
    ...
    SEASON3B::CreateMessageBox(
        MSGBOX_LAYOUT_CLASS(SEASON3B::CDoppelGangerMsgBoxLayout), &pMsgBox);
    pMsgBox->AddMsg(GlobalText[2769], RGBA(255, 255, 255, 255),
                    SEASON3B::MSGBOX_FONT_NORMAL);
    pMsgBox->AddMsg(" ");
    pMsgBox->AddMsg(GlobalText[2770], RGBA(255, 255, 255, 255),
                    SEASON3B::MSGBOX_FONT_NORMAL);
    // 			pMsgBox->AddMsg(" ");
    // 			pMsgBox->AddMsg(" ");
    // 			char szText[256] = { 0, };
    // 			sprintf(szText, GlobalText[2771], Data->dwRewardExp);
    // 			pMsgBox->AddMsg(szText, RGBA(255, 255, 255, 255),
  ...
  case 1: {
    ...
    SEASON3B::CreateMessageBox(
        MSGBOX_LAYOUT_CLASS(SEASON3B::CDoppelGangerMsgBoxLayout), &pMsgBox);
    pMsgBox->AddMsg(GlobalText[2766], RGBA(255, 255, 255, 255),
                    SEASON3B::MSGBOX_FONT_NORMAL);
  } break;
  case 2: {
    SEASON3B::CNewUICommonMessageBox *pMsgBox;
    SEASON3B::CreateMessageBox(
        MSGBOX_LAYOUT_CLASS(SEASON3B::CDoppelGangerMsgBoxLayout), &pMsgBox);
    pMsgBox->AddMsg(GlobalText[2767], RGBA(255, 255, 255, 255),
                    SEASON3B::MSGBOX_FONT_NORMAL);
    pMsgBox->AddMsg(" ");
    pMsgBox->AddMsg(GlobalText[2768], RGBA(255, 255, 255, 255),
                    SEASON3B::MSGBOX_FONT_NORMAL);
  } break;
  }
...
bool ReceiveDoppelGangerMonsterGoal(BYTE *ReceiveBuffer) {
  LPPMSG_DOPPELGANGER_MONSTER_GOAL Data =
      (LPPMSG_DOPPELGANGER_MONSTER_GOAL)ReceiveBuffer;
...
bool ReceiveEnterEmpireGuardianEvent(BYTE *ReceiveBuffer) {
  LPPMSG_RESULT_ENTER_EMPIREGUARDIAN Data =
      (LPPMSG_RESULT_ENTER_EMPIREGUARDIAN)ReceiveBuffer;
  ...
  case 1: {
    SEASON3B::CNewUICommonMessageBox *pMsgBox;
    SEASON3B::CreateMessageBox(
        MSGBOX_LAYOUT_CLASS(SEASON3B::CEmpireGuardianMsgBoxLayout), &pMsgBox);
    pMsgBox->AddMsg(GlobalText[2798], RGBA(255, 255, 255, 255),
                    SEASON3B::MSGBOX_FONT_NORMAL);
    pMsgBox->AddMsg(" ");
    unicode::t_char szText[256] = {
        NULL,
  ...
  case 2: {
    ...
    SEASON3B::CreateMessageBox(
        MSGBOX_LAYOUT_CLASS(SEASON3B::CEmpireGuardianMsgBoxLayout), &pMsgBox);
    pMsgBox->AddMsg(GlobalText[2839], RGBA(255, 255, 255, 255),
                    SEASON3B::MSGBOX_FONT_NORMAL);
  } break;
  case 3: {
    SEASON3B::CNewUICommonMessageBox *pMsgBox;
    SEASON3B::CreateMessageBox(
        MSGBOX_LAYOUT_CLASS(SEASON3B::CEmpireGuardianMsgBoxLayout), &pMsgBox);
    pMsgBox->AddMsg(GlobalText[2841], RGBA(255, 255, 255, 255),
                    SEASON3B::MSGBOX_FONT_NORMAL);
  } break;
  case 4: {
    SEASON3B::CNewUICommonMessageBox *pMsgBox;
    SEASON3B::CreateMessageBox(
        MSGBOX_LAYOUT_CLASS(SEASON3B::CEmpireGuardianMsgBoxLayout), &pMsgBox);
    pMsgBox->AddMsg(GlobalText[2842], RGBA(255, 255, 255, 255),
                    SEASON3B::MSGBOX_FONT_NORMAL);
  } break;
  case 5: {
    SEASON3B::CNewUICommonMessageBox *pMsgBox;
    SEASON3B::CreateMessageBox(
        MSGBOX_LAYOUT_CLASS(SEASON3B::CEmpireGuardianMsgBoxLayout), &pMsgBox);
    pMsgBox->AddMsg(GlobalText[2843], RGBA(255, 255, 255, 255),
                    SEASON3B::MSGBOX_FONT_NORMAL);
  } break;
...
bool ReceiveRemainTickEmpireGuardian(BYTE *ReceiveBuffer) {
  LPPMSG_REMAINTICK_EMPIREGUARDIAN Data =
      (LPPMSG_REMAINTICK_EMPIREGUARDIAN)ReceiveBuffer;

  if (g_pNewUISystem->IsVisible(SEASON3B::INTERFACE_EMPIREGUARDIAN_TIMER) ==
      false) {
...
bool ReceiveResultEmpireGuardian(BYTE *ReceiveBuffer) {
  LPPMSG_CLEAR_RESULT_EMPIREGUARDIAN Data =
      (LPPMSG_CLEAR_RESULT_EMPIREGUARDIAN)ReceiveBuffer;
  ...
  case 0: {
    ...
    SEASON3B::CreateMessageBox(
        MSGBOX_LAYOUT_CLASS(SEASON3B::CEmpireGuardianMsgBoxLayout), &pMsgBox);
    pMsgBox->AddMsg(GlobalText[2803], RGBA(255, 255, 255, 255),
                    SEASON3B::MSGBOX_FONT_NORMAL);
    pMsgBox->AddMsg(GlobalText[2804], RGBA(255, 255, 255, 255),
                    SEASON3B::MSGBOX_FONT_NORMAL);
  } break;
  case 1: {
  ...
  case 2: {
    ...
    pMsgBox->AddMsg(szText, RGBA(255, 255, 255, 255),
                    SEASON3B::MSGBOX_FONT_NORMAL);
    pMsgBox->AddMsg(GlobalText[2802], RGBA(255, 255, 255, 255),
                    SEASON3B::MSGBOX_FONT_NORMAL);
...
bool ReceiveIGS_CashPoint(BYTE *pReceiveBuffer) {
  LPPMSG_CASHSHOP_CASHPOINT_ANS Data =
      (LPPMSG_CASHSHOP_CASHPOINT_ANS)pReceiveBuffer;
...
bool ReceiveIGS_ShopOpenResult(BYTE *pReceiveBuffer) {
  LPPMSG_CASHSHOP_SHOPOPEN_ANS Data =
      (LPPMSG_CASHSHOP_SHOPOPEN_ANS)pReceiveBuffer;
...
bool ReceiveIGS_BuyItem(BYTE *pReceiveBuffer) {
  LPPMSG_CASHSHOP_BUYITEM_ANS Data =
      (LPPMSG_CASHSHOP_BUYITEM_ANS)pReceiveBuffer;
...
bool ReceiveIGS_SendItemGift(BYTE *pReceiveBuffer) {
  LPPMSG_CASHSHOP_GIFTSEND_ANS Data =
      (LPPMSG_CASHSHOP_GIFTSEND_ANS)pReceiveBuffer;
...
bool ReceiveIGS_StorageItemListCount(BYTE *pReceiveBuffer) {
  LPPMSG_CASHSHOP_STORAGECOUNT Data =
      (LPPMSG_CASHSHOP_STORAGECOUNT)pReceiveBuffer;
  g_pInGameShop->InitStorage((int)Data->wTotalItemCount,
                             (int)Data->wCurrentItemCount,
...
bool ReceiveIGS_StorageItemList(BYTE *pReceiveBuffer) {
  LPPMSG_CASHSHOP_STORAGELIST Data =
      (LPPMSG_CASHSHOP_STORAGELIST)pReceiveBuffer;
  ...
  g_pInGameShop->AddStorageItem((int)Data->lStorageIndex, (int)Data->lItemSeq,
                                (int)Data->lStorageGroupCode,
                                (int)Data->lProductSeq, (int)Data->lPriceSeq,
                                (int)Data->dCashPoint, (char)Data->chItemType);
  return true;
}

// (0xD2)(0x0E)
bool ReceiveIGS_StorageGiftItemList(BYTE *pReceiveBuffer) {
  LPPMSG_CASHSHOP_GIFTSTORAGELIST Data =
      (LPPMSG_CASHSHOP_GIFTSTORAGELIST)pReceiveBuffer;
...
bool ReceiveIGS_SendCashGift(BYTE *pReceiveBuffer) {
  LPPMSG_CASHSHOP_CASHSEND_ANS Data =
      (LPPMSG_CASHSHOP_CASHSEND_ANS)pReceiveBuffer;
  return true;
}
...
bool ReceiveIGS_PossibleBuy(BYTE *pReceiveBuffer) {
  LPPMSG_CASHSHOP_ITEMBUY_CONFIRM_ANS Data =
      (LPPMSG_CASHSHOP_ITEMBUY_CONFIRM_ANS)pReceiveBuffer;
  return true;
}

// (0xD2)(0x09)
bool ReceiveIGS_LeftCountItem(BYTE *pReceiveBuffer) {
  LPPMSG_CASHSHOP_ITEMBUY_LEFT_COUNT_ANS Data =
      (LPPMSG_CASHSHOP_ITEMBUY_LEFT_COUNT_ANS)pReceiveBuffer;
  return true;
}

// (0xD2)(0x0B)
bool ReceiveIGS_UseStorageItem(BYTE *pReceiveBuffer) {
  LPPMSG_CASHSHOP_STORAGE_ITEM_USE_ANS Data =
      (LPPMSG_CASHSHOP_STORAGE_ITEM_USE_ANS)pReceiveBuffer;
...
bool ReceiveIGS_UpdateScript(BYTE *pReceiveBuffer) {
  LPPMSG_CASHSHOP_VERSION_UPDATE Data =
      (LPPMSG_CASHSHOP_VERSION_UPDATE)pReceiveBuffer;

#ifdef KJH_MOD_SHOP_SCRIPT_DOWNLOAD
  g_InGameShopSystem->SetScriptVersion(Data->wSaleZone, Data->wYear,
                                       Data->wYearIdentify);
...
bool ReceiveIGS_EventItemlistCnt(BYTE *pReceiveBuffer) {
  LPPMSG_CASHSHOP_EVENTITEM_COUNT Data =
      (LPPMSG_CASHSHOP_EVENTITEM_COUNT)pReceiveBuffer;
...
bool ReceiveIGS_EventItemlist(BYTE *pReceiveBuffer) {
  LPPMSG_CASHSHOP_EVENTITEM_LIST Data =
      (LPPMSG_CASHSHOP_EVENTITEM_LIST)pReceiveBuffer;
...
bool ReceiveIGS_UpdateBanner(BYTE *pReceiveBuffer) {
  ...
  LPPMSG_CASHSHOP_BANNER_UPDATE Data =
      (LPPMSG_CASHSHOP_BANNER_UPDATE)pReceiveBuffer;
#endif // KWAK_FIX_COMPILE_LEVEL4_WARNING_EX

#ifdef KJH_MOD_SHOP_SCRIPT_DOWNLOAD
  g_InGameShopSystem->SetBannerVersion(Data->wBannerZone, Data->wYear,
                                       Data->wYearIdentify);
#else // KJH_MOD_SHOP_SCRIPT_DOWNLOAD
  if (g_InGameShopSystem->IsShopOpen() == false)
    return false;
...
bool ReceiveEquippingInventoryItem(BYTE *pReceiveBuffer) {
  LPPMSG_ANS_INVENTORY_EQUIPMENT_ITEM Data =
      (LPPMSG_ANS_INVENTORY_EQUIPMENT_ITEM)pReceiveBuffer;
  ...
  g_ConsoleDebug->Write(MCD_RECEIVE,
                        "[0xBF][0x20]  [ReceiveEquippingInventoryItem]");
#endif // CONSOLE_DEBUG
...
bool ReceivePeriodItemListCount(BYTE *pReceiveBuffer) {
  LPPMSG_PERIODITEMEX_ITEMCOUNT Data =
      (LPPMSG_PERIODITEMEX_ITEMCOUNT)pReceiveBuffer;
  return true;
}

bool ReceivePeriodItemList(BYTE *pReceiveBuffer) {
  LPPMSG_PERIODITEMEX_ITEMLIST Data =
      (LPPMSG_PERIODITEMEX_ITEMLIST)pReceiveBuffer;

  if (Data->wItemSlotIndex < MAX_EQUIPMENT_INDEX) {
    CharacterMachine->Equipment[Data->wItemSlotIndex].lExpireTime =
        (long)Data->lExpireDate;
  } else {
    ITEM *pItem =
        g_pMyInventory->FindItem(Data->wItemSlotIndex - MAX_EQUIPMENT_INDEX);
...
BOOL ReceiveStraightAttack(BYTE *ReceiveBuffer, int Size, BOOL bEncrypted) {
  ...
  if (TargetIndex != MAX_CHARACTERS_CLIENT || AttackNumber == Skill_Dark_Side ||
      AttackNumber == Skill_Dark_Side_Strengthener) {
    ...
    if (sc != NULL && tc != NULL) {
      ...
      if (g_CMonkSystem.IsRageHalfwaySkillAni(AttackNumber)) {
        if (sc == Hero || (AttackNumber != Skill_Beast_Uppercut &&
                           AttackNumber != Skill_Beast_Uppercut_Strengthener &&
                           AttackNumber != Skill_Beast_Uppercut_Mastery)) {
          if (AttackNumber != Skill_Beast_Uppercut &&
              AttackNumber != Skill_Beast_Uppercut_Strengthener &&
              AttackNumber != Skill_Beast_Uppercut_Mastery) {
      ...
      if (AttackNumber == Skill_Dark_Side ||
          AttackNumber == Skill_Dark_Side_Strengthener) {
        if (sc != Hero)
          g_CMonkSystem.RageCreateEffect(so, AttackNumber);
        g_CMonkSystem.SetRageSkillAni(AttackNumber, so);
...
void ReceiveDarkside(BYTE *ReceiveBuffer) {
  ...
  if (Data->MagicNumber == Skill_Dark_Side ||
      Data->MagicNumber == Skill_Dark_Side_Strengthener) {
...
void ReceiveInventoryHoly(BYTE *ReceiveBuffer) {
  ...
  const int currentPage =
      (g_pInventoryJewel != NULL) ? g_pInventoryJewel->GetCurrentPage() : 0;
  ...
  for (size_t i = 0; i < Data->Value; i++) {
    ...
    // Supongamos que btData tiene 12 bytes y necesitas extraer bItemCount de
    // los primeros 6 bytes
    ...
    short bBundledIndex = *(short *)(btData + 6);

    g_pInventoryJewel->InsertData(btData[11], bItemindex, btData[10],
                                  bItemCount, bBundledIndex);

    offset += sizeof(PRECEIVE_INVENTORY_HOLY);
  }

  if (g_pInventoryJewel != NULL) {
    g_pInventoryJewel->SetCurrentPage(currentPage);
  }
}
...
void UpdateInventoryExtend(BYTE *ReceiveBuffer) {
  LPPMSG_CREATE_INVENTARIO_SEND Data =
      (LPPMSG_CREATE_INVENTARIO_SEND)ReceiveBuffer;
  //--ExtInventory
  g_pInvenExpansion->SetEnableExpansionInven(Data->ExtInventory,
                                             Data->PriceInventoryExt);
...
void settings_reset_manager(BYTE *ReceiveBuffer, BYTE type) {
  if (type == 0) {
    ...
    GMResetManager->SetResetInfo(Data->CurReset, Data->MaxReset, Data->MinLevel,
                                 Data->RewardPoint, Data->ReqMoney,
                                 Data->ItemIndex, Data->ItemCount);
  }
  if (type == 1) {
    LPPMSG_MASTER_RESET_TABLE_INFO Data =
        (LPPMSG_MASTER_RESET_TABLE_INFO)ReceiveBuffer;

    GMResetManager->SetMasterResetInfo(
        Data->CurMasterReset, Data->MaxMasterReset, Data->MinLevel,
        Data->MinReset, Data->RewardPoint, Data->ReqMoney, Data->ItemIndex,
        Data->ItemCount);
  }
}
...
void settings_invasion_kill(BYTE *ReceiveBuffer) {
  ...
  if (gmProtect->InvasionInfoTime) {
    GMInvasionManager->SetMonsterKill(Data->Index, Data->MonsterIndex,
                                      Data->Monster_Kill, Data->MonsterCount);
  }
}

void settings_invasion_manager(BYTE *ReceiveBuffer) {
  LPPMSG_INVASION_DEFAULT_INFO Data =
      (LPPMSG_INVASION_DEFAULT_INFO)ReceiveBuffer;
  ...
  if (gmProtect->InvasionInfoTime) {
    ...
    for (size_t i = 0; i < (size_t)Data->count; i++) {
      LP_INVASION_DEFAULT_MONSTER Data2 =
          (LP_INVASION_DEFAULT_MONSTER)(ReceiveBuffer + Offset);

      GMInvasionManager->SetMonsterCount(Data->Index, Data2->MonsterIndex,
                                         Data2->Monster_Kill,
                                         Data2->MonsterCount);
...
#include "Protocol.h"
BOOL TranslateProtocol(int HeadCode, BYTE *ReceiveBuffer, int Size,
                       BOOL bEncrypted) {
  ...
  case 0xF1: {
    ...
    g_ConsoleDebug->Write(MCD_RECEIVE, "Recv [0xF1][0x%02x][0x%02x]",
                          Data->SubCode, Data->Value);
    ...
    case 0x01: // receive log in
      ...
      case 0xd2:
        ...
        break;
      case RESULT_SUB_GM_ACCESS_DENIED:
        CUIMng::Instance().PopUpMsgWin(RECEIVE_LOG_IN_FAIL_SUB_GM);
        break;
      }
      break;
    case 0x02:
  ...
  case 0xF3: {
    ...
    case 0x10: // receive inventory
      if (!ReceiveInventory(ReceiveBuffer, bEncrypted, false)) {
        return (FALSE);
      }
      break;
    case 0x12: // receive inventory append
      if (!ReceiveInventory(ReceiveBuffer, bEncrypted, true)) {
        return (FALSE);
      }
      break;
    case 0x11:
...
#if CUSTOM_CHOTROI
    case 0x01:
      gCusChoTroi.SetShowItemCache(ReceiveBuffer);
      break;
    case 0x20:
      gCusChoTroi.GCSetListChoTroi(ReceiveBuffer, Size);
      break;
#endif
    case 0x4D:
      gCB_QuaPhucLoi->RecvClientButton(ReceiveBuffer);
      break;
    case 0x4E:
      gCB_QuaPhucLoi->RecvDataInfo(ReceiveBuffer);
      break;
    case 0x8A:
      gVongQuay.GetListVQ(ReceiveBuffer);
      break;
    case 0x8B:
      gVongQuay.RecvListItemVQ(ReceiveBuffer);
      break;
    case 0x8C:
      gVongQuay.GetInfoVQ(ReceiveBuffer);
      break;
    case 0x8D:
      gVongQuay.RecvTichLuyVQ(ReceiveBuffer);
      break;
    case 0x9A:
      gMocNap.GetListDonate(ReceiveBuffer);
      break;
    case 0x9B:
      gMocNap.RecvListPhanThuong(ReceiveBuffer);
      break;
    case 0x9D:
      gMocNap.RecvPaymentLink(ReceiveBuffer);
      break;
    }
    break;
  }
  ...
  if (o && o == &Hero->Object) {
    ...
    case eBuff_LuckOfSanta: {
      ...
      if (buff == eBuff_BlessingOfXmax) {
        g_pChatListBox->AddText("", GlobalText[2591],
                                SEASON3B::TYPE_SYSTEM_MESSAGE);
        CharacterMachine->CalculateDamage();
        CharacterMachine->CalculateDefense();
...
void InsertBuffPhysicalEffect(eBuffState buff, OBJECT *o) {
  ...
  case eBuff_CursedTempleProdection: {
    if (o->Type == MODEL_PLAYER) {
      ...
      CreateEffect(MODEL_CURSEDTEMPLE_PRODECTION_SKILL, o->Position, o->Angle,
                   Light, 0, o);
  ...
  case eBuff_PhysDefense: {
    if (o->Type == MODEL_PLAYER) {
      ...
      for (int j = 0; j < 4; ++j) {
        CreateJoint(MODEL_SPEARSKILL, o->Position, o->Position, o->Angle, 0, o,
                    50.0f);
      }
    }
  } break;
  ...
  case eBuff_Thorns: {
    ...
    CreateJoint(BITMAP_FLARE, o->Position, o->Position, o->Angle, 43, o, 50.f,
                0, 0, 0, 0, vLight);
  } break;
  case eDeBuff_Blind: {
    vec3_t vLight;
    Vector(1.0f, 1.0f, 1.0f, vLight);
    CreateEffect(MODEL_ALICE_BUFFSKILL_EFFECT, o->Position, o->Angle, vLight, 3,
                 o);
  } break;
  case eDeBuff_Sleep: {
    vec3_t vLight;
    Vector(0.7f, 0.1f, 0.9f, vLight);
    CreateEffect(MODEL_ALICE_BUFFSKILL_EFFECT, o->Position, o->Angle, vLight, 4,
                 o);
  } break;
  case eDeBuff_AttackDown: {
    vec3_t vLight;
    Vector(1.4f, 0.2f, 0.2f, vLight);
    CreateEffect(BITMAP_SHINY + 6, o->Position, o->Angle, vLight, 1, o, -1, 0,
                 0, 0, 0.5f);
    CreateEffect(BITMAP_PIN_LIGHT, o->Position, o->Angle, vLight, 1, o, -1, 0,
                 0, 0, 1.f);
  } break;
  ...
  case eDeBuff_DefenseDown: {
    ...
    CreateEffect(BITMAP_SHINY + 6, o->Position, o->Angle, vLight, 2, o, -1, 0,
                 0, 0, 0.5f);
    CreateEffect(BITMAP_PIN_LIGHT, o->Position, o->Angle, vLight, 2, o, -1, 0,
                 0, 0, 1.f);
  } break;
  ...
  case eBuff_Doppelganger_Ascension: {
    ...
    CreateEffect(MODEL_CHANGE_UP_CYLINDER, o->Position, o->Angle, o->Light, 2,
                 o);
  ...
  case eBuff_AG_Addition: {
    ...
    for (int i = 0; i < 5; ++i) {
      CreateParticle(BITMAP_AG_ADDITION_EFFECT, o->Position, o->Angle, o->Light,
                     0, 1.0f, o);
      CreateParticle(BITMAP_AG_ADDITION_EFFECT, o->Position, o->Angle, o->Light,
                     1, 1.0f, o);
      CreateParticle(BITMAP_AG_ADDITION_EFFECT, o->Position, o->Angle, o->Light,
  ...
  case eBuff_Att_up_Ourforces: {
    ...
    CreateEffect(BITMAP_LIGHT_RED, o->Position, o->Angle, o->Light, 1, o, -1, 0,
                 0, 0, 1.5f);
  } break;
  case eBuff_Hp_up_Ourforces: {
    DeleteEffect(BITMAP_LIGHT_RED, o, 0);
    CreateEffect(BITMAP_LIGHT_RED, o->Position, o->Angle, o->Light, 0, o, -1, 0,
                 0, 0, 1.5f);
  } break;
  case eBuff_Def_up_Ourforces: {
    DeleteEffect(BITMAP_LIGHT_RED, o, 2);
    CreateEffect(BITMAP_LIGHT_RED, o->Position, o->Angle, o->Light, 2, o, -1, 0,
                 0, 0, 1.5f);
  } break;
#endif // PBG_ADD_NEWCHAR_MONK_SKILL
  }
...
void RegisterBuff(eBuffState buff, OBJECT *o, const int bufftime, WORD power) {
  ...
  if (CheckExceptionBuff(buff, o, false)) {
    ...
    if (o == &Hero->Object && power > 0 && g_pBuffWindow) {
      g_pBuffWindow->SetBuffPower(buff, power);
    }
  }
}

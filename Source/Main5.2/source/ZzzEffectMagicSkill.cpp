...
namespace
{
	bool SupportsCustomBowProjectile(int effectType, WORD skillKey)
	{
		if (effectType < 0)
		{
			return false;
...
void CreateArrow(CHARACTER* c, OBJECT* o, OBJECT* to, WORD SkillIndex, WORD Skill, WORD SKKey)
{
	...
	{
		CUSTOM_BOW_INFO* customBowInfo = gCustomBow.GetInfoByItem(Weapon1_Type - MODEL_ITEM);

		if (customBowInfo != 0 && SupportsCustomBowProjectile(customBowInfo->SkillShot, SKKey))
		{
			CreateEffect(customBowInfo->SkillShot, o->Position, o->Angle, o->Light, SubType, o, o->PKKey, SkillIndex, Skill);
			return;
		}

...
bool SEASON3B::CUseFruitMsgBoxLayout::SetLayout()
{
	...
	if (pItem->Type == ITEM_HELPER + 15)
	{
		...
		{
		case 0:
			unicode::_sprintf(strName, "%s", GlobalText[168]); //Nang lu?ng
			break;
		case 1:
			unicode::_sprintf(strName, "%s", GlobalText[169]); //Th? l?c
			break;
		case 2:
			unicode::_sprintf(strName, "%s", GlobalText[167]); //Nhanh nh?n
			break;
		case 3:
			unicode::_sprintf(strName, "%s", GlobalText[166]); //S?c m?nh
			break;
		case 4:
			unicode::_sprintf(strName, "%s", GlobalText[1900]); //M?nh L?nh
			break;
		}
	}
...
bool SEASON3B::CNewUIMakeRenderMessageBox::Create(DWORD dwType, float fPriority)
{
	...
	GMRenderGroupMesh->runtime_make_edit(0);

...
namespace {
const float kCommandWindowWidth = 214.0f;
const float kCommandWindowHeight = 230.0f;

void GetCenteredCommandPosition(float &x, float &y) {
	x = (MAX_WIN_WIDTH / 2.0f) - (kCommandWindowWidth / 2.0f);
	y = (MAX_WIN_HEIGHT / 2.0f) - (kCommandWindowHeight / 2.0f);
...
bool SEASON3B::CNewUICommandFilter::UpdateMouseEvent()
{
	...
	GetCenteredCommandPosition(centeredX, centeredY);
	SetPos(centeredX, centeredY);

	float RenderFrameX = m_Pos.x;
	float RenderFrameY = m_Pos.y;

	if (SEASON3B::CheckMouseIn(RenderFrameX, RenderFrameY, kCommandWindowWidth, kCommandWindowHeight))
	{
	...
	return !SEASON3B::CheckMouseIn(RenderFrameX, RenderFrameY, kCommandWindowWidth, kCommandWindowHeight);
}
...
void SEASON3B::CNewUICommandFilter::RenderFrame()
{
	...
	RenderImageF(IMAGE_TOP_BACK3, RenderFrameX, RenderFrameY, kCommandWindowWidth, kCommandWindowHeight, 0.0, 0.0, 854.0, 917.0); //-- background
}

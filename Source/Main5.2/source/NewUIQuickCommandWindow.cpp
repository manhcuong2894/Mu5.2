		...
		case 5:
			{
				...
				if (pCha)
				{
					if (!gH_ViewCharInfo) gH_ViewCharInfo = new H_ViewCharInfo;
					gH_ViewCharInfo->SendRequestViewItem(pCha->Key);
				}
				CloseQuickCommand();
...
void SEASON3B::CNewUIQuickCommandWindow::RenderFrame()
{
	...
	x = m_Pos.x; y = m_Pos.y; width = 112.f; height = 160;
	...
	RenderImage(IMAGE_QUICKCOMMAND_FRAME_DOWN, m_Pos.x, y, 112.f, 20.f);
...
void SEASON3B::CNewUIQuickCommandWindow::RenderContents()
{
	...
	int iGlobalText[] = { 943, 1124, 944, 948, 949, 3400 };
	for(int i = 0; i < 6; ++i)
	{

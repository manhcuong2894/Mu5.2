...
void SCALEFORM::OnResetDevice(SInt bufw, SInt bufh)
{
	for (size_t i = 0; i < hldList.size(); i++)
	{
		hldList[i]->shared.OnResetDevice(bufw, bufh);
	}
}
...
void gfxNotice::addNoticeText(char* Text, DWORD Style)
{
	...
	int NoticeFontSize = FontHeight - 4;
	if (NoticeFontSize < 8)
	{
		NoticeFontSize = 8;
	}

	const char* HTML = SEASON3B::FontHTML(Text, HexColor, NoticeFontSize, true, false, 3);

...
static bool IsWingGlowJointType(int Type, int SubType)
{
	...
	case BITMAP_FLARE:
	case BITMAP_FLARE + 1:
		return SubType == 0 || SubType == 3 || SubType == 6 || SubType == 8 || SubType == 20;
	default:
		return false;
	}
}
...
static bool IsJointTailBatchSafe(const JOINT* o)
{
	if (o == NULL || o->NumTails < 2)
		return false;

	if (o->Type == MODEL_SPEARSKILL || o->Type == BITMAP_JOINT_FORCE || o->Type == BITMAP_FLARE_FORCE)
		return false;
	...
	if (o->Type == BITMAP_JOINT_THUNDER + 1 && (o->SubType == 0 || o->SubType == 6))
		return false;
...
{
	TEXCOORD(textCoords[vertexIndex], u1, v1);
	VectorCopy(p1, vertices[vertexIndex]);
	VectorCopy(color, colors[vertexIndex], 4);
	vertexIndex++;
	TEXCOORD(textCoords[vertexIndex], u2, v2);
	VectorCopy(p2, vertices[vertexIndex]);
	VectorCopy(color, colors[vertexIndex], 4);
	vertexIndex++;
	TEXCOORD(textCoords[vertexIndex], u3, v3);
	VectorCopy(p3, vertices[vertexIndex]);
	VectorCopy(color, colors[vertexIndex], 4);
	vertexIndex++;
	TEXCOORD(textCoords[vertexIndex], u4, v4);
	VectorCopy(p4, vertices[vertexIndex]);
	VectorCopy(color, colors[vertexIndex], 4);
	vertexIndex++;
}

static bool RenderJointTailsBatch(JOINT* o)
{
	if (!IsJointTailBatchSafe(o))
		return false;

	vec2_t textCoordsOne[MAX_TAILS * 4];
	vec3_t verticesOne[MAX_TAILS * 4];
	vec4_t colorsOne[MAX_TAILS * 4];
	vec2_t textCoordsTwo[MAX_TAILS * 4];
	vec3_t verticesTwo[MAX_TAILS * 4];
	vec4_t colorsTwo[MAX_TAILS * 4];
	int vertexOne = 0;
	int vertexTwo = 0;
	GLfloat currentColor[4];
	glGetFloatv(GL_CURRENT_COLOR, currentColor);

	const int lastTail = o->NumTails - 1;
	for (int j = 0; j < lastTail; ++j)
	{
		...
		{
			Light1 = Light1 * 2.f - Scroll;
			Light2 = Light2 * 2.f - Scroll;
		}
		if (o->bTileMapping)
		{
			Scroll *= 2.f;
		...
		{
			AppendJointQuad(textCoordsOne, verticesOne, colorsOne, vertexOne,
				L1, V2, o->Tails[j][2], L1, V1, o->Tails[j][3],
				L2, V1, o->Tails[j + 1][3], L2, V2, o->Tails[j + 1][2], currentColor);
		}
		...
		if ((o->RenderFace & RENDER_FACE_TWO) == RENDER_FACE_TWO)
		{
			...
			if (o->Type == BITMAP_JOINT_THUNDER || o->Type == BITMAP_JOINT_THUNDER + 1)
			{
				faceTwoL1 += Scroll * 2.f;
				faceTwoL2 += Scroll * 2.f;
			}
			AppendJointQuad(textCoordsTwo, verticesTwo, colorsTwo, vertexTwo,
				faceTwoL1, V1, o->Tails[j][0], faceTwoL1, V2, o->Tails[j][1],
				faceTwoL2, V2, o->Tails[j + 1][1], faceTwoL2, V1, o->Tails[j + 1][0], currentColor);
		}
	}

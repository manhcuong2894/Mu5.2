...
{
	const bool hasBotMixList = (gCB_BotMix && gCB_BotMix->HasMixList());
	const bool mouseOnBotMixWindow = (gCB_BotMix && gCB_BotMix->IsMouseOnWindow(m_Pos.x, m_Pos.y));
	bool inventoryCapturedMouse = false;

	if (!hasBotMixList && m_pYourInvenCtrl && false == m_pYourInvenCtrl->UpdateMouseEvent())
	{
		inventoryCapturedMouse = true;
	}

	if (m_pMyInvenCtrl && false == m_pMyInvenCtrl->UpdateMouseEvent())
	{
		inventoryCapturedMouse = true;
	}

	if (inventoryCapturedMouse)
	{
...
bool CNewUITrade::Update()
{
	...
	if (((!hasBotMixList && m_pYourInvenCtrl && false == m_pYourInvenCtrl->Update()))
...
bool CNewUITrade::Render()
{
	...
	const bool bBotMixActive = (gCB_BotMix && gCB_BotMix->HasMixList());

	if (!bBotMixActive && m_pYourInvenCtrl)
	{
	...
	{
		if (bBotMixActive && i == BTN_ZEN_INPUT)
			continue;
		m_abtn[i].Render();
	}
	...
	if (bBotMixActive)
	{
		gCB_BotMix->DrawWindow(m_Pos.x, m_Pos.y);
	}
...
{
	if (gCB_BotMix && gCB_BotMix->IsInputLockActive())
	{
		return 10.65f;
	}
...
bool CNewUITrade::ProcessBtns()
{
	...
	else if (!(gCB_BotMix && gCB_BotMix->HasMixList()) && m_abtn[BTN_ZEN_INPUT].UpdateMouseEvent())
	{
		if (gmProtect->trade_system_pro)
		{
...
void CNewUITrade::ProcessToReceiveTradeExit(BYTE byState)
{
	...
	case 5:
		g_pChatListBox->AddText("", "Trade failed: max Zen/Coin limit reached.", SEASON3B::TYPE_ERROR_MESSAGE);
		break;

	case 6:
		g_pChatListBox->AddText("", "Trade failed: pentagram inventory check failed.", SEASON3B::TYPE_ERROR_MESSAGE);
		break;
	}

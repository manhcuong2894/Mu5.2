...
void CUIWindowMgr::DoAction()
{
	...
	// F1 reserved globally for hide UI
...
BOOL CompareItemEqual(const PART_t* item1, const PART_t* item2)
{
	return (item1->Type == item2->Type &&
		...
		item1->ExtOption == item2->ExtOption);
}
...
void SetItemToPhoto(PART_t* itemDest, const ITEM* itemSrc, int iDefaultValue)
{
	if (itemSrc->Type == -1)
	{
		...
		itemDest->ExtOption = itemSrc->ExtOption;
...
void CUIPhotoViewer::RebuildPhotoHelper()
{
...
void CUIPhotoViewer::UpdateItemChar(int iEquipmentSlot, const ITEM* pItem)
{
	if (m_bIsInitialized == FALSE || pItem == NULL || pItem->Type == -1)
		return;

	if (iEquipmentSlot < EQUIPMENT_WEAPON_RIGHT || iEquipmentSlot >= MAX_NEW_EQUIPMENT)
		return;

	switch (iEquipmentSlot)
	{
	case EQUIPMENT_WEAPON_RIGHT:
	case EQUIPMENT_WEAPON_LEFT:
		SetItemToPhoto(&m_PhotoChar.Weapon[iEquipmentSlot - EQUIPMENT_WEAPON_RIGHT], pItem, -1);
		if (iEquipmentSlot == EQUIPMENT_WEAPON_RIGHT && pItem->TwoHand)
		{
			m_PhotoChar.Weapon[EQUIPMENT_WEAPON_LEFT].Type = -1;
			m_PhotoChar.Weapon[EQUIPMENT_WEAPON_LEFT].Level = 0;
			m_PhotoChar.Weapon[EQUIPMENT_WEAPON_LEFT].Option1 = 0;
			m_PhotoChar.Weapon[EQUIPMENT_WEAPON_LEFT].ExtOption = 0;
		}
		RebuildPhotoHelper();
		break;
	case EQUIPMENT_HELM:
	...
	{
		const int bodyPart = BODYPART_HELM + (iEquipmentSlot - EQUIPMENT_HELM);
		DeleteCloth(&m_PhotoChar, NULL, NULL);
		SetItemToPhoto(&m_PhotoChar.BodyPart[bodyPart], pItem,
			MODEL_BODY_HELM + m_PhotoChar.GetSkinModelIndex() + (MODEL_BODY_NUM * (iEquipmentSlot - EQUIPMENT_HELM)));
	...
	case EQUIPMENT_WING:
		SetItemToPhoto(&m_PhotoChar.Wing, pItem, -1);
		DeleteCloth(NULL, &m_PhotoChar.Object, NULL);
		SetPhotoPose(m_iCurrentAnimation);
		break;
	case EQUIPMENT_HELPER:
		SetItemToPhoto(&m_PhotoChar.Helper, pItem, -1);
		SetPhotoPose(AT_STAND1);
		m_iSettingAnimation = AT_STAND1;
		RebuildPhotoHelper();
		break;
	...
	case EQUIPMENT_WING_CORE:
		SetItemToPhoto(&m_PhotoChar.Core, pItem, -1);
		DeleteCloth(NULL, &m_PhotoChar.Object, NULL);
		SetPhotoPose(m_iCurrentAnimation);
		break;
#endif // SHUTDOWN_CORE_WING4
	...
	case EQUIPMENT_FLAG_NAT:
		SetItemToPhoto(&m_PhotoChar.Flag, pItem, -1);
		break;
#endif // SYSTEM_FLAG_NAT
	default:
		break;
...
void CUIPhotoViewer::SetClass(BYTE byClass)
{
	...
	m_PhotoChar.Object.m_pCloth = NULL;
	m_PhotoChar.Object.m_byNumCloth = 0;
	DeleteParts(&m_PhotoChar);
	SetAction(&m_PhotoHelper, m_iSettingAnimation);
}
...
void CUIPhotoViewer::SetZoomLimit(float fMinZoom, float fMaxZoom)
{
	if (fMinZoom > fMaxZoom)
	{
		float fTempZoom = fMinZoom;
		fMinZoom = fMaxZoom;
		fMaxZoom = fTempZoom;
	}
	...
	if (m_fCurrentZoom < m_fMinZoom)
		m_fCurrentZoom = m_fMinZoom;
	else if (m_fCurrentZoom > m_fMaxZoom)
		m_fCurrentZoom = m_fMaxZoom;

	if (m_fSettingZoom < m_fMinZoom)
		m_fSettingZoom = m_fMinZoom;
	else if (m_fSettingZoom > m_fMaxZoom)
		m_fSettingZoom = m_fMaxZoom;
}
...
BOOL CUIPhotoViewer::DoMouseAction()
{
	...
	const BOOL bRightButtonDown = ((GetAsyncKeyState(VK_RBUTTON) & 0x8000) != 0);
	if (bRightButtonDown == FALSE)
	{
		m_bRightButtonResetLock = FALSE;
	}
	...
	//if (CheckOption(UIPHOTOVIEWER_CANCONTROL))
	{
		...
		else if (CheckMouseIn(m_iPos_x, m_iPos_y, m_iWidth, m_iHeight) == TRUE)
		{
			...
			else if (MouseRButtonPush || (bRightButtonDown == TRUE && m_bRightButtonResetLock == FALSE))
			{
				m_bHelpEnable = FALSE;
				m_fCurrentAngle = m_fSettingAngle;
				m_fCurrentZoom = m_fSettingZoom;
				m_bRightButtonResetLock = TRUE;
			...
			else if (MouseWheel != 0)
			{
				...
				if (m_fCurrentZoom > m_fMaxZoom)
					m_fCurrentZoom = m_fMaxZoom;
				else if (m_fCurrentZoom < m_fMinZoom)
					m_fCurrentZoom = m_fMinZoom;
				MouseWheel = 0;
			}
		}

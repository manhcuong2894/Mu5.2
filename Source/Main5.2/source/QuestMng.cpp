	...
	for (j = 0; j < 2; ++j)
	{
		...
		for (; i < byRewardCount; ++i, ++nLine)
		{
			...
			case QUEST_REWARD_CONTRIBUTE:
				// Fallback to a hardcoded string if GlobalText[2994] is missing or empty
				if (GlobalText[2994] && GlobalText[2994][0] != '\0') {
					::sprintf(aDest[nLine].m_szText, GlobalText[2994], pRewardInfo->m_dwValue);
				} else {
					::sprintf(aDest[nLine].m_szText, "Điểm Cống Hiến: %lu", pRewardInfo->m_dwValue);
				}
				break;
			}
			aDest[nLine].m_szText[QM_MAX_REQUEST_REWARD_TEXT_LEN - 1] = 0;

...
namespace
{
    bool IsVietnameseWideChar(wchar_t ch)
    {
    ...
    int CountVietnameseWideChars(const std::wstring& text)
    {
        int count = 0;

        for (size_t i = 0; i < text.length(); ++i)
        {
            if (IsVietnameseWideChar(text[i]))
            {
                ++count;
            }
        }
    ...
    bool ContainsVietnameseMojibake(const std::wstring& text)
    {
        for (size_t i = 0; i < text.length(); ++i)
        {
            const wchar_t ch = text[i];

            if (ch == 0x00C2 || ch == 0x00C3 || ch == 0x00C6)
            {
                return true;
            }

            if (ch == 0x00E1 && i + 1 < text.length())
            {
                const wchar_t next = text[i + 1];

                if (next == 0x00BA || next == 0x00BB)
                {
                    return true;
                }
            }
    ...
    bool TryRepairVietnameseMojibake(const std::wstring& input, std::wstring& output)
    {
        if (!ContainsVietnameseMojibake(input))
        {
            return false;
        }

        const int encodedLength = WideCharToMultiByte(CP_1252, 0, input.c_str(), -1, NULL, 0, NULL, NULL);
        ...
        char* mojibakeBytes = new char[encodedLength];
        bool repaired = false;

        if (WideCharToMultiByte(CP_1252, 0, input.c_str(), -1, mojibakeBytes, encodedLength, NULL, NULL) != 0)
        {
            const int repairedLength = MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, mojibakeBytes, -1, NULL, 0);

            if (repairedLength > 1)
            {
                wchar_t* repairedText = new wchar_t[repairedLength];

                if (MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, mojibakeBytes, -1, repairedText, repairedLength) != 0)
                {
                    std::wstring candidate(repairedText);

                    if (candidate != input && CountVietnameseWideChars(candidate) > CountVietnameseWideChars(input))
                    {
...
CMultiLanguage::CMultiLanguage(std::string strSelectedML)
{
	...
	else if (_stricmp(strSelectedML.c_str(), "VIE") == 0)
	{
		byLanguage = 7;
		iCodePage = CP_1258;
		iNumByteForOneCharUTF8 = 2;
...
int CMultiLanguage::ConvertCharToWideStr(std::wstring& wstrDest, LPCSTR lpString)
{
	...
	if (this->GetCodePage() == CP_1258)
	{
		std::wstring repairedText = L"";

		if (TryRepairVietnameseMojibake(wstrDest, repairedText))
		{
			wstrDest.swap(repairedText);
		}
	}

...
void SEASON3B::CNewUIMainFrameWindow::LoadImages()
{
	...
	LoadBitmap("Interface\\autohp.jpg", IMAGE_AUTOHP_ICON, GL_LINEAR, GL_CLAMP, 1, 0);
	LoadBitmap("Custom\\NewUIButton.tga", IMAGE_AUTOPK_ICON, GL_LINEAR, GL_CLAMP, 1, 0);
...
void SEASON3B::CNewUIMainFrameWindow::UnloadImages()
{
	...
	DeleteBitmap(Bitmap_button_register);
	DeleteBitmap(IMAGE_AUTOHP_ICON);
	...
	if (!gmProtect->checkold_school())
	{
		...
		for (int i = 0; i < MAX_MAGIC; ++i)
		{
			...
			if (bySkillUseType == SKILL_USE_TYPE_MASTERLEVEL)
			{
				...
				// --- Responsive skill layout: fewer skills per row at 800x600 ---
			int iMaxRow1 = (GetWindowsX <= 800) ? 10 : 14;
			int iOverflowEnd = iMaxRow1 + 4;
			int iHalfRow1 = iMaxRow1 / 2;

			if (iSkillCount == iOverflowEnd)
			{
				y -= height;
			}

			if (iSkillCount < iMaxRow1)
			{
			...
			else if (iSkillCount >= iMaxRow1 && iSkillCount < iOverflowEnd)
			{
				x = fOrigX - ((iHalfRow1 + 1) * width) - ((iSkillCount - iMaxRow1) * width);
			}
			else
			{
				x = fOrigX - ((iHalfRow1 + 5) * width) + ((iSkillCount - (iOverflowEnd - 1)) * width);
			}

...
static int GetEquipmentSourceLinealPos(int pointedSlot, ITEM* pItemObj)
{
	int sourceIndex = EquipmentClientToServer(pointedSlot);

	if (sourceIndex >= 0 && CharacterMachine && CharacterMachine->CheckEquipment(sourceIndex))
	{
		return sourceIndex;
	}

	if (pItemObj && pItemObj->lineal_pos >= 0 && CharacterMachine && CharacterMachine->CheckEquipment(pItemObj->lineal_pos))
	{
		sourceIndex = pItemObj->lineal_pos;
	}
...
void SEASON3B::CNewUIMyInventory::SetEquipmentSlotInfo()
{
	...
	else
	{
		...
		SetEquipmentInfo(&m_Equipment[EQUIPMENT_BOOTS], EQUIPMENT_BOOTS, IMAGE_INVENTORY_ITEM_BOOT, m_Pos.x + 135, m_Pos.y + 150, 46, 46);
		//--
		SetEquipmentInfo(&m_Equipment[EQUIPMENT_RING_LEFT], EQUIPMENT_RING_LEFT, IMAGE_INVENTORY_ITEM_RING, m_Pos.x + 118, m_Pos.y + 150, 20, 20);

		SetEquipmentInfo(&m_Equipment[EQUIPMENT_AMULET], EQUIPMENT_AMULET, IMAGE_INVENTORY_ITEM_NECKLACE, m_Pos.x + 58, m_Pos.y + 87, 20, 20);

		SetEquipmentInfo(&m_Equipment[EQUIPMENT_RING_RIGHT], EQUIPMENT_RING_RIGHT, IMAGE_INVENTORY_ITEM_RING, m_Pos.x + 58, m_Pos.y + 150, 20, 20);
	}
...
void SEASON3B::CNewUIMyInventory::SetButtonInfo()
{
	...
	if (gmProtect->checkold_school())
	{
		...
		if (gmProtect->WindowsJewelInventory)
		{
			m_BtnJewel.ChangeToolTipText(gTextClient.txtClient_Khac[3], true);
		}
...
bool SEASON3B::CNewUIMyInventory::EquipmentWindowProcess()
{
	...
	if ((SEASON3B::IsPress(VK_RBUTTON) || SEASON3B::IsRelease(VK_RBUTTON)) && this->GetRepairMode() != REPAIR_MODE_ON && EquipmentItem == false)
	{
		if (this->m_iPointedSlot != -1)
		{
			...
			if (pItemObj->Type != -1)
			{
				int iSrcIndex = GetEquipmentSourceLinealPos(this->m_iPointedSlot, pItemObj);
	...
	else if (success && GetRepairMode() == REPAIR_MODE_OFF && SEASON3B::IsPress(VK_RBUTTON))
	{
...
#ifdef JEWEL_BANK
		if (g_pNewUISystem->IsVisible(SEASON3B::INTERFACE_INVENTORY_JEWEL) == true)
		{
			g_pInventoryJewel->ProcessInvenItem();
			return false;
		}
#endif
			...
			&& g_pNewUISystem->IsVisible(SEASON3B::INTERFACE_MIXINVENTORY) == false)
		{
			...
			const bool isChoTroiSellTab =
				(gCusChoTroi.m_iNumCurOpenTab == 1 || gCusChoTroi.m_TabBtn.GetCurButtonIndex() == 1);

			if (gInterface->Data[eWindowChoTroi].OnShow && isChoTroiSellTab)
			{
				gCusChoTroi.SendItemRaoBan(pItem, iIndex, true);
				return true;
			}
#endif

#if(CUSTOM_CHANGEITEM)
			if (gInterface->ChangeSendItem(pItem, iIndex + MAX_EQUIPMENT_INDEX, true))
			{
				return true;
			}
#endif
		...
		if (SEASON3B::IsPress(VK_RBUTTON) && this->GetRepairMode() != REPAIR_MODE_ON && EquipmentItem == false)
		{
			...
			if (gInterface->Data[eWindowChangeItem].OnShow)
			{
				...
				if (pItemObj)
				{
					...
					if (gInterface->ChangeSendItem(pItemObj, iSrcIndex + MAX_EQUIPMENT_INDEX, true))
					{
						ResetMouseRButton();
						return true;
					}
			...
			else
			{
				...
				if (pItemObj)
				{
					...
					if (gInterface->Data[eWindowChoTroi].OnShow && isChoTroiSellTab)
					{
						gCusChoTroi.SendItemRaoBan(pItemObj, iSrcIndex, true);
						return true;
					}
#endif

...
namespace
{
	const float kSelectionScreenPaddingX = 180.0f;
	const float kSelectionScreenPaddingY = 220.0f;

	bool IsSelectionCandidateNearCursor(OBJECT* o)
	{
		vec3_t position;
		int screenX = 0;
		int screenY = 0;
		...
		Vector(
			o->Position[0],
			o->Position[1],
			o->Position[2] + ((o->BoundingBoxMax[2] > 0.0f) ? (o->BoundingBoxMax[2] * 0.5f) : 100.0f),
			position);

		Projection(position, &screenX, &screenY);

		return screenX >= (MouseX - kSelectionScreenPaddingX)
			&& screenX <= (MouseX + kSelectionScreenPaddingX)
			&& screenY >= (MouseY - kSelectionScreenPaddingY)
			&& screenY <= (MouseY + kSelectionScreenPaddingY);
	}
}
...
bool CheckAttack_Fenrir(CHARACTER* pCharacter)
{
	...
	if (::IsStrifeMap(gMapManager->currentMap) && pCharacter != Hero && pCharacter->m_byGensInfluence != Hero->m_byGensInfluence)
	{
		...
		{
			if ((SEASON3B::IsRepeat(VK_CONTROL) || gInterface->AutoPK) == 1)
				return 1;
		}
		if (strcmp(GuildMark[Hero->GuildMarkIndex].GuildName, GuildMark[pCharacter->GuildMarkIndex].GuildName))
		{
	...
	else if (pCharacter->Object.Kind == KIND_PLAYER)
	{
		...
		if (pCharacter->GuildRelationShip != GR_RIVAL && pCharacter->GuildRelationShip != GR_RIVALUNION)
		{
				...
				|| strcmp(GuildMark[Hero->GuildMarkIndex].GuildName, GuildMark[pCharacter->GuildMarkIndex].GuildName))
			{
				...
				else if (pCharacter->PK < PVP_MURDERER2 && ((SEASON3B::IsRepeat(VK_CONTROL) || gInterface->AutoPK) != true || pCharacter == Hero))
				{
					success = false;
...
bool CheckAttack()
{
		...
		&& pCharacter->m_byGensInfluence != Hero->m_byGensInfluence)
	{
		...
		else if ((SEASON3B::IsRepeat(VK_CONTROL) || gInterface->AutoPK) == true)
		{
			if (EnableGuildWar)
		...
		if (c == Hero && sc)
		{
				...
				|| strcmp(GuildMark[Hero->GuildMarkIndex].GuildName, GuildMark[sc->GuildMarkIndex].GuildName))
			{
				...
				else if (sc->GuildRelationShip != GR_RIVAL && sc->GuildRelationShip != GR_RIVALUNION)
				{
					...
					{
						if ((signed int)sc->PK < PVP_MURDERER2 && ((SEASON3B::IsRepeat(VK_CONTROL) || gInterface->AutoPK) != true || sc == Hero))
						{
							if (gMapManager->IsCursedTemple())
							{
...
void UseSkillRagefighter(CHARACTER* pCha, OBJECT* pObj)
{
	...
	{
		pObj->start_skill_hide(iSkill);
		if (pObj->animation_hide_traditional() || g_CMonkSystem.IsRideNotUseSkill(iSkill, pCha->Helper.Type) == false)
		{
...
bool FindText(const char* Text, const char* Token, bool First)
{
	...
	for (int i = 0; i <= Length; i += GetUiCharByteLength(reinterpret_cast<const char*>(lpszCheck + i)))
	{
		bool Success = true;
				...
				LABEL_106:

					{
						...
						if (CheckTile(c, TargetX, TargetY, Distance))
						{
							...
							o->start_skill_hide(Skill);
							if (o->animation_hide_traditional() || c->Helper.Type != (MODEL_HELPER + 37) || c->SafeZone)
...
int SelectCharacter(BYTE Kind)
{
	...
	for (int i = 0; i < MAX_CHARACTERS_CLIENT; i++)
	{
		...
		if (o->Live && o->Visible && o->Alpha > 0.f && c->Dead == 0 && !g_isCharacterBuff(o, eBuff_CrywolfNPCHide))
		{
			...
			{
				if (!IsSelectionCandidateNearCursor(o))
				{
					continue;
				}
...
void SelectObjects()
{
	...
	else
	{
		...
		if (sc)
		{
			if ((signed int)sc->Dead > 0 || sc->GetKind() != KIND_MONSTER || sc->Object.CurrentAction == MONSTER01_DIE || !sc->GetLive())
			{
				SelectedCharacter = -1;
				Attacking = -1;
			}
			...
			else if (!gmAIController->IsRunning()
				&& (MouseLButton
					|| MouseLButtonPush
					|| MouseRButton
					|| MouseRButtonPush
	...
	else
	{
		...
		{
			// AI Controller handles target selection - skip mouse-based selection
		}
		else
		{
			if (SelectedCharacter == -1)
...
void MoveInterface()
{
	...
	if (bIsForegroundClient && (GetAsyncKeyState(VK_F1) & 0x8000) && (GetTickCount() - dwLastPressF1 > 300))
	{
	...
	if (gH_ViewCharInfo)
	{
		gH_ViewCharInfo->UpdateMouseInput();
	}
#endif

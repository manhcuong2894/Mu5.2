...
extern void RegisterBuff(eBuffState buff, OBJECT* o, const int bufftime = 0, WORD power = 0);

namespace {
const float kReconnectWindowWidth = 352.0f;
const float kReconnectWindowHeight = 113.0f;
const float kReconnectMainFrameHeight = 51.0f;

void GetCenteredReconnectPosition(int &x, int &y)
{
	float centeredX = (MAX_WIN_WIDTH / 2.0f) - (kReconnectWindowWidth / 2.0f);
	float centeredY = ((MAX_WIN_HEIGHT - kReconnectMainFrameHeight) / 2.0f) -
	                  (kReconnectWindowHeight / 2.0f);
...
static void ReconnectCloseOpenedInterfaces(SEASON3B::CNewUIManager* pNewUIMng)
{
	SEASON3B::CNewUIInventoryCtrl::BackupPickedItem();
	if (g_pNewUISystem->IsVisible(SEASON3B::INTERFACE_MIXINVENTORY))
	{
		g_pMixInventory->ReconnectRollbackItems();
	}
...
bool SEASON3B::CNewUIReconnect::Render()
{
	...
	int centeredX = 0;
	int centeredY = 0;
	GetCenteredReconnectPosition(centeredX, centeredY);
...
void SEASON3B::CNewUIReconnect::RenderFrame()
{
	...
	SEASON3B::RenderImage(BITMAP_MESSAGE_WIN, x, y, kReconnectWindowWidth, kReconnectWindowHeight); //-- background
...
void SEASON3B::CNewUIReconnect::RenderText()
{
	...
	case RECONNECT_PROGRESS_NONE:
		...
		g_pRenderText->RenderFont(x, y, gTextClient.txtClient_KetNoi[0], (int)kReconnectWindowWidth, 55, RT3_SORT_CENTER);
		break;
	case RECONNECT_PROGRESS_CONNECTED:
		//wsprintf(buff, gCustomMessage.GetMessage(2));
		g_pRenderText->RenderFont(x, y, gTextClient.txtClient_KetNoi[1], (int)kReconnectWindowWidth, 55, RT3_SORT_CENTER);
		break;
	case RECONNECT_PROGRESS_JOINED:
		//wsprintf(buff, gCustomMessage.GetMessage(3));
		g_pRenderText->RenderFont(x, y, gTextClient.txtClient_KetNoi[2], (int)kReconnectWindowWidth, 55, RT3_SORT_CENTER);
		break;
	case RECONNECT_PROGRESS_CHAR_LIST:
		//wsprintf(buff, gCustomMessage.GetMessage(4));
		g_pRenderText->RenderFont(x, y, gTextClient.txtClient_KetNoi[3], (int)kReconnectWindowWidth, 55, RT3_SORT_CENTER);
		break;
	}
}
...
void SEASON3B::CNewUIReconnect::ReconnectSetInfo(DWORD status, DWORD progress, DWORD CurWait, DWORD MaxWait)
{
	if (gmProtect->ReconnectTime > 0)
	{
		if (status == RECONNECT_STATUS_RECONNECT && ReconnectStatus != RECONNECT_STATUS_RECONNECT)
		{
			ReconnectCloseOpenedInterfaces(m_pNewUIMng);
		}

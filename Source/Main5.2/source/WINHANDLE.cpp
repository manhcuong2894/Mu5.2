...
void CWINHANDLE::SetFontSize(mu_uint32 FontSize)
{
	if (FontHeight != FontSize || FontHeight == -1)
	{
		...
		g_hFontMini = CreateFont(10, 0, 0, 0, FW_NORMAL, 0, 0, 0, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, NONANTIALIASED_QUALITY, DEFAULT_PITCH | FF_DONTCARE, fontname);
...
LONG CWINHANDLE::WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_SIZE:
		...
		{
			const SInt resizeWidth = LOWORD(lParam);
			const SInt resizeHeight = HIWORD(lParam);

			if (resizeWidth > 0 && resizeHeight > 0)
			{
				gfxinit->OnResetDevice(resizeWidth, resizeHeight);
			}
		}
#endif // SHUTDOWN_SCALEFORM_INFO
		break;

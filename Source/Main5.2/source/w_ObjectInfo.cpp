...
void OBJECT::Initialize()
{
	...
	time_animation_hide = std::chrono::steady_clock::now() - std::chrono::seconds(2);
}
...
void OBJECT::start_skill_hide(int skill)
{
	if (Type == MODEL_PLAYER
		&& (skill == Skill_Inferno
			...
			|| skill == Skill_Fire_Slash_Strengthener
			|| skill == Skill_Fire_Slash_Mastery
			|| skill == Skill_Power_Slash
			|| skill == Skill_Power_Slash_Strengthener
			|| skill == Skill_Killing_Blow_Strengthener
			|| skill == Skill_Killing_Blow_Mastery
			|| skill == Skill_Dragon_Roar_Strengthener
			|| skill == Skill_Dragon_Roar_Mastery
		))
	{

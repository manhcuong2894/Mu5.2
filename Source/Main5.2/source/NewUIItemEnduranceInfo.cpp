// NewUIItemEnduranceInfo.cpp: implementation of the CNewUIItemEnduranceInfo
// class.
//////////////////////////////////////////////////////////////////////
...
CNewUIItemEnduranceInfo::~CNewUIItemEnduranceInfo() { Release(); }

bool SEASON3B::CNewUIItemEnduranceInfo::Create(CNewUIManager *pNewUIMng, int x,
                                               int y) {
  if (NULL == pNewUIMng)
    return false;
...
bool SEASON3B::CNewUIItemEnduranceInfo::UpdateMouseEvent() {
  ...
  if (Hero->Helper.Type >= MODEL_HELPER &&
    ...
    if (CheckMouseIn(m_UIStartPos.x, iNextPosY, PETHP_FRAME_WIDTH,
                     PETHP_FRAME_HEIGHT))
      return false;
  ...
  for (int i = EQUIPMENT_WEAPON_RIGHT; i < MAX_EQUIPMENT; ++i) {
    ...
    if (pItem->Durability <= iMaxDurability * 0.5f) {
      if (i == EQUIPMENT_RING_RIGHT) {
        if (CheckMouseIn(ItemDurPos.x, ItemDurPos.y, ITEM_DUR_WIDTH / 2,
                         ITEM_DUR_HEIGHT)) {
          m_iTooltipIndex = i;
          return false;
        }
        ...
        if (CheckMouseIn(ItemDurPos.x + (ITEM_DUR_WIDTH / 2), ItemDurPos.y,
                         ITEM_DUR_WIDTH / 2, ITEM_DUR_HEIGHT)) {
          m_iTooltipIndex = i;
          return false;
        }
        bRenderRingWarning = false;
      } else {
        if (CheckMouseIn(ItemDurPos.x, ItemDurPos.y, ITEM_DUR_WIDTH,
                         ITEM_DUR_HEIGHT)) {
          m_iTooltipIndex = i;
          return false;
        }
...
bool SEASON3B::CNewUIItemEnduranceInfo::UpdateKeyEvent() { return true; }
...
#if (NOTICE_PKSYSTEM)
  DrawNoticePKSystem((int)GetScreenWidth() - 5, 250);
#endif
...
float SEASON3B::CNewUIItemEnduranceInfo::RenderLeft(float RenderFrameX,
                                                    float RenderFrameY) {
  float iNextPosY = 0.0;
  ...
  if (RenderEquipedHelperLife(&Hero->MuunHelper[0], 237, RenderFrameX,
                              RenderFrameY + iNextPosY)) {
    iNextPosY += (UI_INTERVAL_HEIGHT + PETHP_FRAME_HEIGHT);
  }

  if (RenderEquipedHelperLife(&Hero->MuunHelper[1], 238, RenderFrameX,
                              RenderFrameY + iNextPosY)) {
    iNextPosY += (UI_INTERVAL_HEIGHT + PETHP_FRAME_HEIGHT);
  }
  ...
  if (gmProtect->MenuSwitch != 0 && gmProtect->MenuTypeButton != 2) {
    EnableAlphaTest();
    glColor3f(1.f, 1.f, 1.f);
    RenderImageF(BITMAP_HERO_POSITION_INFO_BEGIN + 6, m_UIStartPos.x,
                 iNextPosY + 4, PETHP_FRAME_WIDTH, PETHP_FRAME_HEIGHT);
  }
...
bool SEASON3B::CNewUIItemEnduranceInfo::runtime_action(int Action) {
  ...
  for (int i = 0; i < 3; i++) {
    if (runtime_action_draw(i, RenderFrameX, RenderFrameY, 100.f, &draw_info[i],
                            Action) &&
        Action == 1) {
      return true;
    }
...
bool SEASON3B::CNewUIItemEnduranceInfo::runtime_action_draw(
    int index, float &RenderFrameX, float &RenderFrameY, float RenderSizeX,
    bool *bEnable, int Action) {
  ...
  if (Action == 0) {
    ...
    RenderImageF(IMAGE_BACK_FADED_R1, RenderFrameX, RenderFrameY, RenderSizeX,
                 15.0, 0.0, 0.0, 130.f, 126.0);
    RenderBtns(RenderFrameX + 2, RenderFrameY + 2, 11.0, 11.0,
               (*bEnable) ? 4 : 3);

    g_pRenderText->RenderText(RenderFrameX + 18, RenderFrameY, text_name[index],
                              0, 15);

    if ((*bEnable) && index == 0) {
      RenderSizeY[index] =
          this->RenderLeft(RenderFrameX + 4.f, RenderFrameY + AddExpansion);
    } else if (index == 1) {
      if (*bEnable) {
      ...
      if (*bEnable) {
        if (!gmProtect->checkold_school())
          g_pPartyListWindow->SetPos(GetScreenWidth() - 75, 18);
        else
          g_pPartyListWindow->SetPos(GetScreenWidth() - 75, 18);
  ...
  if (Action == 1) {
    if (SEASON3B::CheckMouseIn(RenderFrameX + 2, RenderFrameY + 2, 11.0,
                               11.0)) {
    ...
    if ((*bEnable) && index == 0) {
      if (SEASON3B::CheckMouseIn(RenderFrameX + 4.f,
                                 RenderFrameY + AddExpansion, PETHP_FRAME_WIDTH,
                                 RenderSizeY[index])) {
        return true;
      }
    }
...
float SEASON3B::CNewUIItemEnduranceInfo::GetLayerDepth() { return 2.5f; }
...
void SEASON3B::CNewUIItemEnduranceInfo::RenderHPUI(int iX, int iY,
                                                   unicode::t_char *pszName,
                                                   int iLife,
                                                   int iMaxLife /*=255*/,
                                                   bool bWarning /*=false*/) {
  EnableAlphaTest();
  ...
  if (gmProtect->checkold_school()) {
    ...
    RenderImage(IMAGE_PETHP_FRAME, iX, iY, PETHP_FRAME_WIDTH,
                PETHP_FRAME_HEIGHT);

    float fLife = ((float)iLife / (float)iMaxLife) * (float)PETHP_BAR_WIDTH;
    RenderImage(IMAGE_PETHP_BAR, iX + 4,
                iY + PETHP_FRAME_HEIGHT - PETHP_BAR_HEIGHT - 4, fLife,
                PETHP_BAR_HEIGHT);
    g_pRenderText->RenderText(iX + (PETHP_FRAME_WIDTH / 2), iY + 5, pszName, 0,
                              0, RT3_WRITE_CENTER);
  }
#else
  if (bWarning == false)
...
void SEASON3B::CNewUIItemEnduranceInfo::RenderTooltip(
    int iX, int iY, const ITEM *pItem, const DWORD &dwTextColor) {
  Script_Item *pItemAtt = GMItemMng->find(pItem->Type);
  SIZE TextSize = {0, 0};
  ...
  unicode::_sprintf(szText, "%s (%d/%d)", pItemAtt->Name, pItem->Durability,
                    iMaxDurability);
  g_pMultiLanguage->_GetTextExtentPoint32(g_pRenderText->GetFontDC(), szText, 1,
                                          &TextSize);
...
bool SEASON3B::CNewUIItemEnduranceInfo::RenderEquipedHelperLife(int iX,
                                                                int iY) {
  if (Hero->Helper.Type >= MODEL_HELPER &&
          Hero->Helper.Type <= MODEL_HELPER + 4 ||
      Hero->Helper.Type == MODEL_HELPER + 64 ||
    ...
    unicode::_sprintf(szText,
                      GMItemMng->find(Hero->Helper.Type - MODEL_ITEM)->Name);
...
bool SEASON3B::CNewUIItemEnduranceInfo::RenderEquipedHelperLife(PART_t *f,
                                                                int index,
                                                                int iX,
                                                                int iY) {
  ...
  if (f->Type >= MODEL_HELPER && f->Type <= MODEL_HELPER + 4 ||
      f->Type == MODEL_HELPER + 64 || f->Type == MODEL_HELPER + 65 ||
      f->Type == MODEL_HELPER + 67 || f->Type == MODEL_HELPER + 80 ||
      f->Type == MODEL_HELPER + 106 || f->Type == MODEL_HELPER + 123 ||
...
bool SEASON3B::CNewUIItemEnduranceInfo::RenderSummonMonsterLife(int iX,
                                                                int iY) {
  if (SummonLife <= 0)
    return false;
...
bool SEASON3B::CNewUIItemEnduranceInfo::RenderNumArrow(int iX, int iY) {
  ...
  // Quiver equipped - no arrow count display needed
  int weaponKind2 = GMItemMng->GetKind2(
      CharacterMachine->Equipment[EQUIPMENT_WEAPON_LEFT].Type);
  if (weaponKind2 == ItemKind2::QUIVER)
    return false;
  ...
  if (m_iCurArrowType == ARROWTYPE_BOW) {
    ...
    if (CharacterMachine->Equipment[EQUIPMENT_WEAPON_LEFT].Type ==
        ITEM_BOW + 15) {
      iNumEquipedArrowDurability =
          CharacterMachine->Equipment[EQUIPMENT_WEAPON_LEFT].Durability;
    }

    if ((iNumArrowSetInInven == 0) &&
        (CharacterMachine->Equipment[EQUIPMENT_WEAPON_LEFT].Type !=
         ITEM_BOW + 15))
      return false;

    unicode::_sprintf(szText, GlobalText[351], iNumEquipedArrowDurability,
                      iNumArrowSetInInven);
    ...
    unicode::_sprintf(szText, GlobalText[352], iNumEquipedArrowDurability,
                      iNumArrowSetInInven);
  }
...
bool SEASON3B::CNewUIItemEnduranceInfo::RenderItemEndurance(int ix, int iY) {
  ...
  for (int i = EQUIPMENT_WEAPON_RIGHT; i < MAX_EQUIPMENT; ++i) {
    ...
    if (i != EQUIPMENT_RING_LEFT || bRenderRingWarning != true) {
      RenderImage(m_iItemDurImageIndex[i], ItemDurPos.x, ItemDurPos.y,
                  ITEM_DUR_WIDTH, ITEM_DUR_HEIGHT);
    }
    ...
    if (i == EQUIPMENT_RING_RIGHT) {
      RenderColor(ItemDurPos.x, ItemDurPos.y, ITEM_DUR_WIDTH / 2,
                  ITEM_DUR_HEIGHT);
    } else if (i == EQUIPMENT_RING_LEFT) {
      RenderColor(ItemDurPos.x + (ITEM_DUR_WIDTH / 2), ItemDurPos.y,
                  ITEM_DUR_WIDTH / 2, ITEM_DUR_HEIGHT);
      bRenderRingWarning = false;
    } else {
...
void SEASON3B::CNewUIItemEnduranceInfo::LoadImages() {
  ...
  LoadBitmap("Interface\\newui_durable_boots.tga", IMAGE_ITEM_DUR_BOOTS,
             GL_LINEAR);
  LoadBitmap("Interface\\newui_durable_cap.tga", IMAGE_ITEM_DUR_CAP, GL_LINEAR);
  LoadBitmap("Interface\\newui_durable_gloves.tga", IMAGE_ITEM_DUR_GLOVES,
             GL_LINEAR);
  LoadBitmap("Interface\\newui_durable_lower.tga", IMAGE_ITEM_DUR_LOWER,
             GL_LINEAR);
  LoadBitmap("Interface\\newui_durable_necklace.tga", IMAGE_ITEM_DUR_NECKLACE,
             GL_LINEAR);
  LoadBitmap("Interface\\newui_durable_ring.tga", IMAGE_ITEM_DUR_RING,
             GL_LINEAR);
  LoadBitmap("Interface\\newui_durable_shield.tga", IMAGE_ITEM_DUR_SHIELD,
             GL_LINEAR);
  LoadBitmap("Interface\\newui_durable_upper.tga", IMAGE_ITEM_DUR_UPPER,
             GL_LINEAR);
  LoadBitmap("Interface\\newui_durable_weapon.tga", IMAGE_ITEM_DUR_WEAPON,
             GL_LINEAR);
  LoadBitmap("Interface\\newui_durable_wing.tga", IMAGE_ITEM_DUR_WING,
             GL_LINEAR);

  LoadBitmap("Interface\\HUD\\newui_map_back.tga", IMAGE_HUB_WORLD_TAR,
             GL_LINEAR);
  LoadBitmap("Interface\\HUD\\newui_map_user.tga", IMAGE_HUB_WORLD_USER,
             GL_LINEAR);
}
...
void SEASON3B::CNewUIItemEnduranceInfo::runtime_render_fps(float x, float y,
                                                           float RenderSizeX) {
  unicode::t_char szDebugText[128];

  RenderImageF(IMAGE_BACK_FADED_R2, x, y, RenderSizeX, 15.0, 0.0, 0.0, 100.f,
               126.0);
  ...
  g_pRenderText->RenderFont(x + 45, y, szDebugText, 0, 15.0, RT3_SORT_LEFT);

  unicode::_sprintf(szDebugText, "RTT: %d ms", gsteady_clock->get_ping_time());

  g_pRenderText->RenderFont(x + 82, y, szDebugText, 0, 15.0, RT3_SORT_LEFT);
}
...
void SEASON3B::CNewUIItemEnduranceInfo::runtime_render_map() {
  ...
  if (GetScreenWidth() == ((int)GetWindowsX)) {
    ...
    //		g_pRenderText->RenderText(RenderFrameX + 30.0, RenderFrameY,
    //gMapManager->GetMapName(), 83, 15, RT3_SORT_CENTER);
    //	}
    ...
    // else
    //{
    runtime_render_fps(RenderFrameX - 5.f, RenderFrameY, 136.f);
    //}
  }
}
  ...
  if (GetScreenWidth() == ((int)GetWindowsX) && g_pNewUIMiniMap->IsSuccess()) {
    ...
    if (SEASON3B::CheckMouseIn(RenderFrameX, RenderFrameY, 128, 15.f + 128)) {
      if (!this->IsHideInterfaceWorld() &&
        ...
        /*if (SEASON3B::CheckMouseIn(RenderFrameX, RenderFrameY, 15.f, 15.f) &&
        SEASON3B::IsRelease(VK_LBUTTON))
        {
                g_pNewUIMiniMap->Zoom(-0.5f);
                return true;
        }
        if (SEASON3B::CheckMouseIn(RenderFrameX + 15.f,
        RenderFrameY, 15.f, 15.f) && SEASON3B::IsRelease(VK_LBUTTON))
        {
                g_pNewUIMiniMap->Zoom(0.5f);
                return true;
        }
    ...
    if (SEASON3B::CheckMouseIn(RenderFrameX + 113.f, RenderFrameY, 15.f,
                               15.f) &&
        SEASON3B::IsRelease(VK_LBUTTON)) {
...
void SEASON3B::CNewUIItemEnduranceInfo::RenderBtns(float x, float y,
...
#if (NOTICE_PKSYSTEM)
void SEASON3B::CNewUIItemEnduranceInfo::AddNoticePK(DWORD color, DWORD delay,
                                                    const char *message) {
  DATA_NOTICEPK data;
  memset(&data, 0, sizeof(data));
  data.Color = color;
  data.Time = GetTickCount() + (delay > 0 ? delay : 5000);
  data.Delay = 0;
  strncpy(data.Mess, message, sizeof(data.Mess) - 1);
  data.Mess[sizeof(data.Mess) - 1] = '\0';
  this->m_DataNoticePK.push_back(data);
}
#endif

bool SEASON3B::CNewUIItemEnduranceInfo::DrawNoticePKSystem(int ix, int iY) {
  if (this->m_DataNoticePK.empty())
    return false;

  for (int i = (int)this->m_DataNoticePK.size() - 1; i >= 0; i--) {
  ...
  for (int i = 0; i < (int)this->m_DataNoticePK.size(); i++) {
    unicode::t_char szText[256] = {
        NULL,
    };
    unicode::_sprintf(szText, "%s", this->m_DataNoticePK[i].Mess);

    // Tính chiều dài pixel của đoạn text
    SIZE TextSize = {0, 0};

    g_pRenderText->SetFont(g_hFontBold);
    g_pMultiLanguage->_GetTextExtentPoint32(g_pRenderText->GetFontDC(), szText,
                                            unicode::_strlen(szText),
                                            &TextSize);
    int iTextWidth = (int)(TextSize.cx / g_fScreenRate_x);

    // Vị trí x = mép phải màn hình - chiều dài text - 10px padding
    int iPosX = (int)GetScreenWidth() - iTextWidth;

    g_pRenderText->SetBgColor(0, 0, 0, 180);
    g_pRenderText->SetTextColor(this->m_DataNoticePK[i].Color);
    g_pRenderText->RenderText(iPosX, iY + 15 * i, szText, 0, 0, RT3_SORT_LEFT);
  }

...
namespace
{
	...
	size_t GetMessageCharByteLength(const unicode::t_char* pszText)
	{
		if (pszText == NULL || *pszText == '\0')
		{
			return 0;
		...
		{
			return GetUtf8CharByteLength(pszText);
		}

		const int iLength = _mbclen(reinterpret_cast<const unsigned char*>(pszText));
		return (iLength > 0) ? static_cast<size_t>(iLength) : 1;
	}
}
...
int SEASON3B::CNewUITextInputMsgBox::SeparateText(const type_string& strMsg, DWORD dwColor, BYTE byFontType)
{
	...
	while (bLoop)
	{
		...
		for (int cur_offset = 0; cur_offset < (int)strRemainText.size(); )
		{
			...
			size_t offset = GetMessageCharByteLength(strRemainText.c_str() + cur_offset);
			cur_offset += offset;
...
bool SEASON3B::CAddPointsReceiptMsgBoxLayout::SetLayout()
{
	...
	{
	case 0:
		sprintf_s(strText, "%s %s %s", GlobalText[3505], GlobalText[682], GlobalText[166]); // ThÍm /di?m /S?c m?nh
		break;
	case 1:
		sprintf_s(strText, "%s %s %s", GlobalText[3505], GlobalText[682], GlobalText[167]); // ThÍm /di?m /Nhanh nh?n
		break;
	case 2:
		sprintf_s(strText, "%s %s %s", GlobalText[3505], GlobalText[682], GlobalText[169]); // ThÍm /di?m /Th? l?c
		break;
	case 3:
		sprintf_s(strText, "%s %s %s", GlobalText[3505], GlobalText[682], GlobalText[168]); // ThÍm /di?m /Nang lu?ng
		break;
	case 4:
		sprintf_s(strText, "%s %s %s", GlobalText[3505],GlobalText[682],GlobalText[1900]); // ThÍm /di?m /M?nh L?nh
		break;
	}

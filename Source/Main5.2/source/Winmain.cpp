...
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, PSTR szCmdLine, int nCmdShow)
{
...
#if (ANTIHACK_GGNEW)
	gAntiHackSplash.Show(hInstance);
	gAPICB.Init();
	gXShieldClient.Init(szServerIpAddress);
	gAntiHackSplash.PumpMessages();
#endif

	pMultiLanguage = new CMultiLanguage(g_strSelectedML);

	// Apply FPS from registry directly (no popup)
...
#if (ANTIHACK_GGNEW)
	ShowWindow(gwinhandle->GethWnd(), SW_HIDE);
#endif
	...
	{
#if (ANTIHACK_GGNEW)
		gAntiHackSplash.Hide();
#endif
		return FALSE;
	}

#if (ANTIHACK_GGNEW)
	gAntiHackSplash.HideAfterMinimum(1500);
#endif

...
#if MAIN_UPDATE > 603
	if (false == m_pNewChatLogWindow->Create(m_pNewUIMng, (int)pos_left(), (int)pos_botton() + 373, 6))
		return false;
#elif MAIN_UPDATE > 303
	//
	if (gmProtect->checkold_school())
	{
		if (false == m_pNewChatLogWindow->Create(m_pNewUIMng, (int)pos_left(), (int)pos_botton() + 410, 6))
			return false;
	}
	else
	{
		if (false == m_pNewChatLogWindow->Create(m_pNewUIMng, (int)pos_left(), (int)pos_botton() + 383, 6))
			return false;
	}
#else
	if (false == m_pNewChatLogWindow->Create(m_pNewUIMng, (int)pos_left(), (int)GetWindowsY - 50, 6))
		return false;
#endif
...
bool SEASON3B::CNewUISystem::LoadMainSceneInterface()
{
...
#if MAIN_UPDATE > 603
	if (false == m_pNewChatInputBox->Create(m_pNewUIMng, m_pNewChatLogWindow, (int)pos_left(), (int)pos_botton() + 372))
		return false;
#else
	if (false == m_pNewChatInputBox->Create(m_pNewUIMng, m_pNewChatLogWindow, (int)pos_left(), (int)pos_botton() + 382))
		return false;
#endif
	...
	gCB_QuaPhucLoi = new CB_QuaPhucLoi;

#if MAIN_UPDATE > 303
	if (!gmProtect->checkold_school())
	{
		if (m_pNewPartyListWindow->Create(m_pNewUIMng, GetScreenWidth() - 124, 18) == false)
			return false;
	}
	else
	{
	...
	if (m_pNewBuffWindow->Create(m_pNewUIMng, 240, 33) == false)
	{
		return false;
	}
...
void SEASON3B::CNewUISystem::Show(DWORD dwKey)
{
	...
	if (m_pNewUIMng)
	{
		if ((dwKey == SEASON3B::INTERFACE_RANKING_TOP &&
			 !CanOpenTopMostOverlayWindow(eTopMostOverlayRankingTop)) ||
			(dwKey == SEASON3B::INTERFACE_MINI_MAP &&
...
#if MAIN_UPDATE > 303
		m_pNewPartyListWindow->SetPos(GetScreenWidth() - 75, 18);
#else
...
void SEASON3B::CNewUISystem::RenderItem3DFree(float sx, float sy, float Width, float Height, int Type, int Level, int Option1, int ExtOption, bool PickUp, float Scale, bool FixY)
{
	...
	if (FixY)
	{
		...
		{
			x = sx - 5;
			if (Type == GET_ITEM(0, 16) || Type == GET_ITEM(0, 19)) { y = sy + 30; }
			else if ((Type >= GET_ITEM(0, 32) && Type <= GET_ITEM(0, 35))) { y = sy + 15; }
			else if (Type >= GET_ITEM(0, 15) && Type < GET_ITEM(0, 32)) { y = sy + 25; }
			else if (Type > GET_ITEM(0, 35)) { y = sy + 20; }
			else if (Type == GET_ITEM(0, 0)) { y = sy + 30; }
			else { y = sy + 20; }
		}
		...
		{
			if ((Type >= GET_ITEM(8, 2) && Type <= GET_ITEM(8, 4)) ||
				Type == GET_ITEM(8, 7) || Type == GET_ITEM(8, 8) ||
				Type == GET_ITEM(8, 15))
			{
				y = sy + 20;
			}
			else if (Type == GET_ITEM(8, 34) || Type == GET_ITEM(8, 35))
			{
				y = sy + 25;
			}
			else if (Type <= GET_ITEM(8, 73))
			{
				y = sy + 25;
			}
			else if (Type > GET_ITEM(8, 73))

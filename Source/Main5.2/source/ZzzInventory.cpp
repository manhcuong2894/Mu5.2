...
static void RestoreRenderAlphaBlendType(int alphaBlendType)
{
...
void RenderTipTextList(const int sx, const int sy, int TextNum, int Tab, int iSort, int iRenderPoint, BOOL bUseBG, bool bRender3d)
{
	...
	glColor4f(1.0f, 1.0f, 1.0f, 1.0f);
	RestoreRenderAlphaBlendType(BackupAlphaBlendType);
}
...
void GetItemName(int iType, int iLevel, char* Text)
{
	...
	else if (iType == ITEM_POTION + 11)
	{
		...
		{
		case 0: sprintf(Text, "%s", p->Name); break;
		case 1: sprintf(Text, "%s", GlobalText[105]); break; //Ngôi sao may m?n
		case 2: sprintf(Text, "%s", GlobalText[106]); break; //Pháo hoa
		case 3: sprintf(Text, "%s", GlobalText[107]); break; //Trái tim tình yêu
		case 5: sprintf(Text, "%s", GlobalText[109]); break; //Huy chuong B?c
		case 6: sprintf(Text, "%s", GlobalText[110]); break; //Huy chuong Vàng
		case 7: sprintf(Text, "%s", GlobalText[111]); break; //H?p c?a Vua
			break;
		case 8:
		case 9:
		case 10:
		case 11:
		case 12:
			sprintf(Text, "%s +%d", GlobalText[115], iLevel - 7);  //H?p Kundun
			break;
		case 13:
			sprintf(Text, GlobalText[117]); break; //Trái tim Chúa t?
		case 14:
			sprintf(Text, GlobalText[1650]); break; //Bình tang Mana
			break;

		case 15:
			sprintf(Text, GlobalText[1651]); break; //Bình tang HP
			break;
		}
	}
	...
	else if (iType == ITEM_HELPER + 14)
	{
		...
		{
		case 0: sprintf(Text, "%s", p->Name); break;
		case 1: sprintf(Text, "%s", GlobalText[1235]); break; //Huy hi?u Hoàng T?c
		}
	}
	else if (iType == ITEM_HELPER + 31)
	{
		switch (iLevel)
		{
		case 0: sprintf(Text, "%s %s", GlobalText[1187], p->Name); break; //Chi?n Mã
		case 1: sprintf(Text, "%s %s", GlobalText[1214], p->Name); break; //Qu? Tinh
		}
	}
...
{
	if (ip == NULL || g_pNewItemTooltip == NULL)
	{
		return;
	}

	g_pNewItemTooltip->RenderItemTooltip(sx, sy, ip, Sell, Inventype, bItemTextListBoxUse, Render3DItem);
}
...
static GLuint g_GuildMarkTextureCache[MAX_MARKS][2] = { 0 };
static unsigned int g_GuildMarkSignatureCache[MAX_MARKS][2] = { 0 };
static bool g_GuildMarkTextureReady[MAX_MARKS][2] = { false };

static unsigned int GetGuildMarkSignature(const BYTE* MarkBuffer, int PixelCount, bool blend)
{
	unsigned int hash = blend ? 2166136261u : 16777619u;
	for (int i = 0; i < PixelCount; ++i)
	{
		hash ^= (unsigned int)MarkBuffer[i];
		hash *= 16777619u;
	}
	return hash;
}

static void BuildGuildMarkColors(bool blend)
{
...
void CreateGuildMark(int nMarkIndex, bool blend)
{
	...
	const int blendIndex = blend ? 1 : 0;
	const int pixelCount = Width * Height;
	BYTE* MarkBuffer = GuildMark[nMarkIndex].Mark;
	const unsigned int signature = GetGuildMarkSignature(MarkBuffer, pixelCount, blend);

	if (g_GuildMarkTextureReady[nMarkIndex][blendIndex]
		&& g_GuildMarkSignatureCache[nMarkIndex][blendIndex] == signature)
	{
	...
	BuildGuildMarkColors(blend);
	BYTE* WriteBuffer = Buffer;
	for (int i = 0; i < pixelCount; ++i)
	{
		*((unsigned int*)(WriteBuffer)) = MarkColor[MarkBuffer[i]];
		WriteBuffer += 4;
	}

	GLuint& textureNumber = g_GuildMarkTextureCache[nMarkIndex][blendIndex];
	if (textureNumber == 0)
	{
	...
	g_GuildMarkSignatureCache[nMarkIndex][blendIndex] = signature;
	g_GuildMarkTextureReady[nMarkIndex][blendIndex] = true;
}

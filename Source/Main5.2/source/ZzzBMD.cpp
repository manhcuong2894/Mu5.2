...
void BMD::PrepareGpuAssist(OBJECT* pObject, int modelType, int renderTypeHint, bool isBodyModel, bool globalTransform, bool translate)
{
	ClearGpuAssist();
	...
	m_bGpuAssistAttachmentPath = isAttachmentModel;
	m_bGpuAssistGlobalTransform = globalTransform;
	m_bGpuAssistTranslate = translate;
	m_iGpuAssistObjectType = modelType;
	m_byGpuAssistObjectKind = pObject->Kind;
#endif // SHADER_VERSION_TEST
}

void BMD::ClearGpuAssist()
{
	m_bGpuAssistRequested = false;
	m_bGpuAssistTransformReady = false;
	m_bGpuAssistCpuDataReady = false;
	m_bGpuAssistBodyPath = false;
	m_bGpuAssistAttachmentPath = false;
	m_bGpuAssistGlobalTransform = false;
	m_bGpuAssistTranslate = false;
...
void BMD::PrepareGpuAssistBounds(vec3_t BoundingBoxMin, vec3_t BoundingBoxMax, OBB_t* OBB)
{
	float extentX = BoundingBoxMax[0] - BoundingBoxMin[0];
	float extentY = BoundingBoxMax[1] - BoundingBoxMin[1];
	float extentZ = BoundingBoxMax[2] - BoundingBoxMin[2];

	if (m_bGpuAssistTranslate)
	{
		extentX *= BodyScale;
		extentY *= BodyScale;
		extentZ *= BodyScale;
	...
	fTransformedSize = extentX;
	if (extentY > fTransformedSize) fTransformedSize = extentY;
	if (extentZ > fTransformedSize) fTransformedSize = extentZ;
...
void BMD::CacheGpuAssistBoneMatrices()
{
...
void BMD::ComputeGpuAssistLightDirection(vec3_t lightDir) const
{
	vec3_t position;
	float matrix[3][4];
	vec3_t localShadowAngle;

	if (HighLight)
	{
		Vector(1.3f, 0.f, 2.f, position);
		VectorCopy(ShadowAngle, localShadowAngle);
	}
	else if (gMapManager->InBattleCastle())
	{
		Vector(0.5f, -1.f, 1.f, position);
		Vector(0.f, 0.f, -45.f, localShadowAngle);
	}
	else
	{
	...
	AngleMatrix(localShadowAngle, matrix);
	VectorIRotate(position, matrix, lightDir);
}

void BMD::EnsureCpuTransformData()
{
	if (!m_bGpuAssistTransformReady || m_bGpuAssistCpuDataReady || m_pGpuAssistBoneMatrices == NULL)
		return;

	TransformCpuInternal(m_pGpuAssistBoneMatrices, m_vGpuAssistBoundingBoxMin, m_vGpuAssistBoundingBoxMax,
		m_pGpuAssistOBB, m_bGpuAssistTranslate, m_fGpuAssistScale);
		...
		if (m->NumTriangles)
		{
			...
			if (Texture != BITMAP_HIDE)
			{
				...
				if (Texture != -1)
				{
					...
					{
						const bool gpuUsesTexCoordOffset = (gpuRenderMode == 4 || gpuRenderMode == 8 || gpuRenderMode == 11);
						...
						RenderMeshGpuAssist(m, EnableLight, (RenderFlag & RENDER_WAVE) != 0, Alpha,
							(EnableWave || gpuUsesTexCoordOffset) ? BlendMeshTexCoordU : 0.0f,
							(EnableWave || gpuUsesTexCoordOffset) ? BlendMeshTexCoordV : 0.0f,

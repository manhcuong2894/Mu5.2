...
void CreatePoint(vec3_t Position, __int64 Value, vec3_t Color, float scale, bool bMove, bool bRepeatedly, int SubType)
{
...
void RenderPoints(BYTE byRenderOneMore)
{
	...
	for (int i = 0; i < MAX_POINTS; i++)
	{
		...
		if (o->Live)
		{
			...
			if (o->SubType == 0)
			{
				RenderNumber(o->Position, o->NumberValue, o->Angle, o->Gravity * (0.4f), o->Scale);
			}
		}
	}
}

...
enum ObjectID
{
	...
	eWindowQuaPhucLoi = 28,
	IMG_GiftCodeFrame,
	eWindowListItemTrader = 115,

	eWindowViewItemChar = 116,
	eTopDame = 117,
	eWindowRankingTop = 118,

#if(CUSTOM_CHANGEITEM)
	eWindowChangeItem = 119,
#endif
	eWindowMenuUser = 120,
...
public:
	...
	void DrawGlowAroundBox2(float x, float y, float width, float height, int length, float r, float g, float b);
	//==Mess Box
	...
	std::string vCaptcha;
#if(CUSTOM_CHANGEITEM)
	bool CItem_ActiveMix;
	bool CItem_EffectsMix;
	bool SetStateDoiItem;
	BYTE CItem_ItemChinh[16];
	BYTE CItem_ItemPhu[3][16];
	BYTE CItem_ItemKetQua[12][16];
	char requiredInfoItem[256];
	int CostWC;
	int CostWP;
	int CostGP;
	int ChangeItemOpenMap;
	int ChangeItemOpenX;
	int ChangeItemOpenY;
	void DrawChangeItem();
	bool ChangeSendItem(ITEM* ItemSell, int Slot, bool KeyClick);
#endif

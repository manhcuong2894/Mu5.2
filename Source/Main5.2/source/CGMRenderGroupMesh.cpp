...
namespace
{
	bool IsSameRenderMeshEntry(const RENDER_MESH_EFFECT& left, const RENDER_MESH_EFFECT& right)
	{
		return left.Index == right.Index && left.GroupId == right.GroupId;
	}

	void UpsertRenderMeshEntry(std::vector<RENDER_MESH_EFFECT>& items, const RENDER_MESH_EFFECT& info)
	{
		for (size_t i = 0; i < items.size(); ++i)
		{
			if (IsSameRenderMeshEntry(items[i], info))
			{
				items[i] = info;
				return;
			}
	...
	RENDER_MESH_EFFECT CreateDefaultRenderMeshEntry(int itemIndex, int groupId)
	{
		RENDER_MESH_EFFECT info;
		info.Index = itemIndex;
		info.GroupId = groupId;
		info.RenderFlag = RENDER_TEXTURE;
		info.Texture = -1;
		info.BodyLight = static_cast<DWORD>(-1);
		info.RenderType = -1;
		info.BlendLight = 1.f;
		info.TimeEffect = -1.f;
		info.TimeTexture = -1.f;
		info.RenderLevel = TRUE;
		return info;
	}
}
...
bool CGMRenderGroupMesh::runtime_make_item(int modelType, BMD* pModel, OBJECT* pObject)
{
	...
	if (renderaction == TRUE && (&edit_mesh)->Index == itemindex)
	{
		...
		if (edit_item.size() != 0)
		{
			...
			{
				if (IsSameRenderMeshEntry(edit_mesh, edit_item[i]))
				{
					continue;
				}
	...
	if (renderaction == TRUE && (&edit_mesh)->Index == itemindex)
	{
		...
		if (edit_item.size() != 0)
		{
			for (size_t i = 0; i < edit_item.size(); i++)
			{
				...
				if (IsSameRenderMeshEntry(edit_mesh, edit_item[i]) || !b->RenderLevel)
					continue;
...
void CGMRenderGroupMesh::runtime_make_edit(int body)
{
	...
	renderflag = 0;
	edit_mesh_dirty = false;
}

void CGMRenderGroupMesh::runtime_make_item_selector_button(float RenderFrameX, float RenderFrameY, int* type, int* subtype, bool type_selector)
{
	...
	auto isModifierHeld = [](int virtualKey)
	{
		return SEASON3B::IsPress(virtualKey) || SEASON3B::IsRepeat(virtualKey);
	};

	auto getStepDelta = [&]()
	{
		if (isModifierHeld(VK_SHIFT))
		{
			return 100;
		}

		if (isModifierHeld(VK_CONTROL))
		{
			return 10;
		}
	...
	auto applyItemIdDelta = [&](int delta)
	{
		const int maxItemId = (MAX_ITEM_TYPE * MAX_ITEM_INDEX) - 1;
		int itemId = (*type * MAX_ITEM_INDEX) + *subtype;
		...
		else if (itemId > maxItemId)
		{
			itemId = maxItemId;
		}

		*type = itemId / MAX_ITEM_INDEX;
		*subtype = itemId % MAX_ITEM_INDEX;
	};

	auto applyDefaultDelta = [&](bool increase)
	{
		if (type_selector)
		{
			if (increase)
			{
				if (*type < (MAX_ITEM_TYPE - 1))
				{
					++(*type);
				}
				else
	...
	if (SEASON3B::CheckMouseFetch(RenderFrameX, RenderFrameY, true, false))
	{
		...
		{
			const int stepDelta = getStepDelta();

			if (stepDelta > 1)
			{
				applyItemIdDelta(stepDelta);
			}
			else
			{
				applyDefaultDelta(true);
			}
		}
	}
...
bool CGMRenderGroupMesh::runtime_update_interface()
{
	...
	{
		RENDER_MESH_EFFECT previous_edit_mesh = edit_mesh;
		...
		this->runtime_make_item_selector_button(RenderFrameX + 120.f, RenderFrameY + 61.f, &type, &subtype, true);

		this->runtime_make_item_selector_button(RenderFrameX + 120.f, RenderFrameY + 79.f, &type, &subtype, false);
		...
		int maxRenderFlagIndex = render_flag.empty() ? 0 : static_cast<int>(render_flag.size() - 1);
		this->runtime_make_button_steep(RenderFrameX + 260.f, RenderFrameY + 215.f, 1, 0, maxRenderFlagIndex, &renderflag);
		...
		if (!(previous_edit_mesh == edit_mesh))
		{
			edit_mesh_dirty = true;
		}
...
void CGMRenderGroupMesh::runtime_make_interface()
{
	...
	if (render_widget_color(202.f, 136.f, 76.0, &(&edit_mesh)->BodyLight))
	{
		edit_mesh_dirty = true;
	}
...
bool CGMRenderGroupMesh::runtime_make_button(int Image, float RenderFrameX, float RenderFrameY, float RenderSizeX, float RenderSizeY, BYTE Type, bool checkaction)
{
	...
	if (SEASON3B::CheckMouseIn(RenderFrameX, RenderFrameY, RenderSizeX, RenderSizeY))
	{
		...
		if (checkaction && SEASON3B::IsRelease(VK_LBUTTON))
		{
			if (Type == 0)
			{
				UpsertRenderMeshEntry(edit_item, edit_mesh);
...
void CGMRenderGroupMesh::runtime_make_file()
{
	...
	if (fp)
	{
		...
		buffer = "// Type  Index  Red  Green  Blue  Group  RenderEffect  EffectType  TimeEffectType  TextureID  TimeTextureID  LevelGlow";
		textwrite(buffer, true, fp);

		buffer = "//----------------------------------------------------------------------------------------------------------------------------------------";
		textwrite(buffer, true, fp);

		std::vector<RENDER_MESH_EFFECT> export_items;
		RENDER_GROUP_MESH_EFFECT* current_item_data = this->FindItemRender(edit_mesh.Index);

		if (current_item_data)
		{
			for (size_t i = 0; i < current_item_data->size(); ++i)
			{
				UpsertRenderMeshEntry(export_items, current_item_data->Group[i]);
			}
		}

		for (size_t i = 0; i < edit_item.size(); ++i)
		{
			if (edit_item[i].Index != edit_mesh.Index)
			{
				continue;
			}

			UpsertRenderMeshEntry(export_items, edit_item[i]);
		}

		if (export_items.empty())
		{
			export_items.push_back(edit_mesh);
		}
		else if (edit_mesh_dirty)
		{
			UpsertRenderMeshEntry(export_items, edit_mesh);
		}
		...
		{
			for (int groupId = 0; groupId < pModel->NumMeshs; ++groupId)
			{
				bool hasGroup = false;

				for (size_t i = 0; i < export_items.size(); ++i)
				{
					if (export_items[i].Index == edit_mesh.Index && export_items[i].GroupId == groupId)
					{
						hasGroup = true;
						break;
					}
				...
				if (!hasGroup)
				{
					export_items.push_back(CreateDefaultRenderMeshEntry(edit_mesh.Index, groupId));
				}
			}
		}

		std::sort(export_items.begin(), export_items.end(), [](const RENDER_MESH_EFFECT& left, const RENDER_MESH_EFFECT& right)
		{
		...
		for (size_t i = 0; i < export_items.size(); ++i)
		{
			RENDER_MESH_EFFECT* info = &export_items[i];

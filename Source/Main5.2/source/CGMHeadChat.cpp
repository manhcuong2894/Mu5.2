...
static unsigned int GetHeadChatTextHash(const char *text, bool useGmFont,
  ...
  hash ^= useGmFont ? 0x9e3779b9u : 0u;
  hash ^= useBoldFont ? 0x85ebca6bu : 0u;
  return hash;
}

static BOOL GetCachedHeadChatTextExtent(const char *text, bool useGmFont,
                                        bool useBoldFont, SIZE *size) {
  if (size == NULL) {
    return FALSE;
  }
  ...
  const unsigned int hash =
      GetHeadChatTextHash(text, useGmFont, useBoldFont);
  HeadChatExtentCache *cache =
      &g_HeadChatExtentCache[hash % (sizeof(g_HeadChatExtentCache) /
                                     sizeof(g_HeadChatExtentCache[0]))];

  if (cache->Valid && cache->Hash == hash &&
      cache->UseGmFont == useGmFont && cache->UseBoldFont == useBoldFont &&
  ...
  BOOL result = FALSE;
  if (useGmFont) {
    result = GMFontLayer->_GetTextExtentPoint32(text, lstrlen(text), size);
  } else {
    if (useBoldFont) {
      g_pRenderText->SetFont(g_hFontBold);
    }
    result = g_pMultiLanguage->_GetTextExtentPoint32(
        g_pRenderText->GetFontDC(), text, lstrlen(text), size);
    if (useBoldFont) {
  ...
  cache->Valid = true;
  cache->UseGmFont = useGmFont;
  cache->UseBoldFont = useBoldFont;
  cache->Hash = hash;
...
void CGMHeadChat::AddGuildName(CHAT *c, CHARACTER *Owner) {
  ...
  if (gmProtect->m_RenderCharacterName &&
    ...
    if (pCharacter->GuildMarkIndex >= 0 &&
        GuildMark[pCharacter->GuildMarkIndex].UnionName[0]) {
      wsprintf(c->Union, "<%s>",
               GuildMark[pCharacter->GuildMarkIndex].UnionName);
    } else {
      c->Union[0] = NULL;
    }
    ...
    if (pCharacter->GuildMarkIndex >= 0) {
      ...
      if (Owner->GuildStatus == G_MASTER)
        szRole = GlobalText[1300];
      else if (Owner->GuildStatus == G_SUB_MASTER)
        szRole = GlobalText[1301];
      else if (Owner->GuildStatus == G_BATTLE_MASTER)
        szRole = GlobalText[1302];

      if (szRole)
        wsprintf(c->Guild, "[%s] %s",
                 GuildMark[Owner->GuildMarkIndex].GuildName, szRole);
      else
        wsprintf(c->Guild, "[%s]",
                 GuildMark[Owner->GuildMarkIndex].GuildName);
    } else {
      c->GuildColor = 0;
      c->Guild[0] = NULL;
    }
    ...
    if (Owner->GuildMarkIndex >= 0 &&
        GuildMark[Owner->GuildMarkIndex].UnionName[0]) {
      if (Owner->GuildRelationShip == GR_UNION)
        wsprintf(c->Union, "<%s> %s",
                 GuildMark[Owner->GuildMarkIndex].UnionName, GlobalText[1295]);
      if (Owner->GuildRelationShip == GR_UNIONMASTER) {
        if (Owner->GuildStatus == G_MASTER)
          wsprintf(c->Union, "<%s> %s",
                   GuildMark[Owner->GuildMarkIndex].UnionName,
                   GlobalText[1296]);
        else
          wsprintf(c->Union, "<%s> %s",
                   GuildMark[Owner->GuildMarkIndex].UnionName,
        ...
        wsprintf(c->Union, "<%s> %s",
                 GuildMark[Owner->GuildMarkIndex].UnionName, GlobalText[1299]);
      else
    ...
    if (Owner->GuildMarkIndex >= 0) {
      ...
      if (Owner->GuildStatus == G_PERSON)
        wsprintf(c->Guild, "[%s] %s",
                 GuildMark[Owner->GuildMarkIndex].GuildName, GlobalText[1330]);
      else if (Owner->GuildStatus == G_MASTER)
        wsprintf(c->Guild, "[%s] %s",
                 GuildMark[Owner->GuildMarkIndex].GuildName, GlobalText[1300]);
      else if (Owner->GuildStatus == G_SUB_MASTER)
        wsprintf(c->Guild, "[%s] %s",
                 GuildMark[Owner->GuildMarkIndex].GuildName, GlobalText[1301]);
      else if (Owner->GuildStatus == G_BATTLE_MASTER)
        wsprintf(c->Guild, "[%s] %s",
                 GuildMark[Owner->GuildMarkIndex].GuildName, GlobalText[1302]);
      else
...
void CGMHeadChat::CreateChat(char *ID, const char *Text, CHARACTER *Owner,
...
void CGMHeadChat::SetBooleanPosition(CHAT *c) {
  ...
  if (gmProtect->m_RenderCharacterName &&
    ...
    if (g_isCharacterBuff((&pCharacter->Object), eBuff_GMEffect) ||
        (pCharacter->CtlCode == CTLCODE_20OPERATOR) ||
        (pCharacter->CtlCode == CTLCODE_08OPERATOR)) {
      bResult[0] =
          GetCachedHeadChatTextExtent(c->ID, false, true, &Size[0]);
    } else {
      bResult[0] =
          GetCachedHeadChatTextExtent(c->ID, false, false, &Size[0]);
    }

    bResult[1] =
        GetCachedHeadChatTextExtent(c->Text[0], false, false, &Size[1]);
    bResult[2] =
        GetCachedHeadChatTextExtent(c->Text[1], false, false, &Size[2]);
    bResult[3] =
        GetCachedHeadChatTextExtent(c->Union, false, false, &Size[3]);
    bResult[4] =
        GetCachedHeadChatTextExtent(c->Guild, false, false, &Size[4]);

    Size[0].cx += 3;

    if (c->LifeTime[1] > 0)
      c->Width =
          max(max(max(Size[0].cx, Size[1].cx), max(Size[2].cx, Size[3].cx)),
              Size[4].cx);
    ...
    c->Height = FontHeight * (bResult[0] + bResult[1] + bResult[2] +
                              bResult[3] + bResult[4]);

    if (lstrlen(c->szShopTitle) > 0) {
      SIZE sizeT[2];

      if (GetCachedHeadChatTextExtent(c->szShopTitle, false, true,
                                      &sizeT[0]) &&
          GetCachedHeadChatTextExtent(GlobalText[1104], false, true,
                                      &sizeT[1])) {
  ...
  if (gmProtect->m_RenderCharacterName &&
      (pCharacter->GetKind() == KIND_PLAYER ||
       pCharacter->GetKind() == KIND_MONSTER)) {
    bResult[1] =
        GetCachedHeadChatTextExtent(c->Text[0], true, false, &Size[1]);
    bResult[2] =
        GetCachedHeadChatTextExtent(c->Text[1], true, false, &Size[2]);
    bResult[3] =
        GetCachedHeadChatTextExtent(c->Union, true, false, &Size[3]);
    bResult[4] =
        GetCachedHeadChatTextExtent(c->Guild, true, false, &Size[4]);
  } else {
    bResult[1] =
        GetCachedHeadChatTextExtent(c->Text[0], false, false, &Size[1]);
    bResult[2] =
  ...
  c->Height = FontHeight *
              (bResult[0] + bResult[1] + bResult[2] + bResult[3] + bResult[4]);

  if (lstrlen(c->szShopTitle) > 0) {
    SIZE sizeT[2];

    if (GetCachedHeadChatTextExtent(c->szShopTitle, false, true, &sizeT[0]) &&
        GetCachedHeadChatTextExtent(GlobalText[1104], false, true,
                                    &sizeT[1])) {
      if (c->Width < sizeT[0].cx + sizeT[1].cx)
        c->Width = sizeT[0].cx + sizeT[1].cx;
...
void CGMHeadChat::RenderBooleans() {
    ...
    if (ci->IDLifeTime > 0 || ci->LifeTime[0] > 0) {
      ...
      if (ci->Owner != NULL) {
        ...
        if (gmProtect->m_RenderCharacterName &&
            (pCharacter->GetKind() == KIND_PLAYER ||
             pCharacter->GetKind() == KIND_MONSTER)) {
          if (o->Type >= MODEL_LITTLESANTA &&
                  o->Type <= MODEL_LITTLESANTA_END ||
        ...
        Vector(o->Position[0], o->Position[1],
               o->Position[2] + o->BoundingBoxMax[2] + add_boundin, Position);

        Position[2] = RequestTerrainHeight(o->Position[0], o->Position[1]) +
                      o->BoundingBoxMax[2] + add_boundin;
  ...
  /*for (int i = 0; i < MAX_CHAT; i++)
  {
          ...
          if (ci->IDLifeTime > 0 || ci->LifeTime[0] > 0)
          {
                  ...
                  {
                          CHAT* cj = &Chat[j];
                          if (i != j && (cj->IDLifeTime > 0 || cj->LifeTime[0] >
  0))
                          {
                                  if (ci->x + ci->Width > cj->x && ci->x<cj->x +
  cj->Width && ci->y + ci->Height>cj->y && ci->y < cj->y + cj->Height)
                                  {
...
void RenderGensMark(BYTE _GensInfluence, BYTE _GensRankInfo, int _ScreenX,
                    int _ScreenY) {
  if (_GensInfluence == GENS_TYPE_NONE || _GensRankInfo <= 0 ||
      _GensRankInfo > 14) {
    return;
  }
  float u, v, uw, vh;
  ...
  jpexs::Renderjpexs_dds(IMAGE_HUD_PLAYER, scale_jpexs(_ScreenX, -2100),
                         scale_jpexs(_ScreenY, -1060), 24, 30, u, v, uw, vh);
}
...
void CGMHeadChat::RenderBoolean(int x, int y, CHAT *c) {
  ...
  if (pCharacter != Hero && battleCastle::IsBattleCastleStart() == true &&
      g_isCharacterBuff((&pCharacter->Object), eBuff_Cloaking)) {
    if ((Hero->EtcPart == PARTS_ATTACK_KING_TEAM_MARK ||
         Hero->EtcPart == PARTS_ATTACK_TEAM_MARK)) {
      if (!(pCharacter->EtcPart == PARTS_ATTACK_KING_TEAM_MARK ||
      ...
      if (!(pCharacter->EtcPart == PARTS_ATTACK_KING_TEAM_MARK2 ||
      ...
      if (!(pCharacter->EtcPart == PARTS_ATTACK_KING_TEAM_MARK3 ||
      ...
      if (!(pCharacter->EtcPart == PARTS_DEFENSE_KING_TEAM_MARK ||
            pCharacter->EtcPart == PARTS_DEFENSE_TEAM_MARK)) {
        return;
      }
    }
  }
  ...
  if (gmProtect->m_RenderCharacterName &&
    ...
    if (::IsStrifeMap(gMapManager->currentMap) &&
        Hero->m_byGensInfluence != pCharacter->m_byGensInfluence &&
        !::IsGMCharacter()) {
      ...
      if (pCharacter->m_byGensInfluence == 1 ||
          pCharacter->m_byGensInfluence == 2) {
        g_pNewUIGensRanking->RanderMark(
            _TempX / g_fScreenRate_x, y / g_fScreenRate_y,
            pCharacter->m_byGensInfluence, pCharacter->m_byRankIndex,
            SEASON3B::CNewUIGensRanking::MARK_BOOLEAN,
            (float)RenderPos.y / g_fScreenRate_y);
      }
    }
    ...
    // Calculate name text width for positioning (only for players)
    SIZE nameSize = { 0 };
    if (pCharacter->GetKind() == KIND_PLAYER &&
        MODEL_PLAYER == pCharacter->Object.Type) {
      GetCachedHeadChatTextExtent(c->ID, true, false, &nameSize);
    }
    ...
    if (pCharacter->GetKind() == KIND_PLAYER && MODEL_PLAYER == pCharacter->Object.Type) {
      ...
      if (gmProtect->RenderGuildLogo == 1 &&
          pCharacter->GuildStatus != G_NONE &&
          pCharacter->GuildMarkIndex != -1) {
        logoW = 16 + 3; // 16 width + 3 padding
      }
    }
    
    int nameRowW = gensW + nameSize.cx + logoW;
    int startX = RenderPos.x - (nameRowW / 2);
    int curX = startX;

    // --- Render Name Row (Gens + Name + Logo) ---
    if (pCharacter->GetKind() == KIND_PLAYER && MODEL_PLAYER == pCharacter->Object.Type) {
      if (gensW > 0) {
        int gensH = 30;
        int gensX = curX + 105;
        int gensY = RenderPos.y + (iLineHeight / 2) - (gensH / 2) + 53;
        RenderGensMark(pCharacter->m_byGensInfluence, pCharacter->m_byRankIndex, gensX, gensY);
        curX += gensW;
      }
      ...
      if (logoW > 0) {
        ...
        float logoY = (float)RenderPos.y + (iLineHeight - 16) / 2.f;
    ...
    if (pCharacter->GetKind() == KIND_PLAYER) {
      ...
      if (SelectedCharacter != -1 && iSelectedCharacter == SelectedCharacter) {
        g_pBuffWindow->RenderBuffStatus(RenderPos.x / g_fScreenRate_x,
                                        RenderPos.y / g_fScreenRate_y,
                                        &pCharacter->Object);
      }
    }
#endif
    ...
    if (c->LifeTime[1] > 0 || c->LifeTime[0] > 0) {
      ...
      g_pNameWindow->RenderTooltip(RenderPosX, RenderPosY, RenderSizeX,
                                   RenderSizeY);
    }

    if (c->LifeTime[1] > 0) {
      g_pRenderText->SetTextColor(dwTextColor[0]);
      GMFontLayer->RenderText(RenderPos.x, RenderPos.y, c->Text[0], 0,
                              iLineHeight, RT3_WRITE_CENTER);

      RenderPos.y -= iLineHeight;
      g_pRenderText->SetTextColor(dwTextColor[1]);
      GMFontLayer->RenderText(RenderPos.x, RenderPos.y, c->Text[1], 0,
                              iLineHeight, RT3_WRITE_CENTER);
    } else if (c->LifeTime[0] > 0) {
    ...
    if (IsShopInViewport(pCharacter)) {
      ...
      g_pRenderText->RenderTextClipped(RenderPos.x, RenderPos.y,
                                       GlobalText[1104], 0, iLineHeight,
                                       RT3_SORT_LEFT, &TextSize);
      RenderPos.x += TextSize.cx;

      g_pRenderText->SetTextColor(GetShopText2Color(pCharacter));
      g_pRenderText->RenderTextClipped(RenderPos.x, RenderPos.y, c->szShopTitle,
                                       RenderBoxSize.cx - TextSize.cx,
                                       iLineHeight, RT3_SORT_LEFT, &TextSize);
      g_pRenderText->SetFont(g_hFont);
      ...
      if (KIND_PLAYER == pCharacter->GetKind() &&
          MODEL_PLAYER == pCharacter->Object.Type) {
        int _TempX = (int)(c->x + c->Width * 0.5f + 20.0f);
    ...
    if (pCharacter == Hero) {
      ...
      g_pRenderText->SetBgColor(
          GetGuildRelationShipBGColor(pCharacter->GuildRelationShip));
      g_pRenderText->SetTextColor(
          GetGuildRelationShipTextColor(pCharacter->GuildRelationShip));
    }

    if (c->Union && c->Union[0]) {
      g_pRenderText->RenderTextClipped(RenderPos.x, RenderPos.y, c->Union,
                                       RenderBoxSize.cx, iLineHeight,
                                       RT3_SORT_LEFT);
      RenderPos.y += iLineHeight;
    }
    if (c->Guild && c->Guild[0]) {
      g_pRenderText->RenderTextClipped(RenderPos.x, RenderPos.y, c->Guild,
                                       RenderBoxSize.cx, iLineHeight,
                                       RT3_SORT_LEFT);
      RenderPos.y += iLineHeight;
    }
    ...
    if (c->x <= MouseX &&
        MouseX <
            (int)(c->x + c->Width * gwinhandle->GetScreenX() / WindowWidth) &&
        c->y <= MouseY &&
        MouseY <
            (int)(c->y + c->Height * gwinhandle->GetScreenY() / WindowHeight) &&
    ...
    if (pCharacter->GetKind() == KIND_PLAYER) {
      ...
      if (SelectedCharacter != -1 && iSelectedCharacter == SelectedCharacter) {
        g_pBuffWindow->RenderBuffStatus(
            c->x + (c->Width / 2.f) / g_fScreenRate_x, c->y / g_fScreenRate_y,
            &pCharacter->Object);
      }
    }
#endif
    if (bGmMode) {
      ...
      if (GMProtect->IsRenderWave(c->ID)) {
        g_pRenderText->RenderWave(RenderPos.x, RenderPos.y, c->ID,
                                  RenderBoxSize.cx, iLineHeight, RT3_SORT_LEFT,
                                  NULL, strlen(c->ID), false);
      } else {
        g_pRenderText->RenderTextClipped(RenderPos.x, RenderPos.y, c->ID,
                                         RenderBoxSize.cx, iLineHeight,
                                         RT3_SORT_LEFT);
      }
    ...
    if (c->LifeTime[1] > 0) {
      g_pRenderText->SetTextColor(dwTextColor[1]);
      g_pRenderText->RenderTextClipped(RenderPos.x, RenderPos.y, c->Text[1],
                                       RenderBoxSize.cx, iLineHeight,
                                       RT3_SORT_LEFT);
      RenderPos.y += iLineHeight;

      g_pRenderText->SetTextColor(dwTextColor[0]);
      g_pRenderText->RenderTextClipped(RenderPos.x, RenderPos.y, c->Text[0],
                                       RenderBoxSize.cx, iLineHeight);
    } else if (c->LifeTime[0] > 0) {
      g_pRenderText->SetTextColor(dwTextColor[0]);
      g_pRenderText->RenderTextClipped(RenderPos.x, RenderPos.y, c->Text[0],
    ...
    if (pCharacter->GetKind() == KIND_PLAYER &&
      ...
      if (gmProtect->RenderGuildLogo != 0) {
        if (pCharacter->GuildStatus != G_NONE &&
            pCharacter->GuildMarkIndex != -1) {
          x -= 20.f;
          CreateGuildMark(pCharacter->GuildMarkIndex, true);
          RenderBitmap(BITMAP_GUILD, (float)x, (float)y / g_fScreenRate_y, 18.f,
                       18.f, 0.0, 0.0, 1.f, 1.f);
        }
      }
      ...
      if (pCharacter->m_byGensInfluence == 1 ||
          pCharacter->m_byGensInfluence == 2) {
        g_pNewUIGensRanking->RanderMark(
            x, y / g_fScreenRate_y, pCharacter->m_byGensInfluence,
            pCharacter->m_byRankIndex,
            SEASON3B::CNewUIGensRanking::MARK_BOOLEAN,
            (float)RenderPos.y / g_fScreenRate_y);
      }
    }
...
void CGMHeadChat::MoveChat() {
  for (int i = 0; i < MAX_CHAT; i++) {
    ...
    if (pCharacter != NULL &&
        (!pCharacter->Object.Live || !pCharacter->Object.Visible)) {
      c->IDLifeTime = 0;
      c->LifeTime[0] = 0;
      c->LifeTime[1] = 0;
    }

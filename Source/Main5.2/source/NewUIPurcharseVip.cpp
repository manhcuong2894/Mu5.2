...
namespace {
const float kVipWindowWidth = 214.0f;
const float kVipWindowHeight = 230.0f;

void GetCenteredVipPosition(float &x, float &y) {
	x = (MAX_WIN_WIDTH / 2.0f) - (kVipWindowWidth / 2.0f);
	y = (MAX_WIN_HEIGHT / 2.0f) - (kVipWindowHeight / 2.0f);
...
bool SEASON3B::CNewUIPurcharseVip::UpdateMouseEvent()
{
	...
	GetCenteredVipPosition(centeredX, centeredY);
	SetPos(centeredX, centeredY);

	float RenderFrameX = m_Pos.x;
	float RenderFrameY = m_Pos.y;

	if (SEASON3B::CheckMouseIn(RenderFrameX, RenderFrameY, kVipWindowWidth, kVipWindowHeight))
	{
	...
	return !SEASON3B::CheckMouseIn(m_Pos.x, m_Pos.y, kVipWindowWidth, kVipWindowHeight);
}
...
void SEASON3B::CNewUIPurcharseVip::RenderFrame()
{
	...
	SEASON3B::window_backmsg(RenderFrameX, RenderFrameY, kVipWindowWidth, kVipWindowHeight);

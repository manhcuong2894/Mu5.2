...
void CGMHolyItem::GDHolyInventoryRecv(SDHP_INVENTORY_HOLY_RECV* lpMsg, int index)
{
	...
	if (gQueryManager.ExecQuery("SELECT TOP 1 HolyInventory FROM Character WHERE AccountID='%s' AND HolyInventory IS NOT NULL ORDER BY cLevel DESC", lpMsg->account) == 0 || gQueryManager.Fetch() == SQL_NO_DATA)
	{
		gQueryManager.Close();
...
void CGMHolyItem::GDHolyInventorySaveRecv(SDHP_INVENTORY_HOLY_SAVE_RECV* lpMsg)
{
	...
	gQueryManager.ExecQuery("UPDATE Character SET HolyInventory=? WHERE AccountID='%s'", lpMsg->account);

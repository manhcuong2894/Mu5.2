...
#define MAKE_NUMBERW(x, y)                                                     \
  ((WORD)(((BYTE)((y) & 0xFF)) | ((BYTE)((x) & 0xFF) << 8)))
#define MAKE_NUMBERDW(x, y)                                                    \
  ((DWORD)(((WORD)((y) & 0xFFFF)) | ((WORD)((x) & 0xFFFF) << 16)))
#define MAKE_NUMBERQW(x, y)                                                    \
  ((QWORD)(((DWORD)((y) & 0xFFFFFFFF)) | ((DWORD)((x) & 0xFFFFFFFF) << 32)))
...
struct SDHP_CUSTOM_BOSS_KILL_SAVE_RECV {
  PBMSG_HEAD header; // C1:57
  WORD index;
  char account[11];
  char name[11];
  DWORD points;
...
struct SDHP_VONGQUAY_TICHLUY_SAVE_RECV {
  PBMSG_HEAD header; // C1:58
  WORD index;
  char account[11];
  char name[11];
  DWORD DiemTichLuyVQ;
...
struct SDHP_MOCNAP_AUTO_REWARD_RECV {
  PBMSG_HEAD header; // C1:59
  WORD index;
  char account[11];
  char name[11];
};

struct SDHP_MOCNAP_AUTO_REWARD_SEND {
  PBMSG_HEAD header; // C1:59
  WORD index;
  char account[11];
  char name[11];
...
struct SDHP_MOCNAP_PAYMENT_CREATE_SEND {
  PWMSG_HEAD header; // C2:5A
  WORD index;
  char account[11];
  char name[11];
  int Result;
...
void GDCustomBossKillSaveRecv(SDHP_CUSTOM_BOSS_KILL_SAVE_RECV *lpMsg);
void GDVongQuayTichLuySaveRecv(SDHP_VONGQUAY_TICHLUY_SAVE_RECV *lpMsg);
...
void DGGlobalWhisperEchoSend(WORD ServerCode, WORD index, char *account,
                             char *name, char *SourceName, char *message);
...
void GDMocNapPaymentCreateRecv(SDHP_MOCNAP_PAYMENT_CREATE_RECV *lpMsg,
                               int index);
void GDCustomRankingRecv(SDHP_CUSTOM_RANKING_SEND *lpMsg, int index);
...
void GDSetTieuPhi(SDHP_SET_COIN_TIEU_PHI *lpMsg);
void GDGetInfoQuaPhucLoi(SDHP_SET_COIN_TIEU_PHI *lpMsg, int index);
void GDResetQuaPhucLoi(SDHP_SET_COIN_TIEU_PHI *lpMsg, int index);

void CHECK_GIFT_CODE(SEND_DS_GETSTATUS *lpMsg, int index);

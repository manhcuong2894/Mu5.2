...
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,
                     LPSTR lpCmdLine, int nCmdShow) // OK
{
  VM_START
  ...
  GetPrivateProfileString("DataServerInfo", "CustomerName", "", CustomerName,
                          sizeof(CustomerName), ".\\Data\\DataServer.ini");

  GetPrivateProfileString("DataServerInfo", "CustomerHardwareId", "",
                          CustomerHardwareId, sizeof(CustomerHardwareId),
                          ".\\Data\\DataServer.ini");
  ...
  wsprintf(buff, "[%s] %s DataServer (QueueSize : %d)", DATASERVER_VERSION,
           DATASERVER_CLIENT, 0);
  ...
  if (WSAStartup(MAKEWORD(2, 2), &wsa) == 0) {
    ...
    GetPrivateProfileString("DataServerInfo", "DataServerODBC", "",
                            DataServerODBC, sizeof(DataServerODBC),
                            ".\\Data\\DataServer.ini");

    GetPrivateProfileString("DataServerInfo", "DataServerUSER", "",
                            DataServerUSER, sizeof(DataServerUSER),
                            ".\\Data\\DataServer.ini");

    GetPrivateProfileString("DataServerInfo", "DataServerPASS", "",
                            DataServerPASS, sizeof(DataServerPASS),
                            ".\\Data\\DataServer.ini");

    WORD DataServerPort = GetPrivateProfileInt(
        "DataServerInfo", "DataServerPort", 55960, ".\\Data\\DataServer.ini");

    AdvancedLog = GetPrivateProfileInt("DataServerInfo", "AdvancedLog", 0,
                                       ".\\Data\\DataServer.ini");
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow) // OK
{
  ...
  hWnd =
      CreateWindow(szWindowClass, szTitle, WS_OVERLAPPEDWINDOW | WS_THICKFRAME,
                   CW_USEDEFAULT, 0, 600, 600, 0, 0, hInstance, 0);
                         ...
                         LPARAM lParam) // OK
{
  ...
  case WM_COMMAND:
    ...
    case IDM_ABOUT:
      ...
      break;
    case IDM_EXIT:
      if (MessageBox(0, "Are you sure to terminate DataServer?",

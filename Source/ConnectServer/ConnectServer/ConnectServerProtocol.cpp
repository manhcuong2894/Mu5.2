...
void CCServerHwidRecv(PMSG_SERVER_HWID_RECV* lpMsg, int index)
{
	...
	{
		LogAdd(LOG_RED, "[ServerHwid] request blocked, JoinServer offline (Index: %d)", index);
		return;
	}
...
void CCServerInfoRecv(PMSG_SERVER_INFO_RECV* lpMsg, int index) // OK
{
	if (gServerList.CheckJoinServerState() == 0)
	{
		LogAdd(LOG_RED, "[ServerInfo] request blocked, JoinServer offline (Index: %d, ServerCode: %d)", index, lpMsg->ServerCode);
		return;
	}
	...
	{
		LogAdd(LOG_RED, "[ServerInfo] request blocked, server not found (Index: %d, ServerCode: %d)", index, lpMsg->ServerCode);
		return;
	}

	if (lpServerListInfo->ServerShow == 0 || lpServerListInfo->ServerState == 0)
	{
		LogAdd(LOG_RED, "[ServerInfo] request blocked, server hidden/offline (Index: %d, ServerCode: %d, Show: %d, State: %d)", index, lpMsg->ServerCode, lpServerListInfo->ServerShow, lpServerListInfo->ServerState);
		return;
	}
	...
	LogAdd(LOG_BLUE, "[ServerInfo] send address (Index: %d, ServerCode: %d, Address: %s, Port: %d)", index, lpMsg->ServerCode, lpServerListInfo->ServerAddress, lpServerListInfo->ServerPort);
